...
- GitHub rulebook synchronization applies a 15-second timeout to every request and bounds metadata to 512 KiB, commit metadata to 256 KiB, repository trees to 16 MiB, and rulebook content to 4 MiB. Both `Content-Length` and streamed bytes are enforced before rulebook validation or digest use.
