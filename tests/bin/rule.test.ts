...
describe('rule test', () => {
  test('runs every configured project rulebook and reports a failing fixture', async () => {
    await withTempDir('safety-net-rule-test-failing-', async (tempDir) => {
      const env = projectRuleEnv(tempDir);
      const rulesDir = join(tempDir, '.cc-safety-net', 'rules');
      const failingRulebookPath = join(rulesDir, 'failing-rules', 'rulebook.json');
      writeLocalRulebook(join(rulesDir, 'project-rules', 'rulebook.json'), 'project-rules');
      writeLocalRulebook(failingRulebookPath, 'failing-rules');
      writeProjectRulesConfig(tempDir, ['project-rules', 'failing-rules']);
      expect((await runCCSafetyNetCli(['rule', 'sync'], env, tempDir)).exitCode).toBe(0);
      writeFileSync(
        failingRulebookPath,
        JSON.stringify({
          ...readRulebook(failingRulebookPath),
          tests: [
            {
              command: 'echo danger',
              expect: 'blocked',
              rule: 'failing-rules-rule',
            },
            { command: 'echo danger', expect: 'allowed' },
          ],
        }),
      );

      const result = await runCCSafetyNetCli(['rule', 'test'], env, tempDir);

      expect(result.exitCode).toBe(1);
      expect(result.stderr).toContain(
        'echo danger: expected allowed but matched failing-rules-rule',
      );
      expect(result.output).not.toContain('Rulebook tests passed.');
    });
  });

  test('runs a configured passing project rulebook', async () => {
    await withInitializedProjectRules('safety-net-rule-test-passing-', async (tempDir, env) => {
      const result = await runCCSafetyNetCli(['rule', 'test'], env, tempDir);

      expect(result.exitCode).toBe(0);
      expect(result.stderr).toBe('');
      expect(result.output).toContain('Rulebook tests passed.');
      expect(result.output).toContain('project-rules 1.0.0');
      expect(result.output).toContain('Tests: 1');
    });
  });

  test('reports an empty configured source list without a false success', async () => {
    await withTempDir('safety-net-rule-test-empty-', async (tempDir) => {
      const result = await runCCSafetyNetCli(['rule', 'test'], projectRuleEnv(tempDir), tempDir);

      expect(result.exitCode).toBe(0);
      expect(result.stderr).toBe('');
      expect(result.output).toBe('No rulebooks configured; nothing to test.\n');
      expect(result.output).not.toContain('Rulebook tests passed.');
    });
  });

  test('uses the configured user rulebooks with global scope', async () => {
    await withInitializedGlobalRules('safety-net-rule-test-global-', async (_tempDir, env) => {
      const result = await runCCSafetyNetCli(['rule', 'test', '--global'], env);

      expect(result.exitCode).toBe(0);
      expect(result.stderr).toBe('');
      expect(result.output).toContain('Rulebook tests passed.');
      expect(result.output).toContain('user-rules 1.0.0');
      expect(result.output).toContain('Tests: 1');
    });
  });
});
