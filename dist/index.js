...
import { dirname as dirname3, isAbsolute as isAbsolute3, join as join3, resolve } from "node:path";
...
function isLinkedWorktree(cwd) {
  ...
  try {
    ...
    if (rawGitDir === "")
      return !1;
    let gitDir = isAbsolute3(rawGitDir) ? rawGitDir : resolve(dirname3(dotGitPath), rawGitDir);
...
function readWorktreeGitdirBacklink(gitDir) {
  let backlinkPath = join3(gitDir, "gitdir");
  if (!existsSync(backlinkPath))
    return null;
  let rawBacklink = readFileSync(backlinkPath, "utf-8").split(/\r?\n/, 1)[0]?.trim() ?? "";
  return rawBacklink === "" ? null : rawBacklink;
}
function readWorktreeConfigWorktree(gitDir) {
  let configWorktreePath = join3(gitDir, "config.worktree");
...
function resolveGitDirPath(gitDir, target) {
  return isAbsolute3(target) ? target : resolve(gitDir, target);
}
...
function readCoreWorktree(configPath) {
  let content = readFileSync(configPath, "utf-8"), inCore = !1, configuredWorktree = null;
...
function findDotGitInAncestors(cwd) {
  let current = cwd;
  while (!0) {
    let dotGitPath = join3(current, ".git");
...
function isProtectableCommand(token, policy) {
  ...
  return normalized === "git" || basename2 === "busybox" || isStandardCommandWrapper(token) || BUILTIN_ANALYZED_COMMANDS.has(basename2) || policy.transparentWrappers.includes(basename2) || SHELL_WRAPPERS.has(normalized) || token === "$SHELL" || isInterpreterCommand(normalized) || AWK_INTERPRETERS.has(normalized) || policy.rules.some((rule) => rule.command === basename2);
}
...
import { isAbsolute as isAbsolute5, join as join4, normalize, parse, relative, resolve as resolve2, sep as sep2 } from "node:path";
...
function getPolicyFilesystemTargetForPath(scope, path) {
  let relativePath = relative(scope.root, resolve2(path));
...
function readPolicyFile(target) {
  try {
    ...
    try {
      ...
      let content = readFileSync2(descriptor, "utf-8"), after = lstatSync3(target.path);
...
function readPolicyDirectoryEntries(target) {
  ...
  try {
    let entries = names.map((name) => {
      let child = getPolicyFilesystemTarget(target.scope, join4(target.relativePath, name)), stat = lstatSync3(child.path);
...
import { dirname as dirname4, isAbsolute as isAbsolute6, join as join5, relative as relative2, resolve as resolve3, sep as sep3 } from "node:path";
...
function getUserSafetyNetHome() {
  ...
  return home ? resolve3(home) : join5(homedir3(), SAFETY_NET_DIR);
}
...
function getUserPolicyFilesystemScope(_configPath, options2) {
  let root = options2.userConfigPath ? dirname4(dirname4(resolve3(options2.userConfigPath))) : dirname4(resolve3(options2.userConfigDir ?? getUserRulesDir(options2)));
  return bindPolicyFilesystemScope(root, "user policy");
}
function getProjectPolicyFilesystemScope(configPath, options2) {
  let cwd = resolve3(options2.cwd ?? process.cwd()), absoluteConfigPath = resolve3(configPath), fromCwd = relative2(cwd, absoluteConfigPath);
...
function getRulesCacheDir(options2) {
  ...
  if (syncOptions && !syncOptions.global && syncOptions.cwd && resolve3(configDir) === resolve3(syncOptions.cwd))
    return join5(resolve3(syncOptions.cwd), SAFETY_NET_DIR, CACHE_SUBDIR);
...
import { dirname as dirname7, isAbsolute as isAbsolute7, join as join8, relative as relative3, resolve as resolve4, sep as sep4 } from "node:path";
...
function loadLockedRulebook(entry, configDir, options2, filesystemScope) {
  ...
  if (entry.kind === "local-directory") {
    let sourcePath = resolve4(configDir, entry.path), sourceRelative = relative3(resolve4(configDir), sourcePath);
...
function resolveTrackedHeredocPath(source, effectiveCwd) {
  let path = isAbsolute8(source) ? resolve5(source) : effectiveCwd ? resolve5(effectiveCwd, source) : void 0;
  if (!path)
    return;
  try {
...
import { normalize as normalize3, posix, resolve as resolve6, sep as sep6 } from "node:path";

// src/core/analyze/tmpdir.ts
import { lstatSync as lstatSync4, realpathSync as realpathSync6 } from "node:fs";
import { tmpdir } from "node:os";
import { isAbsolute as isAbsolute9, join as join9, normalize as normalize2, parse as parsePath3, sep as sep5 } from "node:path";
...
function resolveExistingPathComponents(path) {
  ...
  for (let i = 0;i < components.length; i++) {
    ...
    current = realpathSync6(candidate);
  }
  return current;
}
function isPathOrSubpath(path, basePath) {
  if (path === basePath)
    return !0;
  let baseWithSlash = basePath.endsWith(sep5) ? basePath : `${basePath}${sep5}`;
...
function isNormalizedPathWithin(target, cwd) {
  ...
  return normalizedTarget.startsWith(`${normalizedCwd}${sep6}`) || normalizedTarget === normalizedCwd;
}
...
import { dirname as dirname8, isAbsolute as isAbsolute10, join as join10, resolve as resolve7 } from "node:path";
...
function resolveGitDirFromDotGit(dotGitPath) {
  try {
    ...
    if (rawGitDir === "")
      return null;
    return isAbsolute10(rawGitDir) ? rawGitDir : resolve7(dirname8(dotGitPath), rawGitDir);
  } catch {
    return null;
  }
}
function resolveCommonGitDir(gitDir) {
  ...
  try {
    ...
    if (rawCommonDir === "")
      return null;
    return isAbsolute10(rawCommonDir) ? rawCommonDir : resolve7(gitDir, rawCommonDir);
  } catch {
    return null;
  }
}
...
function analyzeParallel(tokens, context) {
  ...
  if (template.length === 0) {
    ...
    return firstMatch(commands, (command2) => context.analyzeNested(command2, nestedOverrides));
  }
...
import { dirname as dirname9, isAbsolute as isAbsolute12, normalize as normalize5, resolve as resolve8 } from "node:path";
...
function normalizePolicyCandidatePath(target, cwd, budget) {
  ...
  return resolveExistingPath(normalize5(isAbsolute12(expanded) ? expanded : resolve8(cwd, expanded)), budget).replace(/\\/g, "/");
}
...
import { isAbsolute as isAbsolute13, resolve as resolve9 } from "node:path";
...
function isMetadataOnlyCommand(facts) {
  ...
  if (stripped.length === 0)
    return !1;
  let command2 = basename2(stripped[0] ?? "").toLowerCase(), args = stripped.slice(1);
  if (command2 === "test")
...
function extractSegmentPathTargets(tokens, store, options2) {
  ...
  let executable = stripped[0] ?? "", command2 = basename2(executable).toLowerCase(), post = stripped.slice(1), explainTargets = extractSafetyNetExplainPathTargets(executable, command2, post);
  if (explainTargets)
...
function extractDisplayCommandOperands(tokens) {
  ...
  if (stripped.length === 0)
    return [];
  let command2 = basename2(stripped[0] ?? "").toLowerCase();
  if (!NON_PATH_OPERAND_COMMANDS.has(command2))
    return [];
...
function normalizeCandidatePath(target, cwd, budget) {
  ...
  if (!home)
    return normalized;
  let expanded = expandHomePath(normalized, home), absolute = isAbsolute13(expanded) ? expanded : normalizePathText(resolve9(cwd, expanded)), canonicalAbsolute = normalizePathText(resolveExistingPath(absolute, budget));
...
function normalizeAbsoluteCandidatePath(target, cwd, budget) {
  ...
  return normalizePathText(resolveExistingPath(isAbsolute13(expanded) ? expanded : resolve9(cwd, expanded), budget));
}
...
function createCCSafetyNetPlugin(guardDependencies = {}) {
  return async ({ directory, homeDir }) => {
    let configCwd = resolve10(directory), currentConfig;
    return {
...
function resolveOpenCodeExecutionCwd(configCwd, toolInput) {
  ...
  if (!resolvedWorkdir)
    return null;
  let executionCwd = resolve10(configCwd, resolvedWorkdir);
