...
import { dirname as dirname9, isAbsolute as isAbsolute9, join as join10, normalize as normalize4, resolve as resolve8 } from "node:path";
...
function normalizeCandidatePath(target, cwd) {
  ...
  if (!unix)
    return "";
  const expanded = unix === "~" ? homedir4() : unix.startsWith("~/") ? resolve8(homedir4(), unix.slice(2)) : unix;
  return normalize4(isAbsolute9(expanded) ? expanded : resolve8(cwd, expanded)).replace(/\\/g, "/");
}
  ...
  return {
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (input.tool === "bash") {
        ...
        if (!command2) {
          throwBlocked(REASON_SAFETY_NET_FAILED_CLOSED, undefined, undefined, undefined, undefined, "stop_and_explain");
        }
        let result;
        try {
          const modes = getCCSafetyNetEnvModes(config);
