...
function getUserSafetyNetHome() {
  ...
  return home ? resolve2(home) : join(homedir(), SAFETY_NET_DIR);
}
...
import { dirname as dirname5, isAbsolute as isAbsolute4, join as join4, relative as relative2, resolve as resolve3, sep as sep2 } from "node:path";
...
import { dirname as dirname6, isAbsolute as isAbsolute5, join as join5, resolve as resolve5 } from "node:path";
...
function isLinkedWorktree(cwd) {
  ...
  try {
    ...
    if (rawGitDir === "")
      return !1;
    let gitDir = isAbsolute5(rawGitDir) ? rawGitDir : resolve5(dirname6(dotGitPath), rawGitDir);
...
function resolveGitDirPath(gitDir, target) {
  return isAbsolute5(target) ? target : resolve5(gitDir, target);
}
...
import { dirname as dirname7, isAbsolute as isAbsolute6, join as join6, resolve as resolve6 } from "node:path";
...
function resolveGitDirFromDotGit(dotGitPath) {
  try {
    ...
    if (rawGitDir === "")
      return null;
    return isAbsolute6(rawGitDir) ? rawGitDir : resolve6(dirname7(dotGitPath), rawGitDir);
  } catch {
    return null;
  }
}
function resolveCommonGitDir(gitDir) {
  ...
  try {
    ...
    if (rawCommonDir === "")
      return null;
    return isAbsolute6(rawCommonDir) ? rawCommonDir : resolve6(gitDir, rawCommonDir);
  } catch {
    return null;
  }
}
...
import { isAbsolute as isAbsolute7, join as join7, normalize as normalize2, parse as parsePath3, sep as sep4 } from "node:path";
...
import { dirname as dirname9, isAbsolute as isAbsolute8, join as join9, normalize as normalize4, resolve as resolve7 } from "node:path";
...
function normalizeCandidatePath(target, cwd) {
  ...
  if (!unix)
    return "";
  let expanded = unix === "~" ? homedir4() : unix.startsWith("~/") ? resolve7(homedir4(), unix.slice(2)) : unix;
  return resolveExistingPath(normalize4(isAbsolute8(expanded) ? expanded : resolve7(cwd, expanded))).replace(/\\/g, "/");
}
...
function normalizeCandidatePath2(target, cwd) {
  ...
  if (!home)
    return normalized;
  let expanded = expandHomePath(normalized, home), absolute = isAbsolute9(expanded) ? expanded : normalizePathText(resolve8(cwd, expanded)), canonicalAbsolute = normalizePathText(resolveExistingPath(absolute));
...
function normalizeAbsoluteCandidatePath(target, cwd) {
  ...
  return normalizePathText(resolveExistingPath(isAbsolute9(expanded) ? expanded : resolve8(cwd, expanded)));
}
...
import { isAbsolute as isAbsolute10, join as join10 } from "node:path";
...
function getAuditLogHomeDir(homeFromEnv = process.env.CC_SAFETY_NET_AUDIT_HOME || process.env.HOME) {
  ...
  return home && isAbsolute10(home) ? home : null;
}
