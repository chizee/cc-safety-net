...
function loadLockedRulebook(entry, configDir, options2) {
  ...
  if (entry.kind === "local-directory") {
    let sourcePath = resolve3(configDir, entry.path), sourceRelative = relative2(resolve3(configDir), sourcePath);
...
function isLinkedWorktree(cwd) {
  ...
  try {
    ...
    if (rawGitDir === "")
      return !1;
    let gitDir = isAbsolute6(rawGitDir) ? rawGitDir : resolve5(dirname6(dotGitPath), rawGitDir);
...
function analyzeParallelDynamicEnvValues(values, args, context) {
  for (let value of values) {
    if (!hasParallelPlaceholder(value))
      continue;
    let commands2 = args.length > 0 ? args.map((arg) => replaceParallelPlaceholder(value, arg)) : [value];
...
function normalizeCandidatePath(target, cwd, budget) {
  ...
  return resolveExistingPath(normalize4(isAbsolute9(expanded) ? expanded : resolve7(cwd, expanded)), budget).replace(/\\/g, "/");
}
...
function normalizeCandidatePath2(target, cwd, budget) {
  ...
  if (!home)
    return normalized;
  let expanded = expandHomePath(normalized, home), absolute = isAbsolute10(expanded) ? expanded : normalizePathText(resolve8(cwd, expanded)), canonicalAbsolute = normalizePathText(resolveExistingPath(absolute, budget));
...
function normalizeAbsoluteCandidatePath(target, cwd, budget) {
  ...
  return normalizePathText(resolveExistingPath(isAbsolute10(expanded) ? expanded : resolve8(cwd, expanded), budget));
}
