...
import { isAbsolute as isAbsolute5, join as join2, normalize, parse, relative as relative2, resolve as resolve2, sep as sep2 } from "node:path";
...
import { dirname as dirname2, isAbsolute as isAbsolute6, join as join3, relative as relative3, resolve as resolve3, sep as sep3 } from "node:path";
...
function getUserPolicyFilesystemScope(_configPath, options2) {
  let root = options2.userConfigPath ? dirname2(dirname2(resolve3(options2.userConfigPath))) : dirname2(resolve3(options2.userConfigDir ?? getUserRulesDir(options2)));
...
function getRulesCacheDir(options2) {
  ...
  if (syncOptions && !syncOptions.global && syncOptions.cwd && resolve3(configDir) === resolve3(syncOptions.cwd))
    return join3(resolve3(syncOptions.cwd), SAFETY_NET_DIR, CACHE_SUBDIR);
...
import { dirname as dirname5, isAbsolute as isAbsolute7, join as join6, relative as relative4, resolve as resolve4, sep as sep4 } from "node:path";
...
import { existsSync as existsSync2, lstatSync as lstatSync3, readFileSync as readFileSync3, realpathSync as realpathSync6, statSync as statSync3 } from "node:fs";
import { dirname as dirname6, isAbsolute as isAbsolute8, join as join7, resolve as resolve6 } from "node:path";
...
function isLinkedWorktree(cwd) {
  ...
  try {
    ...
    if (rawGitDir === "")
      return !1;
    let gitDir = isAbsolute8(rawGitDir) ? rawGitDir : resolve6(dirname6(dotGitPath), rawGitDir);
...
import { dirname as dirname7, isAbsolute as isAbsolute9, join as join8, resolve as resolve7 } from "node:path";
...
function resolveGitDirFromDotGit(dotGitPath) {
  try {
    ...
    if (rawGitDir === "")
      return null;
    return isAbsolute9(rawGitDir) ? rawGitDir : resolve7(dirname7(dotGitPath), rawGitDir);
  } catch {
    return null;
  }
}
...
import { isAbsolute as isAbsolute10, join as join9, normalize as normalize3, parse as parsePath3, sep as sep6 } from "node:path";
