...
var COMMAND_NAME = "cc-safety-net", COMMAND_DESCRIPTION = "Manage CC Safety Net rulebooks", DEFAULT_USER_REQUEST = "Help me configure CC Safety Net.";
function registerBuiltinCommands(pi) {
  pi.registerCommand(COMMAND_NAME, {
    description: COMMAND_DESCRIPTION,
    handler: async (args, ctx) => {
      pi.sendUserMessage(buildSafetyNetCommandPrompt(args), ctx.isIdle() ? void 0 : { deliverAs: "followUp" });
    }
  });
}
...
function resolveExistingPath(path, budget = createPathCanonicalizationBudget()) {
  ...
  while (!0) {
    ...
    try {
      let existing = realpathSync3(candidate), resolved = suffixes.length === 0 ? existing : join2(existing, ...suffixes.reverse());
      return budget.resolvedPaths.set(path, resolved), resolved;
    } catch {
      let parent = dirname2(candidate);
      if (parent === candidate) {
...
function scanSelectorCommands(source) {
  let commands2 = [], words = [], i = 0, wordCount = 0, invalidComment = !1, flush = () => {
    if (words.length > 0)
      commands2.push(words);
    words = [];
  };
...
` ? 2 : 1;
    ...
    for (let nested of result.nested)
      commands2.push(...selectorCommandsFromProgram(nested));
    wordCount++, i = result.next > i ? result.next : i + 1;
  }
  return flush(), { commands: commands2, invalidComment };
}
...
function extractAwkSystemCommands(code) {
  let commands2 = [], sawSystem = !1, searchIndex = 0;
  while (searchIndex < code.length) {
    ...
    if (i = skipAwkWhitespace(code, parsed.endIndex), sawSystem = !0, code[i] !== ")")
      return { dynamic: !0, commands: commands2 };
    commands2.push(parsed.value), searchIndex = i + 1;
  }
  if (!sawSystem)
    return null;
  return commands2.length > 0 ? { dynamic: !1, commands: commands2 } : { dynamic: !0, commands: commands2 };
}
function extractAwkPipeCommands(code) {
  let commands2 = [], dynamic = !1, sawPipeCommand = !1, i = 0;
  while (i < code.length) {
    let char = code[i];
    if (!char)
      break;
    ...
    if (isAwkPrintPipe(code, i)) {
      ...
      commands2.push(parsed.value), i = parsed.endIndex;
      continue;
    }
    i++;
  }
...
import { isAbsolute as isAbsolute5, join as join3, normalize, parse, relative as relative2, resolve as resolve2, sep as sep2 } from "node:path";
...
function bindPolicyFilesystemScope(root, label) {
  return { [POLICY_FILESYSTEM_SCOPE]: !0, root: resolve2(root), label };
}
...
import { dirname as dirname3, isAbsolute as isAbsolute6, join as join4, relative as relative3, resolve as resolve3, sep as sep3 } from "node:path";
...
function getUserSafetyNetHome() {
  ...
  return home ? resolve3(home) : join4(homedir3(), SAFETY_NET_DIR);
}
...
function getUserPolicyFilesystemScope(_configPath, options2) {
  let root = options2.userConfigPath ? dirname3(dirname3(resolve3(options2.userConfigPath))) : dirname3(resolve3(options2.userConfigDir ?? getUserRulesDir(options2)));
  return bindPolicyFilesystemScope(root, "user policy");
}
function getProjectPolicyFilesystemScope(configPath, options2) {
  let cwd = resolve3(options2.cwd ?? process.cwd()), absoluteConfigPath = resolve3(configPath), fromCwd = relative3(cwd, absoluteConfigPath);
...
function getRulesCacheDir(options2) {
  ...
  if (syncOptions && !syncOptions.global && syncOptions.cwd && resolve3(configDir) === resolve3(syncOptions.cwd))
    return join4(resolve3(syncOptions.cwd), SAFETY_NET_DIR, CACHE_SUBDIR);
...
import { dirname as dirname6, isAbsolute as isAbsolute7, join as join7, relative as relative4, resolve as resolve4, sep as sep4 } from "node:path";
...
function validateAllowedCommands(commands2, diagnostics) {
  if (!Array.isArray(commands2))
    return;
  let seen = /* @__PURE__ */ new Set;
  for (let i = 0;!diagnostics.stopped && i < commands2.length; i++) {
    let command2 = commands2[i];
...
function loadLockedRulebook(entry, configDir, options2, filesystemScope) {
  ...
  if (entry.kind === "local-directory") {
    let sourcePath = resolve4(configDir, entry.path), sourceRelative = relative4(resolve4(configDir), sourcePath);
...
import { normalize as normalize2, resolve as resolve5, sep as sep5 } from "node:path";
...
function isCwdSelfTarget(target, cwd) {
  if (target === "." || target === "./" || target === ".\\")
    return !0;
  try {
    return normalizePathForComparison(realpathSync6(resolve5(cwd, target))) === normalizePathForComparison(realpathSync6(cwd));
  } catch {
    try {
...
import { existsSync as existsSync2, lstatSync as lstatSync3, readFileSync as readFileSync3, realpathSync as realpathSync7, statSync as statSync3 } from "node:fs";
...
function getGitExecutionContext(tokens, cwd) {
  ...
  let gitCwd;
  try {
    gitCwd = realpathSync7(resolve6(cwd));
  } catch {
    return { gitCwd: null, hasExplicitGitContext: !1 };
  }
...
function isLinkedWorktree(cwd) {
  ...
  try {
    ...
    if (rawGitDir === "")
      return !1;
    let gitDir = isAbsolute8(rawGitDir) ? rawGitDir : resolve6(dirname7(dotGitPath), rawGitDir);
...
function resolveGitDirPath(gitDir, target) {
  return isAbsolute8(target) ? target : resolve6(gitDir, target);
}
...
import { dirname as dirname8, isAbsolute as isAbsolute9, join as join9, resolve as resolve7 } from "node:path";
...
function resolveGitDirFromDotGit(dotGitPath) {
  try {
    ...
    if (rawGitDir === "")
      return null;
    return isAbsolute9(rawGitDir) ? rawGitDir : resolve7(dirname8(dotGitPath), rawGitDir);
  } catch {
    return null;
  }
}
function resolveCommonGitDir(gitDir) {
  ...
  try {
    ...
    if (rawCommonDir === "")
      return null;
    return isAbsolute9(rawCommonDir) ? rawCommonDir : resolve7(gitDir, rawCommonDir);
  } catch {
    return null;
  }
}
...
import { dirname as dirname9, isAbsolute as isAbsolute11, join as join11, normalize as normalize5, resolve as resolve8 } from "node:path";
...
function findPolicyConfigMutationTargetForContext(facts, context, budget) {
  if (facts.invocation.route.kind === "patch")
    return findPolicyConfigMutationTargetInPaths(facts.paths.map((path) => path.raw), !1, context, budget);
  let command2 = getCommandSyntaxFact(facts, "input-candidate");
  if (facts.invocation.route.kind === "command")
    return command2 ? findPolicyConfigMutationTargetInCommand(command2.shell, context, /* @__PURE__ */ new Map, facts.store, budget) : null;
  if (facts.invocation.route.kind === "unknown" && command2) {
    let commandTarget = findPolicyConfigMutationTargetInCommand(command2.shell, context, /* @__PURE__ */ new Map, facts.store, budget);
    if (commandTarget)
      return commandTarget;
  }
  return findPolicyConfigMutationTargetInPaths(facts.paths.map((path) => path.raw), facts.invocation.route.kind === "grep" || facts.invocation.route.kind === "glob" || isReadOnlyTool(facts.invocation.toolName), context, budget);
}
function findPolicyConfigMutationTargetInPaths(paths, readOnly, context, budget) {
  let target = paths.find((value) => isPolicyConfigPath(value, context.configCwd, context.executionCwd, budget));
  if (!target)
    return null;
...
function findPolicyConfigMutationTargetInCommand(syntax, context, variables = /* @__PURE__ */ new Map, store, budget) {
  ...
  for (let entry of syntax.entries) {
    if (entry.kind === "operator") {
      if (!entry.boundary)
        continue;
      let target = findUnsafePolicyConfigSegmentTarget(segment, state, context.configCwd, store, budget);
      if (target)
        return target;
    ...
    if (entry.kind === "redirection") {
      ...
      if (target && isPolicyConfigPath(expandShellVariables(target, state.variables), context.configCwd, state.cwd, budget))
...
function findUnsafePolicyConfigSegmentTarget(segment, state, configCwd, store, budget) {
  if (isAssignmentOnlySegment(segment))
    return null;
  let scriptTarget = findScriptArgumentPolicyConfigTarget(segment, state, configCwd, store, budget);
  if (scriptTarget)
    return scriptTarget;
  let sedWriteTarget = findSedScriptWritePolicyConfigTarget(segment, state, configCwd, budget);
  if (sedWriteTarget)
    return sedWriteTarget;
  let target = segment.flatMap((token) => extractPolicyConfigPathCandidates(token).map((candidate) => expandShellVariables(candidate, state.variables))).find((token) => isPolicyConfigPath(token, configCwd, state.cwd, budget));
  if (!target)
    return null;
  return isReadOnlySegment(segment) ? null : { target };
}
function findScriptArgumentPolicyConfigTarget(segment, state, configCwd, store, budget) {
...
function findSedScriptWritePolicyConfigTarget(segment, state, configCwd, budget) {
  let stripped = stripEnvAssignments(stripWrappers([...segment]));
  if (getBasename(stripped[0] ?? "").toLowerCase() !== "sed")
    return null;
  let target = extractSedScriptArguments(stripped.slice(1)).flatMap((script) => extractSedWritePathCandidates(script)).map((candidate) => expandShellVariables(candidate, state.variables)).find((candidate) => isPolicyConfigPath(candidate, configCwd, state.cwd, budget));
  return target ? { target } : null;
}
...
function getSegmentCwd(segment, state, budget) {
  let stripped = stripEnvAssignments(stripWrappers([...segment]));
...
function findPolicyConfigTargetInText(text, context, budget) {
  let target = extractPolicyConfigPathCandidates(text).find((candidate) => isPolicyConfigPath(candidate, context.configCwd, context.executionCwd, budget));
  return target ? { target } : null;
}
...
function normalizeCandidatePath(target, cwd, budget) {
  ...
  return resolveExistingPath(normalize5(isAbsolute11(expanded) ? expanded : resolve8(cwd, expanded)), budget).replace(/\\/g, "/");
}

// src/core/secret-protection.ts
import { homedir as homedir6 } from "node:os";
import { isAbsolute as isAbsolute12, resolve as resolve9 } from "node:path";
...
function isSensitivePath(target, cwd, config, budget) {
  let normalized = normalizeCandidatePath2(target, cwd, budget);
  if (!normalized)
    return null;
...
function matchesOpenCodePath(normalized, cwd, budget) {
  ...
  return matchesFileInRoot(normalized, dataRoot, ["auth.json", "mcp-auth.json"]) || matchesFileInRoot(normalized, configRoot, ["opencode.json", "opencode.jsonc"]) || matchesOptionalExactPath(normalized, process.env.OPENCODE_CONFIG, cwd, budget) || ["/Library/Application Support/opencode", "/etc/opencode", ...programDataConfig].some((root) => matchesFileInRoot(normalized, normalizeCandidatePath2(root, cwd, budget), [
    "opencode.json",
    "opencode.jsonc"
  ]));
}
...
function codingCliRoot(envValue, fallback, cwd, budget) {
  return normalizeCandidatePath2(envValue?.trim() ? envValue : fallback, cwd, budget);
}
...
function matchesExactPath(normalized, path, cwd, budget) {
  return sameComparablePath(normalized, normalizeCandidatePath2(path, cwd, budget));
}
...
function normalizeCandidatePath2(target, cwd, budget) {
  ...
  if (!home)
    return normalized;
  let expanded = expandHomePath(normalized, home), absolute = isAbsolute12(expanded) ? expanded : normalizePathText(resolve9(cwd, expanded)), canonicalAbsolute = normalizePathText(resolveExistingPath(absolute, budget));
...
function normalizeAbsoluteCandidatePath(target, cwd, budget) {
  ...
  return normalizePathText(resolveExistingPath(isAbsolute12(expanded) ? expanded : resolve9(cwd, expanded), budget));
}
...
// src/pi/tool-call.ts
var PI_COMMAND_TOOL_ADAPTERS = /* @__PURE__ */ new Map([
...
function handlePiToolCallWithDependencies(event, ctx, options2) {
  ...
  try {
    ...
    if (!(error instanceof GuardEvaluationError))
      throw error;
    if (envTruthy(ENV_FLAGS.debug))
      console.error(`CC Safety Net debug: pi tool_call analysis failed: ${formatIntegrationError(error.cause)}`);
