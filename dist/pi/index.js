...
import { isAbsolute as isAbsolute2, relative, resolve, sep as sep2 } from "node:path";
...
function analyzeAwkSystemCallMatch(tokens, analyzeNested, scanWork) {
  ...
  for (let source of extractAwkSourceArgs(tokens)) {
    let commands2 = extractAwkExternalCommands(source, scanWork);
    if (!commands2)
      continue;
...
function extractAwkSystemCommands(code, scanWork) {
  chargeNativeLinearPass(scanWork, code);
  let commands2 = [], sawSystem = !1, dynamic = !1, searchIndex = 0;
...
function extractAwkPipeCommands(code, scanWork) {
  chargeNativeLinearPass(scanWork, code);
  let commands2 = [], dynamic = !1, sawPipeCommand = !1, i = 0, statementStart = 0, printKeywordIndex = null, leadingString = null, lastSignificantEnd = 0;
  while (i < code.length) {
    let char = code[i];
    if (!char)
      break;
...
{}`.includes(char)) {
    ...
    if (isAwkPrintPipe(statementStart, printKeywordIndex)) {
      ...
      commands2.push(parsed.value), lastSignificantEnd = parsed.endIndex, i = parsed.endIndex;
      continue;
    }
...
function resolveDotGitFileTargets(dotGitPath) {
  try {
    ...
    if (!rawGitDir)
      return null;
    let gitDir = realpathSync4(isAbsolute5(rawGitDir) ? rawGitDir : resolve2(dirname3(dotGitPath), rawGitDir));
...
function resolveCommonGitDir(gitDir) {
  try {
    ...
    if (!rawCommonDir)
      return null;
    let commonDir = realpathSync4(isAbsolute5(rawCommonDir) ? rawCommonDir : resolve2(gitDir, rawCommonDir));
...
import { isAbsolute as isAbsolute14, normalize as normalize5, parse as parse2, posix, resolve as resolve8, sep as sep8 } from "node:path";
...
import { isAbsolute as isAbsolute13, join as join10, relative as relative5 } from "node:path";

// src/core/protected-path-scanner.ts
import { homedir as homedir5 } from "node:os";
import { isAbsolute as isAbsolute12, normalize as normalize4, resolve as resolve7 } from "node:path";
...
function normalizeProtectedPathCandidate(target, cwd, budget) {
  ...
  return resolveExistingPath(normalize4(isAbsolute12(expanded) ? expanded : resolve7(cwd, expanded)), budget).replace(/\\/g, "/");
}
...
function isCanonicalHomeTarget(target, ctx) {
  ...
  if (!candidate)
    return !1;
  try {
    let base = isAbsolute14(candidate) ? candidate : ctx.resolvedCwd ? resolve8(ctx.resolvedCwd, candidate) : null;
    if (!base)
      return !1;
...
function isAllowedPathTarget(target, ctx, targetIsLiteral) {
  ...
  if (hasParentDirectoryComponent(trimmed))
    return !1;
  let expanded = targetIsLiteral ? trimmed : expandAllowPathHome(trimmed, ctx.homeDir), base = ctx.resolvedCwd ?? ctx.anchoredCwd, resolved = isAbsolute14(expanded) ? expanded : base ? resolve8(base, expanded) : null;
  if (!resolved)
    return !1;
  try {
...
import { dirname as dirname8, isAbsolute as isAbsolute15, join as join11, resolve as resolve9 } from "node:path";
...
function resolveGitDirFromDotGit(dotGitPath) {
  try {
    ...
    if (rawGitDir === "")
      return null;
    return isAbsolute15(rawGitDir) ? rawGitDir : resolve9(dirname8(dotGitPath), rawGitDir);
  } catch {
    return null;
  }
}
function resolveCommonGitDir2(gitDir) {
  ...
  try {
    ...
    if (rawCommonDir === "")
      return null;
    return isAbsolute15(rawCommonDir) ? rawCommonDir : resolve9(gitDir, rawCommonDir);
  } catch {
    return null;
  }
}
...
import { isAbsolute as isAbsolute17, resolve as resolve10 } from "node:path";
...
function normalizeCandidatePath(target, cwd, budget) {
  ...
  if (!home)
    return normalized;
  let expanded = expandHomePath(normalized, home), absolute = isAbsolute17(expanded) ? expanded : normalizePathText(resolve10(cwd, expanded)), canonicalAbsolute = normalizePathText(resolveExistingPath(absolute, budget));
...
function normalizeAbsoluteCandidatePath(target, cwd, budget) {
  ...
  return normalizePathText(resolveExistingPath(isAbsolute17(expanded) ? expanded : resolve10(cwd, expanded), budget));
}
