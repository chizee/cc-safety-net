...
function normalizeCandidatePath2(target, cwd) {
  ...
  if (!home)
    return normalized;
  let expanded = expandHomePath(normalized, home), absolute = isAbsolute10(expanded) ? expanded : normalizePathText(resolve8(cwd, expanded)), canonicalAbsolute = normalizePathText(resolveExistingPath(absolute));
...
function handlePiToolCallWithDependencies(event, ctx, options2) {
  ...
  if ("malformed" in toolCall)
    return blockPiToolCall(REASON_SAFETY_NET_FAILED_CLOSED, void 0, void 0, void 0, void 0, "stop_and_explain");
  let evaluation;
  try {
    ...
    if (!(error instanceof GuardEvaluationError))
      throw error;
    if (envTruthy(ENV_FLAGS.debug))
      console.error(`CC Safety Net debug: pi tool_call analysis failed: ${redactSecrets(error.cause instanceof Error ? error.cause.message : String(error.cause))}`);
    return blockPiEvaluation(error.evaluation, toolCall.route.kind === "command");
  }
  return writeGuardAudit(evaluation.audit, () => ctx.sessionManager.getSessionFile(), { agent: "pi" }), blockPiEvaluation(evaluation, evaluation.stage !== "config-state");
}
...
function blockPiEvaluation(evaluation, includeEvidence) {
  ...
  return blockPiToolCall(evaluation.decision.reason, evidence?.command, evidence?.segment, void 0, evaluation.decision.ruleId, evaluation.decision.intent);
}
