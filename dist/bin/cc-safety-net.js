...
`.repeat(d.length-1)}${kp(d.length-1)}`:""}\x1B7`);try{for(let f=1;f<=u;f+=1){if(t.signal?.aborted)break;n.write(_s(d.map((L,v)=>Tp(L,v,f,u,p,o,i,a)))),await Sp(m,r,t.signal)}}finally{if(n.write(_s(d.map((f,L)=>Pp(f,L,o,i,a)))),n.write("\x1B8"),d.length>1)n.write(Os(d.length-1));n.write(`
...
`)},a=Om(t,e.fetchVersion??nt).then(async(u)=>{let m=new Set(u.targets);return{targets:u.targets,codexPluginListOutput:u.codexPluginListOutput,available:new Map(await Promise.all(se.filter((f)=>m.has(f.target)&&Ql.has(f.target)).map(async(f)=>[f.target,await yo(f.probeCommand)])))}}),l=await Pt(e.showBanner??!0,()=>({ready:a,finish:()=>a}),()=>Et({input:e.input??process.stdin,output:n}),{loadingMessage:"Checking installed integrations…",output:n}),c=await Promise.resolve().then(()=>(jl(wm(),process.platform,o),null)).catch((u)=>jt(u));if(l.targets.length===0){if(n.write("No installed integrations found. Run `cc-safety-net install` to set one up.\n"),c!==null)console.error(c);return await s(),c===null?0:1}let d=l.targets.some((u)=>ec.has(u))?await Promise.resolve().then(()=>(En(t),null)).catch((u)=>jt(u)):null,p=await nn(Promise.all(l.targets.map((u)=>{if(Ql.has(u)&&!l.available.get(u))return Promise.resolve({message:`${R(u)} not found; skipped`,failed:!1});if(d!==null&&ec.has(u))return Promise.resolve({message:d,failed:!0});return cc("install",u,t,!0,l.codexPluginListOutput).then((m)=>({message:m,failed:!1}),(m)=>({message:jt(m),failed:!0}))})),{loadingMessage:`Updating ${l.targets.length} integration${l.targets.length===1?"":"s"}…`,output:n}),g=c===null?p:[...p,{message:c,failed:!0}];return g.forEach((u)=>{u.failed?console.error(u.message):n.write(`${u.message}
`)}),await s(),g.some((u)=>u.failed)?1:0}function Co(e,t={}){return Promise.resolve().then(()=>_m(e)).then(()=>$m(t)).catch((n)=>(console.error(jt(n)),1))}async function Ut(e,t,n={}){try{let r=await Pt(!0,()=>Dm(e,t,n),()=>Et({input:n.input??process.stdin,output:n.output??process.stdout}),{loadingMessage:e==="install"?"Checking available integrations…":"Checking installed integrations…",output:n.output??process.stdout});if(!r)return(n.output??process.stdout).write(`Cancelled: nothing was ${e}ed.
...
${h.red(y)}`);if(k)if(k.result.errors.length>0)yg(k.path,k.result.errors);else hg(k.path,k.result);if(m)return console.error(`
