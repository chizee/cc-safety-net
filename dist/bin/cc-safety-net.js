...
event = "PreToolUse"
command = "${KIMI_HOOK_COMMAND}"`,KIMI_INLINE_HOOK=`{ event = "PreToolUse", command = "${KIMI_HOOK_COMMAND}" }`;function getKimiConfigPath(homeDir){return join16(process.env.KIMI_CODE_HOME??join16(homeDir,".kimi-code"),"config.toml")}function removeTopLevelEmptyHooksArray(content){return content.split(`
...
${KIMI_HOOK_BLOCK}
`}function removeKimiTableHookBlocks(content){return content.split(/(?=^\s*\[)/m).filter((block)=>!/^\s*\[\[hooks]]\s*$/m.test(block)||!block.includes(KIMI_HOOK_COMMAND)).join("").trimEnd()}function removeKimiInlineHook(content,hooksRange){let itemStart=content.indexOf(KIMI_INLINE_HOOK,hooksRange.start);if(itemStart===-1||itemStart>hooksRange.end)return content;return removeArrayRangeItem(content,{start:itemStart,end:itemStart+KIMI_INLINE_HOOK.length})}function installKimiCode(homeDir){let configPath=getKimiConfigPath(homeDir);if(mkdirSync4(dirname7(configPath),{recursive:!0}),!existsSync14(configPath))return atomicWriteFile(configPath,`${KIMI_HOOK_BLOCK}
`),{path:configPath,alreadyInstalled:!1};let content=readFileSync13(configPath,"utf-8");if(content.includes(KIMI_HOOK_COMMAND))return{path:configPath,alreadyInstalled:!0};return atomicWriteFile(configPath,appendKimiHook(content)),{path:configPath,alreadyInstalled:!1}}function uninstallKimiCode(homeDir){let configPath=getKimiConfigPath(homeDir);if(!existsSync14(configPath))return{path:configPath,alreadyInstalled:!1};let content=readFileSync13(configPath,"utf-8");if(!content.includes(KIMI_HOOK_COMMAND))return{path:configPath,alreadyInstalled:!1};let inlineHooksRange=findTopLevelInlineHooksArray(content),updated=inlineHooksRange?removeKimiInlineHook(content,inlineHooksRange):`${removeKimiTableHookBlocks(content)}
