...
`)}function Rc(e){let{input:t,output:n}=e;Ee.emitKeypressEvents(t);let r=t.isRaw===!0;t.setRawMode(!0),t.resume();let o=0,i=()=>{if(o===0)return;Ee.moveCursor(n,0,-o),Ee.cursorTo(n,0),Ee.clearScreenDown(n)},s=()=>{i();let a=e.render();n.write(`${a}
`),o=a.split(`
...
`))].join(`
`)}function cd(e,t){return t.map((n)=>e==="install"&&n.target==="kimi-code"&&n.unavailableReason==="already installed"?{...n,available:!0,unavailableReason:void 0,label:`${n.label} (global hook installed)`}:n)}function E1(e,t){if(e.selectKimiInstallMethod)return e.selectKimiInstallMethod();if(!Ho(e.input,e.output))return Promise.resolve("global-hook");return Dc({input:e.input,output:e.output,globalHookInstalled:zt({homeDir:t,cwd:process.cwd()}).status==="configured"})}async function gd(e,t,n,r=!1,o){return P1[t][e](n,r,o)}function T1(e){let t=E({label:"update"},e).errors[0];if(t)throw Error(t)}async function I1(e,t=at){let n=await md(e,t),r=Ko(Ut(e),"installed-plugins");return{targets:Bo([...n.hooks.filter((i)=>i.platform!=="copilot-cli"&&i.detected).map((i)=>i.platform),...[An,Va,qa].flatMap((i)=>Wo(Ko(r,...i))?["copilot-cli"]:[]),...Pn(e,Jo)?["claude-code"]:[],...Yo(n.codexPluginListOutput)?["codex"]:[]]),codexPluginListOutput:n.codexPluginListOutput}}async function _1(e){let t=er(),n=e.output??process.stdout,r=(e.scriptPath??process.argv[1]??"").split(/[\\/]/),o=r.find((u)=>/^bunx-\d+-/.test(u)),i=o!==void 0||r.includes("_npx")?null:(e.checkLatestVersion??Pe)(),s=async()=>{let u=i&&await i;if(u?.updateAvailable)n.write(`
...
`;var Wd='<script id="ccsn-data" type="application/json">';function Yd(e){return Jd.replace(Wd,()=>Wd+JSON.stringify({token:e}).replaceAll("<","\\u003c"))}var lr="kenryu42/cc-safety-net",Xh=`https://github.com/${lr}`,ii=1e4,Qh=7;async function au(e,t={}){let n=E({label:"gui",booleans:{noOpen:["--no-open"]}},e),r=t.log??console.log,o=t.error??console.error;if(n.errors.length>0){for(let s of n.errors)o(s);return o("Usage: cc-safety-net gui [--no-open]"),1}let i=await e0(t);if(r(`CC Safety Net policy GUI: ${i.url}`),!n.flags.noOpen)try{await(t.openBrowser??c0)(i.url)}catch(s){o(`Failed to open browser: ${s instanceof Error?s.message:String(s)}`),o(`Open this URL manually: ${i.url}`)}if(t.keepAlive===!1)return await i.close(),0;return await l0(i),0}async function e0(e={}){let t=Wh(24).toString("base64url"),n=Yh((i,s)=>{t0(i,s,t,e)});await new Promise((i,s)=>{n.once("error",s),n.listen(0,"127.0.0.1",()=>{n.off("error",s),i()})});let o=`http://127.0.0.1:${n.address().port}`;return{origin:o,token:t,url:`${o}/?token=${encodeURIComponent(t)}`,close:()=>a0(n)}}async function t0(e,t,n,r){let o=new URL(e.url??"/","http://127.0.0.1");if(e.method==="GET"&&o.pathname==="/favicon.ico"){t.writeHead(204,{"cache-control":"no-store"}),t.end();return}if(!o0(e,o,n)){D(t,403,{error:"Forbidden"});return}if(e.method==="GET"&&o.pathname==="/"){s0(t,Yd(n));return}if(e.method==="GET"&&o.pathname==="/api/policy"){let i=eu(r),s=V(r);D(t,200,{...i,configState:It(s),...s.policyScopes?{projectPolicy:{path:Ne(r.cwd),weakenings:s.policyScopes.weakenings}}:{},destructiveCommandRules:Pt,secretPatterns:Xd,version:_(),preview:i.errors.length>0?null:nu(i.policy)});return}if(e.method==="POST"&&o.pathname==="/api/policy/preview"){let i=await ar(e);if(!i.ok){D(t,i.status,{errors:[i.error]});return}let s=tu(i.value);D(t,s.errors.length>0?400:200,s);return}if(e.method==="POST"&&o.pathname==="/api/policy/explain"){let i=await ar(e);if(!i.ok){D(t,i.status,{errors:[i.error]});return}let s=i.value;if(s===null||typeof s.command!=="string"){D(t,400,{errors:["command must be a string"]});return}let a=Tt(s.policy);if(a.length>0){D(t,400,{errors:a});return}D(t,200,n0(s.command,s.policy,r));return}if(e.method==="POST"&&o.pathname==="/api/policy"){let i=await ar(e);if(!i.ok){D(t,i.status,{errors:[i.error]});return}let s=tn(i.value,r);D(t,s.errors.length>0?400:200,s);return}if(e.method==="POST"&&o.pathname==="/api/reset"){D(t,200,tn(Qd,r));return}if(e.method==="POST"&&o.pathname==="/api/repair"){D(t,200,ru(r));return}if(e.method==="GET"&&o.pathname==="/api/activity"){let i=ot(r),s=r0(o.searchParams.get("days"),i);if(s===null){D(t,400,{error:`days must be an integer between 1 and ${i}`});return}D(t,200,Gd(s,r.activityLogsDir));return}if(e.method==="POST"&&o.pathname==="/api/rules/choose-directory"){D(t,200,await Kd());return}if(e.method==="GET"&&o.pathname==="/api/rules"){let i=Se(r),s=new Map(i.rules.map((a)=>[a.name,a]));D(t,200,{projectPath:r.cwd??process.cwd(),canPickDirectory:zd(process.platform,process.env),rulebooks:i.rulebooks.map((a)=>({source:a.source,spec:a.spec,name:a.name,version:a.version,rules:a.rules.flatMap((l)=>{let c=s.get(l);if(!c)return[];return[{name:c.name,command:c.command,subcommand:c.subcommand,block_args:c.block_args,reason:c.reason}]})})),errors:i.errors,warnings:i.warnings});return}if(e.method==="GET"&&o.pathname==="/api/star/context"){D(t,200,await(r.fetchStarContext??(()=>m0({logsDir:r.activityLogsDir})))());return}if(e.method==="POST"&&o.pathname==="/api/star"){let i=await(r.starRepo??d0)();D(t,200,i.ok?{ok:!0}:{ok:!1,fallbackUrl:Xh});return}if(e.method==="GET"&&o.pathname==="/api/integrations"){D(t,200,await(r.fetchIntegrations??u0)());return}if(e.method==="GET"&&o.pathname==="/api/health"){D(t,200,await(r.fetchHealth??p0)());return}if(e.method==="POST"&&(o.pathname==="/api/install"||o.pathname==="/api/uninstall")){let i=await ar(e);if(!i.ok){D(t,i.status,{errors:[i.error]});return}let s=i.value?.target;if(typeof s!=="string"||!de.some((l)=>l.target===s)){D(t,400,{error:"unknown target"});return}let a=o.pathname==="/api/install"?"install":"uninstall";D(t,200,await(r.runIntegration??f0)(a,s));return}D(t,404,{error:"Not found"})}function n0(e,t,n){let r=Te(t),o=V(n),i=At({rules:o.policy.rules,transparentWrappers:o.policy.transparentWrappers,safety:iu(r.safety),worktreeMode:r.workflow.worktree_mode,destructiveCommandProtectionEnabled:r.destructive_command_protection.enabled,destructiveCommandRuleOverrides:r.destructive_command_protection.overrides,destructiveCommandAllowPaths:r.destructive_command_protection.allow_paths,secretProtection:{enabled:r.secret_protection.enabled,disabledRules:ou(r.secret_protection.overrides),denyPaths:r.secret_protection.deny_paths,allowPaths:r.secret_protection.allow_paths}});return rt(e,{policySnapshot:i,cwd:n.cwd,userConfigDir:n.userConfigDir})}function r0(e,t){if(e===null)return Math.min(Qh,t);let n=Number(e);if(!Number.isInteger(n)||n<1||n>t)return null;return n}function o0(e,t,n){if(t.searchParams.get("token")!==n)return!1;if(e.method!=="POST")return!0;return e.headers["x-cc-safety-net-token"]===n}var i0=1048576;async function ar(e){let t=[],n=0;for await(let r of e){let o=r;if(n+=o.byteLength,n>i0)return{ok:!1,status:413,error:"Request body is too large"};t.push(o)}try{return{ok:!0,value:JSON.parse(Buffer.concat(t).toString("utf-8")||"{}")}}catch(r){return{ok:!1,status:400,error:`Invalid JSON: ${r instanceof Error?r.message:String(r)}`}}}function s0(e,t){e.writeHead(200,{"content-type":"text/html; charset=utf-8","cache-control":"no-store"}),e.end(t)}function D(e,t,n){e.writeHead(t,{"content-type":"application/json; charset=utf-8","cache-control":"no-store"}),e.end(JSON.stringify(n))}function a0(e){return new Promise((t,n)=>{e.close((r)=>r?n(r):t())})}function l0(e){return new Promise((t)=>{let n=()=>{process.off("SIGINT",r),process.off("SIGTERM",r)},r=()=>{n(),e.close().then(t)};process.once("SIGINT",r),process.once("SIGTERM",r)})}function c0(e){let t=process.platform==="darwin"?"open":process.platform==="win32"?"cmd":"xdg-open",n=process.platform==="win32"?["/c","start","",e]:[e];return new Promise((r,o)=>{let i=su(t,n,{detached:!0,stdio:"ignore"}),s=(l)=>{i.off("spawn",a),o(l)},a=()=>{i.off("error",s),i.unref(),r()};i.once("error",s),i.once("spawn",a)})}async function d0(e="gh",t=ii){return{ok:await oi(e,["api","-X","PUT",`/user/starred/${lr}`],t)===0}}async function u0(e={}){let t=await Ft(e.fetcher),n=lu(t,e.homeDir);return{targets:Y.map((r)=>{let o=n.find((i)=>i.platform===r.id);return{target:r.id,label:P(r.id),version:t.versions[r.id]??null,status:o?.configured?"active":o?.detected?"disabled":o?.inspectionStatus==="not-inspected"?"not-inspected":"not-installed"}}),system:{version:t.version,nodeVersion:t.nodeVersion,platform:t.platform}}}function lu(e,t){return ft(process.cwd(),{homeDir:t,ampPluginListOutput:e.ampPluginListOutput,codexPluginListOutput:e.codexPluginListOutput,copilotCliVersion:e.versions["copilot-cli"]})}async function p0(e={}){let[t,n]=await Promise.all([Ft(e.fetcher),(e.checkUpdates??Pe)()]);return{hooks:lu(t,e.homeDir).filter((r)=>r.detected).map((r)=>({platform:r.platform,label:P(r.platform),configured:r.configured})),update:{currentVersion:n.currentVersion,latestVersion:n.latestVersion??null,updateAvailable:n.updateAvailable}}}var Zd=Promise.resolve();function f0(e,t,n={}){let r=async()=>{let i=[],s=console.log,a=console.error;console.log=(...l)=>i.push(l.map(String).join(" ")),console.error=console.log;try{return{ok:await en(e,[],{selectTargets:async()=>[t],output:new Zh({write(c,d,m){i.push(String(c).replace(/\n$/,"")),m()}}),...n})===0,output:i.join(`
`)}}finally{console.log=s,console.error=a}},o=Zd.then(r);return Zd=o.then(()=>{return},()=>{return}),o}async function m0(e={}){let[t,n,r]=await Promise.all([g0(e.command),h0(e.fetchRepo),Promise.resolve(bn(ot(),e.logsDir).totalBlocked)]);return{starred:t,starCount:n,blockedTotal:r}}async function g0(e="gh",t=ii){if(await oi(e,["auth","status"],t)!==0)return null;let n=await oi(e,["api",`/user/starred/${lr}`],t);if(n===0)return!0;if(n===null)return null;return!1}function oi(e,t,n){return new Promise((r)=>{let o=su(e,t,{stdio:"ignore",windowsHide:!0}),i=!1,s,a=(l)=>{if(i)return;if(i=!0,s)clearTimeout(s);r(l)};o.once("error",()=>a(null)),o.once("close",a),s=setTimeout(()=>{o.kill(),a(null)},n)})}async function h0(e=fetch){try{let t=await e(`https://api.github.com/repos/${lr}`,{headers:{accept:"application/vnd.github+json"},signal:AbortSignal.timeout(ii)});if(!t.ok)return null;let n=await t.json();return typeof n.stargazers_count==="number"?n.stargazers_count:null}catch{return null}}function y0(e){if(e[0]!=="help")return!1;let t=e[1];if(!t)Fo(),process.exit(0);if(Yt(t))process.exit(0);console.error(`Unknown command: ${t}`),console.error("Run 'cc-safety-net --help' for available commands."),process.exit(1)}var v0={hook:async(e)=>{let t=qs(e);if(t){await t.run();return}console.error("hook requires exactly one integration flag. Try: cc-safety-net hook --kimi-code"),Yt("hook",console.error),process.exit(1)},install:async(e)=>{process.exit(await en("install",e))},update:async(e)=>{process.exit(await Zo(e))},uninstall:async(e)=>{process.exit(await en("uninstall",e))},rule:async(e)=>{process.exit(await jd(e))},policy:async(e)=>{process.exit(await Ld(e))},status:async(e)=>{if(ue(E({label:"status"},e).errors))process.exit(1);Ud()},statusline:async(e)=>{let t=E({label:"statusline",booleans:{claudeCode:["-cc","--claude-code"]}},e);if(t.errors.length===0&&t.flags.claudeCode){await ni();return}if(ue(t.errors),!t.flags.claudeCode)console.error("statusline requires --claude-code (-cc)");Yt("statusline",console.error),process.exit(1)},doctor:async(e)=>{let t=Io(e);if(!t)process.exit(1);let n=await lc({json:t.json,skipUpdateCheck:t.skipUpdateCheck});process.exit(n)},logs:async(e)=>{process.exit(await ps(e))},gui:async(e)=>{process.exit(await au(e))},explain:async(e)=>{process.exit(await gc(e))}};async function b0(){let e=process.argv.slice(2),t=E({label:"cc-safety-net",booleans:{version:["-V","--version"]},positionals:"list"},e);if(y0(e))return;let n=e[0],r=n?vn(n):void 0;if(t.help&&r&&r.name!=="rule")Yt(r.name),process.exit(0);if(!n||t.help&&!r)Fo(),process.exit(0);if(t.flags.version)vc(),process.exit(0);if(r){await v0[r.name](e.slice(1));return}let o=Vs(n);if(o){await o.run();return}if(n==="--statusline"){await ni();return}console.error(n.startsWith("-")?`Unknown option: ${n}`:`Unknown command: ${n}`),console.error("Run 'cc-safety-net --help' for usage."),process.exit(1)}b0().catch((e)=>{console.error("CC Safety Net error:",e),process.exit(1)});
