...
`,()=>t())})}async function cl(e){let t=io(e);if(!t)return 1;try{let n=Ze(t.command,{cwd:t.cwd}),r=!!process.env.NO_COLOR||!process.stdout.isTTY;return await ll(t.json?lo(n):ao(n,{asciiOnly:r})),0}catch(n){if(!(n instanceof dr)&&!(n instanceof De)&&!(n instanceof pe))throw n;if(t.json)return await ll(JSON.stringify({error:n.message})),1;return console.error(n.message),1}}var dl="2.1.0",W="  ",Oe="cc-safety-net";function ul(e){return e.argument?`${e.flags} ${e.argument}`:e.flags}function vf(e){return Math.max(...e.map((t)=>ul(t).length))}function bf(e){return Math.max(...e.map((t)=>t.usage.length))}function wf(e){return Math.max(...e.map((t)=>`${Oe} ${t.usage}`.length))}function kf(e,t){let n=`${Oe} ${e.usage}`;return`${W}${n.padEnd(t+2)}${e.description}`}function ge(e,t){return`${W}${e.padEnd(Math.max(40,e.length+2))}${t}`}function wn(e,t=console.log){let n=[];if(n.push(`${Oe} ${e.name}`),n.push(""),n.push(`${W}${e.description}`),n.push(""),n.push("USAGE:"),n.push(`${W}${Oe} ${e.usage}`),n.push(""),e.subcommands&&e.subcommands.length>0){n.push("SUBCOMMANDS:");let r=bf(e.subcommands);for(let o of e.subcommands)n.push(`${W}${o.usage.padEnd(r+2)}${o.description}`);n.push("")}if(e.options.length>0){n.push("OPTIONS:");let r=vf(e.options);for(let o of e.options){let i=ul(o),s=o.default?`${o.description} (default: ${o.default})`:o.description;n.push(`${W}${i.padEnd(r+2)}${s}`)}n.push("")}if(e.examples&&e.examples.length>0){n.push("EXAMPLES:");for(let r of e.examples)n.push(`${W}${r}`)}t(n.join(`
