...
`)}function Es(n){let t=["Time","Command"],r=n.map((e)=>{let a=z(e.command.replace(/\r\n|\r|\n/g," ↵ ").replace(/\t/g," ")),l=a.length>40?`${a.slice(0,37)}...`:a;return[e.relativeTime,l]});return m2({headers:t,rows:r})}function La(n){let t=[];if(t.push("Update Check"),n.latestVersion===null&&!n.error)return t.push(Kn([["Status",b.dim("Skipped")],["Installed",n.currentVersion]])),t.join(`
`);if(n.error)return t.push(Kn([["Status",`${b.yellow("⚠")} Error`],["Installed",n.currentVersion],["Error",b.dim(n.error)]])),t.join(`
`);if(n.updateAvailable)return t.push(Kn([["Status",`${b.yellow("⚠")} Update Available`],["Current",n.currentVersion],["Latest",b.green(n.latestVersion??"")]])),t.push(""),t.push("   Run: bunx cc-safety-net@latest doctor"),t.push("   Or:  npx cc-safety-net@latest doctor"),t.join(`
`);return t.push(Kn([["Status",`${b.green("✓")} Up to date`],["Version",n.currentVersion]])),t.join(`
`)}function Kn(n){return m2({rows:n})}function ia(n){let t=[];return t.push("System Info"),t.push(_s(n)),t.join(`
`)}function _s(n){let t=["Component","Version"],r=(l)=>{if(l===null)return b.dim("not found");return l},a=[{label:"cc-safety-net",value:n.version},{label:"Claude Code",value:n.claudeCodeVersion},{label:"Amp Code",value:n.ampVersion},{label:"Antigravity CLI",value:n.antigravityCliVersion},{label:"Codex",value:n.codexCliVersion},{label:"Cursor",value:n.cursorVersion},{label:"Gemini CLI",value:n.geminiCliVersion},{label:"GitHub Copilot CLI",value:n.copilotCliVersion},{label:"Hermes Agent",value:n.hermesAgentVersion},{label:"Kimi Code",value:n.kimiCodeVersion},{label:"OpenClaw",value:n.openClawVersion},{label:"OpenCode",value:n.openCodeVersion},{label:"Pi",value:n.piCliVersion},{label:"Node.js",value:n.nodeVersion},{label:"npm",value:n.npmVersion},{label:"Bun",value:n.bunVersion},{label:"Platform",value:n.platform}].map((l)=>[l.label,r(l.value)]);return m2({headers:t,rows:a})}function pa(n){if(n.findings.length===0)return b.green(`
...
`,r-1);if(l!==-1&&/^\s*$/.test(n.slice(l+1,r)))r=l}return`${n.slice(0,r)}${n.slice(e)}`}var Bn="npx -y cc-safety-net hook --kimi-code",Ir=`[[hooks]]
event = "PreToolUse"
command = "${Bn}"`,ne=`{ event = "PreToolUse", command = "${Bn}" }`;function ql(n){return Dl(process.env.KIMI_CODE_HOME??Dl(n,".kimi-code"),"config.toml")}function Ji(n){return n.split(`
...
`))].join(`
`)}function Rl(n,t){return t.map((r)=>n==="install"&&r.target==="kimi-code"&&r.unavailableReason==="already installed"?{...r,available:!0,unavailableReason:void 0,label:`${r.label} (global hook installed)`}:r)}function d5(n,t){if(n.selectKimiInstallMethod)return n.selectKimiInstallMethod();if(!Cr(n.input,n.output))return Promise.resolve("global-hook");return T0({input:n.input,output:n.output,globalHookInstalled:qn({homeDir:t,cwd:process.cwd()}).status==="configured"})}async function Ul(n,t,r,e=!1){return a5[t][n](r,e)}function s5(n){let t=F({label:"update"},n).errors[0];if(t)throw Error(t)}async function c5(n){let t=await Cl(n),r=ae(Dn(n),"installed-plugins");return Er([...t.hooks.filter((e)=>e.platform!=="copilot-cli"&&e.detected).map((e)=>e.platform),...[_n,Ya,Xa].flatMap((e)=>de(ae(r,...e))?["copilot-cli"]:[]),...En(n,le)?["claude-code"]:[],...se(t.codexPluginListOutput)?["codex"]:[]])}async function L5(){let n=ce(),t=await c5(n);if(t.length===0)return console.log("No installed integrations found. Run `cc-safety-net install` to set one up."),0;let r=new Set(t),e=new Map(await Promise.all(C.filter((l)=>r.has(l.target)&&Hl.has(l.target)).map(async(l)=>[l.target,await Pr(l.probeCommand)]))),a=[];for(let l of t){if(Hl.has(l)&&!e.get(l)){console.log(`${b2(l)} not found; skipped`);continue}try{console.log(await Ul("install",l,n,!0))}catch(d){console.error(ie(d)),a.push(l)}}return a.length>0?1:0}function Le(n){return Promise.resolve().then(()=>s5(n)).then(L5).catch((t)=>{return console.error(ie(t)),1})}async function Jn(n,t,r={}){try{let e=await Un(!0,()=>n5(n,t,r),()=>Cn({input:r.input??process.stdin,output:r.output??process.stdout}),{loadingMessage:n==="install"?"Checking available integrations…":"Checking installed integrations…",output:r.output??process.stdout});if(!e)return(r.output??process.stdout).write(`Cancelled: nothing was ${n}ed.
`),0;if(e==="update")return(r.runUpdate??(()=>Le([])))();let a=ce(),l=r.output??process.stdout;return await fl(e,async(d)=>{if(d==="kimi-code"&&n==="install"){let c=await d5(r,a);if(c===null){l.write(`Cancelled: Kimi Code integration was not installed.
...
`)}),0}catch(e){return console.error(ie(e)),1}}function ie(n){let t=n instanceof Error?n.message:String(n),r=typeof n==="object"&&n!==null&&"code"in n?n.code:null;if(r==="EACCES"||r==="EPERM")return`${t}
