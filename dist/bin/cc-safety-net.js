#!/usr/bin/env node
import{$ as Lt,$a as Pr,A as bn,Aa as Hr,B as rl,Ba as dt,C as I,Ca as Wr,D as O,Da as Ar,E as P1,Ea as T,F as R1,Fa as C1,G as Vn,Ga as I1,H as V1,Ha as N1,I as C2,Ia as E,J as $2,Ja as ql,K as nt,Ka as hn,L as L2,La as jl,M as xe,Ma as $r,N as st,Na as zl,O as Mn,Oa as Fl,P as $e,Pa as Bl,Q as De,Qa as v1,R as Cn,Ra as Jl,S as Un,Sa as Zl,T as xn,Ta as se,U as $n,Ua as Le,V as ne,Va as ce,W as on,Wa as ie,X as D2,Xa as dl,Y as tt,Ya as Er,Z as U2,Za as _r,_ as ge,_a as U1,aa as qe,ab as Ir,b as qr,ba as je,bb as u2,c as jr,ca as te,cb as x2,d as A,da as Zr,db as d2,e as zr,ea as re,eb as Nr,f as H1,fa as ee,fb as K,g as Q,ga as ae,gb as wn,h as g,ha as rt,hb as fn,i as pn,ia as Qr,ib as ke,j as D,ja as m,jb as Dn,k as w2,ka as et,kb as c2,l as Mr,la as me,lb as Ye,m as A2,ma as s2,mb as Gn,n as vn,na as yn,o as pl,oa as at,p as K1,pa as lt,q as Gr,qa as le,r as un,ra as de,s as Cr,sa as Xr,t as _1,ta as M2,u as Ur,ua as M1,v as Or,va as G1,w as _,wa as Tr,x as P,xa as H2,y as R2,ya as kl,z as V2,za as Sr}from"../chunks/index-bmvm3x9s.js";var Wl=["-h","--help"];function F(n,t){let r=Object.entries(n.booleans??{}),e=Object.entries(n.values??{}),a=Object.fromEntries(r.map(([i])=>[i,!1])),l={},d=[],s=[],L=!1,c=-1;for(let[i,v]of t.entries()){if(i<=c)continue;if(v==="--"){d.push(...t.slice(i+1));break}if(Wl.includes(v)){L=!0;continue}let b=r.find(([,f])=>f.includes(v));if(b){a[b[0]]=!0;continue}let p=e.find(([,f])=>f.includes(v));if(p){let f=t[i+1];if(f===void 0||f.startsWith("-")){s.push(`${v} requires a value`);continue}l[p[0]]=f,c=i+1;continue}if(v.startsWith("-")){s.push(`Unknown option for ${n.label}: ${v}`);continue}if(n.positionals==="tail"){d.push(...t.slice(i));break}d.push(v)}if(n.positionals!=="list"&&n.positionals!=="tail")s.push(...d.map((i)=>`Unexpected argument for ${n.label}: ${i}`));return{flags:a,values:l,positionals:d,help:L,errors:s}}function a2(n){for(let t of n)console.error(t);return n.length>0}import{readdirSync as gd,statSync as ve,unlinkSync as md}from"node:fs";import{basename as ue,dirname as kd,join as qd,resolve as jd}from"node:path";function B(n){return Array.from(n,(t)=>{let r=t.charCodeAt(0);if(r<=31||r>=127&&r<=159)return`\\x${r.toString(16).padStart(2,"0")}`;return t}).join("")}var Q1=(n)=>{let t=Date.now()-new Date(n).getTime();if(!Number.isFinite(t))return"";let r=Math.floor(t/60000),e=Math.floor(r/60),a=Math.floor(e/24);if(a>0)return`${a}d ago`;if(e>0)return`${e}h ago`;if(r>0)return`${r}m ago`;return"just now"},X2=(n)=>{let t=(n??"").trim().split(/\s+/).filter((a)=>a&&!/^[A-Za-z_][A-Za-z0-9_]*=/.test(a)),r=t[0]?.split("/").pop();if(!r)return null;let e=t[1];return e&&/^[a-z][a-z0-9-]*$/.test(e)?`${r} ${e}`:r};import{existsSync as Al,readdirSync as Kl,readFileSync as Rl}from"node:fs";import{join as Vl}from"node:path";function v2(n,t){try{return Kl(n,{withFileTypes:!0,encoding:"utf8"}).flatMap((r)=>{let e=Vl(n,r.name);if(r.isDirectory())return v2(e,t);if(r.name.endsWith(".jsonl"))return[e];return[]})}catch{if(t&&Al(n))t.count++;return[]}}function X1(n){let t=(a)=>`${a.sessionId}
${X2(a.segment||a.command)}`,r=n.filter((a)=>a.decision!=="allow"),e=r.filter((a)=>a.sessionId).reduce((a,l)=>a.set(t(l),(a.get(t(l))??0)+1),new Map);return new Set(r.filter((a)=>a.failureStage||(e.get(t(a))??0)>=2))}function cn(n,t){try{return Rl(n,"utf-8").split(`
`).filter(Boolean).flatMap((r)=>{try{return[JSON.parse(r)]}catch{if(t)t.count++;return[]}})}catch{if(t)t.count++;return[]}}import{resolve as od}from"node:path";var Ml=["AKIA","ASIA","ghp_","gho_","ghu_","ghs_","ghr_","github_pat_","glpat-","xox","npm_","pypi-","rk_","sk-","sk_","gsk_","xai-","pplx-","bastn_","tgp_v1_","flp_","wfr_","fw_","fwp_","tp-","psk-"];function Fr(n){let t=0,r={allocateSegment(){return t++},getNextSegmentIndex(){return t},recordGlobal(e){n.record({kind:"step",scope:"global",step:e})},recordSegment(e,a=r.currentSegmentIndex){if(a===void 0)return;n.record({kind:"step",scope:"segment",segmentIndex:a,step:e})}};return r}function Br(n={}){let t=[],r=n.maxEvents??512,e={maxTextLength:n.maxTextLength??2048,maxListLength:n.maxListLength??128,maxObjectProperties:n.maxObjectProperties??n.maxListLength??128,maxDepth:n.maxDepth??16},a=0,l,d=new Set;return{record(s){if(l)return;try{if(!s||t.length>=r){a++;return}t.push(S1(Gl(s,e,d)))}catch{a++}},finish(s){if(l)return l;try{l=S1({events:Object.freeze(t),droppedEvents:a,terminal:Cl(s,e,d)})}catch{a++,l=Object.freeze({events:Object.freeze(t),droppedEvents:a,terminal:Object.freeze({result:"blocked",reason:"trace unavailable".slice(0,e.maxTextLength),segment:"trace unavailable".slice(0,e.maxTextLength)})})}return l}}}function Gl(n,t,r){if(n.kind!=="step")throw TypeError("invalid trace event");let{scope:e,step:a}=n;An(a,r,t);let l=Y2(a,t,r);if(e==="global")return{kind:"step",scope:"global",step:l};if(e!=="segment")throw TypeError("invalid trace event scope");return{kind:"step",scope:"segment",segmentIndex:n.segmentIndex,step:l}}function Cl(n,t,r){let e=n.result;if(e==="allowed")return Object.freeze({result:"allowed"});if(e!=="blocked")throw TypeError("invalid trace terminal");let a=n.ruleId;return Object.freeze({result:"blocked",reason:Y2(n.reason,t,r),segment:Y2(n.segment,t,r),...a?{ruleId:Y2(a,t,r)}:{}})}function An(n,t,r,e=0,a=new WeakSet){if(typeof n==="string"){let s=n.slice(0,r.maxTextLength);if(!H1(s))return;for(let L of zr(s))for(let c of L.match(/[^\s"'()$]+/g)??[])t.add(Jr(c));return}if(!n||typeof n!=="object"||e>=r.maxDepth||a.has(n))return;if(a.add(n),Array.isArray(n)){let s=Math.min(n.length,r.maxListLength);for(let L=0;L<s;L++)An(n[L],t,r,e+1,a);return}let l=0,d=new Set;for(let s in n){if(!Object.hasOwn(n,s))continue;if(l>=r.maxObjectProperties)break;l++,An(s,t,r);let L=Y1(s,r,t);if(d.has(L))continue;d.add(L),An(n[s],t,r,e+1,a)}}function Y2(n,t,r,e=0,a=new WeakSet){if(typeof n==="string")return Y1(n,t,r);if(!n||typeof n!=="object")return n;if(e>=t.maxDepth)return;if(a.has(n))return;if(a.add(n),Array.isArray(n)){let s=[],L=Math.min(n.length,t.maxListLength);for(let c=0;c<L;c++)s.push(Y2(n[c],t,r,e+1,a));return s}let l={},d=0;for(let s in n){if(!Object.hasOwn(n,s))continue;if(d>=t.maxObjectProperties)break;d++;let L=Y1(s,t,r);if(Object.hasOwn(l,L))continue;Object.defineProperty(l,L,{value:Y2(n[s],t,r,e+1,a),enumerable:!0,configurable:!0,writable:!0})}return l}function Y1(n,t,r){let e=n.slice(0,t.maxTextLength),a=H1(e)?jr(e):e,l=r.size>0?Ol(a,r):a;return(Ul(l)?qr(l):l).slice(0,t.maxTextLength)}function Ul(n){return n.includes("PRIVATE KEY")||n.includes("://")||n.includes("eyJ")||n.includes(":")&&/(?:authorization|cookie|x-api-key|api-key|(?:^|\s)(?:-u|--user)(?:\s|=))/i.test(n)||n.length>=14&&Ml.some((t)=>n.includes(t))||n.length>=49&&/\b[a-f0-9]{32}\.[A-Za-z0-9]{16}\b/.test(n)}function Ol(n,t){return n.replace(/[^\s"'()$]+/g,(r)=>t.has(Jr(r))?"<redacted>":r)}function Jr(n){let t=2166136261,r=2166136261;for(let e=0;e<n.length;e++)t=Math.imul(t^n.charCodeAt(e),16777619),r=Math.imul(r^n.charCodeAt(n.length-e-1),16777619);return`${t>>>0}:${r>>>0}:${n.length}`}function S1(n){if(n&&typeof n==="object"&&!Object.isFrozen(n)){for(let t of Object.values(n))S1(t);Object.freeze(n)}return n}function Yr(n,t,r,e){let a=e??Zr(),l=r??a.getCommandProgram(n,t.shell??"auto"),d=Br(),s=Fr(d),L=l.dialect==="powershell"?a.getCommandProgram(n,"posix"):l,c=Qr(L);s.recordGlobal({type:"parse",input:n,segments:c.map((b)=>[...b])});let i=Xr(n,{...t,analyzePartialProgram:!0,trace:s},l,a),v=s.getNextSegmentIndex();if(i&&v>0&&v<c.length)s.recordSegment({type:"segment-skipped",index:v,reason:"prior-segment-blocked"},v);return Object.freeze({decision:i,trace:d.finish(i?{result:"blocked",reason:i.reason,segment:i.evidence.find((b)=>b.kind==="command")?.segment??n,...i.ruleId?{ruleId:i.ruleId}:{}}:{result:"allowed"}),program:l})}import{resolve as Tl}from"node:path";function W1(n){let t=Sr().safeParse(n);return{errors:t.success?[]:Wr(t.error.issues),ruleNames:new Set(Hr(n).map((r)=>r.toLowerCase()))}}function A1(n){let t=Kr(n);if(!t.ok)return t.result;return W1(t.parsed)}function Kr(n){let t=[],r=new Set;try{let e=typeof n==="string"?pn(n):n,a=D(e);if(a===null)return t.push(`File not found: ${e.path}`),{ok:!1,result:{errors:t,ruleNames:r}};if(!a.trim())return t.push("Config file is empty"),{ok:!1,result:{errors:t,ruleNames:r}};return{ok:!0,parsed:JSON.parse(a)}}catch(e){if(e instanceof Q)return t.push(e.message),{ok:!1,result:{errors:t,ruleNames:r}};let a=e instanceof Error?e.message:String(e);return t.push(e instanceof SyntaxError?"Invalid JSON":a),{ok:!1,result:{errors:t,ruleNames:r}}}}function Rr(n){return Tl(n??process.cwd(),".safety-net.json")}function l2(n){let t=Kr(n);if(!t.ok)return t.result;let r=Ar(t.parsed);return{errors:r.errors,ruleNames:r.sources}}import{isAbsolute as El,join as Kn,relative as _l,resolve as Vr,sep as Pl}from"node:path";async function K2(n={}){let t=T1(n);return Il(t,await Rn(t,H2()))}function Il(n,t){if(!t.ok)return t;let r=O(n),e=[...new Set(d2(r.configPath,r.lockPath,n,r.filesystemScope))];if(e.length===0)return t;return{ok:!1,errors:e,warnings:t.warnings,entries:t.entries}}async function Rn(n,t,r,e={}){let a=null,l=!1;try{let d=O(n),s=C1(d.configTarget);if(!s.ok)return s.result;let L=s.config;if(n.check)return ad(L,d,n);a={target:d.lockTarget,content:D(d.lockTarget)};let c=U1(d.lockTarget);if(c.errors.some((y)=>y.startsWith("Unable to access ")))return{ok:!1,errors:c.errors,warnings:[],entries:[]};if(n.only&&c.errors.length>0)return{ok:!1,errors:c.errors,warnings:[],entries:[]};let i=c.errors.length>0?null:c.lock,v=n.only?Er(L,i,n.only):{ok:!0,specs:L.rules};if(!v.ok)return v.result;if(n.only&&!i&&v.specs.length<L.rules.length)return{ok:!1,errors:[`No lockfile available for partial update; run ${Or}`],warnings:[],entries:[]};let b=(await nd(v.specs,(y)=>Pr(y,d.configDir,n,i,d.filesystemScope,t),t)).map((y)=>ld(y,i,r));for(let y of b)Ld(y.content,y.entry,d.configDir,n,d.filesystemScope);let p=n.only?dd(L,i,b):b.map((y)=>y.entry);l=!0,E(d.lockTarget,{version:1,rulebooks:p},void 0,e._testAfterPolicyRename);let f=new Map(b.map((y)=>[y.entry.spec,y.rulebook.rules.length])),u=cd(p,d.configDir,n,d.filesystemScope,e);return{ok:!0,errors:[],warnings:u,entries:p.map((y)=>sd(y,f))}}catch(d){if(l&&a)try{S2(a.target,a.content)}catch(s){return W2(s)}return W2(d)}}async function O1(n,t={}){return Nl(n,T1(t),H2())}async function Nl(n,t,r,e={}){let a=null,l=!1;try{let d=O(t),s=D(d.configTarget);a={target:d.configTarget,content:s};let L=C1(d.configTarget);if(!L.ok)return L.result;let c=L.config,i=Ur(n)?await Ir(n,r):[{spec:n}],v=i.map((f)=>f.spec),b=[...new Set([...c.rules,...v])];if(b.length>M1)return td();if(b.length!==c.rules.length)l=!0,E(d.configTarget,{version:1,rules:b,overrides:c.overrides??{},transparent_wrappers:c.transparent_wrappers??[]},void 0,e._testAfterPolicyRename);let p=await Rn(t,r,new Map(i.filter((f)=>!!f.display_ref).map((f)=>[f.spec,f.display_ref])),e);if(!p.ok)S2(d.configTarget,s);return p}catch(d){if(l&&a)try{S2(a.target,a.content)}catch(s){return W2(s)}return W2(d)}}async function nd(n,t,r=H2()){if(n.length>M1)throw Error(G1);let e=Array(n.length),a=0,l,d=Array.from({length:Math.min(n.length,Tr.concurrency)},async()=>{while(!l){let s=a;if(s>=n.length)return;a++;try{e[s]=await t(n[s],s,r.controller.signal)}catch(L){if(!l)l={value:L},a=n.length,r.controller.abort(L);return}}});if(await Promise.all(d),l)throw l.value;return e}function td(){return{ok:!1,errors:[G1],warnings:[],entries:[]}}function T1(n){return{cwd:n.cwd,cacheConfigDir:n.cacheConfigDir,userConfigDir:n.userConfigDir,userConfigPath:n.userConfigPath,projectConfigPath:n.projectConfigPath,global:n.global,check:n.check,only:n.only,refresh:n.refresh}}function rd(n){return{...T1(n),deleteSource:n.deleteSource}}async function E1(n,t={}){try{return await ed(n,rd(t),{})}catch(r){return W2(r)}}async function ed(n,t,r){let e=O(t),a=T(e.configTarget);if(a.errors.length>0)return{ok:!1,errors:a.errors,warnings:[],entries:[]};if(!a.config)return{ok:!1,errors:[`No config found at ${e.configPath}`],warnings:[],entries:[]};let l=U1(e.lockTarget);if(l.errors.length>0)return{ok:!1,errors:l.errors,warnings:[],entries:[]};let d=_r(a.config.rules,l.lock,n);if(!d.ok)return d.result;let s=t.deleteSource?id(e.configDir,d.specs,l.lock,e.filesystemScope):{ok:!0,dirs:[]};if(!s.ok)return s.result;let L=D(e.configTarget);if(L===null)return W2(Error("Rules config is unavailable."));try{E(e.configTarget,{version:1,rules:a.config.rules.filter((v)=>!d.specs.includes(v)),overrides:a.config.overrides??{},transparent_wrappers:a.config.transparent_wrappers??[]},void 0,r._testAfterPolicyRename)}catch(v){throw S2(e.configTarget,L),v}let c=await Rn(t,H2(),void 0,r);if(!c.ok)return S2(e.configTarget,L),c;let i=vd(s.dirs,r,e.filesystemScope);if(!i.ok){S2(e.configTarget,L);let v=await Rn(t,H2(),void 0,r);if(!v.ok)return{ok:!1,errors:[...i.result.errors,...v.errors],warnings:v.warnings,entries:v.entries};return i.result}return c}async function ad(n,t,r){let e=Nr(n,t.lockPath,t.configDir,r,r.global?"user":"project",t.filesystemScope);return{ok:e.errors.length===0&&e.warnings.length===0,errors:[...e.errors,...e.warnings],warnings:[],entries:e.entries}}function ld(n,t,r){let e=t?.rulebooks.find((l)=>l.spec===n.entry.spec&&l.kind==="github"),a=r?.get(n.entry.spec)??(e?.kind==="github"?e.display_ref:void 0);if(!a||n.entry.kind!=="github")return n;return{...n,entry:{...n.entry,display_ref:a}}}function dd(n,t,r){let e=new Set(n.rules),a=new Set(t?.rulebooks.map((d)=>d.spec)??[]),l=new Map(r.map((d)=>[d.entry.spec,d.entry]));return[...(t?.rulebooks.filter((d)=>e.has(d.spec))??[]).map((d)=>l.get(d.spec)??d),...r.filter((d)=>!a.has(d.entry.spec)).map((d)=>d.entry)]}function sd(n,t){return{...n,ruleCount:t.get(n.spec)}}function Ld(n,t,r,e,a){let l=R1(t,V1(r,e));w2(g(a,l),n)}function cd(n,t,r,e,a){let l=V1(t,r),d=Vn(l),s=g(e,d),L=A2(s);if(!L)return[];let c=n.map((v)=>g(e,R1(v,l))),i=L.filter((v)=>v.kind==="directory").map((v)=>({directory:g(e,Kn(d,v.name)),identity:g(e,Kn(d,v.name,Cr))})).filter((v)=>!c.some((b)=>Mr(v.identity,b))).map((v)=>v.directory);for(let v of i)Gr(v);return i.flatMap((v)=>{try{return ud(v,a),[]}catch{return["Unable to prune rules policy cache safely."]}})}function id(n,t,r,e){let a=new Map(r?.rulebooks.map((c)=>[c.spec,c])??[]),l=t.flatMap((c)=>{let i=a.get(c);if(!i)return un.test(c)?[]:["--delete-source can only delete local rulebook sources"];return i.kind==="local-directory"?[]:["--delete-source can only delete local rulebook sources"]}),d=t.map((c)=>{let i=a.get(c);return Kn(n,i?.kind==="local-directory"?i.path:c)}),s=l.length>0?[]:d.flatMap((c)=>pd(n,c,e)),L=[...l,...s];return L.length>0?{ok:!1,result:{ok:!1,errors:L,warnings:[],entries:[]}}:{ok:!0,dirs:d}}function pd(n,t,r){let e=Vr(n),a=Vr(t),l=_l(e,a);if(l===""||l===".."||l.startsWith(`..${Pl}`)||El(l))return[`Refusing to delete local rulebook source outside ${n}: ${t}`];let d=g(r,a),s=A2(d);if(!s)return[`Local rulebook source directory not found: ${t}`];let L=s.find((c)=>c.name==="rulebook.json");if(!L)return[`Local rulebook source directory is missing rulebook.json: ${t}`];if(L.kind!=="file")throw new Q(r.label);if(D(g(r,Kn(a,"rulebook.json"))),s.length>1)return[`Local rulebook source directory contains extra files: ${t}. delete manually if you really want to remove the directory.`];return[]}function vd(n,t,r){let e=n.flatMap((a)=>{try{return bd(g(r,a),t),[]}catch(l){return[`Failed to delete local rulebook source ${a}: ${l instanceof Error?l.message:String(l)}`]}});return e.length>0?{ok:!1,result:{ok:!1,errors:e,warnings:[],entries:[]}}:{ok:!0}}function ud(n,t){if(t._testPruneRulebookCacheDir){t._testPruneRulebookCacheDir(n.path);return}K1(n)}function bd(n,t){if(t._testDeleteLocalSourceDir){t._testDeleteLocalSourceDir(n.path);return}K1(n)}function S2(n,t){if(t===null){vn(n);return}w2(n,t)}function W2(n){return{ok:!1,errors:[n instanceof Error?n.message:String(n)],warnings:[],entries:[]}}function G2(n,t){let r=yd(t),e=le(r),a={effectiveLevel:e.effectiveLevel,selectedPreset:r.policySnapshot.policy.safety.level??"standard",effectiveCapabilities:e.effectiveCapabilities,destructiveCommandRuleOverrides:r.policySnapshot.policy.destructiveCommandRuleOverrides},{configSource:l,configValid:d}=fd({cwd:t?.cwd,userConfigDir:t?.userConfigDir});if(!n||!n.trim())return{trace:{steps:[{type:"error",message:"No command provided"}],segments:[]},result:"allowed",configSource:l,configValid:d,...a};let s=hd(n,r);if(s)return{trace:{steps:[],segments:[{index:0,steps:[{type:"rule-check",ruleModule:s.ruleModule,ruleFunction:s.ruleFunction,matched:!0,reason:s.reason}]}]},result:"blocked",reason:A(s.reason),segment:A(s.target),...s.ruleId?{ruleId:A(s.ruleId)}:{},configSource:l,configValid:d,...a};let L=Yr(n,r),c=L.decision,i=c?.ruleId??wd(n,r),v=on.find((p)=>p.id===i&&p.activationCapability),b=v?e.policy.effectiveDestructiveCommandRules[v.id]:void 0;return{trace:$d(L.trace),result:c?"blocked":"allowed",reason:c?A(c.reason):void 0,segment:c?A(c.evidence.find((p)=>p.kind==="command")?.segment??n):void 0,ruleId:c?.ruleId?A(c.ruleId):void 0,customRule:xd(Dd(c?.ruleId,r.policySnapshot)),configSource:l,configValid:d,...a,...v&&b?{ruleActivation:{id:v.id,...b}}:{}}}function fd(n){let t=_(n?.cwd),r=n?.userConfigPath??P(n),e=I({cwd:n?.cwd,userConfigDir:n?.userConfigDir,userConfigPath:n?.userConfigPath});try{if(D(e.projectConfigTarget)!==null){if(l2(e.projectConfigTarget).errors.length===0)return{configSource:t,configValid:!0};return{configSource:t,configValid:!1}}}catch(a){if(a instanceof Q)return{configSource:t,configValid:!1};throw a}try{if(D(e.userConfigTarget)!==null){let a=l2(e.userConfigTarget);return{configSource:r,configValid:a.errors.length===0}}return{configSource:null,configValid:!0}}catch(a){if(a instanceof Q)return{configSource:r,configValid:!1};throw a}}function yd(n){let t=od(n?.cwd??process.cwd()),r=n?.policySnapshot??K({cwd:t,userConfigDir:n?.userConfigDir}),e=s2(r.policy);return{cwd:t,effectiveCwd:t,policySnapshot:r,environment:ne(),protectedGitMetadata:ae(t),effectiveCapabilities:e.capabilities,strict:n?.strict??e.strict,paranoidRm:e.paranoidRm,paranoidInterpreters:e.paranoidInterpreters,worktreeMode:e.worktreeMode}}function hd(n,t){let r=t.cwd??process.cwd(),e=te(M2("",{command:n},{kind:"command",shell:"posix"},{executionCwd:r,configCwd:r},n)),a=Le(e);if(a)return{reason:se,target:a.target,ruleId:"policy-protection",ruleModule:"policy-protection",ruleFunction:"findPolicyConfigMutationTarget"};let l=ee(e,t.protectedGitMetadata);if(l)return{reason:re,target:l.target,ruleId:"git-metadata-protection",ruleModule:"git-metadata-protection",ruleFunction:"findGitMetadataMutationTarget"};let d=t.policySnapshot.policy,s=d.secretProtection.enabled===!1?null:ie(e,d.secretProtection,{strict:t.strict});if(s)return{reason:ce,target:s.target,ruleId:s.ruleId,ruleModule:"secret-protection",ruleFunction:"findSensitiveTarget"};return null}function wd(n,t){let r=t.policySnapshot.policy,e=fn({...r,destructiveCommandProtectionEnabled:!0,destructiveCommandRuleOverrides:{...r.destructiveCommandRuleOverrides,...Object.fromEntries(on.flatMap((a)=>a.activationCapability?[[a.id,"on"]]:[]))}},t.policySnapshot.state==="degraded"?{diagnostics:t.policySnapshot.diagnostics,reason:t.policySnapshot.reason}:void 0);return de(n,{...t,policySnapshot:e,strict:!0,paranoidRm:!0,paranoidInterpreters:!0})?.ruleId}function xd(n){if(!n)return;return{id:A(n.id),...n.rulebook?{rulebook:{name:A(n.rulebook.name),version:A(n.rulebook.version)}}:{},...n.source?{source:A(n.source)}:{},...n.override?{override:{type:"reason",reason:A(n.override.reason)}}:{}}}function $d(n){let t=n.events.flatMap((e)=>e.kind==="step"&&e.scope==="global"?[e.step]:[]),r=new Map;for(let e of n.events){if(e.kind!=="step"||e.scope!=="segment")continue;let a=r.get(e.segmentIndex)??{index:e.segmentIndex,steps:[]};a.steps.push(e.step),r.set(e.segmentIndex,a)}return{steps:t,segments:[...r.values()]}}function Dd(n,t){let r=n?.replace(/^custom\./,"");if(!r||!t.policy.rules.some((e)=>e.name===r))return;return t.ruleMetadata[r]??Object.freeze({id:r})}function zd(n){let t=C2(),r=F({label:"logs",booleans:{all:["--all"],suspect:["--suspect"],json:["--json"],pruneLegacy:["--prune-legacy"],dryRun:["--dry-run"]},values:{id:["--id"],limit:["--limit"],since:["--since"],agent:["--agent"],rule:["--rule"],session:["--session"],project:["--project"]}},n);if(a2(r.errors))return null;if(r.values.id!==void 0&&!/^[a-f0-9]{16}$/.test(r.values.id))return console.error("--id must be 16 hexadecimal characters"),null;let e=r.values.limit===void 0?20:pe(r.values.limit);if(e===null)return console.error("--limit must be a positive number"),null;let a=r.values.since===void 0?Math.min(30,t):pe(r.values.since);if(a===null||a>t)return console.error(`--since must be a positive number of days no greater than ${t}`),null;let l={limit:e,limitExplicit:r.values.limit!==void 0,since:a,sinceExplicit:r.values.since!==void 0,all:r.flags.all,json:r.flags.json,suspect:r.flags.suspect,pruneLegacy:r.flags.pruneLegacy,dryRun:r.flags.dryRun,id:r.values.id,agent:r.values.agent,rule:r.values.rule,session:r.values.session,project:r.values.project===void 0?void 0:jd(r.values.project)};if(l.id&&(l.agent!==void 0||l.rule!==void 0||l.session!==void 0||l.project!==void 0||l.suspect||l.sinceExplicit||l.limitExplicit))return console.error("--id cannot be combined with --agent, --rule, --session, --project, --suspect, --since, or --limit"),null;if(l.pruneLegacy&&(l.id!==void 0||l.agent!==void 0||l.rule!==void 0||l.session!==void 0||l.project!==void 0||l.suspect||l.all||l.sinceExplicit||l.limitExplicit))return console.error("--prune-legacy cannot be combined with --id, --agent, --rule, --session, --project, --suspect, --all, --since, or --limit"),null;if(l.dryRun&&!l.pruneLegacy)return console.error("--dry-run requires --prune-legacy"),null;return l}async function be(n,t={}){let r=zd(n);if(!r)return 1;let e=t.logsDir??L2();if(r.pruneLegacy)return Fd(e,r.json,r.dryRun);if(!e)return console.log(r.json?"[]":r.id?`No retained audit log entry found for id ${B(r.id)}.`:"No audit log entries found."),0;$2(e);let a={count:0},l=v2(e,a).flatMap((i)=>cn(i,a).map((v)=>({entry:v,file:i})));if(a.count>0)console.error(`warning: ${a.count} audit log ${a.count===1?"source":"sources"} could not be read; these results are incomplete`);if(r.id)return Qd(l,r,t.timeZone);let d=Date.now()-r.since*24*60*60*1000,s=l.filter((i)=>Xd(i,r,e,d)),L=r.suspect?X1(s.map((i)=>i.entry)):null,c=(L?s.filter((i)=>L.has(i.entry)):s).sort((i,v)=>Date.parse(v.entry.ts)-Date.parse(i.entry.ts)).slice(0,r.limit);if(r.json)return console.log(JSON.stringify(c.map((i)=>i.entry),null,2)),0;if(c.length===0)return console.log("No audit log entries found."),0;for(let i of c)console.log(Hd(i.entry,t.timeZone));return 0}function Fd(n,t,r){let e=n?Jd(n).map((s)=>qd(n,s)):[];if(r)return Bd(e,t);let a=[],l=0,d=0;for(let s of e){let L=ve(s,{throwIfNoEntry:!1})?.size??0,c=Zd(s);if(c){a.push(`${ue(s)}: ${c}`);continue}l++,d+=L}if(t)return console.log(JSON.stringify({removedFiles:l,removedBytes:d,failedFiles:a.length})),a.length===0?0:1;console.log(l===0&&a.length===0?"No legacy audit log files found.":`Removed ${l} legacy audit log ${l===1?"file":"files"} (${oe(d)}).`);for(let s of a)console.error(`Could not remove ${B(s)}`);if(console.log("Nested v2 audit logs were not changed."),l>0)console.log("This deletion cannot be undone.");return a.length===0?0:1}function Bd(n,t){let r=n.reduce((e,a)=>e+(ve(a,{throwIfNoEntry:!1})?.size??0),0);if(t)return console.log(JSON.stringify({dryRun:!0,files:n.length,bytes:r})),0;if(console.log(n.length===0?"No legacy audit log files found.":`Would remove ${n.length} legacy audit log ${n.length===1?"file":"files"} (${oe(r)}).`),console.log("Nested v2 audit logs are not included."),n.length>0)console.log("Run the same command without --dry-run to delete them.");return 0}function Jd(n){try{return gd(n,{withFileTypes:!0}).filter((t)=>t.isFile()&&t.name.endsWith(".jsonl")).map((t)=>t.name)}catch{return[]}}function Zd(n){try{return md(n),null}catch(t){return t instanceof Error?t.message:String(t)}}function oe(n){let t=["B","KiB","MiB","GiB"],r=Math.min(Math.floor(Math.log2(Math.max(n,1))/10),t.length-1);return`${Math.round(n/1024**r*10)/10} ${t[r]}`}function Qd(n,t,r){let e=n.filter((l)=>l.entry.id===t.id);if(e.length>1)return console.error(`Multiple audit log entries found for id ${B(t.id??"")}.`),1;if(t.json)return console.log(JSON.stringify(e.map((l)=>l.entry),null,2)),0;let a=e[0];if(!a)return console.log(`No retained audit log entry found for id ${B(t.id??"")}.`),0;return console.log(Wd(a.entry,r)),0}function Xd(n,t,r,e){if(!t.all&&n.entry.decision==="allow")return!1;if(Date.parse(n.entry.ts)<e)return!1;if(t.agent!==void 0&&n.entry.agent!==t.agent)return!1;if(t.rule!==void 0&&n.entry.ruleId!==t.rule)return!1;if(t.session!==void 0&&!Yd(n,r,t.session))return!1;if(t.project!==void 0&&!Sd(n.entry.cwd,t.project))return!1;return!0}function Yd(n,t,r){if(n.entry.sessionId===r)return!0;return kd(n.file)===t&&ue(n.file,".jsonl")===r}function Sd(n,t){if(!n)return!1;return n===t||n.startsWith(`${t}/`)}function Hd(n,t){let r=B(n.id??"-"),e=B(n.decision??"deny"),a=n.cwd?`  [${B(n.cwd)}]`:"",l=n.segment||n.command,d=l===n.command?"":"↳ ",s=l.length>50?`${l.slice(0,50)}…`:l;return`${r.padEnd(16)}  ${B(fe(n.ts,t))}  ${e.padEnd(5)}  ${B(n.agent??"-").padEnd(15)}  ${B(n.ruleId??"-").padEnd(20)}  ${d}${B(s)}${a}`}function Wd(n,t){let r=(a)=>B(a===void 0||a===null||a===""?"-":a),e=n.shape?`${n.agent??"-"} (shape: ${n.shape})`:n.agent??"-";return[`id:        ${r(n.id)}`,`ts:        ${r(fe(n.ts,t))}`,`decision:  ${r(n.decision)}`,`agent:     ${r(e)}`,`level:     ${r(n.level)}`,`tool:      ${r(n.toolName)}`,`rule:      ${r(n.ruleId)}`,`intent:    ${r(n.intent)}`,`stage:     ${r(n.failureStage)}`,`error:     ${r(n.errorCode)}`,`session:   ${r(n.sessionId)}`,`cwd:       ${r(n.cwd)}`,`version:   ${r(n.v)}`,`truncated: ${r(n.truncated===!0?"yes":void 0)}`,`reason:    ${r(n.reason)}`,`command:   ${r(n.command)}`,`segment:   ${r(n.segment)}`].join(`
`)}function fe(n,t){let r=new Date(n);if(Number.isNaN(r.getTime()))return n;return new Intl.DateTimeFormat("sv-SE",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hourCycle:"h23",timeZone:t}).format(r)}function pe(n){let t=Number(n);return Number.isFinite(t)&&t>0?t:null}var ye={name:"doctor",aliases:["--doctor"],description:"Run diagnostic checks to verify installation and configuration",usage:"doctor [options]",options:[{flags:"--json",description:"Output diagnostics as JSON"},{flags:"--skip-update-check",description:"Skip npm registry version check"},{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net doctor","cc-safety-net doctor --json","cc-safety-net doctor --skip-update-check"]};var he={name:"explain",description:"Show step-by-step analysis trace of how a command would be analyzed",usage:"explain [options] <command>",argument:"<command>",options:[{flags:"--json",description:"Output analysis as JSON"},{flags:"--cwd",argument:"<path>",description:"Use custom working directory"},{flags:"-h, --help",description:"Show this help"}],examples:['cc-safety-net explain "git reset --hard"','cc-safety-net explain --json "rm -rf /"','cc-safety-net explain --cwd /tmp "git status"']};var we={name:"gui",description:"Open the local policy editor GUI",usage:"gui [options]",options:[{flags:"--no-open",description:"Print the URL without opening a browser"},{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net gui","cc-safety-net gui --no-open"]};import{isAbsolute as ze,relative as Ed}from"node:path";var Ad=8388608;function Kd(n,t){console.log(JSON.stringify(n($e(t))))}async function Rd(n){let t;try{t=(await ct(process.stdin)).trim()}catch{n({reason:"Failed to parse hook input JSON."});return}if(!t){n({reason:"Missing hook input JSON."});return}return it(t,n,"Failed to parse hook input JSON.")}async function ct(n){let t=[],r=0;for await(let e of n){let a=typeof e==="string"?Buffer.from(e,"utf-8"):Buffer.from(e.buffer,e.byteOffset,e.byteLength);if(r+=a.byteLength,r>Ad)throw Vd(n),Error("hook input byte limit exceeded");t.push(a)}return Buffer.concat(t,r).toString("utf-8")}function Vd(n){let t=n.destroy??n.cancel;if(!t)return;try{Promise.resolve(t.call(n)).catch(()=>{})}catch{}}function it(n,t,r){try{return JSON.parse(n)}catch{t({reason:r});return}}function Y(n,t){let r=t.get(n);return r?{kind:"command",shell:r}:{kind:ge(n)}}function N(n,t,r,e){let a=n===void 0?process.cwd():n,l=typeof a==="string"&&a.trim()!==""?c2([a]):void 0;if(l)return{configCwd:l,executionCwd:l};return H(e,t,r,Ud(a)),null}function H(n,t,r,e){let a;try{a=Lt(t)}catch(l){if(!(l instanceof U2))throw l}n(Mn({command:a,segment:e,toolName:r}))}async function Md(n){let t=await Rd(n.outputDeny);if(t===void 0)return;if(!t||typeof t!=="object"||Array.isArray(t)){H(n.outputDeny);return}if(!n.isSupported(t))return;let r=n.getAgent?.(t)??n.agent,e=n.agent===r?void 0:n.agent,a=Td(t),l=(p,f)=>{xe(p,()=>n.getSessionId(t),{agent:r,shape:e,toolName:f,cwd:a}),n.outputDeny(p)},d=n.getToolName(t);if(typeof d!=="string"||d.trim()===""){H((p)=>l(p),Od(t));return}let s=d,L=(p)=>l(p,s),c;try{c=n.getToolInput(t,s,L)}catch(p){if(!(p instanceof U2))throw p;H(L,void 0,s);return}if(!c.ok)return;let i=n.getContext(t,c.input,s,L);if(!i)return;let v;try{v=Lt(c.input)}catch(p){if(!(p instanceof U2))throw p;H(L,void 0,s);return}let b=M2(s,c.input,c.route,i,v??null);try{let p=Gn(b,{guard:{auditAllowed:me(),dependencies:n.guardDependencies},audit:{agent:r,shape:e,getSessionId:()=>n.getSessionId(t)}}),f=st(p,{includeEvidence:!0,toolName:p.stage==="command-analysis"?void 0:s});if(f){n.outputDeny(f);return}n.outputAllow?.()}catch(p){if(!(p instanceof ke))throw p;Gd(p);let f=st(p.evaluation,{includeEvidence:!0,toolName:p.evaluation.stage==="command-analysis"?void 0:s});if(f)n.outputDeny(f);return}}function Gd(n){if(!yn(m.debug))return;console.error(`CC Safety Net debug: ${Cd(n.stage)}: ${De(n.cause)}`)}function Cd(n){if(n==="policy-protection")return"hook policy protection failed";if(n==="config-load")return"hook config loading failed";if(n==="secret-protection")return"hook secret protection failed";return"hook analysis failed"}function Ud(n){return typeof n==="string"?n:void 0}function Od(n){if(!n||typeof n!=="object"||Array.isArray(n))return;if(Object.hasOwn(n,"tool_input"))return n.tool_input;let t=n.toolCall;if(t&&typeof t==="object"&&!Array.isArray(t))return t.args;return}function Td(n){if(!n||typeof n!=="object"||Array.isArray(n))return null;let t=n.cwd;if(typeof t==="string")return t;let r=n.toolCall;if(!r||typeof r!=="object"||Array.isArray(r))return null;let e=r.args;if(!e||typeof e!=="object"||Array.isArray(e))return null;let a=e.Cwd;return typeof a==="string"?a:null}async function S(n){let t=(a)=>Kd(n.createDenyOutput,a),r=n.createAllowOutput;await Md({...n,outputDeny:t,outputAllow:r?()=>console.log(JSON.stringify(r())):void 0})}var _d=new Map([["run_command","auto"]]),Pd=new Set(["absolutepath","directorypath","file_path","filepath","path","searchdirectory","searchpath","target_file","targetfile"]);function Fe(n){return Y(n,_d)}async function Be(){await S({agent:"antigravity-cli",createDenyOutput:(n)=>({decision:"deny",reason:n}),isSupported:()=>!0,getToolName:(n)=>n.toolCall?.name,getToolInput:(n,t)=>({ok:!0,input:rs(n.toolCall?.args,t),route:Fe(t)}),getContext:Id,getSessionId:(n)=>n.conversationId})}function Id(n,t,r,e){let l=ts(n).flatMap((c)=>{let i=c2([c]);return i?[i]:[]});if(!l[0])return g2(e,t,r),null;if(r!=="run_command"){let c;try{c=Nd(t,r,l)}catch(i){if(i instanceof U2)return g2(e,void 0,r),null;if(!(i instanceof Cn))throw i;return g2(e,t,r),null}if(!c)return g2(e,t,r),null;return{configCwd:c,executionCwd:c,policyConfigCwds:l}}let d=n.toolCall?.args;if(!d||!Object.hasOwn(d,"Cwd"))return{configCwd:l[0],executionCwd:l[0],policyConfigCwds:l};let s=d.Cwd;if(typeof s!=="string"||s.trim()==="")return g2(e,t,r),null;let L=Dn(s,l);if(L){let c=Je(L,l);if(!c)return g2(e,t,r,s),null;return{configCwd:c,executionCwd:L,policyConfigCwds:l}}return g2(e,t,r,s),null}function Nd(n,t,r){let e=Fe(t),a=[...qe(n,Pd),...e.kind==="patch"?je(n):[]].filter(ze),l=Un(),d=new Set(a.flatMap((s)=>{let L=Je(xn(s,$n,l),r);return L?[L]:[]}));if(d.size>1)return null;return[...d][0]??r[0]??null}function Je(n,t){return t.filter((r)=>ns(n,r)).reduce((r,e)=>e.length>r.length?e:r,"")||null}function ns(n,t){let r=Ed(t,n);return r===""||!r.startsWith("..")&&!ze(r)}function g2(n,t,r,e){let a=t&&typeof t==="object"?t.command:void 0;n(Mn({command:typeof a==="string"?a:void 0,segment:e,toolName:r}))}function ts(n){if(n.workspacePaths===void 0)return[process.cwd()];let t=Array.isArray(n.workspacePaths)?n.workspacePaths.filter((r)=>typeof r==="string"&&r.trim()!==""):[];return c2(t)?t:[]}function rs(n,t){if(!n)return;if(t!=="run_command")return n;return{...n,command:typeof n.CommandLine==="string"&&n.CommandLine!==""?n.CommandLine:void 0}}var O2=[{id:"antigravity-cli",displayName:"Antigravity CLI",doctorOrder:3,runtime:{order:1,flags:["-ac","--agy-cli"],description:"Run as Antigravity CLI PreToolUse hook",legacyTopLevelFlags:[]},install:{order:2,flag:"--agy-cli",installLabel:"Antigravity CLI",helpTarget:"Antigravity CLI hook config",probeCommand:["agy","--version"]}},{id:"claude-code",displayName:"Claude Code",doctorOrder:1,runtime:{order:2,displayName:"Coding CLI",flags:["-cc","--coding-cli"],legacyFlags:["--claude-code"],description:"Run as Coding CLI PreToolUse hook",legacyTopLevelFlags:["-cc","--claude-code"]},install:{order:3,flag:"--claude-code",installLabel:"Claude Code",helpTarget:"Claude Code plugin",probeCommand:["claude","--version"]}},{id:"codex",displayName:"Codex",doctorOrder:4,install:{order:4,flag:"--codex",installLabel:"Codex",helpTarget:"Codex plugin",probeCommand:["codex","--version"]}},{id:"copilot-cli",displayName:"GitHub Copilot CLI",doctorOrder:7,runtime:{order:5,flags:["-cp","--copilot-cli"],description:"Run as GitHub Copilot CLI PreToolUse hook",legacyTopLevelFlags:["-cp","--copilot-cli"]},install:{order:7,flag:"--copilot-cli",installLabel:"GitHub Copilot CLI",helpTarget:"GitHub Copilot CLI plugin",probeCommand:["copilot","--binary-version"]}},{id:"gemini-cli",displayName:"Gemini CLI",doctorOrder:6,runtime:{order:4,flags:["-gc","--gemini-cli"],description:"Run as Gemini CLI BeforeTool hook",legacyTopLevelFlags:["-gc","--gemini-cli"]},install:{order:6,flag:"--gemini-cli",installLabel:"Gemini CLI",helpTarget:"Gemini CLI extension",probeCommand:["gemini","--version"]}},{id:"hermes-agent",displayName:"Hermes Agent",doctorOrder:8,runtime:{order:6,flags:["-ha","--hermes-agent"],description:"Run as Hermes Agent pre_tool_call hook",legacyTopLevelFlags:[]},install:{order:8,flag:"--hermes-agent",installLabel:"Hermes Agent",helpTarget:"Hermes Agent plugin",probeCommand:["hermes","--version"]}},{id:"kimi-code",displayName:"Kimi Code",doctorOrder:9,runtime:{order:7,flags:["-kc","--kimi-code"],description:"Run as Kimi Code PreToolUse hook",legacyTopLevelFlags:[]},install:{order:9,flag:"--kimi-code",installLabel:"Kimi Code",helpTarget:"Kimi Code hook config",probeCommand:["kimi","--version"]}},{id:"openclaw",displayName:"OpenClaw",doctorOrder:10,install:{order:10,flag:"--openclaw",installLabel:"OpenClaw",helpTarget:"OpenClaw plugin",probeCommand:["openclaw","--version"]}},{id:"opencode",displayName:"OpenCode",doctorOrder:11,install:{order:11,flag:"--opencode",installLabel:"OpenCode",helpTarget:"OpenCode plugin",probeCommand:["opencode","--version"]}},{id:"pi",displayName:"Pi",doctorOrder:12,install:{order:12,flag:"--pi",installLabel:"Pi",helpTarget:"Pi package",probeCommand:["pi","--version"]}},{id:"cursor",displayName:"Cursor",doctorOrder:5,runtime:{order:3,flags:["-cu","--cursor"],description:"Run as Cursor preToolUse hook",legacyTopLevelFlags:[]},install:{order:5,flag:"--cursor",installLabel:"Cursor",helpTarget:"Cursor hook config",probeCommand:["cursor","--version"]}},{id:"amp",displayName:"Amp Code",doctorOrder:2,install:{order:1,flag:"--amp",installLabel:"Amp Code",helpTarget:"Amp Code plugin",probeCommand:["amp","--version"]}}],Ze=O2.slice().sort((n,t)=>n.doctorOrder-t.doctorOrder).map((n)=>n.id),Qe=O2.filter((n)=>("runtime"in n)).slice().sort((n,t)=>n.runtime.order-t.runtime.order).map((n)=>({id:n.id,displayName:"displayName"in n.runtime?n.runtime.displayName:n.displayName,flags:n.runtime.flags,legacyFlags:"legacyFlags"in n.runtime?n.runtime.legacyFlags:[],description:n.runtime.description,legacyTopLevelFlags:n.runtime.legacyTopLevelFlags})),i2=O2.slice().sort((n,t)=>n.install.order-t.install.order).map((n)=>({id:n.id,...n.install})).map(({order:n,...t})=>t),R6=Object.fromEntries(O2.map((n)=>[n.id,n.displayName]));function b2(n){return O2.find((t)=>t.id===n)?.displayName??n}function o2(n){return O2.find((t)=>t.id===n)?.install.installLabel??n}import{homedir as es}from"node:os";import{isAbsolute as Xe,join as pt}from"node:path";function Se(n){if(n!==void 0&&n!==null&&!Xe(n))return"unknown";try{let t=Un(),r=n?xn(n,$n,t):void 0,e=process.env.HOME||es(),a=[["codex",process.env.CODEX_HOME||pt(e,".codex")],["copilot-cli",process.env.COPILOT_HOME||pt(e,".copilot")],["claude-code",process.env.CLAUDE_CONFIG_DIR||pt(e,".claude")]],l=r?a.flatMap(([d,s])=>{if(!Xe(s))return[];return Ye(r,xn(s,$n,t))?[d]:[]}):[];if(l.length===1)return l[0]??"unknown";if(l.length>1)return"unknown"}catch(t){if(t instanceof Cn)return"unknown";return"unknown"}if(process.env.CLAUDECODE==="1"||Boolean(process.env.CLAUDE_CODE_ENTRYPOINT))return"claude-code";return"unknown"}var vt="PreToolUse",He="BeforeTool",We="pre_tool_call",ut="PreToolUse";var as=new Map([["Bash","posix"],["PowerShell","powershell"]]);function ls(n){return Y(n,as)}async function Ae(){await S({agent:"claude-code",getAgent:(n)=>Se(n.transcript_path),createDenyOutput:(n)=>({hookSpecificOutput:{hookEventName:vt,permissionDecision:"deny",permissionDecisionReason:n}}),isSupported:(n)=>n.hook_event_name===vt,getToolName:(n)=>n.tool_name,getToolInput:(n,t)=>({ok:!0,input:n.tool_input,route:ls(t)}),getContext:(n,t,r,e)=>N(n.cwd,t,r,e),getSessionId:(n)=>n.session_id})}var ds=new Map([["bash","auto"],["Bash","auto"]]);function ss(n){return Y(n,ds)}async function Ke(){await S({agent:"copilot-cli",createDenyOutput:(n)=>({permissionDecision:"deny",permissionDecisionReason:n}),isSupported:()=>!0,getToolName:(n)=>n.toolName,getToolInput:(n,t,r)=>{if(typeof n.toolArgs!=="string")return r({reason:"Failed to parse toolArgs JSON."}),{ok:!1};let e=it(n.toolArgs,r,"Failed to parse toolArgs JSON.");if(e===void 0)return{ok:!1};return{ok:!0,input:e,route:ss(t)}},getContext:(n,t,r,e)=>N(n.cwd,t,r,e),getSessionId:(n)=>typeof n.sessionId==="string"&&n.sessionId.trim()?n.sessionId:void 0})}var Ls=new Map([["Shell","auto"]]);function cs(n){return Y(n,Ls)}async function Re(){await S({agent:"cursor",createDenyOutput:(n)=>({permission:"deny",user_message:n,agent_message:n}),createAllowOutput:()=>({permission:"allow"}),isSupported:()=>!0,getToolName:(n)=>n.tool_name,getToolInput:(n,t)=>({ok:!0,input:n.tool_input,route:cs(t)}),getContext:is,getSessionId:(n)=>n.conversation_id})}function is(n,t,r,e){let a=ps(n);if(!a[0])return H(e,t,r),null;let l=Dn(us(n.cwd),a);if(!l)return H(e,t,r,typeof n.cwd==="string"?n.cwd:void 0),null;if(t===null||typeof t!=="object"||Array.isArray(t))return{configCwd:l,executionCwd:l,policyConfigCwds:a};if(!Object.hasOwn(t,"working_directory"))return{configCwd:l,executionCwd:l,policyConfigCwds:a};let d=t.working_directory;if(typeof d!=="string"||d.trim()==="")return H(e,t,r),null;let s=Dn(d,a);if(!s)return H(e,t,r,d),null;return{configCwd:l,executionCwd:s,policyConfigCwds:a}}function ps(n){return vs(n).flatMap((t)=>{let r=c2([t]);return r?[r]:[]})}function vs(n){if(n.workspace_roots===void 0)return typeof n.cwd==="string"&&n.cwd.trim()!==""?[n.cwd]:[];if(!Array.isArray(n.workspace_roots))return[];return n.workspace_roots.filter((t)=>typeof t==="string"&&t.trim()!=="")}function us(n){return typeof n==="string"&&n.trim()!==""?n:"."}var bs=new Map([["run_shell_command","auto"]]);function os(n){return Y(n,bs)}async function Ve(){await S({agent:"gemini-cli",createDenyOutput:(n)=>({decision:"deny",reason:n,systemMessage:n}),isSupported:(n)=>n.hook_event_name===He,getToolName:(n)=>n.tool_name,getToolInput:(n,t)=>({ok:!0,input:n.tool_input,route:os(t)}),getContext:(n,t,r,e)=>N(n.cwd,t,r,e),getSessionId:(n)=>n.session_id})}import{resolve as fs}from"node:path";var ys=new Map([["terminal","posix"]]);async function Me(){await S({agent:"hermes-agent",createDenyOutput:(n)=>({action:"block",message:n}),isSupported:(n)=>n.hook_event_name===We,getToolName:(n)=>n.tool_name,getToolInput:(n,t)=>({ok:!0,input:n.tool_input,route:Y(t,ys)}),getContext:hs,getSessionId:(n)=>n.session_id})}function hs(n,t,r,e){let a=N(n.cwd,t,r,e);if(!a)return null;if(!t||typeof t!=="object"||Array.isArray(t))return a;if(!Object.hasOwn(t,"workdir"))return a;let l=t.workdir;if(typeof l!=="string"||l.trim()==="")return H(e,t,r),null;let d=c2([fs(a.configCwd,l)]);if(!d)return H(e,t,r,l),null;return{...a,executionCwd:d}}var ws=new Map([["Bash","posix"]]);function xs(n){return Y(n,ws)}async function Ge(){await S({agent:"kimi-code",createDenyOutput:(n)=>({hookSpecificOutput:{hookEventName:ut,permissionDecision:"deny",permissionDecisionReason:n}}),isSupported:(n)=>n.hook_event_name===ut,getToolName:(n)=>n.tool_name,getToolInput:(n,t)=>({ok:!0,input:n.tool_input,route:xs(t)}),getContext:(n,t,r,e)=>N(n.cwd,t,r,e),getSessionId:(n)=>n.session_id})}var $s={"antigravity-cli":Be,"claude-code":Ae,"copilot-cli":Ke,cursor:Re,"gemini-cli":Ve,"hermes-agent":Me,"kimi-code":Ge},T2=Qe.map((n)=>({...n,run:$s[n.id]}));function Ce(n){let t=F({label:"hook",booleans:Object.fromEntries(T2.map((e)=>[e.id,[...e.flags,...e.legacyFlags]]))},n);if(t.errors.length>0)return;let r=T2.filter((e)=>t.flags[e.id]);return r.length===1?r[0]:void 0}function Ue(n){return T2.find((t)=>t.legacyTopLevelFlags.some((r)=>r===n))}var Ds=T2.map((n)=>({flags:n.flags.join(", "),description:n.description})),gs=T2.flatMap((n)=>n.flags.map((t)=>`cc-safety-net hook ${t}`)),Oe={name:"hook",description:"Run as an agent CLI hook (reads JSON from stdin)",usage:"hook INTEGRATION_FLAG",options:[...Ds,{flags:"-h, --help",description:"Show this help"}],examples:gs};var Te={name:"install",description:"Install CC Safety Net into a coding agent CLI",usage:"install [TARGET_FLAG]",options:[...i2.map((n)=>({flags:n.flag,description:`Install ${n.helpTarget}`})),{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net install",...i2.map((n)=>`cc-safety-net install ${n.flag}`)]},Ee={name:"uninstall",description:"Uninstall CC Safety Net from a coding agent CLI",usage:"uninstall [TARGET_FLAG]",options:[...i2.map((n)=>({flags:n.flag,description:`Uninstall ${n.helpTarget}`})),{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net uninstall",...i2.map((n)=>`cc-safety-net uninstall ${n.flag}`)]},_e={name:"update",description:"Update every installed CC Safety Net integration to the latest version",usage:"update",options:[{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net update"]};var Pe={name:"logs",description:"Browse audit log entries recorded by hooks",usage:"logs [options]",options:[{flags:"--id",argument:"<id>",description:"Show one entry from retained history by its 16-character id (not guaranteed once it is older than the configured retention)"},{flags:"--limit",argument:"<n>",description:"Maximum entries to print",default:"20"},{flags:"--since",argument:"<days>",description:"Only include entries newer than this many days (max: the configured audit retention, 1-365)",default:"30"},{flags:"--agent",argument:"<name>",description:"Filter by agent name"},{flags:"--rule",argument:"<ruleId>",description:"Filter by rule id"},{flags:"--session",argument:"<id>",description:"Filter by session id"},{flags:"--project",argument:"<path>",description:"Filter by project path"},{flags:"--suspect",description:"Only denials that look like false positives"},{flags:"--all",description:"Include allow entries"},{flags:"--prune-legacy",description:"Permanently delete all legacy root-level logs; nested logs are untouched"},{flags:"--dry-run",description:"With --prune-legacy, report what would be deleted and delete nothing"},{flags:"--json",description:"Output entries as JSON"},{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net logs --id 3fa9c2d1a70e8b42","cc-safety-net logs --agent claude-code","cc-safety-net logs --project . --since 7","cc-safety-net logs --suspect --since 7","cc-safety-net logs --json","cc-safety-net logs --prune-legacy --dry-run","cc-safety-net logs --prune-legacy"]};var E2={name:"rule",description:"Manage CC Safety Net rule config and rulebook sources",usage:"rule <subcommand>",subcommands:[{usage:"init [--example]",description:"Create inert rule config"},{usage:"add <source>",description:"Add a rulebook source and sync"},{usage:"remove <source>",description:"Remove a rulebook source and sync"},{usage:"update [source]",description:"Refresh rulebook lock/cache state"},{usage:"sync",description:"Sync configured rulebooks"},{usage:"list",description:"List active rulebooks"},{usage:"wrapper add <command>",description:"Trust a transparent command wrapper"},{usage:"wrapper remove <command>",description:"Remove a transparent command wrapper"},{usage:"wrapper list",description:"List transparent command wrappers"},{usage:"migrate [--cleanup]",description:"Migrate legacy inline rules"},{usage:"doc",description:"Print the rulebook authoring guide"},{usage:"verify",description:"Validate rule config files"}],options:[{flags:"-g, --global",description:"Use user-scope rule config"},{flags:"--check",description:"Check without changing lock/cache state"},{flags:"--cleanup",description:"Delete legacy files after rule migrate verifies them"},{flags:"--delete-source",description:"Delete clean local source directory on remove"},{flags:"--example",description:"Create an inactive example rulebook with rule init"},{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net rule init","cc-safety-net rule init --example","cc-safety-net rule wrapper add rtk","cc-safety-net rule add project-rules","cc-safety-net rule sync","cc-safety-net rule migrate --cleanup","cc-safety-net rule verify"]};var Ie={name:"status",description:"Show what the runtime is enforcing right now",usage:"status",options:[{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net status"]};var Ne={name:"statusline",description:"Print status line with mode indicators for shell integration",usage:"statusline --claude-code",options:[{flags:"-cc, --claude-code",description:"Print status line for Claude Code"},{flags:"-h, --help",description:"Show this help"}],examples:["cc-safety-net statusline -cc","cc-safety-net statusline --claude-code"]};var n0=[Ie,ye,Pe,he,E2,Te,_e,Ee,Oe,we,Ne];function ms(n){return n.aliases??[]}function ks(n){return!n.hidden}function On(n){let t=n.toLowerCase();return n0.find((r)=>r.name.toLowerCase()===t||ms(r).some((e)=>e.toLowerCase()===t))}function t0(){return n0.filter(ks)}import{readFileSync as qs}from"node:fs";import{basename as js}from"node:path";function Tn(n=7,t=L2()){let r=Date.now()-n*24*60*60*1000,e=[],a=new Set,l=0,d,s,L,c;if(t)$2(t);let i={count:0},v=t?v2(t,i):[];for(let p of v)try{let u=qs(p,"utf-8").trim().split(`
`).filter(Boolean);for(let y of u)try{let x=JSON.parse(y);if(x.decision==="allow")continue;let w=new Date(x.ts).getTime();if(w>=r){if(l++,a.add(x.sessionId??js(p,".jsonl")),s===void 0||w<=s)d=x.ts,s=w;if(c===void 0||w>c)L=x.ts,c=w;zs(e,x,w)}}catch{i.count++}}catch{i.count++}let b=e.map((p)=>({timestamp:p.ts,command:p.command,reason:p.reason,relativeTime:Q1(new Date(p.ts))}));return{totalBlocked:l,sessionCount:a.size,recentEntries:b,oldestEntry:d,newestEntry:L,unreadable:i.count}}function zs(n,t,r){let e=n.findIndex((a)=>r>new Date(a.ts).getTime());if(e===-1){if(n.length<3)n.push(t);return}if(n.splice(e,0,t),n.length>3)n.pop()}import{dirname as Fs}from"node:path";function r0(n,t,r,e,a){let l;try{if(D(e)===null)return{path:n,exists:!1,valid:!1,ruleCount:0};l=l2(e),l.errors.push(...d2(n,t,{userConfigDir:r},a))}catch(d){if(!(d instanceof Q))throw d;l={errors:[d.message],ruleNames:new Set}}return{path:n,exists:!0,valid:l.errors.length===0,ruleCount:l.ruleNames.size,...l.errors.length>0?{errors:l.errors}:{}}}function Bs(n,t){return{source:t,name:n.name,command:n.command,subcommand:n.subcommand,blockArgs:[...n.block_args],reason:n.reason}}function e0(n,t){let r=t?.userConfigPath??P(),e=t?.projectConfigPath??_(n),a=Fs(r),l=u2({cwd:n,userConfigPath:r,projectConfigPath:e,userConfigDir:a}),d=I({cwd:n,userConfigPath:r,projectConfigPath:e,userConfigDir:a}),s=new Map(l.rulebooks.flatMap((L)=>L.rules.map((c)=>[c,L.source])));return{userConfig:r0(r,R2({userConfigPath:r}),a,d.userConfigTarget,d.userScope),projectConfig:r0(e,V2(e),a,d.projectConfigTarget,d.projectScope),effectiveRules:l.rules.map((L)=>Bs(L,s.get(L.name)??"project")),shadowedRules:[]}}var Js=[{flag:m.level,description:"Safety level preset: standard, strict, or paranoid",defaultBehavior:"standard"},{flag:m.strict,description:"Legacy; equivalent to safety.overrides.fail_closed",defaultBehavior:"permissive"},{flag:m.paranoid,description:"Legacy; equivalent to safety.overrides.paranoid_rm and paranoid_interpreters",defaultBehavior:"off"},{flag:m.paranoidRm,description:"Legacy; equivalent to safety.overrides.paranoid_rm",defaultBehavior:"off"},{flag:m.paranoidInterpreters,description:"Legacy; equivalent to safety.overrides.paranoid_interpreters",defaultBehavior:"off"},{flag:m.worktree,description:"Allow local git discards in linked worktrees",defaultBehavior:"off"},{flag:m.debug,description:"Print diagnostic messages to stderr",defaultBehavior:"off"},{flag:m.auditScope,description:"Command decisions recorded: all, or blocked (privacy-minimizing, denials only)",defaultBehavior:"all"}];function a0(){return[...Js.map((n)=>({name:n.flag.name,value:at(n.flag),isSet:lt(n.flag),legacyName:n.flag.legacyName,legacyValue:n.flag.legacyName?process.env[n.flag.legacyName]:void 0,legacyIsSet:n.flag.legacyName?process.env[n.flag.legacyName]!==void 0:void 0,description:n.description,defaultBehavior:n.defaultBehavior})),{name:"CC_SAFETY_NET_HOME",value:process.env.CC_SAFETY_NET_HOME,isSet:process.env.CC_SAFETY_NET_HOME!==void 0,description:"Override user-scope config/cache directory",defaultBehavior:"~/.cc-safety-net"}]}var l0={error:0,warning:1,info:2},Zs=["policy","config","audit"];function Qs(n){return n.map((t)=>{if(t==="ownership")return"is not owned by the current user";if(t==="permissions")return"has unsafe permissions";if(t==="symlink")return"is a symbolic link";return"is not a directory"}).join(" and ")}var Xs=[{derive:(n)=>n.hooks.length>0&&n.hooks.every((t)=>!t.configured)?[{checkId:"integration.none-configured",severity:"error",title:"No integration configured",detail:"CC Safety Net is not connected to any supported coding-agent integration.",fixHint:"Run `cc-safety-net install` and configure at least one integration."}]:[]},{derive:(n)=>n.hooks.filter((t)=>t.inspectionStatus==="failed").map((t)=>{let r=b2(t.platform);return{checkId:"integration.inspection-failed",severity:"error",title:`${r} inspection failed`,detail:`Doctor could not verify the ${r} integration configuration.`,fixHint:`Correct the reported ${r} configuration error, then run \`cc-safety-net doctor\` again.`,integration:t.platform}})},{derive:(n)=>n.userConfig.exists&&!n.userConfig.valid?[{checkId:"config.user-invalid",severity:"error",title:"User configuration is invalid",detail:"Doctor could not load a valid user rules configuration.",fixHint:"Run `cc-safety-net rule verify`, correct the reported error, then rerun doctor.",path:n.userConfig.path}]:[]},{derive:(n)=>n.projectConfig.exists&&!n.projectConfig.valid?[{checkId:"config.project-invalid",severity:"error",title:"Project configuration is invalid",detail:"Doctor could not load a valid project rules configuration.",fixHint:"Run `cc-safety-net rule verify`, correct the reported error, then rerun doctor.",path:n.projectConfig.path}]:[]},{derive:(n)=>n.configState.state==="degraded"?[{checkId:"config.runtime-degraded",severity:"warning",title:"Runtime is enforcing a fallback configuration",detail:`The rejected candidate configuration is not active: ${n.configState.reason}`,fixHint:"Correct the named source, run `cc-safety-net rule sync` for a rule source, then rerun doctor."}]:[]},{derive:(n)=>{let t=n.environment.find((r)=>r.name==="CC_SAFETY_NET_AUDIT_SCOPE");return et(t?.value)==="invalid"?[{checkId:"environment.audit-scope-invalid",severity:"warning",title:"Audit scope value is invalid",detail:"CC_SAFETY_NET_AUDIT_SCOPE is not `all` or `blocked`, so allowed command decisions are not recorded.",fixHint:"Set CC_SAFETY_NET_AUDIT_SCOPE to `all` or `blocked`, then restart the integration."}]:[]}},...Zs.map((n)=>({derive:(t)=>t.posture.directories.filter((r)=>r.kind===n&&r.status==="unsafe").map((r)=>({checkId:`posture.${n}-directory-unsafe`,severity:"error",title:`${n[0]?.toUpperCase()}${n.slice(1)} directory is unsafe`,detail:`The ${n} directory ${Qs(r.issues)}.`,fixHint:"Ensure this is a real directory owned by the current user with no group or other write access, then rerun doctor.",...r.path?{path:r.path}:{}}))})),{derive:(n)=>{let t=[...n.effectiveSafety.weakenedRuleOverrides].sort();return t.length>0?[{checkId:"posture.rule-overrides-weaken-preset",severity:"warning",title:"Rule overrides weaken the selected preset",detail:`Explicit overrides disable rules the resolved preset would enable: ${t.join(", ")}.`,fixHint:`Remove these \`off\` overrides or set them to \`on\`: ${t.join(", ")}.`}]:[]}}];function d0(n){return Xs.flatMap((t,r)=>t.derive(n).map((e,a)=>({finding:e,catalogOrder:r,occurrence:a}))).sort((t,r)=>l0[t.finding.severity]-l0[r.finding.severity]||t.catalogOrder-r.catalogOrder||t.occurrence-r.occurrence).map((t)=>t.finding)}function n2(){return Boolean(process.stdout.isTTY&&!process.env.NO_COLOR)}var Ys=(n)=>n2()?`\x1B[32m${n}\x1B[0m`:n,Ss=(n)=>n2()?`\x1B[33m${n}\x1B[0m`:n,Hs=(n)=>n2()?`\x1B[34m${n}\x1B[0m`:n,Ws=(n)=>n2()?`\x1B[35m${n}\x1B[0m`:n,As=(n)=>n2()?`\x1B[36m${n}\x1B[0m`:n,Ks=(n)=>n2()?`\x1B[31m${n}\x1B[0m`:n,Rs=(n)=>n2()?`\x1B[2m${n}\x1B[0m`:n,Vs=(n)=>n2()?`\x1B[1m${n}\x1B[0m`:n,o={green:Ys,yellow:Ss,blue:Hs,magenta:Ws,cyan:As,red:Ks,dim:Rs,bold:Vs},Ms="\x1B[0m",Gs=[39,82,198,226,208,51,196,46,201,214,93,154,220,27,49,190,200,33,129,227,45,160,63,118,123,202];function Cs(n){let t=n;return()=>{return t=(t*1664525+1013904223)%4294967296,t/4294967296}}function Us(n){let t=[...Gs],r=Cs(n);for(let e=t.length-1;e>0;e--){let a=Math.floor(r()*(e+1)),l=t[e];t[e]=t[a],t[a]=l}return t}function Os(n,t=0){if(!n2())return"";let r=Us(t);return`\x1B[38;5;${r[n%r.length]}m`}function s0(n,t,r=0){if(!n2())return`"${n}"`;return`${Os(t,r)}"${n}"${Ms}`}var Ts=new RegExp("\x1B\\[[0-9;]*m","g"),bt=(n)=>n.replace(Ts,"").length;function m2(n){let t=(n.headers??n.rows[0]??[]).map((d,s)=>{let L=Math.max(...n.rows.map((c)=>bt(c[s]??"")));return Math.max(bt(d),L)}),r=(d,s)=>d+" ".repeat(Math.max(0,s-bt(d))),e=(d,s)=>s[0]+t.map((L)=>d.repeat(L+2)).join(s[1])+s[2],a=(d)=>`│ ${d.map((s,L)=>r(s,t[L]??0)).join(" │ ")} │`,l=n.headers?[`   ${a(n.headers)}`,`   ${e("─",["├","┼","┤"])}`]:[];return[`   ${e("─",["┌","┬","┐"])}`,...l,...n.rows.map((d)=>`   ${a(d)}`),`   ${e("─",["└","┴","┘"])}`].join(`
`)}function L0(n){let t=[];t.push("Hook Integration"),t.push(Es(n));let r=[],e=[];for(let a of n){let l=b2(a.platform);if(a.errors&&a.errors.length>0)for(let d of a.errors)if(a.configured)r.push({platform:l,message:d});else e.push({platform:l,message:d})}for(let a of r)t.push(`   Warning (${a.platform}): ${a.message}`);for(let a of e)t.push(o.red(`   Error (${a.platform}): ${a.message}`));return t.join(`
`)}function Es(n){let t=["Platform","Discovery","Configuration","Inspection"],r=n.map((e)=>{let a=b2(e.platform);if(e.inspectionStatus==="not-inspected"){let L=o.dim("Not inspected");return[a,L,L,L]}let l=e.detected?o.green("Detected"):e.inspectionStatus==="failed"?o.red("Unknown"):o.dim("Not detected"),d=e.configured?o.green("Configured"):e.detected?o.yellow("Not configured"):e.inspectionStatus==="failed"?o.red("Unknown"):o.dim("Not applicable"),s=e.inspectionStatus==="verified"?o.green("Verified"):e.inspectionStatus==="failed"?o.red("Failed"):o.dim("Not applicable");return[a,l,d,s]});return m2({headers:t,rows:r})}function c0(n){let r=["Guard Engine Verification",`   Synthetic self-test: ${n.failed>0?o.red(`${n.passed}/${n.total} FAIL`):o.green(`${n.passed}/${n.total} passed`)}`],e=n.results.filter((a)=>!a.passed);if(e.length>0){r.push(""),r.push(o.red("   Failures:"));for(let a of e)r.push(o.red(`   • ${a.description}`)),r.push(o.red(`     expected ${a.expected}, got ${a.actual}`))}return r.join(`
`)}function _s(n){if(n.length===0)return"   (no custom rules)";let t=["Source","Name","Command","Block Args"],r=n.map((e)=>[e.source,e.name,e.subcommand?`${e.command} ${e.subcommand}`:e.command,e.blockArgs.join(", ")]);return m2({headers:t,rows:r})}function i0(n){let t=[];if(t.push("Configuration"),t.push(Ps(n.userConfig,n.projectConfig)),t.push(""),n.effectiveRules.length>0)t.push(`   Effective rules (${n.effectiveRules.length} total):`),t.push(_s(n.effectiveRules));else t.push("   Effective rules: (none - using built-in rules only)");for(let r of n.shadowedRules)t.push(""),t.push(`   Note: Project rule "${r.name}" shadows user rule with same name`);return t.join(`
`)}function Ps(n,t){let r=["Scope","Status"],e=(l)=>{if(!l.exists)return o.dim("N/A");if(!l.valid)return o.red(`Invalid (${l.errors?.[0]??"unknown error"})`);return o.green("Configured")},a=[["User",e(n)],["Project",e(t)]];return m2({headers:r,rows:a})}function p0(n){let t=[];return t.push("Environment"),t.push(Is(n)),t.join(`
`)}function v0(n){let t=["Effective Safety",`   Selected preset: ${n.effectiveSafety.selectedPreset}`,`   Effective: ${n.effectiveSafety.level}`],r=[["fail_closed","fail_closed"],["paranoid_rm","paranoid_rm"],["paranoid_interpreters","paranoid_interpreters"]];for(let[e,a]of r){let l=n.effectiveSafety.capabilities[e],d=l.enabled?o.green("ON"):o.dim("OFF"),s=l.sources.length>0?` (${l.sources.join(", ")})`:"";t.push(`   ${a}: ${d} via ${l.source}${s}`)}t.push(`   Stored rule customizations: ${n.effectiveSafety.ruleCounts.stored}`),t.push(`   Effective rule customizations: ${n.effectiveSafety.ruleCounts.effective}`);for(let[e,a]of Object.entries(n.effectiveSafety.ruleOverrides))t.push(`   ${e}: ${a}`);return t.join(`
`)}function u0(n){let t=["Findings"];if(n.length===0)return t.push("   No findings from inspected doctor facts."),t.join(`
`);for(let r of n){let e=`[${r.severity.toUpperCase()}] ${r.checkId}: ${B(r.title)}`,a=r.severity==="error"?o.red:r.severity==="warning"?o.yellow:o.blue;if(t.push(`   ${a(e)}`),t.push(`      ${B(r.detail)}`),r.path)t.push(`      Path: ${B(r.path)}`);if(r.fixHint)t.push(`      Fix: ${B(r.fixHint)}`)}return t.join(`
`)}function Is(n){let t=["Variable","Status","Legacy"],r=n.map((e)=>{let a=e.isSet?o.green("✓"):o.dim("✗"),l=e.legacyName&&e.legacyIsSet?`${e.legacyName} ${o.green("✓")}`:e.legacyName??"";return[e.name,a,l]});return m2({headers:t,rows:r})}function b0(n){let t=[];if(n.totalBlocked===0)t.push("Recent Activity"),t.push("   No blocked commands in the last 7 days"),t.push("   Tip: This is normal for new installations");else t.push(`Recent Activity · last 7 days (${n.totalBlocked} blocked / ${n.sessionCount} sessions)`),t.push(Ns(n.recentEntries));if(n.unreadable>0)t.push(`   Warning: ${n.unreadable} audit log ${n.unreadable===1?"source":"sources"} could not be read; this summary is incomplete`);return t.join(`
`)}function Ns(n){let t=["Time","Command"],r=n.map((e)=>{let a=B(e.command.replace(/\r\n|\r|\n/g," ↵ ").replace(/\t/g," ")),l=a.length>40?`${a.slice(0,37)}...`:a;return[e.relativeTime,l]});return m2({headers:t,rows:r})}function o0(n){let t=[];if(t.push("Update Check"),n.latestVersion===null&&!n.error)return t.push(En([["Status",o.dim("Skipped")],["Installed",n.currentVersion]])),t.join(`
...
`)}function En(n){return m2({rows:n})}function f0(n){let t=[];return t.push("System Info"),t.push(nL(n)),t.join(`
`)}function nL(n){let t=["Component","Version"],r=(l)=>{if(l===null)return o.dim("not found");return l},a=[{label:"cc-safety-net",value:n.version},{label:"Claude Code",value:n.claudeCodeVersion},{label:"Amp Code",value:n.ampVersion},{label:"Antigravity CLI",value:n.antigravityCliVersion},{label:"Codex",value:n.codexCliVersion},{label:"Cursor",value:n.cursorVersion},{label:"Gemini CLI",value:n.geminiCliVersion},{label:"GitHub Copilot CLI",value:n.copilotCliVersion},{label:"Hermes Agent",value:n.hermesAgentVersion},{label:"Kimi Code",value:n.kimiCodeVersion},{label:"OpenClaw",value:n.openClawVersion},{label:"OpenCode",value:n.openCodeVersion},{label:"Pi",value:n.piCliVersion},{label:"Node.js",value:n.nodeVersion},{label:"npm",value:n.npmVersion},{label:"Bun",value:n.bunVersion},{label:"Platform",value:n.platform}].map((l)=>[l.label,r(l.value)]);return m2({headers:t,rows:a})}function y0(n){if(n.findings.length===0)return o.green(`
...
${n.findings.length} ${e}: ${r.join(", ")}.`;if(t.error>0)return o.red(a);if(t.warning>0)return o.yellow(a);return o.blue(a)}import{lstatSync as tL}from"node:fs";import{dirname as ot}from"node:path";function ft(n,t){try{let r=tL(t);if(r.isSymbolicLink())return{kind:n,path:t,status:"unsafe",issues:["symlink"]};if(!r.isDirectory())return{kind:n,path:t,status:"unsafe",issues:["not-directory"]};if(process.platform==="win32"||typeof process.getuid!=="function")return{kind:n,path:t,status:"unknown",issues:[]};let e=[...r.uid!==process.getuid()?["ownership"]:[],...(r.mode&18)!==0?["permissions"]:[]];return{kind:n,path:t,status:e.length>0?"unsafe":"safe",issues:e}}catch(r){if(typeof r==="object"&&r!==null&&"code"in r&&r.code==="ENOENT")return{kind:n,path:t,status:"not-applicable",issues:[]};return{kind:n,path:t,status:"unknown",issues:[]}}}function h0(n){let t=L2();return{directories:[ft("policy",ot(ot(n))),ft("config",ot(n)),...t?[ft("audit",t)]:[{kind:"audit",status:"unknown",issues:[]}]]}}import{spawn as rL}from"node:child_process";import{existsSync as w0}from"node:fs";import{delimiter as eL,extname as aL,join as lL}from"node:path";import{stripVTControlCharacters as x0}from"node:util";var D0="2.0.5",dL=5000,sL="_CC_SAFETY_NET_TEST_SPAWN_PLATFORM";function z(){return D0}function yt(n,t){let r=n[t];if(r)return r;let e=Object.keys(n).find((a)=>a.toLowerCase()===t.toLowerCase()&&!!n[a]);return e?n[e]:r}function LL(n){return(yt(n,"PATHEXT")||".COM;.EXE;.BAT;.CMD").split(";").filter((t)=>t.length>0)}function cL(n,t){let r=aL(n)?[n]:[...LL(t).map((e)=>`${n}${e}`),n];if(n.includes("/")||n.includes("\\"))return r.find((e)=>w0(e))??n;return(yt(t,"PATH")??"").split(eL).flatMap((e)=>r.map((a)=>lL(e,a))).find((e)=>w0(e))??n}function $0(n){if(!/[\s"&|<>^]/.test(n))return n;return`"${n.replace(/"/g,'""')}"`}function f2(n,t){let[r,...e]=n,a=t[sL]==="win32"?"win32":process.platform;if(!r||a!=="win32")return{cmd:r??"",args:e};let l=cL(r,t);if(!/\.(?:bat|cmd)$/i.test(l))return{cmd:l,args:e};return{cmd:yt(t,"COMSPEC")??"cmd.exe",args:["/d","/c",["call",$0(l),...e.map($0)].join(" ")]}}var _2=async(n,t=dL)=>{let r=await iL(n,{timeoutMs:t});if(r.code!==0)return null;return x0(r.stdout).trim()||x0(r.stderr).trim()||null};function iL(n,t){let[r,...e]=n;if(!r)return Promise.resolve({code:null,stdout:"",stderr:""});return new Promise((a)=>{try{let l=f2([r,...e],process.env),d=rL(l.cmd,l.args,{stdio:["ignore","pipe","pipe"]}),s=!1,L="",c="";d.stdout.on("data",(b)=>{L+=b.toString()}),d.stderr.on("data",(b)=>{c+=b.toString()});let i=(b)=>{if(s)return;s=!0,clearTimeout(v),a(b)},v=setTimeout(()=>{d.kill(),i({code:null,stdout:L,stderr:c})},t.timeoutMs);d.on("close",(b)=>{i({code:b,stdout:L,stderr:c})}),d.on("error",()=>{i({code:null,stdout:L,stderr:c})})}catch{a({code:null,stdout:"",stderr:""})}})}function X(n){if(!n)return null;let t=/Claude Code\s+(\d+\.\d+\.\d+)/i.exec(n);if(t)return t[1]??null;let r=/v?(\d+\.\d+\.\d+(?:-[a-zA-Z0-9.]+)?)/i.exec(n);if(r)return r[1]??null;return n.split(`
`)[0]?.trim()||null}async function gn(n=_2){let[t,r,e,a,l,d,s,L,c,i,v,b,p,f,u,y,x]=await Promise.all([n(["claude","--version"]),n(["agy","--version"]),n(["opencode","--version"]),n(["codex","--version"]),n(["codex","plugin","list"]),n(["gemini","--version"]),n(["copilot","--binary-version"]),n(["hermes","--version"]),n(["kimi","--version"]),n(["openclaw","--version"]),n(["pi","--version"]),n(["cursor","--version"]),n(["amp","--version"]),n(["amp","plugins","list"],30000),n(["node","--version"]),n(["npm","--version"]),n(["bun","--version"])]);return{version:D0,claudeCodeVersion:X(t),antigravityCliVersion:X(r),openCodeVersion:X(e),codexCliVersion:X(a),codexPluginListOutput:l,geminiCliVersion:X(d),copilotCliVersion:X(s),hermesAgentVersion:X(L),kimiCodeVersion:X(c),openClawVersion:X(i),piCliVersion:X(v),cursorVersion:X(b),ampVersion:X(p),ampPluginListOutput:f,nodeVersion:X(u),npmVersion:X(y),bunVersion:X(x),platform:`${process.platform} ${process.arch}`}}function ht(n,t){if(t==="dev")return!1;let r=n.split(".").map(Number),e=t.split(".").map(Number),[a=0,l=0,d=0]=r,[s=0,L=0,c=0]=e;if(a!==s)return a>s;if(l!==L)return l>L;return d>c}async function P2(){let n=z(),t=new AbortController,r=setTimeout(()=>t.abort(),3000);try{let e=await fetch("https://registry.npmjs.org/cc-safety-net/latest",{signal:t.signal});if(!e.ok)return{currentVersion:n,latestVersion:null,updateAvailable:!1,error:`npm registry returned ${e.status}`};let a=await e.json(),l=ht(a.version,n);return{currentVersion:n,latestVersion:a.version,updateAvailable:l}}catch(e){return{currentVersion:n,latestVersion:null,updateAvailable:!1,error:e instanceof Error?e.message:"Network error"}}finally{clearTimeout(r)}}import*as B0 from"node:readline";var q0=(n)=>`\x1B[${n}B`,pL=(n)=>`\x1B[${n}A`;var g0=["░","▒","▓","╱","╲","┃","━","┏","┓","┗","┛","╋"];function vL(n){return new Promise((t)=>setTimeout(t,n))}function uL(n,t,r){if(!r)return t(n);if(r.aborted)return Promise.resolve();return new Promise((e,a)=>{let l=()=>r.removeEventListener("abort",d),d=()=>{l(),e()};r.addEventListener("abort",d,{once:!0}),t(n).then(()=>{l(),e()},(s)=>{l(),a(s)})})}function mn(n,t){return n&&n>0?n:t}function _n(n){return Math.max(0,Math.min(1,n))}function I2(n){return Math.max(0,Math.min(255,Math.round(n)))}function wt(n){return n<=0.0031308?12.92*n:1.055*n**0.4166666666666667-0.055}function bL(n,t,r){let e=r*Math.PI/180,a=t*Math.cos(e),l=t*Math.sin(e),d=(n+0.3963377774*a+0.2158037573*l)**3,s=(n-0.1055613458*a-0.0638541728*l)**3,L=(n-0.0894841775*a-1.291485548*l)**3;return{blue:I2(wt(_n(-0.0041960863*d-0.7034186147*s+1.707614701*L))*255),green:I2(wt(_n(-1.2684380046*d+2.6097574011*s-0.3413193965*L))*255),red:I2(wt(_n(4.0767416621*d-3.3077115913*s+0.2309699292*L))*255)}}function xt(n,t){let r=(t*n*180/Math.PI%360+360)%360;return bL(0.72,0.15,r)}function j0(n,t=0.1){let r=xt(t,n);return`\x1B[38;2;${r.red};${r.green};${r.blue}m`}function oL(n,t){return{blue:I2(n.blue+(255-n.blue)*t),green:I2(n.green+(255-n.green)*t),red:I2(n.red+(255-n.red)*t)}}function z0(n,t,r){let e=Math.imul(n+2654435769,2246822507)^Math.imul(t+3266489909,668265263)^Math.imul(r+374761393,2654435761),a=e^e>>>15,l=Math.imul(a,739982445),d=l^l>>>12,s=Math.imul(d,695872825);return((s^s>>>15)>>>0)/4294967296}function fL(n,t,r){let e=Math.floor(z0(n,t,r)*g0.length);return g0[e]??"░"}function m0(n){let t=_n(n);return t*t*t*(t*(t*6-15)+10)}function yL(n){if(n.length===0)return"";let t=[],r=!1,e="";for(let a of n){let l=`${a.red};${a.green};${a.blue}`;if(a.bold!==r)t.push(a.bold?"\x1B[1m":"\x1B[22m"),r=a.bold;if(l!==e)t.push(`\x1B[38;2;${l}m`),e=l;t.push(a.character)}return`${t.join("")}\x1B[22m\x1B[39m`}function hL(n,t,r,e,a){return n.map((l,d)=>({...xt(r,e+t+d/a),bold:!1,character:l}))}function wL(n,t,r,e,a,l,d,s){let L=Math.max(1,e*0.75),c=Math.min(1,r/L),i=a*m0(c),v=Math.max(0,(r-L)/Math.max(1,e-L)),b=(1-m0(r/e))*s*2,p=0.35*Math.max(0,1-v*2),f=c>=1,u=Math.min(n.length,Math.ceil(i+2+1));return n.slice(0,u).map((y,x)=>{let w=xt(l,d+t+x/s+b),h=x+z0(t,x,7919)*2-1;if(h>i+2)return{...w,bold:!1,character:" "};let Z=i-h,Q2=0.8*Math.exp(-(Z*Z)/12.5),Ln=Math.min(0.9,Q2+p),J1=!f&&h>i-4;return{...oL(w,Ln),bold:Ln>0.3,character:J1?fL(t,x,r):y}})}function k0(n){return`\x1B[?2026h${n.map((t,r)=>`\x1B8${r>0?q0(r):""}${yL(t)}`).join("")}\x1B[?2026l`}async function $t(n,t={}){if(!n)return;let r=t.output??process.stdout,e=t.sleep??vL,a=mn(t.frequency,0.1),l=t.seed??0,d=mn(t.speed,40),s=mn(t.spread,3),L=mn(t.frameRate,60),c=Math.max(1,Math.floor(mn(t.duration,12))),i=n.split(`
`).map((u)=>Array.from(u)),v=Math.max(...i.map((u)=>u.length)),b=1000*c*i.filter((u)=>u.length>0).length/d,p=v>0?Math.max(1,Math.ceil(b/(1000/L))):0,f=p>0?b/p:0;r.write(`\x1B[?25l${i.length>1?`${`
`.repeat(i.length-1)}${pL(i.length-1)}`:""}\x1B7`);try{for(let u=1;u<=p;u+=1){if(t.signal?.aborted)break;r.write(k0(i.map((y,x)=>wL(y,x,u,p,v,a,l,s)))),await uL(f,e,t.signal)}}finally{if(r.write(k0(i.map((u,y)=>hL(u,y,a,l,s)))),r.write("\x1B8"),i.length>1)r.write(q0(i.length-1));r.write(`
\x1B[0m\x1B[?25h`)}}var F0=["┏━┛┏━┛  ┏━┛┏━┃┏━┛┏━┛━┏┛┃ ┃  ┏━ ┏━┛━┏┛","┃  ┃    ━━┃┏━┃┏━┛┏━┛ ┃ ━┏┛  ┃ ┃┏━┛ ┃ ","━━┛━━┛  ━━┛┛ ┛┛  ━━┛ ┛  ┛   ┛ ┛━━┛ ┛ "].join(`
`);function xL(n){return Boolean(n.isTTY)}async function kn(n={}){let t=n.output??process.stdout;if(!xL(t))return;let r=n.input??process.stdin,e={duration:n.duration,frequency:n.frequency,output:t,seed:n.seed??Math.random()*8192,sleep:n.sleep,speed:n.speed,spread:n.spread};if(!r.isTTY||typeof r.setRawMode!=="function"){await $t(F0,e);return}let a=new AbortController,l=r.readableFlowing===!0,d=r.isRaw===!0,s=!1,L=(c,i)=>{if(i.ctrl&&i.name==="c")s=!0;if(s||i.name==="return"||i.name==="enter")a.abort()};B0.emitKeypressEvents(r),r.on("keypress",L),r.setRawMode(!0),r.resume();try{await $t(F0,{...e,signal:a.signal})}finally{if(r.off("keypress",L),r.setRawMode(d),!l)r.pause()}if(!s)return;if(n.onInterrupt){n.onInterrupt();return}process.kill(process.pid,"SIGINT")}var J0="\r\x1B[2K",$L="\x1B[?25l",DL="\x1B[39m",gL="\x1B[?25h",mL=100,kL=0.55,qL=80,Z0=["⠋","⠙","⠹","⠸","⠼","⠴","⠦","⠧","⠇","⠏"];function jL(n){return new Promise((t)=>setTimeout(t,n))}async function Pn(n,t={}){let r=t.output??process.stdout;if(!r.isTTY)return n;let e=t.sleep??jL,a=!1,l=n.then((s)=>{return a=!0,s},(s)=>{throw a=!0,s});if(await Promise.race([l.then(()=>!0),e(mL).then(()=>!1)]))return l;r.write($L);try{for(let s=0;!a;s+=1)r.write(`${J0}${j0(s*kL)}${Z0[s%Z0.length]}${DL} ${t.loadingMessage??"Loading…"}`),await Promise.race([l,e(qL)]);return await l}finally{r.write(`${J0}${gL}`)}}async function qn(n,t,r,e={}){let a=t();if(n)await r();if(n&&a.ready)await Pn(a.ready,e);return a.finish()}import{homedir as Wc}from"node:os";import{stripVTControlCharacters as zL}from"node:util";var In="amp plugins list",FL=/^\s*[✓✗]\s+cc-safety-net(?:\.ts)?\s+\(User Plugins\)\s+(\S+)\s*$/;function Q0(n){if(!n.ampPluginListOutput)return{platform:"amp",status:"n/a"};let t=zL(n.ampPluginListOutput).split(`
`).map((r)=>FL.exec(r)?.[1]).find((r)=>r!==void 0);if(!t)return{platform:"amp",status:"n/a"};if(t!=="active")return{platform:"amp",status:"disabled",method:In,configPath:In,errors:[`Amp personal plugin cc-safety-net is ${t}; run "plugins: reload" in Amp or reinstall with install --amp`]};return{platform:"amp",status:"configured",method:In,configPath:In}}import{existsSync as JL,readFileSync as ZL}from"node:fs";import{join as BL}from"node:path";function jn(n){return BL(n,".gemini","config","hooks.json")}var QL=/cc-safety-net\s+hook\s+(?:[^\s]+\s+)*(?:--agy-cli|-ac)(\s|["']|$)/;function XL(n){if(!n||typeof n!=="object"||Array.isArray(n))return[];return Object.values(n).flatMap((t)=>{if(!t||typeof t!=="object"||Array.isArray(t))return[];let r=t,e=r.PreToolUse;if(!Array.isArray(e))return[];return e.flatMap((a)=>{if(!a||typeof a!=="object"||Array.isArray(a))return[];let l=a.hooks;if(!Array.isArray(l))return[];return l.flatMap((d)=>{if(!d||typeof d!=="object"||Array.isArray(d))return[];let s=d.command;if(typeof s!=="string"||!QL.test(s))return[];return[{command:s,enabled:r.enabled!==!1}]})})})}function X0(n){let t=jn(n.homeDir);if(!JL(t))return{platform:"antigravity-cli",status:"n/a",configPath:t};let r;try{r=XL(JSON.parse(ZL(t,"utf-8")))}catch(e){return{platform:"antigravity-cli",status:"n/a",configPath:t,errors:[`Failed to parse Antigravity hooks config ${t}: ${e instanceof Error?e.message:String(e)}`]}}if(r.some((e)=>e.enabled))return{platform:"antigravity-cli",status:"configured",method:"hook config",configPath:t};if(r.length>0)return{platform:"antigravity-cli",status:"disabled",method:"hook config",configPath:t};return{platform:"antigravity-cli",status:"n/a",configPath:t}}import{join as Y0}from"node:path";import{existsSync as YL,lstatSync as SL,readFileSync as HL}from"node:fs";function t2(n,t=(r)=>r){if(!YL(n))return{kind:"missing"};try{return{kind:"ok",value:JSON.parse(t(HL(n,"utf-8")))}}catch{return{kind:"unreadable"}}}function j(n){try{return SL(n)}catch{return}}function Nn(n,t){let r=j(t);if(!r)return{platform:n,status:"n/a",configPath:t};if(!r.isSymbolicLink()&&r.isDirectory())return;return{platform:n,status:"n/a",configPath:t,errors:[`${t} is a symlink or not a directory; move or remove it before installing`]}}function $(n,t){return typeof n==="object"&&n!==null?n[t]:void 0}var Dt="cc-safety-net@cc-marketplace";function S0(n){return Y0(n,".claude","plugins","installed_plugins.json")}function H0(n,t){let r=$($(n,"plugins"),t);return Array.isArray(r)&&r.length>0}function n1(n,t){let r=t2(S0(n));return r.kind==="ok"&&H0(r.value,t)}function gt(n){let t=S0(n),r=t2(t);if(r.kind==="unreadable")return{platform:"claude-code",status:"not-inspected"};if(r.kind==="missing")return{platform:"claude-code",status:"n/a"};if(!H0(r.value,Dt))return{platform:"claude-code",status:"n/a"};let e=Y0(n,".claude","settings.json"),a=t2(e);if(a.kind==="unreadable")return{platform:"claude-code",status:"not-inspected"};if(!(a.kind==="ok"&&$($(a.value,"enabledPlugins"),Dt)===!0))return{platform:"claude-code",status:"disabled",method:"plugin config",configPath:e,errors:[`${Dt} is installed but not enabled in Claude Code`]};return{platform:"claude-code",status:"configured",method:"plugin config",configPath:t}}function W0(n){return gt(n.homeDir)}function A0(n){if(!n.codexPluginListOutput)return{platform:"codex",status:"n/a"};let t=n.codexPluginListOutput.split(`
`).find((r)=>r.includes("https://github.com/kenryu42/cc-safety-net.git"));if(!t)return{platform:"codex",status:"n/a"};if(!t.includes("installed, enabled"))return{platform:"codex",status:"disabled",method:"codex plugin list",configPath:"codex plugin list",errors:["Codex plugin line for https://github.com/kenryu42/cc-safety-net.git must contain installed, enabled."]};return{platform:"codex",status:"configured",method:"codex plugin list",configPath:"codex plugin list"}}import{existsSync as e1,readdirSync as WL,readFileSync as AL}from"node:fs";import{join as V}from"node:path";var r2="cc-safety-net@cc-marketplace",t1=["cc-marketplace","cc-safety-net"],K0=["_direct","copilot-safety-net"],R0=["cc-marketplace","safety-net"],V0="safety-net@cc-marketplace";function r1(n,t){let r=t.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");return new RegExp(`(^|[^a-z0-9-])${r}([^a-z0-9-]|$)`,"m").test(n??"")}function M0(n){return r1(n,"cc-safety-net@cc-marketplace")}function G0(n){return r1(n,"cc-marketplace")}function C0(n){return r1(n,"copilot-safety-net")}function U0(n){return r1(n,"safety-net@cc-marketplace")}function R(n){let t="",r=0,e=!1,a=!1,l=-1;while(r<n.length){let d=n[r],s=n[r+1];if(a){t+=d,a=!1,r++;continue}if(d==='"'&&!e){e=!0,l=-1,t+=d,r++;continue}if(d==='"'&&e){e=!1,t+=d,r++;continue}if(d==="\\"&&e){a=!0,t+=d,r++;continue}if(e){t+=d,r++;continue}if(d==="/"&&s==="/"){while(r<n.length&&n[r]!==`
`)r++;continue}if(d==="/"&&s==="*"){r+=2;while(r<n.length-1){if(n[r]==="*"&&n[r+1]==="/"){r+=2;break}r++}continue}if(d===","){l=t.length,t+=d,r++;continue}if(d==="}"||d==="]"){if(l!==-1){let L=t.slice(l+1);if(/^\s*$/.test(L))t=t.slice(0,l)+L}l=-1,t+=d,r++;continue}if(!/\s/.test(d))l=-1;t+=d,r++}return t}function mt(n){if(!n?.includes("cc-safety-net"))return!1;return/(^|\s)hook\s+(?:[^\s]+\s+)*(--copilot-cli|-cp)(\s|$)/.test(n)}function T0(n,t){if(!n)return null;let r=n.match(/(\d+)\.(\d+)\.(\d+)/);if(!r)return null;let e=[Number(r[1]),Number(r[2]),Number(r[3])];for(let a=0;a<t.length;a++){let l=e[a]??0,d=t[a]??0;if(l!==d)return l>d}return!0}function KL(n){return T0(n,[0,0,422])}function RL(n){return T0(n,[1,0,8])}function zn(n){return process.env.COPILOT_HOME||V(n,".copilot")}function qt(n){return(n.hooks?.preToolUse??[]).some((r)=>{if(r.type!=="command")return!1;return mt(r.command)||mt(r.bash)||mt(r.powershell)})}function jt(n,t){try{return JSON.parse(R(AL(n,"utf-8")))}catch(r){t?.push(`Failed to parse ${n}: ${r instanceof Error?r.message:String(r)}`);return}}function E0(n,t){try{return WL(n).filter((r)=>r.endsWith(".json")).sort((r,e)=>r.localeCompare(e))}catch(r){return t?.push(`Failed to read ${n}: ${r instanceof Error?r.message:String(r)}`),[]}}function VL(n,t){if(!e1(n))return[];let r=[];for(let e of E0(n,t)){let a=V(n,e),l=jt(a,t);if(l&&qt(l))r.push(a)}return r}function kt(n,t){if(!e1(n))return;let r=jt(n,t);if(!r)return;return{path:n,config:r}}function O0(n,t,r,e){if(t){n.push(`GitHub Copilot CLI ${t} does not support ${r}; requires ${e}+`);return}n.push(`GitHub Copilot CLI version unavailable; skipping ${r} because it requires ${e}+`)}function ML(n){let t=[n.localSettings,n.repoSettings,n.userConfig];for(let r of t){if(r?.config.disableAllHooks===!0)return r.path;if(r?.config.disableAllHooks===!1)return}return}function GL(n,t,r,e){let a=zn(n),l=V(t,".github","hooks"),d=V(a,"hooks"),s=V(t,".github","copilot"),L=RL(r),c=L===!0?e:void 0,i={userConfig:kt(V(a,"config.json"),c),repoSettings:kt(V(s,"settings.json"),c),localSettings:kt(V(s,"settings.local.json"),c)};if(L!==!1){let w=ML(i);if(w){if(L===null)e.push(`GitHub Copilot CLI version unavailable; treating disableAllHooks in ${w} as active`);return{activeConfigPaths:[],disabledBy:w}}}let v=VL(l,e),b=KL(r),p=b===!0?e:void 0,f=e1(d)?E0(d,p):[],u=[];for(let w of f){let h=V(d,w),Z=jt(h,p);if(Z&&qt(Z))u.push(h)}if(b!==!0&&u.length>0)O0(e,r,`user hook files in ${d}`,"0.0.422"),u.length=0;let y=[],x=[i.localSettings,i.repoSettings,i.userConfig];for(let w of x){if(!w)continue;if(!qt(w.config))continue;if(L===!0){y.push(w.path);continue}O0(e,r,"inline hook definitions in Copilot config files","1.0.8");break}return{activeConfigPaths:[...y.filter((w)=>w.endsWith("settings.local.json")),...y.filter((w)=>w.endsWith("settings.json")),...v,...y.filter((w)=>w.endsWith("config.json")),...u]}}function _0(n){let t=[],r=GL(n.homeDir,n.cwd,n.copilotCliVersion,t);if(r.disabledBy)return{platform:"copilot-cli",status:"disabled",method:"hook config",configPath:r.disabledBy,configPaths:[r.disabledBy],errors:t.length>0?t:void 0};let e=zn(n.homeDir),a=V(e,"installed-plugins",...t1),l=e1(a),d=V(e,"settings.json"),s=t2(d,R);if(l&&s.kind==="unreadable")return{platform:"copilot-cli",status:"not-inspected"};if(l&&s.kind==="ok"&&$($(s.value,"enabledPlugins"),r2)===!1)return{platform:"copilot-cli",status:"disabled",method:"plugin config",configPath:d,errors:[`${r2} is installed but not enabled in Copilot CLI`]};if(l||r.activeConfigPaths.length>0){let L=l,c=r.activeConfigPaths[0];return{platform:"copilot-cli",status:"configured",method:L?"plugin config":"hook config",configPath:c??(L?a:void 0),configPaths:r.activeConfigPaths.length>0?r.activeConfigPaths:void 0,errors:t.length>0?t:void 0}}return{platform:"copilot-cli",status:"n/a",errors:t.length>0?t:void 0}}import{existsSync as nc,readFileSync as tc}from"node:fs";import{existsSync as P0,mkdirSync as OL,readFileSync as TL}from"node:fs";import{dirname as EL,join as _L}from"node:path";import{renameSync as CL,writeFileSync as UL}from"node:fs";function J(n,t){let r=`${n}.${process.pid}.tmp`;UL(r,t),CL(r,n)}var Fn="npx -y cc-safety-net hook --cursor",I0=30;function l1(n){return _L(n,".cursor","hooks.json")}function k2(n){return typeof n==="object"&&n!==null&&!Array.isArray(n)}function zt(){return{command:Fn,timeout:I0,failClosed:!0}}function a1(n){return k2(n)&&n.command===Fn}function PL(n){return Object.keys(n).length===3&&n.command===Fn&&n.timeout===I0&&n.failClosed===!0}function IL(n){try{return JSON.parse(TL(n,"utf-8"))}catch(t){if(t instanceof SyntaxError)throw Error(`Failed to parse Cursor hooks config ${n}: ${t.message}`);throw t}}function N0(n){let t=IL(n);if(!k2(t))throw Error(`Cursor hooks config ${n} must be a JSON object`);if(t.version!==1)throw Error(`Cursor hooks config ${n} must set "version": 1`);if(t.hooks!==void 0&&!k2(t.hooks))throw Error(`Cursor hooks config ${n} "hooks" must be an object`);let r=k2(t.hooks)?t.hooks.preToolUse:void 0;if(r!==void 0&&!Array.isArray(r))throw Error(`Cursor hooks config ${n} "hooks.preToolUse" must be an array`);return t}function na(n){let t=k2(n.hooks)?n.hooks.preToolUse:void 0;return Array.isArray(t)?t:[]}function NL(n){if(!n.some(a1))return[...n,zt()];return n.reduce((t,r)=>{if(!a1(r))return t.result.push(r),t;if(!t.inserted)t.result.push(zt()),t.inserted=!0;return t},{result:[],inserted:!1}).result}function ta(n,t,r){let e=k2(t.hooks)?t.hooks:{},a={...t,hooks:{...e,preToolUse:r}};J(n,`${JSON.stringify(a,null,2)}
`)}function ra(n){let t=l1(n);if(!P0(t))return OL(EL(t),{recursive:!0}),J(t,`${JSON.stringify({version:1,hooks:{preToolUse:[zt()]}},null,2)}
`),{path:t,alreadyInstalled:!1};let r=N0(t),e=na(r),a=e.filter(a1);if(k2(r.hooks)&&Array.isArray(r.hooks.preToolUse)&&a.length===1&&a[0]!==void 0&&PL(a[0]))return{path:t,alreadyInstalled:!0};return ta(t,r,NL(e)),{path:t,alreadyInstalled:!1}}function ea(n){let t=l1(n);if(!P0(t))return{path:t,alreadyInstalled:!1};let r=N0(t),e=na(r),a=e.filter((l)=>!a1(l));if(a.length===e.length)return{path:t,alreadyInstalled:!1};return ta(t,r,a),{path:t,alreadyInstalled:!0}}function rc(n){if(!n||typeof n!=="object"||Array.isArray(n))return[];let t=n.hooks;if(!t||typeof t!=="object"||Array.isArray(t))return[];let r=t.preToolUse;if(!Array.isArray(r))return[];return r.filter((e)=>!!e&&typeof e==="object"&&!Array.isArray(e)&&e.command===Fn)}function ec(n){let t=[];if(n.length>1)t.push("Multiple managed cc-safety-net hooks found; reinstall to collapse duplicates");let r=n[0];if(r&&r.failClosed!==!0)t.push('Managed hook is missing "failClosed": true; reinstall to repair');if(r&&r.timeout!==30)t.push('Managed hook "timeout" is not 30; reinstall to repair');return t}function aa(n){let t=l1(n.homeDir);if(!nc(t))return{platform:"cursor",status:"n/a",configPath:t};let r;try{r=JSON.parse(tc(t,"utf-8"))}catch(l){return{platform:"cursor",status:"n/a",configPath:t,errors:[`Failed to parse Cursor hooks config ${t}: ${l instanceof Error?l.message:String(l)}`]}}let e=rc(r);if(e.length===0)return{platform:"cursor",status:"n/a",configPath:t};let a=ec(e);return{platform:"cursor",status:"configured",method:"hook config",configPath:t,errors:a.length>0?a:void 0}}import{existsSync as ac}from"node:fs";import{join as Ft}from"node:path";var Bt="gemini-safety-net";function Jt(n){let t=Ft(n,".gemini","extensions"),r=Ft(t,Bt);if(!ac(r))return{platform:"gemini-cli",status:"n/a"};let e=Ft(t,"extension-enablement.json"),a=t2(e);if(a.kind==="unreadable")return{platform:"gemini-cli",status:"not-inspected"};let l=a.kind==="ok"?$($(a.value,Bt),"overrides"):void 0;if(Array.isArray(l)&&l.some((s)=>typeof s==="string"&&s.startsWith("!")))return{platform:"gemini-cli",status:"disabled",method:"extension config",configPath:e,errors:[`${Bt} is disabled in Gemini CLI`]};return{platform:"gemini-cli",status:"configured",method:"extension config",configPath:r}}function la(n){return Jt(n.homeDir)}import{readFileSync as ba}from"node:fs";import{join as oa}from"node:path";var M="cc-safety-net",da="# cc-safety-net managed Hermes Agent plugin. Do not edit. Reinstall with: npx -y cc-safety-net install --hermes-agent";function sa(n){return`# cc-safety-net managed Hermes Agent plugin. Do not edit. Reinstall with: npx -y cc-safety-net install --hermes-agent
# version: ${n}
`}function lc(n){return`${sa(n)}name: cc-safety-net
version: "${n}"
description: "Block destructive commands and secret-file access before Hermes runs a tool."
author: "cc-safety-net"
provides_hooks:
  - pre_tool_call
`}function dc(n){return`${sa(n)}"""CC Safety Net guard for Hermes Agent.
...
`}function Bn(n){return[{name:"__init__.py",content:dc(n)},{name:"plugin.yaml",content:lc(n)}]}import{mkdirSync as sc,readdirSync as Lc,readFileSync as cc,rmSync as Zt}from"node:fs";import{join as q2}from"node:path";var ic="__pycache__";function Qt(n){let t=process.env.HERMES_HOME?.trim();return t?t:q2(n,".hermes")}function Xt(n){return q2(Qt(n),"plugins",M)}function Yt(n){return n.startsWith(da)}function St(n,t){let r=Xt(n),e=j(r);if(e&&(e.isSymbolicLink()||!e.isDirectory()))throw Error(`Refusing to ${t} ${r}: not a regular directory. Move or remove it and rerun ${t==="install"?"install":"uninstall"} --hermes-agent.`);return r}function La(n,t){let r=j(n);if(!r)return;if(r.isSymbolicLink()||!r.isFile())throw Error(`Refusing to ${t} ${n}: not a regular file. Move or remove it.`);let e=cc(n,"utf-8");if(!Yt(e))throw Error(`Refusing to ${t} unmanaged file at ${n}. Move or remove it.`);return e}function ca(n){let t=St(n,"install"),r=Bn(z());if(r.map((a)=>La(q2(t,a.name),"overwrite")).every((a,l)=>a===r[l]?.content))return{path:t,alreadyInstalled:!0};return sc(t,{recursive:!0}),r.forEach((a)=>{J(q2(t,a.name),a.content)}),{path:t,alreadyInstalled:!1}}function Ht(n){let t=St(n,"remove");if(!j(t))return[];return Bn(z()).filter((r)=>La(q2(t,r.name),"remove")!==void 0)}function ia(n){let t=St(n,"remove");if(!j(t))return{path:t,alreadyInstalled:!1};let r=Ht(n);if(r.forEach((e)=>{Zt(q2(t,e.name))}),Zt(q2(t,ic),{recursive:!0,force:!0}),Lc(t).length===0)Zt(t,{recursive:!0});return{path:t,alreadyInstalled:r.length>0}}var d1="hermes-agent",pa=/^([^\s#][^:]*):/,pc=/^\s+([A-Za-z_][\w-]*):/,va=/^\s+-\s*(.*)$/;function vc(n){return n.trim().replace(/^(["'])(.*)\1$/,"$2")}function uc(n){let t=n.split(/\r?\n/),r=t.findIndex((l)=>pa.exec(l)?.[1]?.trim()==="plugins");if(r===-1)return[];let e=t.slice(r+1),a=e.findIndex((l)=>pa.test(l));return a===-1?e:e.slice(0,a)}function ua(n,t){let r=uc(n),e=r.findIndex((d)=>pc.exec(d)?.[1]===t);if(e===-1)return[];let a=r.slice(e+1),l=a.findIndex((d)=>!va.test(d));return(l===-1?a:a.slice(0,l)).map((d)=>vc(va.exec(d)?.[1]??""))}function bc(n){try{return ba(oa(Qt(n),"config.yaml"),"utf-8")}catch{return}}function Wt(n){let t=bc(n)??"";return ua(t,"enabled").includes(M)&&!ua(t,"disabled").includes(M)}function fa(n){return/^# version:\s*(.+)$/m.exec(n)?.[1]?.trim()}function oc(n,t){let r=j(n);if(!r)return{error:`${t.name} is missing from ${n}; run install --hermes-agent`};if(r.isSymbolicLink()||!r.isFile())return{error:`${n} is a symlink or not a regular file; move or remove it`};try{let e=ba(n,"utf-8");if(!Yt(e))return{error:`Unmanaged ${t.name} occupies ${n}; move or remove it`};if(fa(e)===z()&&e!==t.content)return{error:`Modified ${t.name} occupies ${n}; run install --hermes-agent to restore it`};return{content:e}}catch(e){return{error:`Failed to read ${n}: ${e instanceof Error?e.message:String(e)}`}}}function ya(n){let t=Xt(n.homeDir),r=Nn(d1,t);if(r)return r;let e=Bn(z()).map((s)=>oc(oa(t,s.name),s)),a=e.flatMap((s)=>("error"in s)?[s.error]:[]);if(a.length>0)return{platform:d1,status:"n/a",configPath:t,errors:a};let l=e.some((s)=>("content"in s)&&fa(s.content)!==z()),d=l?["Installed Hermes Agent plugin is outdated; run install --hermes-agent to update"]:[];if(!Wt(n.homeDir))return{platform:d1,status:"disabled",method:"plugin directory",configPath:t,errors:[`${M} is not enabled in Hermes; run \`hermes plugins enable ${M}\``,...d]};return{platform:d1,status:"configured",method:"plugin directory",configPath:t,errors:l?d:void 0}}import{existsSync as fc,readFileSync as yc}from"node:fs";import{join as ha}from"node:path";var hc=/cc-safety-net\s+hook\s+(?:[^\s]+\s+)*--kimi-code(\s|["']|$)/;function wc(n){return ha(process.env.KIMI_CODE_HOME||ha(n,".kimi-code"),"config.toml")}function Jn(n){let t=wc(n.homeDir);if(!fc(t))return{platform:"kimi-code",status:"n/a",configPath:t};try{if(!hc.test(yc(t,"utf-8")))return{platform:"kimi-code",status:"n/a",configPath:t}}catch(r){return{platform:"kimi-code",status:"n/a",configPath:t,errors:[`Failed to read ${t}: ${r instanceof Error?r.message:String(r)}`]}}return{platform:"kimi-code",status:"configured",method:"hook config",configPath:t}}import{readFileSync as za}from"node:fs";import{join as Qn}from"node:path";var q="cc-safety-net",G="index.js",N2="openclaw.plugin.json",nn="package.json";var s1="// cc-safety-net managed OpenClaw plugin. Do not edit. Reinstall with: npx -y cc-safety-net install --openclaw";import{existsSync as Dc,lstatSync as gc,readdirSync as mc,readFileSync as kc}from"node:fs";import{dirname as $a,join as j2}from"node:path";import{fileURLToPath as qc}from"node:url";import{spawn as xc}from"node:child_process";function $c(n){return n.join(" ")}function At(n,t,r){return[`Failed to run ${$c(n)}${t===null?"":` (exit ${t})`}.`,r.trim()].filter(Boolean).join(`
`)}function Kt(n){let t={stdout:"",stderr:""};return n.stdout.setEncoding("utf-8"),n.stderr.setEncoding("utf-8"),n.stdout.on("data",(r)=>{t.stdout+=r}),n.stderr.on("data",(r)=>{t.stderr+=r}),t}function e2(n,t){return new Promise((r,e)=>{let a=f2([...n],process.env),l=xc(a.cmd,a.args,{stdio:["ignore","pipe","pipe"]}),d=Kt(l),s=()=>[d.stdout,d.stderr].filter(Boolean).join(`
`),L=t?.timeoutMs??120000,c=setTimeout(()=>{l.kill(),e(Error(At(n,null,`Timed out after ${L}ms.
${s()}`.trim())))},L);l.on("error",(i)=>{clearTimeout(c),e(Error(At(n,null,`${i.message}
${s()}`.trim())))}),l.on("close",(i)=>{if(clearTimeout(c),i!==0){e(Error(At(n,i,s())));return}r(t?.stdoutOnly?d.stdout:s())})})}async function Rt(n){for(let t of n)await e2(t)}async function wa(n){for(let t of n)try{await e2(t)}catch(r){console.warn(r instanceof Error?r.message:String(r))}}var xa=j2("openclaw",q),jc=[G,N2,nn];function Da(n){let t=process.env.OPENCLAW_STATE_DIR?.trim();if(t)return t;let r=process.env.OPENCLAW_CONFIG_PATH?.trim();return r?$a(r):j2(n,".openclaw")}function ga(n){let t=process.env.OPENCLAW_CONFIG_PATH?.trim();return t?t:j2(Da(n),"openclaw.json")}function Vt(n){return j2(Da(n),"extensions",q)}function zc(n){let t=mc(n);if(t.length===0)return!0;if(t.some((a)=>!jc.includes(a)))return!1;let r=j2(n,G),e=j(r);return e!==void 0&&!e.isSymbolicLink()&&e.isFile()&&kc(r,"utf-8").startsWith(s1)}function Mt(n){let t=Vt(n),r=j(t);if(!r)return;if(!r.isSymbolicLink()&&r.isDirectory()&&zc(t))return;throw Error(`Refusing to modify ${t}: it does not hold a cc-safety-net managed OpenClaw plugin. Move or remove it, then run the command again.`)}function ma(){let n=$a(qc(import.meta.url));return[j2(n,"..",xa),j2(n,"..","..","..","dist",xa)]}function Gt(n=ma()){return n.find((t)=>Dc(t)&&gc(t).isDirectory())}function Fc(n=ma()){let t=Gt(n);if(!t)throw Error("Packaged OpenClaw plugin directory not found. Reinstall cc-safety-net and try again.");return t}function ka(n=Fc()){return[["openclaw","plugins","install",n,"--force"],["openclaw","plugins","enable",q]]}function Bc(n){let t=(()=>{try{return JSON.parse(n)}catch{return}})(),r=$($(t,"plugin"),"status");return typeof r==="string"?r:void 0}async function qa(){let n=Bc(await e2(["openclaw","plugins","inspect",q,"--runtime","--json"],{stdoutOnly:!0}));if(n==="loaded")return;throw Error(`${n===void 0?`The ${q} plugin's load state could not be verified: OpenClaw's runtime inspect report was unreadable.`:`OpenClaw reports the ${q} plugin with status "${n}".`} Run \`openclaw plugins inspect ${q} --runtime\` for details.`)}var L1="openclaw",Zn=`run \`openclaw plugins enable ${q}\``;function tn(n,t){let r=Qn(n,t),e=j(r);if(!e)return{error:`${t} is missing from ${r}; run install --openclaw`};if(e.isSymbolicLink()||!e.isFile())return{error:`${r} is a symlink or not a regular file; move or remove it`};try{return{content:za(r,"utf-8")}}catch(a){return{error:`Failed to read ${r}: ${a instanceof Error?a.message:String(a)}`}}}function Fa(n){try{return JSON.parse(R(n))}catch{return}}function Jc(n){let t=tn(n,N2);if("error"in t)return t.error;if($(Fa(t.content),"id")===q)return;return`${Qn(n,N2)} is not a valid ${q} manifest; run install --openclaw`}function Zc(n){let t=tn(n,nn);if("error"in t)return t.error;let r=$($(Fa(t.content),"openclaw"),"extensions");if(Array.isArray(r)&&r.includes(`./${G}`))return;return`${Qn(n,nn)} does not point OpenClaw at ${G}; run install --openclaw`}function ja(n){return Array.isArray(n)?n.filter((t)=>typeof t==="string"):[]}function Qc(n){let t=ga(n);if(!j(t))return`${q} is not enabled; ${Zn}`;let r=(()=>{try{return JSON.parse(R(za(t,"utf-8")))}catch{return}})();if(r===void 0)return`Failed to read ${t}; fix it, then ${Zn}`;let e=$(r,"plugins");if($(e,"enabled")===!1)return`plugins.enabled is false in ${t}; no OpenClaw plugin loads`;let a=$($($(e,"entries"),q),"enabled");if(ja($(e,"deny")).includes(q)||a===!1)return`${q} is disabled in ${t}; ${Zn}`;let l=ja($(e,"allow"));if(l.length>0&&!l.includes(q))return`plugins.allow in ${t} does not list ${q}; add it, then ${Zn}`;if(l.includes(q)||a===!0)return;return`${q} is not enabled; ${Zn}`}function Ba(n){return/^\/\/ version:\s*(.+)$/m.exec(n)?.[1]?.trim()}function Xc(n,t,r){if(r===void 0)return[];let e=tn(r,G);if("error"in e||Ba(e.content)!==t)return[];return[G,N2,nn].flatMap((a)=>{let l=tn(n,a),d=tn(r,a);if("error"in l||"error"in d||l.content===d.content)return[];return[`Modified ${a} occupies ${Qn(n,a)}; run install --openclaw to restore it`]})}function Ja(n){let t=Vt(n.homeDir),r=Nn(L1,t);if(r)return r;let e=tn(t,G),l=["error"in e?e.error:e.content.startsWith(s1)?void 0:`Unmanaged ${G} occupies ${Qn(t,G)}; move or remove it`,Jc(t),Zc(t)].filter((i)=>i!==void 0),d="content"in e?Ba(e.content):void 0,s=l.length>0?l:Xc(t,d,Gt());if(s.length>0)return{platform:L1,status:"n/a",configPath:t,errors:s};let L=d===z()?[]:["Installed OpenClaw plugin is outdated; run install --openclaw to update"],c=Qc(n.homeDir);if(c)return{platform:L1,status:"disabled",method:"plugin directory",configPath:t,errors:[c,...L]};return{platform:L1,status:"configured",method:"plugin directory",configPath:t,errors:L.length>0?L:void 0}}import{existsSync as Yc,readFileSync as Sc}from"node:fs";import{join as Za}from"node:path";function Qa(n){let t=[],r=Za(n.homeDir,".config","opencode"),e=["opencode.json","opencode.jsonc"];for(let a of e){let l=Za(r,a);if(Yc(l))try{let d=Sc(l,"utf-8"),s=R(d);if((JSON.parse(s).plugin??[]).some((v)=>v.includes("cc-safety-net")))return{platform:"opencode",status:"configured",method:"plugin array",configPath:l,errors:t.length>0?t:void 0}}catch(d){t.push(`Failed to parse ${a}: ${d instanceof Error?d.message:String(d)}`)}}return{platform:"opencode",status:"n/a",errors:t.length>0?t:void 0}}import{join as Hc}from"node:path";function Ct(n){return Hc(n,".pi","agent","settings.json")}function Ut(n){if(typeof n!=="string")return!1;return n==="npm:cc-safety-net"||n.startsWith("npm:cc-safety-net@")}function Xa(n){let t=Ct(n.homeDir),r=t2(t);if(r.kind==="unreadable")return{platform:"pi",status:"not-inspected"};if(r.kind==="missing")return{platform:"pi",status:"n/a"};let e=$(r.value,"packages");if(!Array.isArray(e))return{platform:"pi",status:"n/a"};let a=e.find((s)=>Ut(typeof s==="string"?s:$(s,"source")));if(a===void 0)return{platform:"pi",status:"n/a"};let l=$(a,"extensions");if(Array.isArray(l)&&l.some((s)=>typeof s==="string"&&s.startsWith("-")))return{platform:"pi",status:"disabled",method:"package config",configPath:t,errors:["npm:cc-safety-net is installed but its extension is disabled in Pi settings"]};return{platform:"pi",status:"configured",method:"package config",configPath:t}}var Ac={amp:Q0,"antigravity-cli":X0,"claude-code":W0,codex:A0,"copilot-cli":_0,cursor:aa,"gemini-cli":la,"hermes-agent":ya,"kimi-code":Jn,openclaw:Ja,opencode:Qa,pi:Xa};function rn(n,t){let r={...t,cwd:n,homeDir:t?.homeDir??Wc()};return Ze.map((e)=>Kc(Ac[e](r)))}function Kc(n){if(n.status==="not-inspected")return{platform:n.platform,detected:!1,configured:!1,inspectionStatus:"not-inspected"};return{platform:n.platform,detected:n.status!=="n/a",configured:n.status==="configured",inspectionStatus:n.status!=="n/a"?"verified":n.errors&&n.errors.length>0?"failed":"not-applicable",method:n.method,configPath:n.configPath,configPaths:n.configPaths,errors:n.errors}}import{tmpdir as Rc}from"node:os";import{join as Vc}from"node:path";var Mc=Object.freeze([{command:"git reset --hard",description:"git reset --hard",expectBlocked:!0},{command:"rm -rf /",description:"rm -rf /",expectBlocked:!0},{command:"rm -rf ./node_modules",description:"rm in cwd (safe)",expectBlocked:!1}]),Gc=Object.freeze({state:"ready",diagnostics:Object.freeze([]),ruleMetadata:Object.freeze({}),policy:Object.freeze({rules:Object.freeze([]),transparentWrappers:Object.freeze([]),safety:Object.freeze({}),worktreeMode:!1,destructiveCommandProtectionEnabled:!0,destructiveCommandRuleOverrides:Object.freeze({}),destructiveCommandAllowPaths:Object.freeze([]),secretProtection:Object.freeze({enabled:!0,disabledRules:Object.freeze([]),denyPaths:Object.freeze([])})})}),Cc={strict:!1,paranoidRm:!1,paranoidInterpreters:!1,worktreeMode:!1,effectiveLevel:"standard",capabilities:{fail_closed:{enabled:!1,source:"preset",sources:[]},paranoid_rm:{enabled:!1,source:"preset",sources:[]},paranoid_interpreters:{enabled:!1,source:"preset",sources:[]}},sources:{failClosed:[],paranoidRm:[],paranoidInterpreters:[],worktreeMode:[]}};function Ya(){let n=Vc(Rc(),"cc-safety-net-self-test"),t=Mc.map((r)=>{let e=Gn(M2("self-test",{command:r.command},{kind:"command",shell:"auto"},{configCwd:n,executionCwd:n},r.command),{guard:{dependencies:{loadPolicySnapshot:()=>Gc,getModes:()=>Cc,findPolicyMutation:()=>null}},audit:{agent:"self-test",getSessionId:()=>{return}}}),a=r.expectBlocked?"blocked":"allowed",l=e.decision.kind==="deny"?"blocked":"allowed";return{command:r.command,description:r.description,expected:a,actual:l,passed:a===l,reason:e.decision.kind==="deny"?e.decision.reason:void 0,ruleId:e.decision.kind==="deny"?e.decision.ruleId:void 0}});return{passed:t.filter((r)=>r.passed).length,failed:t.filter((r)=>!r.passed).length,total:t.length,results:t}}function Ot(n){let t=F({label:"doctor",booleans:{json:["--json"],skipUpdateCheck:["--skip-update-check"]}},n);if(a2(t.errors))return null;return{json:t.flags.json,skipUpdateCheck:t.flags.skipUpdateCheck}}async function Sa(n={}){let t=await qn(!n.json,()=>{let r=Uc(n);return{ready:r,finish:()=>r}},()=>kn(),{loadingMessage:"Checking system status…"});if(n.json)console.log(JSON.stringify(t,null,2));else Tc(t);return Oc(t.hooks,t.engineSelfTest,{userConfig:t.userConfig,projectConfig:t.projectConfig})?1:0}async function Uc(n){let t=n.cwd??process.cwd(),r=await gn(),e=rn(t,{ampPluginListOutput:r.ampPluginListOutput,codexPluginListOutput:r.codexPluginListOutput,copilotCliVersion:r.copilotCliVersion}),a=e0(t),l=a0(),d=K({cwd:t}),s=d.policy,L=s2(s),c=D2(s,L.capabilities),i=Tn(7),v=n.skipUpdateCheck?{currentVersion:z(),latestVersion:null,updateAvailable:!1}:await P2(),b={hooks:e,engineSelfTest:Ya(),userConfig:a.userConfig,projectConfig:a.projectConfig,configState:wn(d),effectiveRules:a.effectiveRules,shadowedRules:a.shadowedRules,environment:l,effectiveSafety:{selectedPreset:s.safety.level??"standard",level:L.effectiveLevel,capabilities:L.capabilities,ruleOverrides:s.destructiveCommandRuleOverrides,weakenedRuleOverrides:Object.entries(c).filter(([,p])=>p.source==="rule_override"&&p.override==="off"&&p.inheritedEnabled&&p.changesInherited).map(([p])=>p),ruleCounts:{stored:Object.keys(s.destructiveCommandRuleOverrides).length,effective:Object.values(c).filter((p)=>p.changesInherited).length}},posture:h0(a.userConfig.path),activity:i,update:v,system:r};return{...b,findings:d0(b)}}function Oc(n,t,r){return n.length>0&&n.every((e)=>!e.configured)||n.some((e)=>e.inspectionStatus==="failed")||t.failed>0||r.userConfig.exists&&!r.userConfig.valid||r.projectConfig.exists&&!r.projectConfig.valid}function Tc(n){console.log(),console.log(L0(n.hooks)),console.log(),console.log(c0(n.engineSelfTest)),console.log(),console.log(i0(n)),console.log(),console.log(p0(n.environment)),console.log(),console.log(v0(n)),console.log(),console.log(u0(n.findings)),console.log(),console.log(b0(n.activity)),console.log(),console.log(f0(n.system)),console.log(),console.log(o0(n.update)),console.log(y0(n))}import{existsSync as Ec}from"node:fs";var _c=/^[A-Za-z0-9_@%+=:,./-]+$/,Ha="Usage: cc-safety-net explain [--json] [--cwd <path>] <command>";function Tt(n){let t=F({label:"explain",booleans:{json:["--json"]},values:{cwd:["--cwd"]},positionals:"tail"},n);if(a2(t.errors))return console.error(Ha),console.error("Pass -- before a command that starts with dashes."),null;if(t.values.cwd!==void 0&&!Ec(t.values.cwd))return console.error(`Error: --cwd path does not exist: ${t.values.cwd}`),null;let r=t.positionals.length===1?t.positionals[0]:t.positionals.map((e)=>_c.test(e)?e:`'${e.replaceAll("'","'\\''")}'`).join(" ");if(!r)return console.error("Error: No command provided"),console.error(Ha),null;return{json:t.flags.json,cwd:t.values.cwd,command:r}}function Wa(n){if(n)return{dh:"=",dv:"|",dtl:"+",dtr:"+",dbl:"+",dbr:"+",h:"-",v:"|",tl:"+",tr:"+",bl:"+",br:"+",sh:"="};return{dh:"═",dv:"║",dtl:"╔",dtr:"╗",dbl:"╚",dbr:"╝",h:"─",v:"│",tl:"┌",tr:"┐",bl:"└",br:"┘",sh:"━"}}function Aa(n,t){let e=t-18;return[`${n.dtl}${n.dh.repeat(t)}${n.dtr}`,`${n.dv}  Command Analysis${" ".repeat(e)}${n.dv}`,`${n.dbl}${n.dh.repeat(t)}${n.dbr}`]}function Et(n){return JSON.stringify(n)}function Ka(n,t=0){return`[${n.map((e,a)=>s0(e,a,t)).join(",")}]`}function c1(n,t,r=70){let e=n.split(" "),a=[],l="";for(let d of e)if(l&&l.length+d.length+1>r)a.push(l),l=d;else l=l?`${l} ${d}`:d;if(l)a.push(l);return a.map((d,s)=>s===0?d:`${t}${d}`)}function Ra(n,t,r){let e=[];switch(n.type){case"parse":return null;case"env-strip":{e.push(""),e.push(`STEP ${t} ${r.h} Strip environment variables`);let a=Object.keys(n.envVars);return e.push(`  Removed: ${a.map((l)=>`${l}=<redacted>`).join(", ")}`),e.push(`  Tokens:  ${Et(n.output)}`),{lines:e,incrementStep:!0}}case"leading-tokens-stripped":return e.push(""),e.push(`STEP ${t} ${r.h} Strip wrappers`),e.push(`  Removed: ${n.removed.join(", ")}`),e.push(`  Tokens:  ${Et(n.output)}`),{lines:e,incrementStep:!0};case"shell-wrapper":return e.push(""),e.push(`STEP ${t} ${r.h} Detect shell wrapper`),e.push(`  Wrapper: ${n.wrapper} -c`),e.push(`  Inner:   ${n.innerCommand}`),{lines:e,incrementStep:!0};case"interpreter":{if(e.push(""),e.push(`STEP ${t} ${r.h} Detect interpreter`),e.push(`  Interpreter: ${n.interpreter}`),e.push(`  Code:        ${n.codeArg}`),n.paranoidBlocked)e.push("  Result:      ✗ BLOCKED (paranoid mode)");return{lines:e,incrementStep:!0}}case"busybox":return e.push(""),e.push(`STEP ${t} ${r.h} Busybox wrapper`),e.push(`  Subcommand: ${n.subcommand}`),{lines:e,incrementStep:!0};case"transparent-wrapper":return e.push(""),e.push(`STEP ${t} ${r.h} Transparent wrapper`),e.push(`  Wrapper: ${n.wrapper}`),e.push(`  Tokens:  ${Et(n.output)}`),{lines:e,incrementStep:!0};case"recurse":return{lines:[],incrementStep:!1};case"rule-check":{e.push(""),e.push(`STEP ${t} ${r.h} Match rules`);let a=`${n.ruleModule}:${n.ruleFunction}()`;if(e.push(`  Rule:   ${a}`),n.matched)e.push("  Result: MATCHED");else e.push("  Result: No match");return{lines:e,incrementStep:!0}}case"worktree-relaxation":return e.push(""),e.push(`STEP ${t} ${r.h} Worktree relaxation`),e.push(`  Mode:   ${m.worktree.name}`),e.push(`  Git cwd: ${n.gitCwd}`),e.push("  Result: Allowed local discard in linked worktree"),{lines:e,incrementStep:!0};case"tmpdir-check":return null;case"fallback-scan":{if(n.embeddedCommandFound)return e.push(""),e.push(`STEP ${t} ${r.h} Fallback scan`),e.push(`  Found: ${n.embeddedCommandFound}`),{lines:e,incrementStep:!0};return null}case"custom-rules-check":{if(n.rulesChecked){if(e.push(""),e.push(`STEP ${t} ${r.h} Custom rules`),n.matched)e.push("  Result: MATCHED");else e.push("  Result: No match");return{lines:e,incrementStep:!0}}return null}case"cwd-change":return null;case"dangerous-text":{if(n.matched)return e.push(""),e.push(`STEP ${t} ${r.h} Dangerous text check`),e.push(`  Token:  ${n.token}`),e.push("  Result: MATCHED"),{lines:e,incrementStep:!0};return null}case"strict-unparseable":return e.push(""),e.push(`STEP ${t} ${r.h} Strict mode check`),e.push(`  Command: ${n.rawCommand}`),e.push("  Result:  ✗ UNPARSEABLE"),{lines:e,incrementStep:!0};case"segment-skipped":return null;case"error":return e.push(""),e.push(`ERROR: ${n.message}`),{lines:e,incrementStep:!1};default:return null}}function _t(n,t){let r=Wa(t?.asciiOnly??!1),e=58,a=[],l=1;a.push(...Aa(r,58)),a.push("");let d=n.trace.steps.find((p)=>p.type==="error");if(d&&d.type==="error"){a.push("ERROR"),a.push(`  ${d.message}`),a.push(""),a.push("RESULT"),a.push(`  Status: ${n.result==="blocked"?o.red("BLOCKED"):o.green("ALLOWED")}`),a.push(""),a.push("CONFIG");let p=n.configSource??"none";return a.push(`  Path: ${p}`),a.join(`
`)}let s=n.trace.steps.find((p)=>p.type==="parse");if(s&&s.type==="parse"){a.push("INPUT"),a.push(`  ${s.input}`),a.push(""),a.push(`STEP ${l} ${r.h} Split shell commands`),l++;for(let p=0;p<s.segments.length;p++){let f=s.segments[p];if(f){let u=Math.random();a.push(`  Segment ${p+1}: ${Ka(f,u)}`)}}}let L=n.trace.segments,c=L.length>1;for(let p of L){if(c){a.push("");let x="";if(s&&s.type==="parse"){let Z1=s.segments[p.index];if(Z1)x=Z1.join(" ")}let w=54,h=x,Z=` Segment ${p.index+1}: `,Q2=" ";if(x){if(Z.length+x.length+Q2.length>w){let Hl=w-Z.length-Q2.length;h=`${x.substring(0,Hl-1)}…`}}let Ln=x?`${Z}${h}${Q2}`:` Segment ${p.index+1} `,J1=x?`${Z}${o.cyan(h)}${Q2}`:Ln,mr=58-Ln.length,kr=Math.floor(mr/2),Sl=mr-kr;a.push(`${r.sh.repeat(kr)}${J1}${r.sh.repeat(Sl)}`)}if(p.steps.find((x)=>x.type==="segment-skipped")){a.push(""),a.push("  (skipped — prior segment blocked)");continue}let u=!1,y=!1;for(let x of p.steps){let w=Ra(x,l,r);if(w){if(y=!0,x.type==="recurse"){a.push("");let h=" RECURSING ",Z=58-h.length-4;a.push(`  ${r.tl}${r.h}${h}${r.h.repeat(Z)}`),a.push(`  ${r.v}`),u=!0;continue}for(let h of w.lines)if(u)a.push(`  ${r.v} ${h}`);else a.push(h);if(w.incrementStep)l++}}if(u)a.push(`  ${r.v}`),a.push(`  ${r.bl}${r.h.repeat(56)}`),u=!1;if(!y)a.push(""),a.push(`  ${o.green("✓")} Allowed (no matching rules)`)}if(a.push(""),a.push("RESULT"),n.result==="blocked"){if(a.push(`  Status: ${o.red("BLOCKED")}`),n.customRule){if(a.push(`  Rule: ${n.customRule.id}`),n.customRule.rulebook)a.push(`  Rulebook: ${n.customRule.rulebook.name} ${n.customRule.rulebook.version}`);if(n.customRule.source)a.push(`  Source: ${n.customRule.source}`);if(n.customRule.override)a.push(`  Override: reason ${n.customRule.override.reason}`)}if(n.reason){let p=c1(n.reason,"          ");a.push(`  Reason: ${p[0]}`);for(let f=1;f<p.length;f++)a.push(p[f]??"")}}else a.push(`  Status: ${o.green("ALLOWED")}`);a.push(""),a.push("CONFIG");let i=n.configSource??"none",v=n.configValid?"":" (invalid)";a.push(`  Path: ${i}${v}`),a.push(`  Safety preset: ${n.selectedPreset??"standard"}`),a.push(`  Effective capabilities: ${n.effectiveLevel}`);let b=Object.entries(n.destructiveCommandRuleOverrides??{});if(a.push(`  Rule customizations: ${b.length}`),n.ruleActivation)a.push(`  Rule activation: ${n.ruleActivation.id} — ${n.ruleActivation.enabled?"on":"off"} via ${n.ruleActivation.source}`);return a.join(`
`)}function Pt(n){return JSON.stringify(n,null,2)}var Va="2.0.5",C="  ",z2="cc-safety-net";function Ma(n){return n.argument?`${n.flags} ${n.argument}`:n.flags}function Pc(n){return Math.max(...n.map((t)=>Ma(t).length))}function Ic(n){return Math.max(...n.map((t)=>t.usage.length))}function Nc(n){return Math.max(...n.map((t)=>`${z2} ${t.usage}`.length))}function ni(n,t){let r=`${z2} ${n.usage}`;return`${C}${r.padEnd(t+2)}${n.description}`}function p2(n,t){return`${C}${n.padEnd(Math.max(40,n.length+2))}${t}`}function i1(n,t=console.log){let r=[];if(r.push(`${z2} ${n.name}`),r.push(""),r.push(`${C}${n.description}`),r.push(""),r.push("USAGE:"),r.push(`${C}${z2} ${n.usage}`),r.push(""),n.subcommands&&n.subcommands.length>0){r.push("SUBCOMMANDS:");let e=Ic(n.subcommands);for(let a of n.subcommands)r.push(`${C}${a.usage.padEnd(e+2)}${a.description}`);r.push("")}if(n.options.length>0){r.push("OPTIONS:");let e=Pc(n.options);for(let a of n.options){let l=Ma(a),d=a.default?`${a.description} (default: ${a.default})`:a.description;r.push(`${C}${l.padEnd(e+2)}${d}`)}r.push("")}if(n.examples&&n.examples.length>0){r.push("EXAMPLES:");for(let e of n.examples)r.push(`${C}${e}`)}t(r.join(`
`))}function It(){let n=t0(),t=Nc(n),r=[];r.push(`${z2} v${Va}`),r.push(""),r.push("Blocks destructive commands and secret access."),r.push(""),r.push("COMMANDS:");for(let e of n)r.push(ni(e,t));r.push(""),r.push("GLOBAL OPTIONS:"),r.push(`${C}-h, --help       Show help (use with command for command-specific help)`),r.push(`${C}-V, --version    Show version`),r.push(""),r.push("HELP:"),r.push(`${C}${z2} help <command>     Show help for a specific command`),r.push(`${C}${z2} <command> --help   Show help for a specific command`),r.push(""),r.push("ENVIRONMENT VARIABLES:"),r.push(p2(`${m.level.name}=standard|strict|paranoid`,"Set session safety level")),r.push(p2(`${m.worktree.name}=1`,"Allow local git discards in linked worktrees")),r.push(p2(`${m.debug.name}=1`,"Print diagnostic messages to stderr")),r.push(p2(`${m.auditScope.name}=all|blocked`,"Record all command decisions, or denials only")),r.push(p2("CC_SAFETY_NET_HOME","Override rule config home directory")),r.push(""),r.push("LEGACY ENVIRONMENT VARIABLES (STILL SUPPORTED):"),r.push(p2(`${m.strict.name}=1`,"Force safety.overrides.fail_closed on")),r.push(p2(`${m.paranoid.name}=1`,"Force paranoid_rm and paranoid_interpreters on")),r.push(p2(`${m.paranoidRm.name}=1`,"Force safety.overrides.paranoid_rm on")),r.push(p2(`${m.paranoidInterpreters.name}=1`,"Force safety.overrides.paranoid_interpreters on")),r.push(""),r.push("Documentation:        https://ccsafetynet.com/docs"),console.log(r.join(`
`))}function Ga(){console.log(Va)}function Xn(n,t=console.log){let r=On(n);if(!r)return!1;if(r.hidden||r.name.toLowerCase()!==n.toLowerCase())return!1;return i1(r,t),!0}import{existsSync as br,readFileSync as C3}from"node:fs";import{homedir as s5}from"node:os";import{join as vr}from"node:path";import*as y2 from"node:readline";function ti(n){return n==="install"?"Install":"Uninstall"}function ri(n){return n==="install"?"Installing":"Uninstalling"}function ei(n){return n==="install"?"into":"from"}function Oa(n){return n?.available===!0}function ai(n,t){let r=new Set(t);return n.filter((e)=>r.has(e.target)).map((e)=>e.target)}function Ca(n,t,r){if(n.length===0||n.every((e)=>!e.available))return t;return Array.from({length:n.length},(e,a)=>a+1).map((e)=>(t+e*r+n.length)%n.length).find((e)=>Oa(n[e]))}function li(n,t,r){if(r.ctrl&&r.name==="c")return"interrupt";if(r.name==="escape"||t==="q")return"abort";if(n==="install"&&(t==="u"||t==="U"))return"update";if(r.name==="up"||t==="k")return"up";if(r.name==="down"||t==="j")return"down";if(r.name==="space"||t===" ")return"toggle";if(r.name==="return"||r.name==="enter")return"confirm";return null}function di(n){return{cursor:n.findIndex((t)=>t.available),selected:[]}}function si(n,t,r){if(r==="confirm"||r==="update"||r==="abort"||r==="interrupt")return{state:n,done:r};if(r==="up")return{state:{...n,cursor:Ca(t,n.cursor,-1)}};if(r==="down")return{state:{...n,cursor:Ca(t,n.cursor,1)}};let e=t[n.cursor];if(!Oa(e))return{state:n};let a=n.selected.includes(e.target)?n.selected.filter((l)=>l!==e.target):ai(t,[...n.selected,e.target]);return{state:{...n,selected:a}}}var Ta="◉",Ea="◯",_a=">",Pa=" ";function Li(n,t,r,e={}){let a=e.color!==!1,l=a?o.dim:(L)=>L,d=a?o.green:(L)=>L,s=a?o.bold:(L)=>L;return["",`${ti(n)} CC Safety Net ${ei(n)}:`,"",...t.map((L,c)=>{let i=r.selected.includes(L.target),v=c===r.cursor,b=i?Ta:Ea,p=v?_a:Pa,f=L.available?"":` (${L.unavailableReason??"not installed"})`,u=`${b} ${L.label}${f}`,y=!L.available?l(u):i?d(u):v?s(u):u;return`${p} ${y}`}),"",n==="install"?"Space: select  Enter: confirm  u: update installed  Up/Down: move  q/Esc: cancel":t.some((L)=>L.available)?"Space: select  Enter: confirm  Up/Down: move  q/Esc: cancel":`No selectable integrations found for ${n}. q/Esc: close`].join(`
`)}var Ua=["global-hook","plugin"];function ci(n,t,r={}){let e=r.color!==!1?o.bold:(l)=>l;return["","Install the Kimi Code integration as:","",...[`Global hook — ${t?"already installed; selecting it reports the current state":"write the hook into ~/.kimi-code/config.toml now"}`,"Native Kimi plugin — print the steps to run inside Kimi Code"].map((l,d)=>{let s=d===n,L=`${s?Ta:Ea} ${l}`;return`${s?_a:Pa} ${s?e(L):L}`}),"","Enter: confirm  Up/Down: move  q/Esc: cancel"].join(`
`)}function Ia(n){let{input:t,output:r}=n;y2.emitKeypressEvents(t);let e=t.isRaw===!0;t.setRawMode(!0),t.resume();let a=0,l=()=>{if(a===0)return;y2.moveCursor(r,0,-a),y2.cursorTo(r,0),y2.clearScreenDown(r)},d=()=>{l();let s=n.render();r.write(`${s}
`),a=s.split(`
`).length};return new Promise((s)=>{let L=(i)=>{t.off("keypress",c),t.setRawMode(e),t.pause(),l(),s(i)};function c(i,v){n.onKey(i,v,{finish:L,draw:d})}t.on("keypress",c),d()})}function Na(n={}){let t=0;return Ia({input:n.input??process.stdin,output:n.output??process.stdout,render:()=>ci(t,n.globalHookInstalled===!0),onKey:(r,e,a)=>{if(e.ctrl&&e.name==="c"){a.finish(null),(n.onInterrupt??(()=>process.kill(process.pid,"SIGINT")))();return}if(e.name==="escape"||r==="q")return a.finish(null);if(e.name==="return"||e.name==="enter")return a.finish(Ua[t]);if(e.name==="up"||e.name==="down"||r==="k"||r==="j")t=(t+1)%Ua.length,a.draw()}})}function Nt(n=process.stdin,t=process.stdout){return Boolean(n.isTTY&&t.isTTY&&typeof n.setRawMode==="function")}function n3(n,t,r={}){let e=r.output??process.stdout,a=di(t);return Ia({input:r.input??process.stdin,output:e,render:()=>Li(n,t,a),onKey:(l,d,s)=>{let L=li(n,l,d);if(!L)return;let c=si(a,t,L);if(a=c.state,c.done==="interrupt"){s.finish(null),(r.onInterrupt??(()=>process.kill(process.pid,"SIGINT")))();return}if(c.done==="abort")return s.finish(null);if(c.done==="update")return s.finish("update");if(c.done==="confirm"){if(a.selected.length===0){e.write("\x07"),s.draw();return}s.finish([...a.selected]),e.write(`${ri(n)} selected integrations...
`);return}s.draw()}})}import{existsSync as r3,lstatSync as pi,mkdirSync as vi,mkdtempSync as ui,readdirSync as bi,readFileSync as an,rmSync as u1}from"node:fs";import{tmpdir as oi}from"node:os";import{basename as fi,dirname as yi,join as W}from"node:path";import{fileURLToPath as hi}from"node:url";var nr="// cc-safety-net managed Amp plugin. Do not edit. Reinstall with: npx -y cc-safety-net install --amp",F2="cc-safety-net",B2="cc-safety-net/index.ts";import{spawn as ii}from"node:child_process";var tr=(n,t)=>{let r=f2([...n],process.env);return new Promise((e)=>{let a=ii(r.cmd,r.args,{cwd:t,stdio:["ignore","pipe","pipe"]}),l=Kt(a),d=!1,s=setTimeout(()=>{d=!0,a.kill()},120000);a.on("error",(L)=>{clearTimeout(s),e({status:null,errorCode:L.code,stdout:l.stdout,stderr:[L.message,l.stderr].filter(Boolean).join(`
`)})}),a.on("close",(L)=>{clearTimeout(s),e({status:d?null:L,errorCode:d?"ETIMEDOUT":void 0,stdout:l.stdout,stderr:l.stderr})})})};var en="cc-safety-net.ts",t3=W("amp",B2);function wi(n){return W(n,".config","amp","plugins","cc-safety-net.ts")}function xi(){let n=yi(hi(import.meta.url));return[W(n,"..",t3),W(n,"..","..","..","dist",t3)]}function $i(n=xi()){let t=n.find((r)=>r3(r)&&pi(r).isFile());if(!t)throw Error("Packaged Amp plugin artifact not found. Reinstall cc-safety-net and try again.");return t}function e3(n){try{return JSON.parse(n)}catch{return}}function b1(n){return n.subarray(0,Buffer.byteLength(nr)).toString("utf-8")===nr}async function Yn(n,t,r){let e=await n(t,r);if(e.status===0)return e;throw Error([`Failed to run ${t.join(" ")}${e.status===null?"":` (exit ${e.status})`}.`,[e.stdout,e.stderr].filter(Boolean).join(`
`).trim()].filter(Boolean).join(`
`))}async function a3(n){let t=await n(["amp","plugins","repositories","--json"]);if(t.status===null)throw Error(`${t.errorCode==="ENOENT"?'Amp CLI not found. Install the amp CLI, sign in with "amp login", and rerun install --amp.':`amp plugins repositories --json did not finish (${t.errorCode??"terminated"}). Check that the amp CLI responds and rerun install --amp.`}
...
`)}`.trim());let r=e3(t.stdout),e=(Array.isArray(r)?r:[]).filter((a)=>$(a,"scope")==="user"&&$(a,"exists")===!0&&$(a,"viewerCanWrite")===!0).map((a)=>$(a,"cloneRef")).find((a)=>typeof a==="string"&&a.length>0);if(!e)throw Error('Your Amp account has no writable Personal Plugins repository. Sign in with "amp login", open Amp once to create it, and rerun install --amp.');return e}async function l3(n,t){let r=ui(W(oi(),"cc-safety-net-amp-"));try{return await Yn(n,["amp","clone","user-plugins",r]),await t(r)}finally{u1(r,{recursive:!0,force:!0})}}function rr(n){return`rerun ${n==="overwrite"?"install":"uninstall"} --amp`}function d3(n,t,r){let e=W(n,t),a=j(e);if(!a)return;if(a.isSymbolicLink()||!a.isFile())throw Error(`Refusing to ${r} ${t} in your Amp personal plugins repository: not a regular file. Remove it there and ${rr(r)}.`);let l=an(e);if(b1(l))return l;throw Error(`Refusing to ${r} unmanaged file ${t} in your Amp personal plugins repository. Remove it there and ${rr(r)}.`)}function s3(n,t){let r=W(n,F2),e=j(r);if(!e)return;if(e.isSymbolicLink()||!e.isDirectory())throw Error(`Refusing to ${t} ${F2} in your Amp personal plugins repository: not a regular directory. Remove it there and ${rr(t)}.`);return d3(n,B2,t)}function Di(n){let t=W(n,en),r=j(t);if(!r||r.isSymbolicLink()||!r.isFile())return;let e=an(t);return b1(e)?e:void 0}async function L3(n,t,r,e){if(await Yn(n,r,t),(await Yn(n,["git","status","--porcelain"],t)).stdout.trim()==="")return!1;return await Yn(n,["git","-c","commit.gpgsign=false","-c","user.name=cc-safety-net","-c","user.email=cc-safety-net@localhost","commit","-m",e],t),await Yn(n,["git","push","origin","HEAD"],t),!0}function p1(n,t){gi(n,t),mi(n,t)}function c3(n,t){if(t==="keep")return;throw Error(`Local Amp plugin ${n} is not a managed copy and masks the personal plugin. Remove it and rerun install --amp.`)}function gi(n,t){let r=wi(n),e=j(r);if(!e)return;if(!e.isSymbolicLink()&&e.isFile()&&b1(an(r))){u1(r);return}c3(r,t)}function mi(n,t){let r=W(n,".config","amp","plugins",F2),e=j(r);if(!e)return;if(!e.isSymbolicLink()&&e.isDirectory()&&ki(r)){u1(r,{recursive:!0});return}c3(r,t)}function ki(n){let t=fi(B2);if(bi(n).join("\x00")!==t)return!1;let r=W(n,t),e=j(r);return!!e&&!e.isSymbolicLink()&&e.isFile()&&b1(an(r))}function qi(){let n=hn();if(!r3(n))return"";let t=e3(an(n,"utf-8"));if(!t||typeof t!=="object"||Array.isArray(t))return"";return`;globalThis.__CC_SAFETY_NET_EMBEDDED_POLICY__ = ${JSON.stringify(v1(t))};
`}async function i3(n,t=$i(),r=tr){let e=Buffer.concat([an(t),Buffer.from(qi(),"utf-8")]),a=await a3(r);return l3(r,async(l)=>{let d=`${a}/${F2}`,s=s3(l,"overwrite"),L=d3(l,en,"overwrite");if(s?.equals(e)&&!L)return p1(n,"fail"),{path:d,alreadyInstalled:!0};if(vi(W(l,F2),{recursive:!0}),J(W(l,B2),e),L)u1(W(l,en));let c=await L3(r,l,["git","add","--",B2,...L?[en]:[]],`chore: update cc-safety-net plugin to v${z()}`);return p1(n,"fail"),{path:d,alreadyInstalled:!c}})}async function p3(n,t=tr){let r=await a3(t);return l3(t,async(e)=>{let a=s3(e,"remove"),l=Di(e),d=`${r}/${l&&!a?en:F2}`;if(!a&&!l)return p1(n,"keep"),{path:d,alreadyInstalled:!1};return await L3(t,e,["git","rm","--",...a?[B2]:[],...l?[en]:[]],`chore: remove cc-safety-net plugin v${z()}`),p1(n,"keep"),{path:d,alreadyInstalled:!0}})}import{existsSync as v3,mkdirSync as ji,readFileSync as zi}from"node:fs";import{dirname as Fi}from"node:path";var er="npx -y cc-safety-net hook --agy-cli",h2="cc-safety-net";function f1(){return{PreToolUse:[{hooks:[{type:"command",command:er,timeout:30}]}]}}function u3(n){try{let t=JSON.parse(zi(n,"utf-8"));if(!t||typeof t!=="object"||Array.isArray(t))throw Error("Antigravity hooks config must be a JSON object");return t}catch(t){if(t instanceof SyntaxError)throw Error(`Failed to parse Antigravity hooks config ${n}: ${t.message}`);throw t}}function b3(n){let t=n[h2];if(t===void 0)return n[h2]=f1(),n[h2];if(!t||typeof t!=="object"||Array.isArray(t))throw Error(`Antigravity hooks config entry "${h2}" must be an object`);if(!Array.isArray(t.PreToolUse))t.PreToolUse=[];return t}function o3(n){if(!Array.isArray(n.PreToolUse))return!1;return n.PreToolUse.some((t)=>Array.isArray(t.hooks)&&t.hooks.some((r)=>r.command===er))}function Bi(n){return Object.values(n).some((t)=>t.enabled!==!1&&o3(t))}function Ji(n){if(n[h2]===void 0)return!1;let t=b3(n);if(t.enabled!==!1||!o3(t))return!1;return t.enabled=!0,!0}function Zi(n){if(n[h2]===void 0){n[h2]=f1();return}let t=b3(n);t.PreToolUse??=[],t.enabled=!0,t.PreToolUse.push(f1().PreToolUse?.[0]??{hooks:[]})}function Qi(n){let t=!1;for(let r of Object.values(n)){if(!Array.isArray(r.PreToolUse))continue;r.PreToolUse=r.PreToolUse.flatMap((e)=>{if(!Array.isArray(e.hooks))return[e];let a=e.hooks.filter((l)=>l.command!==er);if(a.length!==e.hooks.length)t=!0;return a.length===0?[]:[{...e,hooks:a}]})}return t}function o1(n,t){J(n,`${JSON.stringify(t,null,2)}
`)}function f3(n){let t=jn(n);if(ji(Fi(t),{recursive:!0}),!v3(t))return o1(t,{[h2]:f1()}),{path:t,alreadyInstalled:!1};let r=u3(t);if(Bi(r))return{path:t,alreadyInstalled:!0};if(Ji(r))return o1(t,r),{path:t,alreadyInstalled:!1};return Zi(r),o1(t,r),{path:t,alreadyInstalled:!1}}function y3(n){let t=jn(n);if(!v3(t))return{path:t,alreadyInstalled:!1};let r=u3(t);if(!Qi(r))return{path:t,alreadyInstalled:!1};return o1(t,r),{path:t,alreadyInstalled:!0}}import{spawn as Xi,spawnSync as Yi}from"node:child_process";var U=i2.map((n)=>({target:n.id,flag:n.flag,label:n.installLabel,probeCommand:n.probeCommand})),h3=new Map(U.map((n)=>[n.flag,n.target]));function ar(n){let t=new Set(n);return U.map((r)=>r.target).filter((r)=>t.has(r))}async function w3(n,t){for(let r of n)await t(r)}var x3=5000;function Si(n){let t=f2([...n],process.env),r=Yi(t.cmd,t.args,{env:process.env,stdio:"ignore",timeout:x3});return!r.error&&r.status===0}function dr(n){return new Promise((t)=>{let r=f2([...n],process.env),e=Xi(r.cmd,r.args,{env:process.env,stdio:"ignore"}),a=!1,l=(s)=>{if(a)return;a=!0,clearTimeout(d),t(s)},d=setTimeout(()=>{e.kill(),l(!1)},x3);e.on("error",()=>l(!1)),e.on("close",(s)=>l(s===0))})}function Hi(n=Si,t={}){let r=new Set(t.configuredTargets??[]);if(t.async)return Promise.all(U.map(async(a)=>({target:a.target,flag:a.flag,label:a.label,...lr(t.action,await n(a.probeCommand),r.has(a.target))})));let e=n;return U.map((a)=>({target:a.target,flag:a.flag,label:a.label,...lr(t.action,e(a.probeCommand),r.has(a.target))}))}function $3(n=dr,t={}){return Hi(n,{...t,async:!0})}function D3(n,t){let r=new Set(t.configuredTargets??[]);return n.map((e)=>({...e,...lr(t.action,e.available,r.has(e.target))}))}function lr(n,t,r){if(n==="uninstall")return r?{available:!0}:{available:!1,unavailableReason:"not installed"};if(n==="install"&&r)return{available:!1,unavailableReason:"already installed"};if(!t)return{available:!1,unavailableReason:"CLI not installed"};return{available:!0}}import{existsSync as g3,readdirSync as Wi,rmSync as Ai}from"node:fs";import{join as ln}from"node:path";function y1(n,t=process.platform){let r=ln(process.env.npm_config_cache||(t==="win32"?ln(process.env.LOCALAPPDATA||ln(n,"AppData","Local"),"npm-cache"):ln(n,".npm")),"_npx");if(!g3(r))return;Wi(r).filter((e)=>g3(ln(r,e,"node_modules","cc-safety-net"))).forEach((e)=>{Ai(ln(r,e),{recursive:!0,force:!0})})}import{existsSync as j3,mkdirSync as Ri,readFileSync as z3}from"node:fs";import{dirname as Vi,join as q3}from"node:path";function m3(n){return n!==void 0&&/\s/.test(n)}function Ki(n,t,r){let e=t+1,a=!1;while(e<n.length){let l=n[e];if(a){a=!1,e++;continue}if(l==="\\"){a=!0,e++;continue}if(l==='"')return e+1;e++}throw Error(r)}function h1(n,t,r){let e=n[t],a=e==="["?"]":"}",l=0,d=t;while(d<n.length){let s=r.skipComment?.(n,d)??d;if(s!==d){d=s;continue}if(n[d]==='"'){d=Ki(n,d,r.stringError);continue}if(n[d]===e)l++;if(n[d]===a){if(l--,l===0)return d}d++}throw Error(r.bracketError)}function k3(n,t){let r=n.lastIndexOf(`
`,t)+1;return/^[ \t]*/.exec(n.slice(r))?.[0]??""}function w1(n,t){let{start:r,end:e,end:a}=t;while(m3(n[a]))a++;if(n[a]===","){if(e=a+1,n[e]===`
`)e++;return`${n.slice(0,r)}${n.slice(e)}`}a=t.start-1;while(m3(n[a]))a--;if(n[a]===","){r=a;let l=n.lastIndexOf(`
`,r-1);if(l!==-1&&/^\s*$/.test(n.slice(l+1,r)))r=l}return`${n.slice(0,r)}${n.slice(e)}`}var Sn="npx -y cc-safety-net hook --kimi-code",sr=`[[hooks]]
event = "PreToolUse"
command = "${Sn}"`,Lr=`{ event = "PreToolUse", command = "${Sn}" }`;function F3(n){return q3(process.env.KIMI_CODE_HOME??q3(n,".kimi-code"),"config.toml")}function Mi(n){return n.split(`
...
`)}function Gi(n,t){if(n[t]!=="#")return t;let r=n.indexOf(`
`,t+1);return r===-1?n.length:r+1}function Ci(n,t){return h1(n,t,{skipComment:Gi,stringError:"Unterminated string in Kimi Code config",bracketError:"Unmatched hooks array in Kimi Code config"})}function B3(n){let t=!1,r=0;while(r<n.length){let e=n.indexOf(`
`,r),a=e===-1?n.length:e,l=n.slice(r,a);if(/^\s*\[/.test(l))t=!0;if(!t){let d=/^(\s*)hooks\s*=\s*\[/.exec(l);if(d){let s=r+d[0].lastIndexOf("[");return{start:s,end:Ci(n,s)}}}r=e===-1?n.length:e+1}return}function Ui(n,t){let r=n.slice(0,t.end).trimEnd(),e=k3(n,t.end),a=e===""?"     ":`${e}  `,l=!r.endsWith("[")&&!r.endsWith(",");return`${r}${l?",":""}
${a}${Lr}${n.slice(t.end)}`}function Oi(n){let t=B3(n);if(t&&n.slice(t.start+1,t.end).trim())return Ui(n,t);let r=Mi(n).trimEnd();if(r==="")return`${sr}
`;return`${r}

${sr}
`}function Ti(n){return n.split(/(?=^\s*\[)/m).filter((r)=>!/^\s*\[\[hooks]]\s*$/m.test(r)||!r.includes(Sn)).join("").trimEnd()}function Ei(n,t){let r=n.indexOf(Lr,t.start);if(r===-1||r>t.end)return n;return w1(n,{start:r,end:r+Lr.length})}function J3(n){let t=F3(n);if(Ri(Vi(t),{recursive:!0}),!j3(t))return J(t,`${sr}
`),{path:t,alreadyInstalled:!1};let r=z3(t,"utf-8");if(r.includes(Sn))return{path:t,alreadyInstalled:!0};return J(t,Oi(r)),{path:t,alreadyInstalled:!1}}function Z3(n){let t=F3(n);if(!j3(t))return{path:t,alreadyInstalled:!1};let r=z3(t,"utf-8");if(!r.includes(Sn))return{path:t,alreadyInstalled:!1};let e=B3(r),a=e?Ei(r,e):`${Ti(r)}
`;return J(t,a),{path:t,alreadyInstalled:!0}}import{existsSync as Q3,readFileSync as _i,rmSync as Pi}from"node:fs";import{join as cr}from"node:path";var ir="cc-safety-net",Ii=`${ir}@latest`,Y3=["opencode.json","opencode.jsonc"];function Ni(n){return cr(n,".config","opencode",Y3[0])}function n5(n){return Y3.map((t)=>cr(n,".config","opencode",t))}function t5(n){return cr(n,".cache","opencode","packages",Ii)}function pr(n){Pi(t5(n),{recursive:!0,force:!0})}function x1(n,t){if(n[t]==="/"&&n[t+1]==="/"){let r=n.indexOf(`
`,t+2);return r===-1?n.length:r+1}if(n[t]==="/"&&n[t+1]==="*"){let r=n.indexOf("*/",t+2);return r===-1?n.length:r+2}return t}function X3(n,t){let r=t;while(r<n.length){if(/\s/.test(n[r]??"")){r++;continue}let e=x1(n,r);if(e===r)return r;r=e}return r}function S3(n,t){let r=t+1,e=!1;while(r<n.length){if(e){e=!1,r++;continue}if(n[r]==="\\"){e=!0,r++;continue}if(n[r]==='"')return r+1;r++}throw Error("Unterminated string in OpenCode config")}function H3(n,t,r){return JSON.parse(n.slice(t,r))}function r5(n,t){return h1(n,t,{skipComment:x1,stringError:"Unterminated string in OpenCode config",bracketError:"Unmatched plugin array in OpenCode config"})}function e5(n){let t=0,r=0;while(r<n.length){let e=x1(n,r);if(e!==r){r=e;continue}if(n[r]==='"'){let a=S3(n,r);if(t===1&&H3(n,r,a)==="plugin"){let l=X3(n,a),d=X3(n,l+1);if(n[l]===":"&&n[d]==="[")return{start:d,end:r5(n,d)}}r=a;continue}if(n[r]==="{"||n[r]==="[")t++;if(n[r]==="}"||n[r]==="]")t--;r++}return}function a5(n,t){let r=[],e=t.start+1;while(e<t.end){let a=x1(n,e);if(a!==e){e=a;continue}if(n[e]==='"'){let l=S3(n,e),d=H3(n,e,l);if(typeof d==="string"&&d.includes(ir))r.push({start:e,end:l});e=l;continue}e++}return r}function W3(n,t){try{return JSON.parse(R(n))}catch(r){if(r instanceof SyntaxError)throw Error(`Failed to parse OpenCode config ${t}: ${r.message}`);throw r}}function l5(n){if(!n||typeof n!=="object"||Array.isArray(n))return!1;let t=n.plugin;if(!Array.isArray(t))return!1;return t.some((r)=>typeof r==="string"&&r.includes(ir))}function d5(n,t){let r=e5(n);if(!r)throw Error(`Failed to locate OpenCode plugin array in ${t}`);let e=[...a5(n,r)].reverse().reduce(w1,n);return W3(e,t),e}function A3(n){pr(n);let t=n5(n),r=t.find((a)=>Q3(a)),e=[];for(let a of t){if(!Q3(a))continue;try{let l=_i(a,"utf-8");if(!l5(W3(l,a)))continue;return J(a,d5(l,a)),{path:a,alreadyInstalled:!0}}catch(l){e.push(l instanceof Error?l.message:String(l))}}if(e.length>0)throw Error(e.join(`
`));return{path:r??Ni(n),alreadyInstalled:!1}}var ur="safety-net@cc-marketplace",K3=new Set(["claude-code","codex","copilot-cli","gemini-cli","hermes-agent","openclaw","opencode","pi"]),R3=new Set(["antigravity-cli","cursor","hermes-agent","kimi-code"]);function or(n){return/^\s*safety-net@cc-marketplace[^a-z0-9-][^\n]*installed,/m.test(n??"")}function U3(n){return/^\s*cc-safety-net[^a-z0-9-][^\n]*installed,/m.test(n??"")}function L5(n){return/^Marketplace `cc-marketplace`\s*$/m.test(n??"")}var O3={"claude-code":{installCommands:(n)=>{let t=n1(n,"cc-safety-net@cc-marketplace");return{commands:[...t?[["claude","plugin","marketplace","update","cc-marketplace"],["claude","plugin","update","cc-safety-net@cc-marketplace"]]:[["claude","plugin","marketplace","add","kenryu42/cc-marketplace"],["claude","plugin","marketplace","update","cc-marketplace"],["claude","plugin","install","cc-safety-net@cc-marketplace"]],...gt(n).status==="disabled"?[["claude","plugin","enable","cc-safety-net@cc-marketplace"]]:[]],cleanupCommands:n1(n,ur)?[["claude","plugin","uninstall",ur]]:[],update:t}},uninstallCommands:[["claude","plugin","uninstall","cc-safety-net@cc-marketplace"],["claude","plugin","marketplace","remove","cc-marketplace"]]},codex:{installCommands:async(n,t)=>{let r=t??await e2(["codex","plugin","list"]),e=U3(r);return{commands:[e||L5(r)?["codex","plugin","marketplace","upgrade","cc-marketplace"]:["codex","plugin","marketplace","add","kenryu42/cc-marketplace"],["codex","plugin","add","cc-safety-net@cc-marketplace"]],cleanupCommands:or(r)?[["codex","plugin","remove","safety-net@cc-marketplace"]]:[],update:e}},uninstallCommands:[["codex","plugin","remove","cc-safety-net@cc-marketplace"],["codex","plugin","marketplace","remove","cc-marketplace"]],postInstallMessage:"Start Codex, open `/hooks`, select the cc-safety-net PreToolUse hook, and press `t` to trust it."},"copilot-cli":{installCommands:async()=>{let n=await e2(["copilot","plugin","list"]),t=[...C0(n)?[["copilot","plugin","uninstall","copilot-safety-net"]]:[],...U0(n)?[["copilot","plugin","uninstall",V0]]:[]];if(M0(n))return{commands:[["copilot","plugin","marketplace","update","cc-marketplace"],["copilot","plugin","update",r2]],cleanupCommands:t,update:!0};return{commands:[G0(await e2(["copilot","plugin","marketplace","list"]))?["copilot","plugin","marketplace","update","cc-marketplace"]:["copilot","plugin","marketplace","add","kenryu42/cc-marketplace"],["copilot","plugin","install",r2]],cleanupCommands:t}},uninstallCommands:[["copilot","plugin","uninstall","cc-safety-net@cc-marketplace"],["copilot","plugin","marketplace","remove","cc-marketplace"]]},"gemini-cli":{installCommands:(n)=>{let t=Jt(n);if(t.status==="configured")return{commands:[["gemini","extensions","update","gemini-safety-net"]],update:!0};if(t.status==="disabled")return{commands:[["gemini","extensions","update","gemini-safety-net"],["gemini","extensions","enable","gemini-safety-net"]],update:!0};return{commands:[["gemini","extensions","install","https://github.com/kenryu42/gemini-safety-net","--consent"]]}},uninstallCommands:[["gemini","extensions","uninstall","gemini-safety-net"]]},openclaw:{beforeInstall:Mt,installCommands:()=>({commands:ka()}),uninstallCommands:[["openclaw","plugins","uninstall",q,"--force"]],postInstallMessage:["Restart the OpenClaw Gateway to apply the change.","If plugins.allow is set in openclaw.json, it must also list cc-safety-net."].join(`
`)},opencode:{beforeInstall:pr,installCommands:[["opencode","plugin","-g","-f","cc-safety-net@latest"]]},pi:{installCommands:[["pi","install","npm:cc-safety-net"]],uninstallCommands:[["pi","uninstall","npm:cc-safety-net"]]}};function g1(){return process.env.HOME??s5()}function T3(n,t=(r)=>r){try{let r=JSON.parse(t(C3(n,"utf-8")));if(!r||typeof r!=="object"||Array.isArray(r))throw Error(`Settings file ${n} must be a JSON object`);return r}catch(r){if(r instanceof SyntaxError)throw Error(`Failed to parse ${n}: ${r.message}`);throw r}}function c5(n){let t=vr(zn(n),"settings.json");if(!br(t))return;let r=T3(t,R),e=r.enabledPlugins;if(!e||typeof e!=="object"||Array.isArray(e))return;if(e[r2]!==!1)return;let a=C3(t,"utf-8"),l=a.replace(new RegExp(`("${r2}"\\s*:\\s*)false`),"$1true");return e[r2]=!0,J(t,l!==a?l:`${JSON.stringify(r,null,2)}
`),`Enabled ${r2} plugin in ${t}`}function i5(n){let t=Ct(n);if(!br(t))return;let r=T3(t);if(!Array.isArray(r.packages))return;let e=r.packages.find((a)=>!!a&&typeof a==="object"&&!Array.isArray(a)&&Ut(a.source)&&("extensions"in a));if(!e)return;return delete e.extensions,J(t,`${JSON.stringify(r,null,2)}
`),`Enabled npm:cc-safety-net extensions in ${t}`}function V3(n,t){let r=F({label:t,booleans:Object.fromEntries(U.map((l)=>[l.target,[l.flag]]))},n),e=r.errors[0];if(e)throw Error(e);let a=U.filter((l)=>r.flags[l.target]).map((l)=>l.target);if(a.length!==1)throw Error(`Choose exactly one ${t} target: ${[...h3.keys()].join(", ")}`);return a[0]}async function E3(n=g1(),t=_2){let[r,e,a]=await Promise.all([t(["amp","plugins","list"],30000),t(["codex","plugin","list"],30000),t(["copilot","--binary-version"])]);return{codexPluginListOutput:e,hooks:rn(process.cwd(),{homeDir:n,ampPluginListOutput:r,codexPluginListOutput:e,copilotCliVersion:a})}}async function p5(n,t=_2){let r=await E3(g1(),t);return r.hooks.filter((e)=>n==="install"?e.configured:e.detected||e.inspectionStatus==="not-inspected").filter((e)=>e.platform!=="codex"||!or(r.codexPluginListOutput)||U3(r.codexPluginListOutput)).map((e)=>e.platform)}function v5(n,t,r){if(t.length>0)return{finish:async()=>[V3(t,n)]};if(!r.selectTargets&&!Nt(r.input,r.output))return{finish:async()=>[V3(t,n)]};let e=r.detectConfiguredTargets??(()=>p5(n,r.fetchVersion)),a=Promise.all([$3(r.probeTargets),e()]);return{ready:a,finish:async()=>{let[l,d]=await a,s=D3(l,{action:n,configuredTargets:d}),L=r.selectTargets?await r.selectTargets(n,G3(n,s)):await n3(n,G3(n,s),{input:r.input,output:r.output});if(L==="update")return L;if(!L||L.length===0)return null;return ar(L)}}}async function J2(n,t,r=!1,e){let a=O3[n];a.beforeInstall?.(t);let l=typeof a.installCommands==="function"?await a.installCommands(t,e):{commands:a.installCommands};return await Rt(l.commands),await wa(l.cleanupCommands??[]),[`${l.update||r?"Updated":"Installed"} ${o2(n)} integration`,a.postInstallMessage].filter(Boolean).join(`
`)}async function dn(n){let t=O3[n];if(!t.uninstallCommands)throw Error(`${o2(n)} uninstall is not supported`);return await Rt(t.uninstallCommands),`Uninstalled ${o2(n)} integration`}function u5(n){let t=A3(n);return t.alreadyInstalled?`Uninstalled OpenCode plugin from ${t.path}`:`OpenCode plugin not installed in ${t.path}`}var b5={"antigravity-cli":{install:f3,uninstall:y3},cursor:{install:ra,uninstall:ea},"kimi-code":{install:J3,uninstall:Z3}};function sn(n,t,r,e=!1){if(n==="install"&&!e)y1(r);let a=b5[t][n](r),l=o2(t),d=n!=="install"?"Uninstalled":e?"Updated":"Installed";return n==="install"&&a.alreadyInstalled?e?`${l} hook up to date in ${a.path}`:`${l} hook already installed in ${a.path}`:n==="uninstall"&&!a.alreadyInstalled?`${l} hook not installed in ${a.path}`:`${d} ${l} hook ${n==="install"?"in":"from"} ${a.path}`}var o5={amp:{install:i3,uninstall:p3,restartNote:'Amp personal plugins apply to every Amp session, including Orb threads. Restart Amp or run "plugins: reload" to apply the change.'},"hermes-agent":{install:ca,uninstall:ia,afterInstall:async(n)=>{let t=Wt(n);return await e2(["hermes","plugins","enable",M,"--no-allow-tool-override"]),!t},beforeUninstall:async(n)=>{Ht(n);try{await e2(["hermes","plugins","disable",M])}catch(t){console.warn(`${t instanceof Error?t.message:String(t)}
Removing the plugin files anyway; ${M} may still be listed in the Hermes config.`)}},restartNote:"Restart Hermes to apply the change."}};async function $1(n,t,r,e=!1){let a=o5[t];if(n==="uninstall")await a.beforeUninstall?.(r);let l=n==="install"?await a.install(r):await a.uninstall(r),d=n==="install"&&await a.afterInstall?.(r),s=o2(t),L=!d&&(n==="install"&&l.alreadyInstalled||n==="uninstall"&&!l.alreadyInstalled);return[L?n==="install"?`${s} plugin ${e?"up to date":"already installed"} at ${l.path}`:`${s} plugin not installed at ${l.path}`:`${n!=="install"?"Uninstalled":e?"Updated":"Installed"} ${s} plugin ${n==="install"?"at":"from"} ${l.path}`,L?void 0:a.restartNote].filter(Boolean).join(`
`)}var f5={amp:{install:(n,t)=>$1("install","amp",n,t),uninstall:(n)=>$1("uninstall","amp",n)},"antigravity-cli":{install:(n,t)=>sn("install","antigravity-cli",n,t),uninstall:(n)=>sn("uninstall","antigravity-cli",n)},"claude-code":{install:(n,t)=>J2("claude-code",n,t),uninstall:()=>dn("claude-code")},codex:{install:(n,t,r)=>J2("codex",n,t,r),uninstall:()=>dn("codex")},"copilot-cli":{install:async(n,t)=>[await J2("copilot-cli",n,t),c5(n)].filter(Boolean).join(`
`),uninstall:()=>dn("copilot-cli")},cursor:{install:(n,t)=>sn("install","cursor",n,t),uninstall:(n)=>sn("uninstall","cursor",n)},"gemini-cli":{install:(n,t)=>J2("gemini-cli",n,t),uninstall:()=>dn("gemini-cli")},"hermes-agent":{install:(n,t)=>{if(!t)y1(n);return $1("install","hermes-agent",n,t)},uninstall:(n)=>$1("uninstall","hermes-agent",n)},"kimi-code":{install:(n,t)=>sn("install","kimi-code",n,t),uninstall:(n)=>sn("uninstall","kimi-code",n)},openclaw:{install:async(n,t)=>{let r=await J2("openclaw",n,t);return await qa(),r},uninstall:(n)=>{return Mt(n),dn("openclaw")}},opencode:{install:(n,t)=>J2("opencode",n,t),uninstall:(n)=>u5(n)},pi:{install:async(n,t)=>[await J2("pi",n,t),i5(n)].filter(Boolean).join(`
`),uninstall:()=>dn("pi")}},M3=["Install CC Safety Net as a native Kimi Code plugin:","","  1. Start Kimi Code and run: /plugins install https://github.com/kenryu42/cc-safety-net","     Confirm the trust prompt; it defaults to cancel.","  2. Run /reload, or start a new session.","","Note: Kimi Code hooks are fail-open. When the hook process cannot start, crashes, or times","out, Kimi Code allows the tool call."].join(`
`);function y5(n){if(Jn({homeDir:n,cwd:process.cwd()}).status!=="configured")return M3;return[M3,"",o.red(["CAUTION: the global Kimi Code hook is installed and will run alongside the plugin.","After the plugin is active, remove it with: cc-safety-net uninstall --kimi-code"].join(`
`))].join(`
`)}function G3(n,t){return t.map((r)=>n==="install"&&r.target==="kimi-code"&&r.unavailableReason==="already installed"?{...r,available:!0,unavailableReason:void 0,label:`${r.label} (global hook installed)`}:r)}function h5(n,t){if(n.selectKimiInstallMethod)return n.selectKimiInstallMethod();if(!Nt(n.input,n.output))return Promise.resolve("global-hook");return Na({input:n.input,output:n.output,globalHookInstalled:Jn({homeDir:t,cwd:process.cwd()}).status==="configured"})}async function _3(n,t,r,e=!1,a){return f5[t][n](r,e,a)}function w5(n){let t=F({label:"update"},n).errors[0];if(t)throw Error(t)}async function x5(n,t=_2){let r=await E3(n,t),e=vr(zn(n),"installed-plugins");return{targets:ar([...r.hooks.filter((l)=>l.platform!=="copilot-cli"&&l.detected).map((l)=>l.platform),...[t1,R0,K0].flatMap((l)=>br(vr(e,...l))?["copilot-cli"]:[]),...n1(n,ur)?["claude-code"]:[],...or(r.codexPluginListOutput)?["codex"]:[]]),codexPluginListOutput:r.codexPluginListOutput}}async function $5(n){let t=g1(),r=n.output??process.stdout,e=x5(t,n.fetchVersion??_2).then(async(s)=>{let L=new Set(s.targets);return{targets:s.targets,codexPluginListOutput:s.codexPluginListOutput,available:new Map(await Promise.all(U.filter((c)=>L.has(c.target)&&K3.has(c.target)).map(async(c)=>[c.target,await dr(c.probeCommand)])))}}),a=await qn(n.showBanner??!0,()=>({ready:e,finish:()=>e}),()=>kn({input:n.input??process.stdin,output:r}),{loadingMessage:"Checking installed integrations…",output:r});if(a.targets.length===0)return r.write("No installed integrations found. Run `cc-safety-net install` to set one up.\n"),0;let l=a.targets.some((s)=>R3.has(s))?await Promise.resolve().then(()=>{return y1(t),null}).catch((s)=>D1(s)):null,d=await Pn(Promise.all(a.targets.map((s)=>{if(K3.has(s)&&!a.available.get(s))return Promise.resolve({message:`${o2(s)} not found; skipped`,failed:!1});if(l!==null&&R3.has(s))return Promise.resolve({message:l,failed:!0});return _3("install",s,t,!0,a.codexPluginListOutput).then((L)=>({message:L,failed:!1}),(L)=>({message:D1(L),failed:!0}))})),{loadingMessage:`Updating ${a.targets.length} integration${a.targets.length===1?"":"s"}…`,output:r});return d.forEach((s)=>{s.failed?console.error(s.message):r.write(`${s.message}
`)}),d.some((s)=>s.failed)?1:0}function fr(n,t={}){return Promise.resolve().then(()=>w5(n)).then(()=>$5(t)).catch((r)=>{return console.error(D1(r)),1})}async function Hn(n,t,r={}){try{let e=await qn(!0,()=>v5(n,t,r),()=>kn({input:r.input??process.stdin,output:r.output??process.stdout}),{loadingMessage:n==="install"?"Checking available integrations…":"Checking installed integrations…",output:r.output??process.stdout});if(!e)return(r.output??process.stdout).write(`Cancelled: nothing was ${n}ed.
`),0;if(e==="update")return(r.runUpdate??(()=>fr([],{fetchVersion:r.fetchVersion,input:r.input,output:r.output,showBanner:!1})))();let a=g1(),l=r.output??process.stdout;return await w3(e,async(d)=>{if(d==="kimi-code"&&n==="install"){let L=await h5(r,a);if(L===null){l.write(`Cancelled: Kimi Code integration was not installed.
`);return}if(L==="plugin"){l.write(`${y5(a)}
`);return}}let s=await Pn(_3(n,d,a),{loadingMessage:`${n==="install"?"Installing":"Uninstalling"} ${o2(d)} integration…`,output:l});l.write(`${s}
`)}),0}catch(e){return console.error(D1(e)),1}}function D1(n){let t=n instanceof Error?n.message:String(n),r=typeof n==="object"&&n!==null&&"code"in n?n.code:null;if(r==="EACCES"||r==="EPERM")return`${t}
...
Check that every parent path component is a directory.`;return t}import{join as l8}from"node:path";var P3="# Custom Rules Reference\n\nAgent reference for generating CC Safety Net rulebook configuration.\n\n## Config Locations\n\n| Scope | Config path | Rulebook path | Cache path | Priority |\n|-------|-------------|---------------|------------|----------|\n| User | `~/.cc-safety-net/rules/rule.json` | `~/.cc-safety-net/rules/<rulebook-name>/rulebook.json` | `~/.cc-safety-net/cache/rulebooks/` | First |\n| Project | `.cc-safety-net/rules/rule.json` | `.cc-safety-net/rules/<rulebook-name>/rulebook.json` | `.cc-safety-net/cache/rulebooks/` | Second |\n| GitHub source | Listed in a local `rule.json` | `.cc-safety-net/rules/<rulebook-name>/rulebook.json` in the source repository | Consumer local cache | Source order |\n\nUser scope is evaluated before project scope; within a scope, sources apply in `rules` array order. A duplicate active rulebook name keeps the first claim and ignores the later rulebook with a warning, so a user-scoped name shadows a project-scoped one.\n\nUse `cc-safety-net rule init` to create an inert local config. Use `--global` for user scope. Use `cc-safety-net rule init --example` to also create an inactive example rulebook. `CC_SAFETY_NET_HOME` overrides the `~/.cc-safety-net` user root.\n\nLegacy inline `.safety-net.json` and `~/.cc-safety-net/config.json` files are not loaded at runtime. Convert them with `cc-safety-net rule migrate`.\n\n## rule.json Schema\n\n```json\n{\n  \"version\": 1,\n  \"rules\": [\"project-rules\", \"owner/repo#main/team-rules\"],\n  \"overrides\": {\n    \"project-rules/block-docker-system-prune\": {\n      \"reason\": \"Use targeted Docker cleanup commands.\"\n    },\n    \"team-rules/block-npm-global\": \"off\"\n  },\n  \"transparent_wrappers\": [\"rtk\"]\n}\n```\n\n- `version`: Required. Must be `1`.\n- `$schema`: Optional. `cc-safety-net rule verify` inserts it into a valid `rule.json` that lacks it.\n- `rules`: Optional array of rulebook source strings. Missing `rules` is treated as `[]`.\n- `overrides`: Optional object keyed by `<rulebook-name>/<rule-name>`.\n- `overrides` values are either `\"off\"` to disable a rule or an object with a required `reason` (replacement block reason) and an optional `intent` (one of `hard_stop`, `use_alternative`, `scope_down`, `manual_only`, `stop_and_explain`).\n- A project override cannot target a user-scoped rule: only that override is ignored, the user rule keeps its configured state, and `rule sync`/`rule verify` report the diagnostic as a failure.\n- `transparent_wrappers`: Optional array of command names that transparently execute a visible child command.\n- Transparent wrappers have no built-in defaults. Configure only wrappers you intentionally trust, such as `\"rtk\"`.\n- Use `cc-safety-net rule wrapper add rtk` to configure RTK without manually editing `rule.json`.\n\n## Rulebook Sources\n\n- Local sources are bare rulebook names such as `project-rules`; the rulebook file is `.cc-safety-net/rules/project-rules/rulebook.json`.\n- GitHub sources use `owner/repo#ref/<rulebook-name>`.\n- GitHub refs must be one path segment, such as a tag, SHA, or branch name without `/`.\n- The GitHub source name, the repository directory name, and the rulebook `name` must match exactly.\n- Rulebook source strings must be unique in a config.\n\n## rulebook.json Schema\n\n```json\n{\n  \"rulebook_version\": 1,\n  \"name\": \"project-rules\",\n  \"version\": \"1.0.0\",\n  \"description\": \"Project-specific CC Safety Net rules.\",\n  \"author\": \"project\",\n  \"allowed_commands\": [\"docker\"],\n  \"rules\": [\n    {\n      \"name\": \"block-docker-system-prune\",\n      \"command\": \"docker\",\n      \"subcommand\": \"system\",\n      \"block_args\": [\"prune\"],\n      \"reason\": \"Use targeted cleanup instead.\"\n    }\n  ],\n  \"tests\": [\n    {\n      \"command\": \"docker system prune\",\n      \"expect\": \"blocked\",\n      \"rule\": \"block-docker-system-prune\"\n    },\n    {\n      \"command\": \"docker ps\",\n      \"expect\": \"allowed\"\n    }\n  ]\n}\n```\n\n### Rulebook Fields\n\n| Field | Required | Constraints |\n|-------|----------|-------------|\n| `rulebook_version` | Yes | Must be `1` |\n| `name` | Yes | `^[a-zA-Z][a-zA-Z0-9_-]{0,63}$` |\n| `version` | Yes | Non-empty string |\n| `description` | No | Free text; not type-checked at runtime |\n| `author` | No | Free text; not type-checked at runtime |\n| `allowed_commands` | Yes | Unique command names matching `^[a-zA-Z][a-zA-Z0-9_-]*$` |\n| `rules` | Yes | Array of rule objects |\n| `tests` | No | Array of fixtures |\n\n### Rule Fields\n\n| Field | Required | Constraints |\n|-------|----------|-------------|\n| `name` | Yes | Unique within the rulebook (case-insensitive); same pattern as rulebook `name` |\n| `command` | Yes | Must be listed in `allowed_commands`; basename only, not path |\n| `subcommand` | No | Same pattern as `command`; omit to match any subcommand |\n| `intent` | No | One of `hard_stop`, `use_alternative`, `scope_down`, `manual_only`, `stop_and_explain` |\n| `block_args` | Yes | Non-empty array of non-empty strings |\n| `reason` | Yes | Non-empty string, max 256 chars |\n\n### Test Fixture Fields\n\n| Field | Required | Constraints |\n|-------|----------|-------------|\n| `command` | Yes | Non-empty shell command string |\n| `expect` | Yes | `\"blocked\"` or `\"allowed\"` |\n| `rule` | Required for blocked fixtures | Rule name expected to block the command |\n\nFixtures are optional documentation of intended behavior. Fixtures are shape-validated only; CC Safety Net does not execute them.\n\n## Matching Behavior\n\n- **Command**: Normalized to lowercase basename with any trailing `.exe` removed (`/usr/bin/git` → `git`).\n- **Subcommand**: The first command token after recognized Git and Docker global options and their values; `--` ends option parsing. An unrecognized option without `=` may consume the following token as its value.\n- **Arguments**: Each `block_args` value is compared literally against every command token, including expanded short options. The command is blocked if **any** item matches.\n- **Short options**: Expanded (`-Ap` matches `-A`).\n- **Long options**: Exact match (`--all-files` does not match `--all`).\n- **Execution order**: Built-in rules first, then custom rulebooks. Custom rules only add restrictions.\n- **Transparent wrappers**: A configured wrapper such as `rtk` lets `rtk git commit` be analyzed as `git commit` only when `git` is protected by built-in analyzers or active custom rules. `rtk -- git commit` is also supported.\n\n## Workflow\n\n1. Run `cc-safety-net rule init` or create `rule.json` manually.\n2. Optionally run `cc-safety-net rule init --example` to create an inactive example rulebook.\n3. Use `cc-safety-net rule wrapper add rtk` for trusted transparent wrappers.\n4. Run `cc-safety-net rule add <source>` after creating or choosing a rulebook source; it adds the source and syncs it.\n5. Run `cc-safety-net rule sync` after manual `rule.json` changes or local rulebook edits.\n6. Run `cc-safety-net rule verify` to validate config, lock/cache state, local rulebooks, and shareable GitHub-source rulebook directories in the current repository (it does not fetch remote content).\n7. Run `cc-safety-net rule list` to inspect active rulebooks and transparent wrappers.\n\nAn edited or invalid local rulebook keeps its last synced, digest-verified cached version enforced until `cc-safety-net rule sync` validates the edit. A missing lock entry or cache, a cache digest mismatch, or an invalid cached rulebook makes that source inactive; a missing lockfile or an unreadable or invalid `rule.json` makes every source in its scope inactive. Inactive sources stop applying their rules while other custom rules and all built-in protections stay active. Repair the reported condition, then run `cc-safety-net rule sync`. Run `cc-safety-net status` to see degraded sources.\n";function Wn(n,t){if(!n.ok){k5(n);return}q5(n),console.log(t),console.log("Rule config synced."),console.log(""),D5(n.entries)}function D5(n){if(n.length===0){console.log("Active rulebooks: (none)");return}console.log(`Active rulebooks (${n.length}):`);for(let t of n)console.log(`  - ${t.name} ${t.version} (${g5(t.ruleCount??0)})`),console.log(`    Source: ${P1(t)}`)}function g5(n){return`${n} ${n===1?"rule":"rules"}`}function N3(n,t){Z2("Active sources",n.rulebooks,(r)=>[`[${r.source}] ${r.name} ${r.version}`,`  Source: ${t[r.source].get(r.spec)??r.spec}`]),Z2("Active rules",n.rules,(r)=>[`[${m5(n,r.name)}] ${r.name}`,`  Command: ${r.subcommand?`${r.command} ${r.subcommand}`:r.command}`,`  Block args: ${r.block_args.join(", ")}`,`  Reason: ${r.reason}`]),Z2("Disabled rules",I3(n,"off"),(r)=>[r.key]),Z2("Reason overrides",I3(n,"reason"),(r)=>[r.key,`  Reason: ${r.value.reason}`]),Z2("Transparent wrappers",n.transparent_wrappers,(r)=>[r]),Z2("Issues",n.errors,(r)=>[r]),Z2("Warnings",n.warnings,(r)=>[r])}function Z2(n,t,r){if(t.length===0){console.log(`${n}: (none)`);return}console.log(`${n} (${t.length}):`);for(let e of t){let[a,...l]=r(e);console.log(`  - ${a}`);for(let d of l)console.log(`    ${d}`)}}function m5(n,t){return n.rulebooks.find((r)=>r.rules.includes(t))?.source??"project"}function I3(n,t){return Object.entries({...n.userConfig?.overrides??{},...n.projectConfig?.overrides??{}}).filter((r)=>{if(t==="off")return r[1]==="off";return!!r[1]&&typeof r[1]==="object"}).map(([r,e])=>({key:r,value:e}))}function k5(n){for(let t of n.errors)console.error(t)}function q5(n){if(!n.warnings||n.warnings.length===0)return;for(let t of n.warnings)console.warn(t)}import{dirname as nl,join as m1}from"node:path";var j5=".safety-net.json",z5="~/.cc-safety-net/config.json";async function el(n){return[await tl({legacyPath:rl({cwd:n.cwd}),configPath:_(n.cwd),defaultRulebookName:"project-rules",migratedFrom:j5,cleanup:n.cleanup,syncOptions:{cwd:n.cwd}}),await tl({legacyPath:bn(),configPath:P(),defaultRulebookName:"user-rules",migratedFrom:z5,cleanup:n.cleanup,syncOptions:{cwd:n.cwd,global:!0}})].every((r)=>r)?0:1}async function tl(n){let t=O(n.syncOptions),r=g(t.filesystemScope,n.legacyPath),e=D(r);if(e===null)return console.log(`No legacy config found at ${n.legacyPath}`),!0;let a=B5(e);if(!a.ok){for(let b of a.errors)console.error(b);return!1}let l=T(t.configTarget);if(l.errors.length>0){for(let b of l.errors)console.error(b);return!1}let d=l.config??{version:1,rules:[],overrides:{},transparent_wrappers:[]},s=J5(nl(n.configPath),d.rules,n.defaultRulebookName,n.migratedFrom,t.filesystemScope),L=m1(nl(n.configPath),s,"rulebook.json"),c=g(t.filesystemScope,L),i=[yr(t.configTarget),yr(c),yr(t.lockTarget)],v=await F5(n,t.configTarget,c,s,a.config.rules,d.rules.includes(s)?d.rules:[...d.rules,s],d.overrides??{},d.transparent_wrappers??[]);if(!v.ok){X5(i);for(let b of v.errors)console.error(b);return!1}if(!n.cleanup)return console.log(`Migrated legacy config at ${n.legacyPath}. Legacy file is no longer used.`),!0;if(!Q5(t.configTarget,c,s,n.migratedFrom,a.config.rules))return console.error(`Migration cleanup verification failed for ${n.legacyPath}`),!1;return vn(r),console.log(`Deleted legacy config at ${n.legacyPath}`),!0}async function F5(n,t,r,e,a,l,d,s){try{return E(t,{version:1,rules:l,overrides:d,transparent_wrappers:s}),E(r,Z5(e,n.migratedFrom,a)),await K2(n.syncOptions)}catch(L){return{ok:!1,errors:[L instanceof Error?L.message:String(L)]}}}function B5(n){try{let t=JSON.parse(n),r=W1(t);if(r.errors.length>0)return{ok:!1,errors:r.errors};return{ok:!0,config:{version:1,rules:t.rules??[]}}}catch{return{ok:!1,errors:["Invalid JSON"]}}}function J5(n,t,r,e,a){let l=t.find((d)=>Y5(g(a,m1(n,d,"rulebook.json")))===e);if(l)return l;if(D(g(a,m1(n,r,"rulebook.json")))===null)return r;for(let d=2;;d++){let s=`${r}-${d}`;if(D(g(a,m1(n,s,"rulebook.json")))===null)return s}}function Z5(n,t,r){return{rulebook_version:1,name:n,version:"1.0.0",description:"Migrated CC Safety Net rules.",author:"project",migrated_from:t,allowed_commands:[...new Set(r.map((e)=>e.command))],rules:r,tests:r.map((e)=>({command:[e.command,e.subcommand,e.block_args[0]].filter(Boolean).join(" "),expect:"blocked",rule:e.name}))}}function Q5(n,t,r,e,a){if(!T(n).config?.rules.includes(r))return!1;try{let d=D(t);if(d===null)return!1;let s=JSON.parse(d);return s.migrated_from===e&&JSON.stringify(s.rules)===JSON.stringify(a)}catch{return!1}}function yr(n){return{target:n,content:D(n)}}function X5(n){for(let t of n){if(t.content===null){vn(t.target);continue}w2(t.target,t.content)}}function Y5(n){let t=D(n);if(t===null)return null;try{let r=JSON.parse(t);return typeof r.migrated_from==="string"?r.migrated_from:null}catch{return null}}import{mkdir as S5,readFile as H5,writeFile as W5}from"node:fs/promises";import{dirname as A5,join as K5}from"node:path";var R5=86400000,V5=604800000;async function ll(n=Date.now()){if(process.env.CC_SAFETY_NET_NO_UPDATE_CHECK)return null;let t=nt();if(!t)return null;let r=K5(t,".cc-safety-net","update-check.json"),e=await M5(r,n);if(!e.lastCheck||n-e.lastCheck>R5){let d=await P2();if(e.lastCheck=n,d.latestVersion)e.latestVersion=d.latestVersion;if(!await al(r,e))return null;if(d.error)return null}let a=e.latestVersion,l=z();if(!a||!ht(a,l))return null;if(e.notifiedVersion===a&&e.notifiedAt!==void 0&&n-e.notifiedAt<V5)return null;if(e.notifiedVersion=a,e.notifiedAt=n,!await al(r,e))return null;return`UPDATE_AVAILABLE: cc-safety-net v${a} is available (running v${l}). Ask the user once whether to run \`npx -y cc-safety-net@latest update\`; continue the current task either way and do not raise this again.`}async function M5(n,t){let r=await H5(n,"utf8").then((l)=>JSON.parse(l)).catch(()=>{return});if(!r||typeof r!=="object"||Array.isArray(r))return{};let e=r,a=(l)=>typeof l==="number"&&Number.isFinite(l)&&l<=t?l:void 0;return{lastCheck:a(e.lastCheck),latestVersion:typeof e.latestVersion==="string"?e.latestVersion:void 0,notifiedVersion:typeof e.notifiedVersion==="string"?e.notifiedVersion:void 0,notifiedAt:a(e.notifiedAt)}}async function al(n,t){return S5(A5(n),{recursive:!0,mode:448}).then(()=>W5(n,JSON.stringify(t),{mode:384})).then(()=>!0).catch(()=>!1)}import{dirname as G5,join as C5,resolve as hr}from"node:path";var sl="CC Safety Net Config",U5="═".repeat(sl.length),O5="https://raw.githubusercontent.com/kenryu42/cc-safety-net/main/assets/cc-safety-net.schema.json",T5=new Set(["rule.json","rule.lock","cache"]);function Ll(n={}){try{return E5(n)}catch(t){if(t instanceof Q)return console.error(t.message),1;throw t}}function E5(n){let t=n.cwd??process.cwd(),r=n.userConfigPath??P(),e=n.projectConfigPath??_(t),a=n.legacyUserConfigPath??bn(),l=n.legacyProjectConfigPath??Rr(t),d=hr(t,_1),s=G5(r),L=I({cwd:t,userConfigPath:r,projectConfigPath:e}),c=I({cwd:t}),i=g(L.userScope,r),v=g(L.projectScope,e),b=n.legacyUserConfigPath?pn(n.legacyUserConfigPath,"user policy"):g(c.userScope,a),p=n.legacyProjectConfigPath?pn(n.legacyProjectConfigPath,"project policy"):g(c.projectScope,l),f=!1,u=!1,y=[],x=[],w=_5(g(c.projectScope,d));if(I5(),D(i)!==null){let h=l2(i);if(h.errors.push(...d2(r,R2({userConfigDir:s}),{userConfigDir:s},L.userScope)),y.push({scope:"User",path:r,result:h,schema:"rules",sourceDisplayMap:x2(r,L.userScope),target:i}),h.errors.length>0)f=!0}if(D(b)!==null)if(u=!0,D(i)!==null)x.push(k1("user","cleanup"));else{let h=A1(b);if(y.push({scope:"User",path:a,result:h,schema:"legacy",sourceDisplayMap:new Map,inactive:!0,target:b}),x.push(k1("user",h.errors.length>0?"fix-or-delete":"migrate")),h.errors.length>0)f=!0}if(D(v)!==null){let h=l2(v);if(h.errors.push(...d2(e,V2(e),{userConfigDir:s},L.projectScope)),y.push({scope:"Project",path:hr(e),result:h,schema:"rules",sourceDisplayMap:x2(e,L.projectScope),target:v}),h.errors.length>0)f=!0;if(D(p)!==null)u=!0,x.push(k1("project","cleanup"))}else if(D(p)!==null){u=!0,f=!0;let h=A1(p);y.push({scope:"Project",path:hr(l),result:h,schema:"legacy",sourceDisplayMap:new Map,inactive:!0,target:p}),x.push(k1("project",h.errors.length>0?"fix-or-delete":"migrate"))}if(w?.result.errors.length)f=!0;if(y.length===0&&!w)return console.log(`
No config files found. Using built-in rules only.`),0;for(let h of y)if(h.inactive)n8(h.scope,h.path,h.result,h.sourceDisplayMap);else if(h.result.errors.length>0)t8(h.scope,h.path,h.result.errors);else{if(h.schema==="rules"&&a8(h.target))console.log(`
Added $schema to ${h.scope.toLowerCase()} config.`);N5(h.scope,h.path,h.result,h.schema,h.sourceDisplayMap)}for(let h of x)console.error(`
${o.red(h)}`);if(w)if(w.result.errors.length>0)e8(w.path,w.result.errors);else r8(w.path,w.result);if(f)return console.error(`
Config validation failed.`),1;return console.log(u?`
Configs valid with warnings.`:`
All configs valid.`),0}function k1(n,t){let r=`legacy ${n} config`;if(t==="cleanup")return`Warning: Legacy ${n} config is no longer needed. Run \`npx -y cc-safety-net rule migrate --cleanup\` to clean it up safely.`;if(t==="migrate")return`Warning: Legacy ${n} config is ignored by CC Safety Net. Run \`npx -y cc-safety-net rule migrate\`.`;return`Warning: Legacy ${n} config is no longer supported. Fix or delete the ${r}, then run \`npx -y cc-safety-net rule migrate\`.`}function _5(n){if(A2(n)===null)return null;let t=P5(n);if(t.ruleNames.size===0&&t.errors.length===0)return null;return{path:n.path,result:t}}function P5(n){let t=[],r=new Set,e=(A2(n)??[]).filter((a)=>!T5.has(a.name)).sort((a,l)=>a.name.localeCompare(l.name));if(e.length===0)return{errors:t,ruleNames:r};for(let a of e){if(!un.test(a.name)){t.push(`rulebook directory names must match ${un}: ${a.name}`);continue}if(a.kind!=="directory"){t.push(`${a.name} must be a rulebook directory`);continue}let l=g(n.scope,C5(n.path,a.name,"rulebook.json")),d=D(l);if(d===null){t.push(`${a.name}/rulebook.json is required`);continue}try{let s;try{s=JSON.parse(d)}catch{t.push(`${a.name}/rulebook.json: invalid JSON`);continue}let L=dl(s);if(L.name!==a.name){t.push(`rulebook name "${L.name}" must match folder "${a.name}"`);continue}r.add(a.name)}catch(s){t.push(s instanceof Error?`${a.name}/rulebook.json: ${s.message}`:`${a.name}/rulebook.json: ${String(s)}`)}}return{errors:t,ruleNames:r}}function I5(){console.log(sl),console.log(U5)}function N5(n,t,r,e,a){if(console.log(`
✓ ${n} config: ${t}`),console.log(`  Schema: ${e==="rules"?"rulebook sources":"legacy inline rules"}`),r.ruleNames.size>0){console.log(`  ${e==="rules"?"Sources":"Rules"}:`);let l=1;for(let d of r.ruleNames)console.log(`    ${l}. ${a.get(d)??d}`),l++}else console.log(`  ${e==="rules"?"Sources":"Rules"}: (none)`)}function n8(n,t,r,e){if(console.error(`
✗ Legacy ${n.toLowerCase()} config: ${t}`),console.error("  Schema: legacy inline rules"),console.error("  Status: ignored by CC Safety Net"),r.errors.length>0){console.error("  Errors:");let a=1;for(let l of r.errors)for(let d of l.split("; "))console.error(`    ${a}. ${d}`),a++;return}if(r.ruleNames.size>0){console.error("  Rules:");let a=1;for(let l of r.ruleNames)console.error(`    ${a}. ${e.get(l)??l}`),a++;return}console.error("  Rules: (none)")}function t8(n,t,r){cl(`${n} config`,t,r)}function r8(n,t){console.log(`
✓ GitHub source rules: ${n}`),console.log("  Rulebooks:");let r=1;for(let e of t.ruleNames)console.log(`    ${r}. ${e}`),r++}function e8(n,t){cl("GitHub source rules",n,t)}function cl(n,t,r){console.error(`
✗ ${n}: ${t}`),console.error("  Errors:");let e=1;for(let a of r)for(let l of a.split("; "))console.error(`    ${e}. ${l}`),e++}function a8(n){try{let t=D(n);if(t===null)return!1;let r=JSON.parse(t);if(r.$schema)return!1;return w2(n,JSON.stringify({$schema:O5,...r},null,2)),!0}catch(t){if(t instanceof Q)throw t;return!1}}var il=new Set(["init","add","remove","update","sync","list","wrapper","migrate","doc","verify"]),d8=new Set(["add","remove","list"]);async function vl(n){try{return await s8(n)}catch(t){if(t instanceof Q)return console.error(t.message),1;throw t}}async function s8(n){let t=c8(n),r=t.help?L8(t.positionals):null;if(r)return i1(r),0;if(t.errors.length>0){for(let d of t.errors)console.error(d);return 1}let e=t.positionals[0];if(!e)return i1(E2,console.error),1;let a=t.positionals[1],l={global:t.global,check:t.check};if(e==="init"){let d=O(l),s=d.configDir;v8(d.configTarget),pl(g(d.filesystemScope,Vn({...l,cacheConfigDir:s})));let L=l8(s,"example-rules","rulebook.json"),c=g(d.filesystemScope,L);if(t.example&&D(c)===null)N1(c,"example-rules");let i=await K2(l);return Wn(i,"Rule config initialized."),i.ok?0:1}if(e==="add"){if(!a)return console.error("rule add requires a source"),1;let d=await O1(a,l);return Wn(d,`Added rulebook source: ${a}`),d.ok?0:1}if(e==="remove"){if(!a)return console.error("rule remove requires a source"),1;let d=await E1(a,{...l,deleteSource:t.deleteSource});return Wn(d,`Removed rulebook source: ${a}`),d.ok?0:1}if(e==="update"||e==="sync"){let d=await K2({...l,only:e==="update"?a:void 0});return Wn(d,t.check?"Rule config checked.":"Rule config synced."),d.ok?0:1}if(e==="list"){let d=u2(),s=I({});return N3(d,{user:x2(d.userConfigPath,s.userScope),project:x2(d.projectConfigPath,s.projectScope)}),d.errors.length>0?1:0}if(e==="wrapper")return u8(t);if(e==="migrate")return el({cleanup:t.cleanup,cwd:process.cwd()});if(e==="doc"){console.log(P3);let d=await ll();if(d)console.error(d);return 0}if(e==="verify")return Ll();return 1}function L8(n){if(n.length===0)return E2;let t=E2.subcommands.filter((e)=>e.usage.split(" ")[0]===n[0]);if(t.length===0)return null;if(n.length===1&&t.length>1)return{name:`rule ${n[0]}`,description:`Subcommands of rule ${n[0]}`,usage:`rule ${n[0]} <subcommand>`,subcommands:t,options:[]};let r=n.length===1?t[0]:t.find((e)=>e.usage.split(" ")[1]===n[1]);if(!r)return null;return{name:`rule ${n[0]}`,description:r.description,usage:`rule ${r.usage}`,options:[]}}function c8(n){let t=F({label:"rule",booleans:{global:["-g","--global"],check:["--check"],cleanup:["--cleanup"],deleteSource:["--delete-source"],example:["--example"]},positionals:"list"},n),r={...t.flags,help:t.help,positionals:t.positionals,errors:t.errors};return i8(r),r}function i8(n){let[t]=n.positionals;if(t&&!il.has(t))n.errors.push(`Unknown rule subcommand: ${t}`);if(n.deleteSource&&t!=="remove")if(t&&il.has(t))n.errors.push(`Unknown option for rule ${t}: --delete-source`);else n.errors.push("--delete-source is only valid with 'rule remove'");if(n.cleanup&&t!=="migrate")n.errors.push(q1(t,"--cleanup"));if(n.example&&t!=="init")n.errors.push(q1(t,"--example"));if(t==="migrate"){if(n.global)n.errors.push(q1(t,"--global"));if(n.check)n.errors.push(q1(t,"--check"));if(n.positionals.length>1)n.errors.push(`Unexpected rule migrate argument: ${n.positionals[1]}`)}else if(t==="wrapper")p8(n);else if(n.positionals.length>2)n.errors.push(`Unexpected rule argument: ${n.positionals[2]}`);if(t==="list"&&n.global)n.errors.push("Unknown option for rule list: --global")}function q1(n,t){return n?`Unknown option for rule ${n}: ${t}`:`Unknown option for rule: ${t}`}function p8(n){let t=n.positionals[1],r=n.positionals[2];if(!t){n.errors.push("rule wrapper requires add, remove, or list");return}if(!d8.has(t)){n.errors.push(`Unknown rule wrapper action: ${t}`);return}if(t==="list"){if(r)n.errors.push(`Unexpected rule wrapper argument: ${r}`);return}if(!r){n.errors.push(`rule wrapper ${t} requires a command`);return}if(n.positionals.length>3)n.errors.push(`Unexpected rule wrapper argument: ${n.positionals[3]}`)}function v8(n){if(D(n)===null){I1(n);return}let t=T(n);if(!t.config)return;E(n,{version:1,rules:t.config.rules,overrides:t.config.overrides??{},transparent_wrappers:t.config.transparent_wrappers??[]})}async function u8(n){let t=n.positionals[1],r=n.positionals[2],e=O({global:n.global}).configTarget;if(t==="list"){let s=T(e);if(s.errors.length>0){for(let L of s.errors)console.error(L);return 1}return b8(s.config?.transparent_wrappers??[]),0}if(!r||!tt.test(r))return console.error("transparent wrapper must match command pattern"),1;if(rt(r))return console.error(`reserved command "${r}" cannot be a wrapper`),1;let a=T(e);if(a.errors.length>0){for(let s of a.errors)console.error(s);return 1}let l=a.config??{version:1,rules:[],overrides:{},transparent_wrappers:[]},d=t==="add"?[...new Set([...l.transparent_wrappers??[],r])]:(l.transparent_wrappers??[]).filter((s)=>s!==r);return E(e,{version:1,rules:l.rules,overrides:l.overrides??{},transparent_wrappers:d}),console.log(t==="add"?`Added transparent wrapper: ${r}`:`Removed transparent wrapper: ${r}`),0}function b8(n){if(n.length===0){console.log("Transparent wrappers: (none)");return}console.log(`Transparent wrappers (${n.length}):`);for(let t of n)console.log(`  - ${t}`)}import{homedir as ul}from"node:os";import{existsSync as o8,readFileSync as f8}from"node:fs";import{homedir as y8}from"node:os";import{join as h8}from"node:path";async function w8(n){if(n.isTTY)return null;return(await ct(n).catch(()=>null))?.trim()||null}function x8(){if(process.env.CLAUDE_SETTINGS_PATH)return process.env.CLAUDE_SETTINGS_PATH;return h8(y8(),".claude","settings.json")}function wr(){let n=x8();if(!o8(n))return!1;try{let t=f8(n,"utf-8"),r=JSON.parse(t);if(!r.enabledPlugins)return!1;let e="cc-safety-net@cc-marketplace";if(!(e in r.enabledPlugins))return!1;return r.enabledPlugins[e]===!0}catch(t){if(yn(m.debug))console.error(`CC Safety Net debug: failed to read Claude settings: ${n}: ${t instanceof Error?t.message:String(t)}`);return!1}}async function xr(n=process.stdin){let t=wr(),r;if(!t)r="\uD83D\uDEE1️ CC Safety Net ❌";else{let a=K({cwd:process.cwd()}),l=a.policy,d=s2(l),s=Object.values(D2(l,d.capabilities)).some((c)=>c.changesInherited);r=`\uD83D\uDEE1️ CC Safety Net ${{standard:"✅",strict:"\uD83D\uDD12",paranoid:"\uD83D\uDC41️",custom:"\uD83D\uDD27"}[s?"custom":d.effectiveLevel]}${d.worktreeMode?"\uD83C\uDF33":""}${a.state==="degraded"?"⚠️":""}`}let e=await w8(n);if(e&&!e.startsWith("{"))console.log(`${e} | ${r}`);else console.log(r)}function bl(){let n=K({cwd:process.cwd()}),t=n.policy,r=s2(t),e=!!process.env.NO_COLOR||!process.stdout.isTTY,a=Math.min(process.stdout.columns||80,100),l=e?"ok":"✔",d=e?"OFF":"✘",s=(p,f)=>{let u=`  ${p.padEnd(13)}${f}`;return(u.length>a?`${u.slice(0,a-1)}…`:u).replaceAll(d,o.red(d))},L=Object.values(D2(t,r.capabilities)).some((p)=>p.changesInherited),c=hn(),i={ready:o.green,degraded:o.yellow}[n.state],v=[...wr()?[]:["plugin cc-safety-net@cc-marketplace is disabled in Claude Code; nothing is enforced in Claude Code until it is re-enabled. Other integrations are not affected."],...n.diagnostics],b=e?"-":"·";console.log([`${e?"":"\uD83D\uDEE1️  "}CC Safety Net — ${i(n.state)}`,"",s("Protection",`destructive ${t.destructiveCommandProtectionEnabled?l:d}   secrets ${t.secretProtection.enabled?l:d}`),s("Level",L?`${r.effectiveLevel} (customised)`:r.effectiveLevel),s("Rules",t.rules.length===0?"none active":`${t.rules.length} active`),s("Policy",c.startsWith(ul())?`~${c.slice(ul().length)}`:c),...r.worktreeMode?[s("Worktree","relaxations active")]:[],"",...v.length===0?["  Everything configured is active."]:["  Not active",...v.flatMap((p)=>c1(p,"      ",a-6).map((f,u)=>u===0?`    ${b} ${f}`:f)),"","  Full report: cc-safety-net doctor"]].join(`
`))}import{spawn as Ql}from"node:child_process";import{randomBytes as J8}from"node:crypto";import{createServer as Z8}from"node:http";import{Writable as Q8}from"node:stream";import{homedir as $8}from"node:os";var j1=500;function D8(n){let t=n.filter((a)=>a.decision!=="allow"),r=n.filter((a)=>a.decision==="allow"),e=Math.min(t.length,Math.max(j1-r.length,Math.ceil(j1/2)));return[...t.slice(0,e),...r.slice(0,j1-e)]}function ol(n,t=L2()){if(t)$2(t);let r=(u)=>new Date(u.getFullYear(),u.getMonth(),u.getDate()).getTime(),e=r(new Date),a=new Date(e);a.setDate(a.getDate()-(n-1));let l=a.getTime(),d=[],s={count:0};for(let u of t?v2(t,s):[])for(let y of cn(u,s)){if(!y||typeof y.ts!=="string"||typeof y.command!=="string")continue;let x=new Date(y.ts).getTime();if(!Number.isFinite(x))continue;if(x>=l)d.push(y)}d.sort((u,y)=>new Date(y.ts).getTime()-new Date(u.ts).getTime());let L=Array.from({length:n},()=>0),c=Array.from({length:n},()=>0),i={},v={},b={},p=0,f=0;for(let u of d){let y=u.agent||"unknown";i[y]=(i[y]??0)+1;let x=Math.round((e-r(new Date(u.ts)))/86400000),w=n-1-x,h=x>=0&&x<n;if(h)c[w]=(c[w]??0)+1;if(u.decision!=="allow"){if(p++,u.ruleId)v[u.ruleId]=(v[u.ruleId]??0)+1;let Z=X2(u.segment||u.command);if(Z)b[Z]=(b[Z]??0)+1;if(u.failureStage)f++;if(h)L[w]=(L[w]??0)+1}}return{days:n,logsDir:t,homeDir:$8(),totalInWindow:d.length,truncated:d.length>j1,unreadable:s.count,counts:{blocked:p,allowed:d.length-p,agents:i,blockedByDay:L,analyzedByDay:c,rules:v,commands:b,errors:f},entries:D8(d).sort((u,y)=>new Date(y.ts).getTime()-new Date(u.ts).getTime())}}import{spawn as g8}from"node:child_process";import{existsSync as fl,statSync as m8}from"node:fs";import{delimiter as k8,join as q8}from"node:path";var j8=120000,z1="Choose the project folder",z8=`try
  return POSIX path of (choose folder with prompt "${z1}")
on error number -128
  return ""
end try`,F8=`Add-Type -AssemblyName System.Windows.Forms
$dialog = New-Object System.Windows.Forms.FolderBrowserDialog
$dialog.Description = '${z1}'
if ($dialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { [Console]::Out.Write($dialog.SelectedPath) }`,yl=[{binary:"zenity",args:["--file-selection","--directory",`--title=${z1}`]},{binary:"kdialog",args:["--getexistingdirectory",".","--title",z1]}],hl=(n,t)=>(t.PATH??"").split(k8).some((r)=>r.length>0&&fl(q8(r,n)));function wl(n,t){if(n==="darwin"||n==="win32")return!0;if(n!=="linux")return!1;if(!t.DISPLAY&&!t.WAYLAND_DISPLAY)return!1;return yl.some((r)=>hl(r.binary,t))}function B8(n,t){if(n==="darwin")return{cmd:"osascript",args:["-e",z8]};if(n==="win32")return{cmd:"powershell.exe",args:["-NoProfile","-STA","-Command",F8]};let r=yl.find((e)=>hl(e.binary,t));return r?{cmd:r.binary,args:r.args}:null}function xl(n=process.platform,t=process.env){let r=B8(n,t);if(!r)return Promise.resolve({error:"No folder dialog is available on this system"});return new Promise((e)=>{let a=g8(r.cmd,r.args,{env:t,stdio:["ignore","pipe","pipe"]}),l="",d=!1,s=(c)=>{if(d)return;d=!0,clearTimeout(L),e(c)},L=setTimeout(()=>{a.kill(),s({error:"The folder dialog timed out"})},j8);a.stdout.on("data",(c)=>{l+=c.toString()}),a.on("error",()=>s({error:`Could not open the folder dialog (${r.cmd})`})),a.on("close",()=>{let c=l.trim().replace(/\/+$/,"");if(!c)return s({cancelled:!0});if(!fl(c)||!m8(c).isDirectory())return s({error:"That selection is not a folder on disk"});s({path:c})})})}var $l=`<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
`;var Dl='<script id="ccsn-data" type="application/json">';function gl(n){return $l.replace(Dl,()=>Dl+JSON.stringify({token:n}).replaceAll("<","\\u003c"))}var B1="kenryu42/cc-safety-net",X8=`https://github.com/${B1}`,gr=1e4,Y8=7;async function Xl(n,t={}){let r=F({label:"gui",booleans:{noOpen:["--no-open"]}},n),e=t.log??console.log,a=t.error??console.error;if(r.errors.length>0){for(let d of r.errors)a(d);return a("Usage: cc-safety-net gui [--no-open]"),1}let l=await S8(t);if(e(`CC Safety Net policy GUI: ${l.url}`),!r.flags.noOpen)try{await(t.openBrowser??G8)(l.url)}catch(d){a(`Failed to open browser: ${d instanceof Error?d.message:String(d)}`),a(`Open this URL manually: ${l.url}`)}if(t.keepAlive===!1)return await l.close(),0;return await M8(l),0}async function S8(n={}){let t=n.token??J8(24).toString("base64url"),r=Z8((l,d)=>{H8(l,d,t,n)});await new Promise((l,d)=>{r.once("error",d),r.listen(0,"127.0.0.1",()=>{r.off("error",d),l()})});let a=`http://127.0.0.1:${r.address().port}`;return{origin:a,token:t,url:`${a}/?token=${encodeURIComponent(t)}`,close:()=>V8(r)}}async function H8(n,t,r,e){let a=new URL(n.url??"/","http://127.0.0.1");if(n.method==="GET"&&a.pathname==="/favicon.ico"){t.writeHead(204,{"cache-control":"no-store"}),t.end();return}if(!K8(n,a,r)){k(t,403,{error:"Forbidden"});return}if(n.method==="GET"&&a.pathname==="/"){R8(t,gl(r));return}if(n.method==="GET"&&a.pathname==="/api/policy"){let l=jl(e);k(t,200,{...l,configState:wn(K(e)),destructiveCommandRules:on,secretPatterns:kl,version:z(),preview:l.errors.length>0?null:Fl(l.policy)});return}if(n.method==="POST"&&a.pathname==="/api/policy/preview"){let l=await F1(n);if(!l.ok){k(t,400,{errors:[l.error]});return}let d=zl(l.value);k(t,d.errors.length>0?400:200,d);return}if(n.method==="POST"&&a.pathname==="/api/policy/explain"){let l=await F1(n);if(!l.ok){k(t,400,{errors:[l.error]});return}let d=l.value;if(d===null||typeof d.command!=="string"){k(t,400,{errors:["command must be a string"]});return}let s=dt(d.policy);if(s.length>0){k(t,400,{errors:s});return}k(t,200,W8(d.command,d.policy,e));return}if(n.method==="POST"&&a.pathname==="/api/policy"){let l=await F1(n);if(!l.ok){k(t,400,{errors:[l.error]});return}let d=$r(l.value,e);k(t,d.errors.length>0?400:200,d);return}if(n.method==="POST"&&a.pathname==="/api/reset"){k(t,200,$r(ql,e));return}if(n.method==="POST"&&a.pathname==="/api/repair"){k(t,200,Bl(e));return}if(n.method==="GET"&&a.pathname==="/api/activity"){let l=C2(e),d=A8(a.searchParams.get("days"),l);if(d===null){k(t,400,{error:`days must be an integer between 1 and ${l}`});return}k(t,200,ol(d,e.activityLogsDir));return}if(n.method==="POST"&&a.pathname==="/api/rules/choose-directory"){k(t,200,await xl());return}if(n.method==="GET"&&a.pathname==="/api/rules"){let l=u2(e),d=new Map(l.rules.map((s)=>[s.name,s]));k(t,200,{projectPath:e.cwd??process.cwd(),canPickDirectory:wl(process.platform,process.env),rulebooks:l.rulebooks.map((s)=>({source:s.source,spec:s.spec,name:s.name,version:s.version,rules:s.rules.flatMap((L)=>{let c=d.get(L);if(!c)return[];return[{name:c.name,command:c.command,subcommand:c.subcommand,block_args:c.block_args,reason:c.reason}]})})),errors:l.errors,warnings:l.warnings});return}if(n.method==="GET"&&a.pathname==="/api/star/context"){k(t,200,await(e.fetchStarContext??(()=>_8({logsDir:e.activityLogsDir})))());return}if(n.method==="POST"&&a.pathname==="/api/star"){let l=await(e.starRepo??C8)();k(t,200,l.ok?{ok:!0}:{ok:!1,fallbackUrl:X8});return}if(n.method==="GET"&&a.pathname==="/api/integrations"){k(t,200,await(e.fetchIntegrations??O8)());return}if(n.method==="GET"&&a.pathname==="/api/health"){k(t,200,await(e.fetchHealth??T8)());return}if(n.method==="POST"&&(a.pathname==="/api/install"||a.pathname==="/api/uninstall")){let l=await F1(n);if(!l.ok){k(t,400,{errors:[l.error]});return}let d=l.value?.target;if(typeof d!=="string"||!U.some((L)=>L.target===d)){k(t,400,{error:"unknown target"});return}let s=a.pathname==="/api/install"?"install":"uninstall";k(t,200,await(e.runIntegration??E8)(s,d));return}k(t,404,{error:"Not found"})}function W8(n,t,r){let e=v1(t),a=K(r),l=fn({rules:a.policy.rules,transparentWrappers:a.policy.transparentWrappers,safety:Zl(e.safety),worktreeMode:e.workflow.worktree_mode,destructiveCommandProtectionEnabled:e.destructive_command_protection.enabled,destructiveCommandRuleOverrides:e.destructive_command_protection.overrides,destructiveCommandAllowPaths:e.destructive_command_protection.allow_paths,secretProtection:{enabled:e.secret_protection.enabled,disabledRules:[...Jl(e.secret_protection.overrides)],denyPaths:e.secret_protection.deny_paths}});return G2(n,{policySnapshot:l,cwd:r.cwd,userConfigDir:r.userConfigDir})}function A8(n,t){if(n===null)return Math.min(Y8,t);let r=Number(n);if(!Number.isInteger(r)||r<1||r>t)return null;return r}function K8(n,t,r){if(t.searchParams.get("token")!==r)return!1;if(n.method!=="POST")return!0;return n.headers["x-cc-safety-net-token"]===r}async function F1(n){let t=[];for await(let r of n)t.push(r);try{return{ok:!0,value:JSON.parse(Buffer.concat(t).toString("utf-8")||"{}")}}catch(r){return{ok:!1,error:`Invalid JSON: ${r instanceof Error?r.message:String(r)}`}}}function R8(n,t){n.writeHead(200,{"content-type":"text/html; charset=utf-8","cache-control":"no-store"}),n.end(t)}function k(n,t,r){n.writeHead(t,{"content-type":"application/json; charset=utf-8","cache-control":"no-store"}),n.end(JSON.stringify(r))}function V8(n){return new Promise((t,r)=>{n.close((e)=>e?r(e):t())})}function M8(n){return new Promise((t)=>{let r=()=>{process.off("SIGINT",e),process.off("SIGTERM",e)},e=()=>{r(),n.close().then(t)};process.once("SIGINT",e),process.once("SIGTERM",e)})}function G8(n){let t=process.platform==="darwin"?"open":process.platform==="win32"?"cmd":"xdg-open",r=process.platform==="win32"?["/c","start","",n]:[n];return new Promise((e,a)=>{let l=Ql(t,r,{detached:!0,stdio:"ignore"}),d=(L)=>{l.off("spawn",s),a(L)},s=()=>{l.off("error",d),l.unref(),e()};l.once("error",d),l.once("spawn",s)})}async function C8(n="gh",t=gr){return{ok:await Dr(n,["api","-X","PUT",`/user/starred/${B1}`],t)===0}}var U8={amp:"ampVersion","antigravity-cli":"antigravityCliVersion","claude-code":"claudeCodeVersion",codex:"codexCliVersion","copilot-cli":"copilotCliVersion",cursor:"cursorVersion","gemini-cli":"geminiCliVersion","hermes-agent":"hermesAgentVersion","kimi-code":"kimiCodeVersion",openclaw:"openClawVersion",opencode:"openCodeVersion",pi:"piCliVersion"};async function O8(n={}){let t=await gn(n.fetcher),r=Yl(t,n.homeDir);return{targets:i2.map((e)=>{let a=r.find((l)=>l.platform===e.id);return{target:e.id,label:b2(e.id),version:t[U8[e.id]],status:a?.configured?"active":a?.detected?"disabled":a?.inspectionStatus==="not-inspected"?"not-inspected":"not-installed"}}),system:{version:t.version,nodeVersion:t.nodeVersion,platform:t.platform}}}function Yl(n,t){return rn(process.cwd(),{homeDir:t,ampPluginListOutput:n.ampPluginListOutput,codexPluginListOutput:n.codexPluginListOutput,copilotCliVersion:n.copilotCliVersion})}async function T8(n={}){let[t,r]=await Promise.all([gn(n.fetcher),(n.checkUpdates??P2)()]);return{hooks:Yl(t,n.homeDir).filter((e)=>e.detected).map((e)=>({platform:e.platform,label:b2(e.platform),configured:e.configured})),update:{currentVersion:r.currentVersion,latestVersion:r.latestVersion??null,updateAvailable:r.updateAvailable}}}var ml=Promise.resolve();function E8(n,t,r={}){let e=async()=>{let l=[],d=console.log,s=console.error;console.log=(...L)=>l.push(L.map(String).join(" ")),console.error=console.log;try{return{ok:await Hn(n,[],{selectTargets:async()=>[t],output:new Q8({write(c,i,v){v()}}),...r})===0,output:l.join(`
`)}}finally{console.log=d,console.error=s}},a=ml.then(e);return ml=a.then(()=>{return},()=>{return}),a}async function _8(n={}){let[t,r,e]=await Promise.all([P8(n.command),I8(n.fetchRepo),Promise.resolve(Tn(C2(),n.logsDir).totalBlocked)]);return{starred:t,starCount:r,blockedTotal:e}}async function P8(n="gh",t=gr){if(await Dr(n,["auth","status"],t)!==0)return null;let r=await Dr(n,["api",`/user/starred/${B1}`],t);if(r===0)return!0;if(r===null)return null;return!1}function Dr(n,t,r){return new Promise((e)=>{let a=Ql(n,t,{stdio:"ignore",windowsHide:!0}),l=!1,d,s=(L)=>{if(l)return;if(l=!0,d)clearTimeout(d);e(L)};a.once("error",()=>s(null)),a.once("close",s),d=setTimeout(()=>{a.kill(),s(null)},r)})}async function I8(n=fetch){try{let t=await n(`https://api.github.com/repos/${B1}`,{headers:{accept:"application/vnd.github+json"},signal:AbortSignal.timeout(gr)});if(!t.ok)return null;let r=await t.json();return typeof r.stargazers_count==="number"?r.stargazers_count:null}catch{return null}}function N8(n){if(n[0]!=="help")return!1;let t=n[1];if(!t)It(),process.exit(0);if(Xn(t))process.exit(0);console.error(`Unknown command: ${t}`),console.error("Run 'cc-safety-net --help' for available commands."),process.exit(1)}var n4={hook:async(n)=>{let t=Ce(n);if(t){await t.run();return}console.error("hook requires exactly one integration flag. Try: cc-safety-net hook --kimi-code"),Xn("hook",console.error),process.exit(1)},install:async(n)=>{process.exit(await Hn("install",n))},update:async(n)=>{process.exit(await fr(n))},uninstall:async(n)=>{process.exit(await Hn("uninstall",n))},rule:async(n)=>{process.exit(await vl(n))},status:async(n)=>{if(a2(F({label:"status"},n).errors))process.exit(1);bl()},statusline:async(n)=>{let t=F({label:"statusline",booleans:{claudeCode:["-cc","--claude-code"]}},n);if(t.errors.length===0&&t.flags.claudeCode){await xr();return}if(a2(t.errors),!t.flags.claudeCode)console.error("statusline requires --claude-code (-cc)");Xn("statusline",console.error),process.exit(1)},doctor:async(n)=>{let t=Ot(n);if(!t)process.exit(1);let r=await Sa({json:t.json,skipUpdateCheck:t.skipUpdateCheck});process.exit(r)},logs:async(n)=>{process.exit(await be(n))},gui:async(n)=>{process.exit(await Xl(n))},explain:async(n)=>{let t=Tt(n);if(!t)process.exit(1);let r=G2(t.command,{cwd:t.cwd}),e=!!process.env.NO_COLOR||!process.stdout.isTTY;if(t.json)console.log(Pt(r));else console.log(_t(r,{asciiOnly:e}));process.exit(0)}};async function t4(){let n=process.argv.slice(2),t=F({label:"cc-safety-net",booleans:{version:["-V","--version"]},positionals:"list"},n);if(N8(n))return;let r=n[0],e=r?On(r):void 0;if(t.help&&e&&e.name!=="rule")Xn(e.name),process.exit(0);if(!r||t.help&&!e)It(),process.exit(0);if(t.flags.version)Ga(),process.exit(0);if(e){await n4[e.name](n.slice(1));return}let a=Ue(r);if(a){await a.run();return}if(r==="--statusline"){await xr();return}console.error(r.startsWith("-")?`Unknown option: ${r}`:`Unknown command: ${r}`),console.error("Run 'cc-safety-net --help' for usage."),process.exit(1)}t4().catch((n)=>{console.error("CC Safety Net error:",n),process.exit(1)});
