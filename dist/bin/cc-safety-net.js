...
`);for(let r of n){let e=`[${r.severity.toUpperCase()}] ${r.checkId}: ${B(r.title)}`,a=r.severity==="error"?b.red:r.severity==="warning"?b.yellow:b.blue;if(t.push(`   ${a(e)}`),t.push(`      ${B(r.detail)}`),r.path)t.push(`      Path: ${B(r.path)}`);if(r.fixHint)t.push(`      Fix: ${B(r.fixHint)}`)}return t.join(`
`)}function tc(n){let t=["Variable","Status","Legacy"],r=n.map((e)=>{let a=e.isSet?b.green("✓"):b.dim("✗"),l=e.legacyName&&e.legacyIsSet?`${e.legacyName} ${b.green("✓")}`:e.legacyName??"";return[e.name,a,l]});return F2({headers:t,rows:r})}function va(n){let t=[];if(n.totalBlocked===0)t.push("Recent Activity"),t.push("   No blocked commands in the last 7 days"),t.push("   Tip: This is normal for new installations");else t.push(`Recent Activity · last 7 days (${n.totalBlocked} blocked / ${n.sessionCount} sessions)`),t.push(rc(n.recentEntries));if(n.unreadable>0)t.push(`   Warning: ${n.unreadable} audit log ${n.unreadable===1?"source":"sources"} could not be read; this summary is incomplete`);return t.join(`
`)}function rc(n){let t=["Time","Command"],r=n.map((e)=>{let a=B(e.command.replace(/\r\n|\r|\n/g," ↵ ").replace(/\t/g," ")),l=a.length>40?`${a.slice(0,37)}...`:a;return[e.relativeTime,l]});return F2({headers:t,rows:r})}function oa(n){let t=[];if(t.push("Update Check"),n.latestVersion===null&&!n.error)return t.push(On([["Status",b.dim("Skipped")],["Installed",n.currentVersion]])),t.join(`
...
${s()}`.trim())))}),l.on("close",(i)=>{if(clearTimeout(L),i!==0){e(Error(Ar(n,i,s())));return}r(t?.stdoutOnly?d.stdout:s())})})}async function Rr(n){for(let t of n)await e2(t)}async function h0(n){for(let t of n)try{await e2(t)}catch(r){console.warn(r instanceof Error?r.message:String(r))}}var w0=x2("openclaw",j),BL=[G,rn,en];function Kr(n,t){if(n==="~")return t;if(n.startsWith("~/")||n.startsWith("~\\"))return x2(t,n.slice(2));return n}function $0(n){let t=process.env.OPENCLAW_STATE_DIR?.trim();if(t)return Kr(t,n);let r=process.env.OPENCLAW_CONFIG_PATH?.trim();return r?x0(Kr(r,n)):x2(n,".openclaw")}function g0(n){let t=process.env.OPENCLAW_CONFIG_PATH?.trim();return t?Kr(t,n):x2($0(n),"openclaw.json")}function Mr(n){return x2($0(n),"extensions",j)}function JL(n){let t=jL(n);if(t.length===0)return!0;if(t.some((a)=>!BL.includes(a)))return!1;let r=x2(n,G),e=W(r);return e!==void 0&&!e.isSymbolicLink()&&e.isFile()&&zL(r,"utf-8").startsWith(dt)}function Vr(n){let t=Mr(n),r=W(t);if(!r)return;if(!r.isSymbolicLink()&&r.isDirectory()&&JL(t))return;throw Error(`Refusing to modify ${t}: it does not hold a cc-safety-net managed OpenClaw plugin. Move or remove it, then run the command again.`)}function D0(){let n=x0(FL(import.meta.url));return[x2(n,"..",w0),x2(n,"..","..","..","dist",w0)]}function Gr(n=D0()){return n.find((t)=>kL(t)&&qL(t).isDirectory())}function ZL(n=D0()){let t=Gr(n);if(!t)throw Error("Packaged OpenClaw plugin directory not found. Reinstall cc-safety-net and try again.");return t}function m0(n=ZL()){return[["openclaw","plugins","install",n,"--force"],["openclaw","plugins","enable",j]]}function QL(n){let t=(()=>{try{return JSON.parse(n)}catch{return}})(),r=x(x(t,"plugin"),"status");return typeof r==="string"?r:void 0}async function k0(){let n=QL(await e2(["openclaw","plugins","inspect",j,"--runtime","--json"],{stdoutOnly:!0}));if(n==="loaded")return;throw Error(`${n===void 0?`The ${j} plugin's load state could not be verified: OpenClaw's runtime inspect report was unreadable.`:`OpenClaw reports the ${j} plugin with status "${n}".`} Run \`openclaw plugins inspect ${j} --runtime\` for details.`)}var st="openclaw",Jn=`run \`openclaw plugins enable ${j}\``;function an(n,t){let r=Zn(n,t),e=W(r);if(!e)return{error:`${t} is missing from ${r}; run install --openclaw`};if(e.isSymbolicLink()||!e.isFile())return{error:`${r} is a symlink or not a regular file; move or remove it`};try{return{content:j0(r,"utf-8")}}catch(a){return{error:`Failed to read ${r}: ${a instanceof Error?a.message:String(a)}`}}}function z0(n){try{return JSON.parse(M(n))}catch{return}}function XL(n){let t=an(n,rn);if("error"in t)return t.error;if(x(z0(t.content),"id")===j)return;return`${Zn(n,rn)} is not a valid ${j} manifest; run install --openclaw`}function YL(n){let t=an(n,en);if("error"in t)return t.error;let r=x(x(z0(t.content),"openclaw"),"extensions");if(Array.isArray(r)&&r.includes(`./${G}`))return;return`${Zn(n,en)} does not point OpenClaw at ${G}; run install --openclaw`}function q0(n){return Array.isArray(n)?n.filter((t)=>typeof t==="string"):[]}function SL(n){let t=g0(n);if(!W(t))return`${j} is not enabled; ${Jn}`;let r=(()=>{try{return JSON.parse(M(j0(t,"utf-8")))}catch{return}})();if(r===void 0)return`Failed to read ${t}; fix it, then ${Jn}`;let e=x(r,"plugins");if(x(e,"enabled")===!1)return`plugins.enabled is false in ${t}; no OpenClaw plugin loads`;let a=x(x(x(e,"entries"),j),"enabled");if(q0(x(e,"deny")).includes(j)||a===!1)return`${j} is disabled in ${t}; ${Jn}`;let l=q0(x(e,"allow"));if(l.length>0&&!l.includes(j))return`plugins.allow in ${t} does not list ${j}; add it, then ${Jn}`;if(l.includes(j)||a===!0)return;return`${j} is not enabled; ${Jn}`}function F0(n){return/^\/\/ version:\s*(.+)$/m.exec(n)?.[1]?.trim()}function HL(n,t,r){if(r===void 0)return[];let e=an(r,G);if("error"in e||F0(e.content)!==t)return[];return[G,rn,en].flatMap((a)=>{let l=an(n,a),d=an(r,a);if("error"in l||"error"in d||l.content===d.content)return[];return[`Modified ${a} occupies ${Zn(n,a)}; run install --openclaw to restore it`]})}function B0(n){let t=Mr(n.homeDir),r=Nn(st,t);if(r)return r;let e=an(t,G),l=["error"in e?e.error:e.content.startsWith(dt)?void 0:`Unmanaged ${G} occupies ${Zn(t,G)}; move or remove it`,XL(t),YL(t)].filter((i)=>i!==void 0),d="content"in e?F0(e.content):void 0,s=l.length>0?l:HL(t,d,Gr());if(s.length>0)return{platform:st,status:"n/a",configPath:t,errors:s};let c=d===z()?[]:["Installed OpenClaw plugin is outdated; run install --openclaw to update"],L=SL(n.homeDir);if(L)return{platform:st,status:"disabled",method:"plugin directory",configPath:t,errors:[L,...c]};return{platform:st,status:"configured",method:"plugin directory",configPath:t,errors:c.length>0?c:void 0}}import{existsSync as OL,readFileSync as _L}from"node:fs";import{join as EL}from"node:path";import{existsSync as Ur,readFileSync as Y0,rmSync as WL}from"node:fs";import{join as p2}from"node:path";import{pathToFileURL as RL}from"node:url";function J0(n){return n!==void 0&&/\s/.test(n)}function AL(n,t,r){let e=t+1,a=!1;while(e<n.length){let l=n[e];if(a){a=!1,e++;continue}if(l==="\\"){a=!0,e++;continue}if(l==='"')return e+1;e++}throw Error(r)}function ct(n,t,r){let e=n[t],a=e==="["?"]":"}",l=0,d=t;while(d<n.length){let s=r.skipComment?.(n,d)??d;if(s!==d){d=s;continue}if(n[d]==='"'){d=AL(n,d,r.stringError);continue}if(n[d]===e)l++;if(n[d]===a){if(l--,l===0)return d}d++}throw Error(r.bracketError)}function Z0(n,t){let r=n.lastIndexOf(`
...
`)}function Ll(n){let{input:t,output:r}=n;$2.emitKeypressEvents(t);let e=t.isRaw===!0;t.setRawMode(!0),t.resume();let a=0,l=()=>{if(a===0)return;$2.moveCursor(r,0,-a),$2.cursorTo(r,0),$2.clearScreenDown(r)},d=()=>{l();let s=n.render();r.write(`${s}
`),a=s.split(`
...
`)}`.trim());let r=yl(t.stdout),e=(Array.isArray(r)?r:[]).filter((a)=>x(a,"scope")==="user"&&x(a,"exists")===!0&&x(a,"viewerCanWrite")===!0).map((a)=>x(a,"cloneRef")).find((a)=>typeof a==="string"&&a.length>0);if(!e)throw Error('Your Amp account has no writable Personal Plugins repository. Sign in with "amp login", open Amp once to create it, and rerun install --amp.');return e}async function xl(n,t){let r=ki(Q2(qi(),"cc-safety-net-amp-"));try{return await Xn(n,["amp","clone","user-plugins",r]),await t(r)}finally{bl(r,{recursive:!0,force:!0})}}function $l(n,t){let r=Q2(n,v2),e=fl(r);if(!e)return;if(e.isSymbolicLink()||!e.isFile())throw Error(`Refusing to ${t} ${v2} in your Amp personal plugins repository: not a regular file. Remove it there and rerun install --amp.`);let a=yt(r);if(hl(a))return a;throw Error(`Refusing to ${t} unmanaged file ${v2} in your Amp personal plugins repository. Remove it there and rerun install --amp.`)}async function gl(n,t,r,e){if(await Xn(n,r,t),(await Xn(n,["git","status","--porcelain"],t)).stdout.trim()==="")return!1;return await Xn(n,["git","-c","commit.gpgsign=false","-c","user.name=cc-safety-net","-c","user.email=cc-safety-net@localhost","commit","-m",e],t),await Xn(n,["git","push","origin","HEAD"],t),!0}function bt(n,t){let r=Fi(n),e=fl(r);if(!e)return;if(!e.isSymbolicLink()&&e.isFile()&&hl(yt(r))){bl(r);return}if(t==="keep")return;throw Error(`Local Amp plugin ${r} is not a managed copy and masks the personal plugin. Remove it and rerun install --amp.`)}function Zi(){let n=hn();if(!vl(n))return"";let t=yl(yt(n,"utf-8"));if(!t||typeof t!=="object"||Array.isArray(t))return"";return`;globalThis.__CC_SAFETY_NET_EMBEDDED_POLICY__ = ${JSON.stringify(ft(t))};
`}async function Dl(n,t=Ji(),r=ee){let e=Buffer.concat([yt(t),Buffer.from(Zi(),"utf-8")]),a=await wl(r);return xl(r,async(l)=>{let d=`${a}/${v2}`;if($l(l,"overwrite")?.equals(e))return bt(n,"fail"),{path:d,alreadyInstalled:!0};J(Q2(l,v2),e);let s=await gl(r,l,["git","add",v2],`chore: update cc-safety-net plugin to v${z()}`);return bt(n,"fail"),{path:d,alreadyInstalled:!s}})}async function ml(n,t=ee){let r=await wl(t);return xl(t,async(e)=>{let a=`${r}/${v2}`;if(!$l(e,"remove"))return bt(n,"keep"),{path:a,alreadyInstalled:!1};return await gl(t,e,["git","rm",v2],`chore: remove cc-safety-net plugin v${z()}`),bt(n,"keep"),{path:a,alreadyInstalled:!0}})}import{existsSync as kl,mkdirSync as Qi,readFileSync as Xi}from"node:fs";import{dirname as Yi}from"node:path";var ae="npx -y cc-safety-net hook --agy-cli",g2="cc-safety-net";function wt(){return{PreToolUse:[{hooks:[{type:"command",command:ae,timeout:30}]}]}}function ql(n){try{let t=JSON.parse(Xi(n,"utf-8"));if(!t||typeof t!=="object"||Array.isArray(t))throw Error("Antigravity hooks config must be a JSON object");return t}catch(t){if(t instanceof SyntaxError)throw Error(`Failed to parse Antigravity hooks config ${n}: ${t.message}`);throw t}}function jl(n){let t=n[g2];if(t===void 0)return n[g2]=wt(),n[g2];if(!t||typeof t!=="object"||Array.isArray(t))throw Error(`Antigravity hooks config entry "${g2}" must be an object`);if(!Array.isArray(t.PreToolUse))t.PreToolUse=[];return t}function zl(n){if(!Array.isArray(n.PreToolUse))return!1;return n.PreToolUse.some((t)=>Array.isArray(t.hooks)&&t.hooks.some((r)=>r.command===ae))}function Si(n){return Object.values(n).some((t)=>t.enabled!==!1&&zl(t))}function Hi(n){if(n[g2]===void 0)return!1;let t=jl(n);if(t.enabled!==!1||!zl(t))return!1;return t.enabled=!0,!0}function Ai(n){if(n[g2]===void 0){n[g2]=wt();return}let t=jl(n);t.PreToolUse??=[],t.enabled=!0,t.PreToolUse.push(wt().PreToolUse?.[0]??{hooks:[]})}function Wi(n){let t=!1;for(let r of Object.values(n)){if(!Array.isArray(r.PreToolUse))continue;r.PreToolUse=r.PreToolUse.flatMap((e)=>{if(!Array.isArray(e.hooks))return[e];let a=e.hooks.filter((l)=>l.command!==ae);if(a.length!==e.hooks.length)t=!0;return a.length===0?[]:[{...e,hooks:a}]})}return t}function ht(n,t){J(n,`${JSON.stringify(t,null,2)}
`)}function Fl(n){let t=qn(n);if(Qi(Yi(t),{recursive:!0}),!kl(t))return ht(t,{[g2]:wt()}),{path:t,alreadyInstalled:!1};let r=ql(t);if(Si(r))return{path:t,alreadyInstalled:!0};if(Hi(r))return ht(t,r),{path:t,alreadyInstalled:!1};return Ai(r),ht(t,r),{path:t,alreadyInstalled:!1}}function Bl(n){let t=qn(n);if(!kl(t))return{path:t,alreadyInstalled:!1};let r=ql(t);if(!Wi(r))return{path:t,alreadyInstalled:!1};return ht(t,r),{path:t,alreadyInstalled:!0}}import{spawn as Ri,spawnSync as Ki}from"node:child_process";var C=i2.map((n)=>({target:n.id,flag:n.flag,label:n.installLabel,probeCommand:n.probeCommand})),Jl=new Map(C.map((n)=>[n.flag,n.target]));function le(n){let t=new Set(n);return C.map((r)=>r.target).filter((r)=>t.has(r))}async function Zl(n,t){for(let r of n)await t(r)}var Ql=5000;function Mi(n){let t=w2([...n],process.env),r=Ki(t.cmd,t.args,{env:process.env,stdio:"ignore",timeout:Ql});return!r.error&&r.status===0}function se(n){return new Promise((t)=>{let r=w2([...n],process.env),e=Ri(r.cmd,r.args,{env:process.env,stdio:"ignore"}),a=!1,l=(s)=>{if(a)return;a=!0,clearTimeout(d),t(s)},d=setTimeout(()=>{e.kill(),l(!1)},Ql);e.on("error",()=>l(!1)),e.on("close",(s)=>l(s===0))})}function Vi(n=Mi,t={}){let r=new Set(t.configuredTargets??[]);if(t.async)return Promise.all(C.map(async(a)=>({target:a.target,flag:a.flag,label:a.label,...de(t.action,await n(a.probeCommand),r.has(a.target))})));let e=n;return C.map((a)=>({target:a.target,flag:a.flag,label:a.label,...de(t.action,e(a.probeCommand),r.has(a.target))}))}function Xl(n=se,t={}){return Vi(n,{...t,async:!0})}function Yl(n,t){let r=new Set(t.configuredTargets??[]);return n.map((e)=>({...e,...de(t.action,e.available,r.has(e.target))}))}function de(n,t,r){if(n==="uninstall")return r?{available:!0}:{available:!1,unavailableReason:"not installed"};if(n==="install"&&r)return{available:!1,unavailableReason:"already installed"};if(!t)return{available:!1,unavailableReason:"CLI not installed"};return{available:!0}}import{existsSync as Sl,readdirSync as Gi,rmSync as Ui}from"node:fs";import{join as dn}from"node:path";function xt(n,t=process.platform){let r=dn(process.env.npm_config_cache||(t==="win32"?dn(process.env.LOCALAPPDATA||dn(n,"AppData","Local"),"npm-cache"):dn(n,".npm")),"_npx");if(!Sl(r))return;Gi(r).filter((e)=>Sl(dn(r,e,"node_modules","cc-safety-net"))).forEach((e)=>{Ui(dn(r,e),{recursive:!0,force:!0})})}import{existsSync as Al,mkdirSync as Ci,readFileSync as Wl}from"node:fs";import{dirname as Ti,join as Hl}from"node:path";var Yn="npx -y cc-safety-net hook --kimi-code",ce=`[[hooks]]
event = "PreToolUse"
command = "${Yn}"`,Le=`{ event = "PreToolUse", command = "${Yn}" }`;function Rl(n){return Hl(process.env.KIMI_CODE_HOME??Hl(n,".kimi-code"),"config.toml")}function Oi(n){return n.split(`
...
`,t+1);return r===-1?n.length:r+1}function Ei(n,t){return ct(n,t,{skipComment:_i,stringError:"Unterminated string in Kimi Code config",bracketError:"Unmatched hooks array in Kimi Code config"})}function Kl(n){let t=!1,r=0;while(r<n.length){let e=n.indexOf(`
...
✗ ${n}: ${t}`),console.error("  Errors:");let e=1;for(let a of r)for(let l of a.split("; "))console.error(`    ${e}. ${l}`),e++}function N5(n){try{let t=g(n);if(t===null)return!1;let r=JSON.parse(t);if(r.$schema)return!1;return D2(n,JSON.stringify({$schema:K5,...r},null,2)),!0}catch(t){if(t instanceof Q)throw t;return!1}}var o3=new Set(["init","add","remove","update","sync","list","wrapper","migrate","doc","verify"]),n8=new Set(["add","remove","list"]);async function f3(n){try{return await t8(n)}catch(t){if(t instanceof Q)return console.error(t.message),1;throw t}}async function t8(n){let t=e8(n),r=t.help?r8(t.positionals):null;if(r)return ot(r),0;if(t.errors.length>0){for(let d of t.errors)console.error(d);return 1}let e=t.positionals[0];if(!e)return ot(P2,console.error),1;let a=t.positionals[1],l={global:t.global,check:t.check};if(e==="init"){let d=T(l),s=d.configDir;d8(d.configTarget),b3(m(d.filesystemScope,Kn({...l,cacheConfigDir:s})));let c=I5(s,"example-rules","rulebook.json"),L=m(d.filesystemScope,c);if(t.example&&g(L)===null)It(L,"example-rules");let i=await M2(l);return Hn(i,"Rule config initialized."),i.ok?0:1}if(e==="add"){if(!a)return console.error("rule add requires a source"),1;let d=await Tt(a,l);return Hn(d,`Added rulebook source: ${a}`),d.ok?0:1}if(e==="remove"){if(!a)return console.error("rule remove requires a source"),1;let d=await _t(a,{...l,deleteSource:t.deleteSource});return Hn(d,`Removed rulebook source: ${a}`),d.ok?0:1}if(e==="update"||e==="sync"){let d=await M2({...l,only:e==="update"?a:void 0});return Hn(d,t.check?"Rule config checked.":"Rule config synced."),d.ok?0:1}if(e==="list"){let d=f2(),s=N({});return e3(d,{user:m2(d.userConfigPath,s.userScope),project:m2(d.projectConfigPath,s.projectScope)}),d.errors.length>0?1:0}if(e==="wrapper")return s8(t);if(e==="migrate")return s3({cleanup:t.cleanup,cwd:process.cwd()});if(e==="doc"){console.log(t3);let d=await L3();if(d)console.error(d);return 0}if(e==="verify")return u3();return 1}function r8(n){if(n.length===0)return P2;let t=P2.subcommands.filter((e)=>e.usage.split(" ")[0]===n[0]);if(t.length===0)return null;if(n.length===1&&t.length>1)return{name:`rule ${n[0]}`,description:`Subcommands of rule ${n[0]}`,usage:`rule ${n[0]} <subcommand>`,subcommands:t,options:[]};let r=n.length===1?t[0]:t.find((e)=>e.usage.split(" ")[1]===n[1]);if(!r)return null;return{name:`rule ${n[0]}`,description:r.description,usage:`rule ${r.usage}`,options:[]}}function e8(n){let t=F({label:"rule",booleans:{global:["-g","--global"],check:["--check"],cleanup:["--cleanup"],deleteSource:["--delete-source"],example:["--example"]},positionals:"list"},n),r={...t.flags,help:t.help,positionals:t.positionals,errors:t.errors};return a8(r),r}function a8(n){let[t]=n.positionals;if(t&&!o3.has(t))n.errors.push(`Unknown rule subcommand: ${t}`);if(n.deleteSource&&t!=="remove")if(t&&o3.has(t))n.errors.push(`Unknown option for rule ${t}: --delete-source`);else n.errors.push("--delete-source is only valid with 'rule remove'");if(n.cleanup&&t!=="migrate")n.errors.push(qt(t,"--cleanup"));if(n.example&&t!=="init")n.errors.push(qt(t,"--example"));if(t==="migrate"){if(n.global)n.errors.push(qt(t,"--global"));if(n.check)n.errors.push(qt(t,"--check"));if(n.positionals.length>1)n.errors.push(`Unexpected rule migrate argument: ${n.positionals[1]}`)}else if(t==="wrapper")l8(n);else if(n.positionals.length>2)n.errors.push(`Unexpected rule argument: ${n.positionals[2]}`);if(t==="list"&&n.global)n.errors.push("Unknown option for rule list: --global")}function qt(n,t){return n?`Unknown option for rule ${n}: ${t}`:`Unknown option for rule: ${t}`}function l8(n){let t=n.positionals[1],r=n.positionals[2];if(!t){n.errors.push("rule wrapper requires add, remove, or list");return}if(!n8.has(t)){n.errors.push(`Unknown rule wrapper action: ${t}`);return}if(t==="list"){if(r)n.errors.push(`Unexpected rule wrapper argument: ${r}`);return}if(!r){n.errors.push(`rule wrapper ${t} requires a command`);return}if(n.positionals.length>3)n.errors.push(`Unexpected rule wrapper argument: ${n.positionals[3]}`)}function d8(n){if(g(n)===null){Nt(n);return}let t=O(n);if(!t.config)return;_(n,{version:1,rules:t.config.rules,overrides:t.config.overrides??{},transparent_wrappers:t.config.transparent_wrappers??[]})}async function s8(n){let t=n.positionals[1],r=n.positionals[2],e=T({global:n.global}).configTarget;if(t==="list"){let s=O(e);if(s.errors.length>0){for(let c of s.errors)console.error(c);return 1}return c8(s.config?.transparent_wrappers??[]),0}if(!r||!tr.test(r))return console.error("transparent wrapper must match command pattern"),1;if(rr(r))return console.error(`reserved command "${r}" cannot be a wrapper`),1;let a=O(e);if(a.errors.length>0){for(let s of a.errors)console.error(s);return 1}let l=a.config??{version:1,rules:[],overrides:{},transparent_wrappers:[]},d=t==="add"?[...new Set([...l.transparent_wrappers??[],r])]:(l.transparent_wrappers??[]).filter((s)=>s!==r);return _(e,{version:1,rules:l.rules,overrides:l.overrides??{},transparent_wrappers:d}),console.log(t==="add"?`Added transparent wrapper: ${r}`:`Removed transparent wrapper: ${r}`),0}function c8(n){if(n.length===0){console.log("Transparent wrappers: (none)");return}console.log(`Transparent wrappers (${n.length}):`);for(let t of n)console.log(`  - ${t}`)}import{homedir as y3}from"node:os";import{existsSync as L8,readFileSync as i8}from"node:fs";import{homedir as p8}from"node:os";import{join as u8}from"node:path";async function v8(n){if(n.isTTY)return null;return(await Lr(n).catch(()=>null))?.trim()||null}function o8(){if(process.env.CLAUDE_SETTINGS_PATH)return process.env.CLAUDE_SETTINGS_PATH;return u8(p8(),".claude","settings.json")}function ye(){let n=o8();if(!L8(n))return!1;try{let t=i8(n,"utf-8"),r=JSON.parse(t);if(!r.enabledPlugins)return!1;let e="cc-safety-net@cc-marketplace";if(!(e in r.enabledPlugins))return!1;return r.enabledPlugins[e]===!0}catch(t){if(yn(k.debug))console.error(`CC Safety Net debug: failed to read Claude settings: ${n}: ${t instanceof Error?t.message:String(t)}`);return!1}}async function he(n=process.stdin){let t=ye(),r;if(!t)r="\uD83D\uDEE1️ CC Safety Net ❌";else{let a=K({cwd:process.cwd()}),l=a.policy,d=s2(l),s=Object.values(q2(l,d.capabilities)).some((L)=>L.changesInherited);r=`\uD83D\uDEE1️ CC Safety Net ${{standard:"✅",strict:"\uD83D\uDD12",paranoid:"\uD83D\uDC41️",custom:"\uD83D\uDD27"}[s?"custom":d.effectiveLevel]}${d.worktreeMode?"\uD83C\uDF33":""}${a.state==="degraded"?"⚠️":""}`}let e=await v8(n);if(e&&!e.startsWith("{"))console.log(`${e} | ${r}`);else console.log(r)}function h3(){let n=K({cwd:process.cwd()}),t=n.policy,r=s2(t),e=!!process.env.NO_COLOR||!process.stdout.isTTY,a=Math.min(process.stdout.columns||80,100),l=e?"ok":"✔",d=e?"OFF":"✘",s=(p,f)=>{let o=`  ${p.padEnd(13)}${f}`;return(o.length>a?`${o.slice(0,a-1)}…`:o).replaceAll(d,b.red(d))},c=Object.values(q2(t,r.capabilities)).some((p)=>p.changesInherited),L=hn(),i={ready:b.green,degraded:b.yellow}[n.state],u=[...ye()?[]:["plugin cc-safety-net@cc-marketplace is disabled in Claude Code; nothing is enforced in Claude Code until it is re-enabled. Other integrations are not affected."],...n.diagnostics],v=e?"-":"·";console.log([`${e?"":"\uD83D\uDEE1️  "}CC Safety Net — ${i(n.state)}`,"",s("Protection",`destructive ${t.destructiveCommandProtectionEnabled?l:d}   secrets ${t.secretProtection.enabled?l:d}`),s("Level",c?`${r.effectiveLevel} (customised)`:r.effectiveLevel),s("Rules",t.rules.length===0?"none active":`${t.rules.length} active`),s("Policy",L.startsWith(y3())?`~${L.slice(y3().length)}`:L),...r.worktreeMode?[s("Worktree","relaxations active")]:[],"",...u.length===0?["  Everything configured is active."]:["  Not active",...u.flatMap((p)=>vt(p,"      ",a-6).map((f,o)=>o===0?`    ${v} ${f}`:f)),"","  Full report: cc-safety-net doctor"]].join(`
`))}import{spawn as H3}from"node:child_process";import{randomBytes as k8}from"node:crypto";import{createServer as q8}from"node:http";import{Writable as j8}from"node:stream";import{homedir as b8}from"node:os";var jt=500;function f8(n){let t=n.filter((a)=>a.decision!=="allow"),r=n.filter((a)=>a.decision==="allow"),e=Math.min(t.length,Math.max(jt-r.length,Math.ceil(jt/2)));return[...t.slice(0,e),...r.slice(0,jt-e)]}function w3(n,t=c2()){if(t)k2(t);let r=(o)=>new Date(o.getFullYear(),o.getMonth(),o.getDate()).getTime(),e=r(new Date),a=new Date(e);a.setDate(a.getDate()-(n-1));let l=a.getTime(),d=[],s={count:0};for(let o of t?b2(t,s):[])for(let h of Ln(o,s)){if(!h||typeof h.ts!=="string"||typeof h.command!=="string")continue;let w=new Date(h.ts).getTime();if(!Number.isFinite(w))continue;if(w>=l)d.push(h)}d.sort((o,h)=>new Date(h.ts).getTime()-new Date(o.ts).getTime());let c=Array.from({length:n},()=>0),L=Array.from({length:n},()=>0),i={},u={},v={},p=0,f=0;for(let o of d){let h=o.agent||"unknown";i[h]=(i[h]??0)+1;let w=Math.round((e-r(new Date(o.ts)))/86400000),$=n-1-w,y=w>=0&&w<n;if(y)L[$]=(L[$]??0)+1;if(o.decision!=="allow"){if(p++,o.ruleId)u[o.ruleId]=(u[o.ruleId]??0)+1;let D=S2(o.segment||o.command);if(D)v[D]=(v[D]??0)+1;if(o.failureStage)f++;if(y)c[$]=(c[$]??0)+1}}return{days:n,logsDir:t,homeDir:b8(),totalInWindow:d.length,truncated:d.length>jt,unreadable:s.count,counts:{blocked:p,allowed:d.length-p,agents:i,blockedByDay:c,analyzedByDay:L,rules:u,commands:v,errors:f},entries:f8(d).sort((o,h)=>new Date(h.ts).getTime()-new Date(o.ts).getTime())}}import{spawn as y8}from"node:child_process";import{existsSync as x3,statSync as h8}from"node:fs";import{delimiter as w8,join as x8}from"node:path";var $8=120000,zt="Choose the project folder",g8=`try
...
`;var q3='<script id="ccsn-data" type="application/json">';function j3(n){return k3.replace(q3,()=>q3+JSON.stringify({token:n}).replaceAll("<","\\u003c"))}var Bt="kenryu42/cc-safety-net",z8=`https://github.com/${Bt}`,$e=1e4,F8=7;async function A3(n,t={}){let r=F({label:"gui",booleans:{noOpen:["--no-open"]}},n),e=t.log??console.log,a=t.error??console.error;if(r.errors.length>0){for(let d of r.errors)a(d);return a("Usage: cc-safety-net gui [--no-open]"),1}let l=await B8(t);if(e(`CC Safety Net policy GUI: ${l.url}`),!r.flags.noOpen)try{await(t.openBrowser??A8)(l.url)}catch(d){a(`Failed to open browser: ${d instanceof Error?d.message:String(d)}`),a(`Open this URL manually: ${l.url}`)}if(t.keepAlive===!1)return await l.close(),0;return await H8(l),0}async function B8(n={}){let t=n.token??k8(24).toString("base64url"),r=q8((l,d)=>{J8(l,d,t,n)});await new Promise((l,d)=>{r.once("error",d),r.listen(0,"127.0.0.1",()=>{r.off("error",d),l()})});let a=`http://127.0.0.1:${r.address().port}`;return{origin:a,token:t,url:`${a}/?token=${encodeURIComponent(t)}`,close:()=>S8(r)}}async function J8(n,t,r,e){let a=new URL(n.url??"/","http://127.0.0.1");if(n.method==="GET"&&a.pathname==="/favicon.ico"){t.writeHead(204,{"cache-control":"no-store"}),t.end();return}if(!X8(n,a,r)){q(t,403,{error:"Forbidden"});return}if(n.method==="GET"&&a.pathname==="/"){Y8(t,j3(r));return}if(n.method==="GET"&&a.pathname==="/api/policy"){let l=J3(e);q(t,200,{...l,configState:wn(K(e)),destructiveCommandRules:bn,secretPatterns:F3,version:z(),preview:l.errors.length>0?null:Q3(l.policy)});return}if(n.method==="POST"&&a.pathname==="/api/policy/preview"){let l=await Ft(n);if(!l.ok){q(t,400,{errors:[l.error]});return}let d=Z3(l.value);q(t,d.errors.length>0?400:200,d);return}if(n.method==="POST"&&a.pathname==="/api/policy/explain"){let l=await Ft(n);if(!l.ok){q(t,400,{errors:[l.error]});return}let d=l.value;if(d===null||typeof d.command!=="string"){q(t,400,{errors:["command must be a string"]});return}let s=dr(d.policy);if(s.length>0){q(t,400,{errors:s});return}q(t,200,Z8(d.command,d.policy,e));return}if(n.method==="POST"&&a.pathname==="/api/policy"){let l=await Ft(n);if(!l.ok){q(t,400,{errors:[l.error]});return}let d=we(l.value,e);q(t,d.errors.length>0?400:200,d);return}if(n.method==="POST"&&a.pathname==="/api/reset"){q(t,200,we(B3,e));return}if(n.method==="POST"&&a.pathname==="/api/repair"){q(t,200,X3(e));return}if(n.method==="GET"&&a.pathname==="/api/activity"){let l=T2(e),d=Q8(a.searchParams.get("days"),l);if(d===null){q(t,400,{error:`days must be an integer between 1 and ${l}`});return}q(t,200,w3(d,e.activityLogsDir));return}if(n.method==="POST"&&a.pathname==="/api/rules/choose-directory"){q(t,200,await m3());return}if(n.method==="GET"&&a.pathname==="/api/rules"){let l=f2(e),d=new Map(l.rules.map((s)=>[s.name,s]));q(t,200,{projectPath:e.cwd??process.cwd(),canPickDirectory:D3(process.platform,process.env),rulebooks:l.rulebooks.map((s)=>({source:s.source,spec:s.spec,name:s.name,version:s.version,rules:s.rules.flatMap((c)=>{let L=d.get(c);if(!L)return[];return[{name:L.name,command:L.command,subcommand:L.subcommand,block_args:L.block_args,reason:L.reason}]})})),errors:l.errors,warnings:l.warnings});return}if(n.method==="GET"&&a.pathname==="/api/star/context"){q(t,200,await(e.fetchStarContext??(()=>G8({logsDir:e.activityLogsDir})))());return}if(n.method==="POST"&&a.pathname==="/api/star"){let l=await(e.starRepo??W8)();q(t,200,l.ok?{ok:!0}:{ok:!1,fallbackUrl:z8});return}if(n.method==="GET"&&a.pathname==="/api/integrations"){q(t,200,await(e.fetchIntegrations??K8)());return}if(n.method==="GET"&&a.pathname==="/api/health"){q(t,200,await(e.fetchHealth??M8)());return}if(n.method==="POST"&&(a.pathname==="/api/install"||a.pathname==="/api/uninstall")){let l=await Ft(n);if(!l.ok){q(t,400,{errors:[l.error]});return}let d=l.value?.target;if(typeof d!=="string"||!C.some((c)=>c.target===d)){q(t,400,{error:"unknown target"});return}let s=a.pathname==="/api/install"?"install":"uninstall";q(t,200,await(e.runIntegration??V8)(s,d));return}q(t,404,{error:"Not found"})}function Z8(n,t,r){let e=ft(t),a=K(r),l=fn({rules:a.policy.rules,transparentWrappers:a.policy.transparentWrappers,safety:S3(e.safety),worktreeMode:e.workflow.worktree_mode,destructiveCommandProtectionEnabled:e.destructive_command_protection.enabled,destructiveCommandRuleOverrides:e.destructive_command_protection.overrides,destructiveCommandAllowPaths:e.destructive_command_protection.allow_paths,secretProtection:{enabled:e.secret_protection.enabled,disabledRules:[...Y3(e.secret_protection.overrides)],denyPaths:e.secret_protection.deny_paths,allowPaths:e.secret_protection.allow_paths}});return C2(n,{policySnapshot:l,cwd:r.cwd,userConfigDir:r.userConfigDir})}function Q8(n,t){if(n===null)return Math.min(F8,t);let r=Number(n);if(!Number.isInteger(r)||r<1||r>t)return null;return r}function X8(n,t,r){if(t.searchParams.get("token")!==r)return!1;if(n.method!=="POST")return!0;return n.headers["x-cc-safety-net-token"]===r}async function Ft(n){let t=[];for await(let r of n)t.push(r);try{return{ok:!0,value:JSON.parse(Buffer.concat(t).toString("utf-8")||"{}")}}catch(r){return{ok:!1,error:`Invalid JSON: ${r instanceof Error?r.message:String(r)}`}}}function Y8(n,t){n.writeHead(200,{"content-type":"text/html; charset=utf-8","cache-control":"no-store"}),n.end(t)}function q(n,t,r){n.writeHead(t,{"content-type":"application/json; charset=utf-8","cache-control":"no-store"}),n.end(JSON.stringify(r))}function S8(n){return new Promise((t,r)=>{n.close((e)=>e?r(e):t())})}function H8(n){return new Promise((t)=>{let r=()=>{process.off("SIGINT",e),process.off("SIGTERM",e)},e=()=>{r(),n.close().then(t)};process.once("SIGINT",e),process.once("SIGTERM",e)})}function A8(n){let t=process.platform==="darwin"?"open":process.platform==="win32"?"cmd":"xdg-open",r=process.platform==="win32"?["/c","start","",n]:[n];return new Promise((e,a)=>{let l=H3(t,r,{detached:!0,stdio:"ignore"}),d=(c)=>{l.off("spawn",s),a(c)},s=()=>{l.off("error",d),l.unref(),e()};l.once("error",d),l.once("spawn",s)})}async function W8(n="gh",t=$e){return{ok:await xe(n,["api","-X","PUT",`/user/starred/${Bt}`],t)===0}}var R8={amp:"ampVersion","antigravity-cli":"antigravityCliVersion","claude-code":"claudeCodeVersion",codex:"codexCliVersion","copilot-cli":"copilotCliVersion",cursor:"cursorVersion","gemini-cli":"geminiCliVersion","hermes-agent":"hermesAgentVersion","kimi-code":"kimiCodeVersion",openclaw:"openClawVersion",opencode:"openCodeVersion",pi:"piCliVersion"};async function K8(n={}){let t=await gn(n.fetcher),r=W3(t,n.homeDir);return{targets:i2.map((e)=>{let a=r.find((l)=>l.platform===e.id);return{target:e.id,label:y2(e.id),version:t[R8[e.id]],status:a?.configured?"active":a?.detected?"disabled":a?.inspectionStatus==="not-inspected"?"not-inspected":"not-installed"}}),system:{version:t.version,nodeVersion:t.nodeVersion,platform:t.platform}}}function W3(n,t){return ln(process.cwd(),{homeDir:t,ampPluginListOutput:n.ampPluginListOutput,codexPluginListOutput:n.codexPluginListOutput,copilotCliVersion:n.copilotCliVersion})}async function M8(n={}){let[t,r]=await Promise.all([gn(n.fetcher),(n.checkUpdates??I2)()]);return{hooks:W3(t,n.homeDir).filter((e)=>e.detected).map((e)=>({platform:e.platform,label:y2(e.platform),configured:e.configured})),update:{currentVersion:r.currentVersion,latestVersion:r.latestVersion??null,updateAvailable:r.updateAvailable}}}var z3=Promise.resolve();function V8(n,t,r={}){let e=async()=>{let l=[],d=console.log,s=console.error;console.log=(...c)=>l.push(c.map(String).join(" ")),console.error=console.log;try{return{ok:await Sn(n,[],{selectTargets:async()=>[t],output:new j8({write(L,i,u){u()}}),...r})===0,output:l.join(`
`)}}finally{console.log=d,console.error=s}},a=z3.then(e);return z3=a.then(()=>{return},()=>{return}),a}async function G8(n={}){let[t,r,e]=await Promise.all([U8(n.command),C8(n.fetchRepo),Promise.resolve(Tn(T2(),n.logsDir).totalBlocked)]);return{starred:t,starCount:r,blockedTotal:e}}async function U8(n="gh",t=$e){if(await xe(n,["auth","status"],t)!==0)return null;let r=await xe(n,["api",`/user/starred/${Bt}`],t);if(r===0)return!0;if(r===null)return null;return!1}function xe(n,t,r){return new Promise((e)=>{let a=H3(n,t,{stdio:"ignore",windowsHide:!0}),l=!1,d,s=(c)=>{if(l)return;if(l=!0,d)clearTimeout(d);e(c)};a.once("error",()=>s(null)),a.once("close",s),d=setTimeout(()=>{a.kill(),s(null)},r)})}async function C8(n=fetch){try{let t=await n(`https://api.github.com/repos/${Bt}`,{headers:{accept:"application/vnd.github+json"},signal:AbortSignal.timeout($e)});if(!t.ok)return null;let r=await t.json();return typeof r.stargazers_count==="number"?r.stargazers_count:null}catch{return null}}function T8(n){if(n[0]!=="help")return!1;let t=n[1];if(!t)ne(),process.exit(0);if(Qn(t))process.exit(0);console.error(`Unknown command: ${t}`),console.error("Run 'cc-safety-net --help' for available commands."),process.exit(1)}var O8={hook:async(n)=>{let t=G1(n);if(t){await t.run();return}console.error("hook requires exactly one integration flag. Try: cc-safety-net hook --kimi-code"),Qn("hook",console.error),process.exit(1)},install:async(n)=>{process.exit(await Sn("install",n))},update:async(n)=>{process.exit(await oe(n))},uninstall:async(n)=>{process.exit(await Sn("uninstall",n))},rule:async(n)=>{process.exit(await f3(n))},status:async(n)=>{if(a2(F({label:"status"},n).errors))process.exit(1);h3()},statusline:async(n)=>{let t=F({label:"statusline",booleans:{claudeCode:["-cc","--claude-code"]}},n);if(t.errors.length===0&&t.flags.claudeCode){await he();return}if(a2(t.errors),!t.flags.claudeCode)console.error("statusline requires --claude-code (-cc)");Qn("statusline",console.error),process.exit(1)},doctor:async(n)=>{let t=_r(n);if(!t)process.exit(1);let r=await T0({json:t.json,skipUpdateCheck:t.skipUpdateCheck});process.exit(r)},logs:async(n)=>{process.exit(await u1(n))},gui:async(n)=>{process.exit(await A3(n))},explain:async(n)=>{let t=Er(n);if(!t)process.exit(1);let r=C2(t.command,{cwd:t.cwd}),e=!!process.env.NO_COLOR||!process.stdout.isTTY;if(t.json)console.log(Ir(r));else console.log(Nr(r,{asciiOnly:e}));process.exit(0)}};async function _8(){let n=process.argv.slice(2),t=F({label:"cc-safety-net",booleans:{version:["-V","--version"]},positionals:"list"},n);if(T8(n))return;let r=n[0],e=r?Cn(r):void 0;if(t.help&&e&&e.name!=="rule")Qn(e.name),process.exit(0);if(!r||t.help&&!e)ne(),process.exit(0);if(t.flags.version)tl(),process.exit(0);if(e){await O8[e.name](n.slice(1));return}let a=U1(r);if(a){await a.run();return}if(r==="--statusline"){await he();return}console.error(r.startsWith("-")?`Unknown option: ${r}`:`Unknown command: ${r}`),console.error("Run 'cc-safety-net --help' for usage."),process.exit(1)}_8().catch((n)=>{console.error("CC Safety Net error:",n),process.exit(1)});
