...
${output}`.trim()));if(result.status!==0)throw Error(formatCommandFailure(command,result.status,output));return output}function runNativeCommands(commands2){commands2.forEach((command)=>{runNativeCommand(command)})}import{existsSync as existsSync14,readdirSync as readdirSync4,rmSync as rmSync2}from"node:fs";import{join as join16}from"node:path";function clearNpxSafetyNetCache(homeDir,platform=process.platform){let npxDir=join16(process.env.npm_config_cache||(platform==="win32"?join16(process.env.LOCALAPPDATA||join16(homeDir,"AppData","Local"),"npm-cache"):join16(homeDir,".npm")),"_npx");if(!existsSync14(npxDir))return;readdirSync4(npxDir).filter((entry)=>existsSync14(join16(npxDir,entry,"node_modules","cc-safety-net"))).forEach((entry)=>{rmSync2(join16(npxDir,entry),{recursive:!0,force:!0})})}import{existsSync as existsSync15,mkdirSync as mkdirSync4,readFileSync as readFileSync13}from"node:fs";import{dirname as dirname7,join as join17}from"node:path";function isWhitespace(char){return char!==void 0&&/\s/.test(char)}function skipString(content,index,errorMessage){let current=index+1,isEscaped=!1;while(current<content.length){let char=content[current];if(isEscaped){isEscaped=!1,current++;continue}if(char==="\\"){isEscaped=!0,current++;continue}if(char==='"')return current+1;current++}throw Error(errorMessage)}function findMatchingBracket(content,openIndex,options){let open=content[openIndex],close=open==="["?"]":"}",depth=0,index=openIndex;while(index<content.length){let nextIndex=options.skipComment?.(content,index)??index;if(nextIndex!==index){index=nextIndex;continue}if(content[index]==='"'){index=skipString(content,index,options.stringError);continue}if(content[index]===open)depth++;if(content[index]===close){if(depth--,depth===0)return index}index++}throw Error(options.bracketError)}function getLineIndent(content,index){let lineStart=content.lastIndexOf(`
...
event = "PreToolUse"
command = "${KIMI_HOOK_COMMAND}"`,KIMI_INLINE_HOOK=`{ event = "PreToolUse", command = "${KIMI_HOOK_COMMAND}" }`;function getKimiConfigPath(homeDir){return join17(process.env.KIMI_CODE_HOME??join17(homeDir,".kimi-code"),"config.toml")}function removeTopLevelEmptyHooksArray(content){return content.split(`
...
if ($dialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { [Console]::Out.Write($dialog.SelectedPath) }`,LINUX_DIALOGS=[{binary:"zenity",args:["--file-selection","--directory",`--title=${PROMPT}`]},{binary:"kdialog",args:["--getexistingdirectory",".","--title",PROMPT]}],onPath=(binary,env)=>(env.PATH??"").split(delimiter2).some((dir)=>dir.length>0&&existsSync19(join25(dir,binary)));function isDirectoryPickerAvailable(platform,env){if(platform==="darwin"||platform==="win32")return!0;if(platform!=="linux")return!1;if(!env.DISPLAY&&!env.WAYLAND_DISPLAY)return!1;return LINUX_DIALOGS.some((dialog)=>onPath(dialog.binary,env))}function getDialogCommand(platform,env){if(platform==="darwin")return{cmd:"osascript",args:["-e",MACOS_SCRIPT]};if(platform==="win32")return{cmd:"powershell.exe",args:["-NoProfile","-STA","-Command",WINDOWS_SCRIPT]};let dialog=LINUX_DIALOGS.find((candidate)=>onPath(candidate.binary,env));return dialog?{cmd:dialog.binary,args:dialog.args}:null}function chooseDirectory(platform=process.platform,env=process.env){let command=getDialogCommand(platform,env);if(!command)return Promise.resolve({error:"No folder dialog is available on this system"});return new Promise((resolve6)=>{let child=spawn3(command.cmd,command.args,{env,stdio:["ignore","pipe","pipe"]}),stdout2="",settled=!1,finish=(result)=>{if(settled)return;settled=!0,clearTimeout(timeout),resolve6(result)},timeout=setTimeout(()=>{child.kill(),finish({error:"The folder dialog timed out"})},DIALOG_TIMEOUT_MS);child.stdout.on("data",(chunk)=>{stdout2+=chunk.toString()}),child.on("error",()=>finish({error:`Could not open the folder dialog (${command.cmd})`})),child.on("close",()=>{let picked=stdout2.trim().replace(/\/+$/,"");if(!picked)return finish({cancelled:!0});if(!existsSync19(picked)||!statSync2(picked).isDirectory())return finish({error:"That selection is not a folder on disk"});finish({path:picked})})})}var guiDocument=`<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
