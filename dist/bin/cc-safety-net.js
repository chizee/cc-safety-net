...
`}function buildHermesAgentPluginFiles(version){return[{name:"__init__.py",content:buildPluginSource(version)},{name:"plugin.yaml",content:buildManifest(version)}]}import{mkdirSync as mkdirSync3,readdirSync as readdirSync4,readFileSync as readFileSync10,rmSync as rmSync2}from"node:fs";import{join as join12}from"node:path";var BYTECODE_CACHE_DIR="__pycache__";function getHermesHomeDir(homeDir){let hermesHome=process.env.HERMES_HOME?.trim();return hermesHome?hermesHome:join12(homeDir,".hermes")}function getHermesAgentPluginDir(homeDir){return join12(getHermesHomeDir(homeDir),"plugins",HERMES_AGENT_PLUGIN_NAME)}function isManagedHermesAgentFile(content){return content.startsWith(HERMES_AGENT_MANAGED_HEADER)}function resolvePluginDir(homeDir,action){let dir=getHermesAgentPluginDir(homeDir),info=lstatOrUndefined2(dir);if(info&&(info.isSymbolicLink()||!info.isDirectory()))throw Error(`Refusing to ${action} ${dir}: not a regular directory. Move or remove it and rerun ${action==="install"?"install":"uninstall"} --hermes-agent.`);return dir}function readManagedFile(path,action){let info=lstatOrUndefined2(path);if(!info)return;if(info.isSymbolicLink()||!info.isFile())throw Error(`Refusing to ${action} ${path}: not a regular file. Move or remove it.`);let content=readFileSync10(path,"utf-8");if(!isManagedHermesAgentFile(content))throw Error(`Refusing to ${action} unmanaged file at ${path}. Move or remove it.`);return content}function installHermesAgent(homeDir){let dir=resolvePluginDir(homeDir,"install"),files=buildHermesAgentPluginFiles(getPackageVersion());if(files.map((file)=>readManagedFile(join12(dir,file.name),"overwrite")).every((content,index)=>content===files[index]?.content))return{path:dir,alreadyInstalled:!0};return mkdirSync3(dir,{recursive:!0}),files.forEach((file)=>{atomicWriteFile(join12(dir,file.name),file.content)}),{path:dir,alreadyInstalled:!1}}function readOwnedHermesAgentFiles(homeDir){let dir=resolvePluginDir(homeDir,"remove");if(!lstatOrUndefined2(dir))return[];return buildHermesAgentPluginFiles(getPackageVersion()).filter((file)=>readManagedFile(join12(dir,file.name),"remove")!==void 0)}function uninstallHermesAgent(homeDir){let dir=resolvePluginDir(homeDir,"remove");if(!lstatOrUndefined2(dir))return{path:dir,alreadyInstalled:!1};let present=readOwnedHermesAgentFiles(homeDir);if(present.forEach((file)=>{rmSync2(join12(dir,file.name))}),rmSync2(join12(dir,BYTECODE_CACHE_DIR),{recursive:!0,force:!0}),readdirSync4(dir).length===0)rmSync2(dir,{recursive:!0});return{path:dir,alreadyInstalled:present.length>0}}var PLATFORM="hermes-agent",TOP_LEVEL_KEY=/^([^\s#][^:]*):/,NESTED_KEY=/^\s+([A-Za-z_][\w-]*):/,SEQUENCE_ITEM=/^\s+-\s*(.*)$/;function unquote(value){return value.trim().replace(/^(["'])(.*)\1$/,"$2")}function pluginsBlock(raw){let lines=raw.split(/\r?\n/),start=lines.findIndex((line)=>TOP_LEVEL_KEY.exec(line)?.[1]?.trim()==="plugins");if(start===-1)return[];let body=lines.slice(start+1),end=body.findIndex((line)=>TOP_LEVEL_KEY.test(line));return end===-1?body:body.slice(0,end)}function readPluginList(raw,key){let block=pluginsBlock(raw),start=block.findIndex((line)=>NESTED_KEY.exec(line)?.[1]===key);if(start===-1)return[];let body=block.slice(start+1),end=body.findIndex((line)=>!SEQUENCE_ITEM.test(line));return(end===-1?body:body.slice(0,end)).map((line)=>unquote(SEQUENCE_ITEM.exec(line)?.[1]??""))}function readHermesConfig(homeDir){try{return readFileSync11(join13(getHermesHomeDir(homeDir),"config.yaml"),"utf-8")}catch{return}}function isHermesAgentPluginEnabled(homeDir){let config=readHermesConfig(homeDir)??"";return readPluginList(config,"enabled").includes(HERMES_AGENT_PLUGIN_NAME)&&!readPluginList(config,"disabled").includes(HERMES_AGENT_PLUGIN_NAME)}function artifactVersion(content){return/^# version:\s*(.+)$/m.exec(content)?.[1]?.trim()}function inspectFile(path,expected){let info=lstatOrUndefined2(path);if(!info)return{error:`${expected.name} is missing from ${path}; run install --hermes-agent`};if(info.isSymbolicLink()||!info.isFile())return{error:`${path} is a symlink or not a regular file; move or remove it`};try{let content=readFileSync11(path,"utf-8");if(!isManagedHermesAgentFile(content))return{error:`Unmanaged ${expected.name} occupies ${path}; move or remove it`};if(artifactVersion(content)===getPackageVersion()&&content!==expected.content)return{error:`Modified ${expected.name} occupies ${path}; run install --hermes-agent to restore it`};return{content}}catch(e){return{error:`Failed to read ${path}: ${e instanceof Error?e.message:String(e)}`}}}function detect8(context){let configPath=getHermesAgentPluginDir(context.homeDir),unusable=inspectManagedPluginDir(PLATFORM,configPath);if(unusable)return unusable;let files=buildHermesAgentPluginFiles(getPackageVersion()).map((file)=>inspectFile(join13(configPath,file.name),file)),errors=files.flatMap((file)=>("error"in file)?[file.error]:[]);if(errors.length>0)return{platform:PLATFORM,status:"n/a",configPath,errors};let outdated=files.some((file)=>("content"in file)&&artifactVersion(file.content)!==getPackageVersion()),outdatedError=outdated?["Installed Hermes Agent plugin is outdated; run install --hermes-agent to update"]:[];if(!isHermesAgentPluginEnabled(context.homeDir))return{platform:PLATFORM,status:"disabled",method:"plugin directory",configPath,errors:[`${HERMES_AGENT_PLUGIN_NAME} is not enabled in Hermes; run \`hermes plugins enable ${HERMES_AGENT_PLUGIN_NAME}\``,...outdatedError]};return{platform:PLATFORM,status:"configured",method:"plugin directory",configPath,errors:outdated?outdatedError:void 0}}import{existsSync as existsSync10,readFileSync as readFileSync12}from"node:fs";import{join as join14}from"node:path";var KIMI_HOOK_COMMAND_PATTERN=/cc-safety-net\s+hook\s+(?:[^\s]+\s+)*--kimi-code(\s|["']|$)/;function _getKimiConfigPath(homeDir){return join14(process.env.KIMI_CODE_HOME||join14(homeDir,".kimi-code"),"config.toml")}function detect9(context){let configPath=_getKimiConfigPath(context.homeDir);if(!existsSync10(configPath))return{platform:"kimi-code",status:"n/a",configPath};try{if(!KIMI_HOOK_COMMAND_PATTERN.test(readFileSync12(configPath,"utf-8")))return{platform:"kimi-code",status:"n/a",configPath}}catch(e){return{platform:"kimi-code",status:"n/a",configPath,errors:[`Failed to read ${configPath}: ${e instanceof Error?e.message:String(e)}`]}}return{platform:"kimi-code",status:"configured",method:"hook config",configPath}}import{readFileSync as readFileSync14}from"node:fs";import{join as join16}from"node:path";var OPENCLAW_PLUGIN_ID="cc-safety-net",OPENCLAW_PLUGIN_ENTRY_FILE="index.js",OPENCLAW_PLUGIN_MANIFEST_FILE="openclaw.plugin.json",OPENCLAW_PLUGIN_PACKAGE_FILE="package.json";var OPENCLAW_MANAGED_HEADER="// cc-safety-net managed OpenClaw plugin. Do not edit. Reinstall with: npx -y cc-safety-net install --openclaw";import{existsSync as existsSync11,lstatSync as lstatSync5,readdirSync as readdirSync5,readFileSync as readFileSync13}from"node:fs";import{dirname as dirname6,join as join15}from"node:path";import{fileURLToPath as fileURLToPath2}from"node:url";import{spawnSync}from"node:child_process";function formatNativeCommand(command){return command.join(" ")}function formatCommandFailure(command,status,output){return[`Failed to run ${formatNativeCommand(command)}${status===null?"":` (exit ${status})`}.`,output.trim()].filter(Boolean).join(`
`)}function runNativeCommand(command,options){let result=spawnSync(command[0],command.slice(1),{encoding:"utf-8",stdio:"pipe"}),output=[result.stdout,result.stderr].filter(Boolean).join(`
`);if(result.error)throw Error(formatCommandFailure(command,null,`${result.error.message}
${output}`.trim()));if(result.status!==0)throw Error(formatCommandFailure(command,result.status,output));return options?.stdoutOnly?result.stdout:output}function runNativeCommands(commands2){commands2.forEach((command)=>{runNativeCommand(command)})}var OPENCLAW_ARTIFACT_RELATIVE=join15("openclaw",OPENCLAW_PLUGIN_ID),INSTALLED_PLUGIN_FILES=[OPENCLAW_PLUGIN_ENTRY_FILE,OPENCLAW_PLUGIN_MANIFEST_FILE,OPENCLAW_PLUGIN_PACKAGE_FILE];function getOpenClawStateDir(homeDir){let stateDir=process.env.OPENCLAW_STATE_DIR?.trim();if(stateDir)return stateDir;let configPath=process.env.OPENCLAW_CONFIG_PATH?.trim();return configPath?dirname6(configPath):join15(homeDir,".openclaw")}function getOpenClawConfigPath(homeDir){let configPath=process.env.OPENCLAW_CONFIG_PATH?.trim();return configPath?configPath:join15(getOpenClawStateDir(homeDir),"openclaw.json")}function getOpenClawPluginDir(homeDir){return join15(getOpenClawStateDir(homeDir),"extensions",OPENCLAW_PLUGIN_ID)}function holdsOnlyOurPlugin(dir){let entries=readdirSync5(dir);if(entries.length===0)return!0;if(entries.some((name)=>!INSTALLED_PLUGIN_FILES.includes(name)))return!1;let entry=join15(dir,OPENCLAW_PLUGIN_ENTRY_FILE),info=lstatOrUndefined2(entry);return info!==void 0&&!info.isSymbolicLink()&&info.isFile()&&readFileSync13(entry,"utf-8").startsWith(OPENCLAW_MANAGED_HEADER)}function assertOpenClawPluginDirIsOurs(homeDir){let dir=getOpenClawPluginDir(homeDir),info=lstatOrUndefined2(dir);if(!info)return;if(!info.isSymbolicLink()&&info.isDirectory()&&holdsOnlyOurPlugin(dir))return;throw Error(`Refusing to modify ${dir}: it does not hold a cc-safety-net managed OpenClaw plugin. Move or remove it, then run the command again.`)}function openClawArtifactCandidates(){let moduleDir=dirname6(fileURLToPath2(import.meta.url));return[join15(moduleDir,"..",OPENCLAW_ARTIFACT_RELATIVE),join15(moduleDir,"..","..","..","dist",OPENCLAW_ARTIFACT_RELATIVE)]}function findOpenClawArtifactDir(candidates=openClawArtifactCandidates()){return candidates.find((path)=>existsSync11(path)&&lstatSync5(path).isDirectory())}function resolveOpenClawArtifactDir(candidates=openClawArtifactCandidates()){let found=findOpenClawArtifactDir(candidates);if(!found)throw Error("Packaged OpenClaw plugin directory not found. Reinstall cc-safety-net and try again.");return found}function getOpenClawInstallCommands(artifactDir=resolveOpenClawArtifactDir()){return[["openclaw","plugins","install",artifactDir,"--force"],["openclaw","plugins","enable",OPENCLAW_PLUGIN_ID]]}function readOpenClawPluginStatus(inspectOutput){let report=(()=>{try{return JSON.parse(inspectOutput)}catch{return}})(),status=readRecord(readRecord(report,"plugin"),"status");return typeof status==="string"?status:void 0}function verifyOpenClawPluginRuntime(){let status=readOpenClawPluginStatus(runNativeCommand(["openclaw","plugins","inspect",OPENCLAW_PLUGIN_ID,"--runtime","--json"],{stdoutOnly:!0}));if(status==="loaded")return;throw Error(`${status===void 0?`The ${OPENCLAW_PLUGIN_ID} plugin's load state could not be verified: OpenClaw's runtime inspect report was unreadable.`:`OpenClaw reports the ${OPENCLAW_PLUGIN_ID} plugin with status "${status}".`} Run \`openclaw plugins inspect ${OPENCLAW_PLUGIN_ID} --runtime\` for details.`)}var PLATFORM2="openclaw",ENABLE_HINT=`run \`openclaw plugins enable ${OPENCLAW_PLUGIN_ID}\``;function readPluginFile(dir,name){let path=join16(dir,name),info=lstatOrUndefined2(path);if(!info)return{error:`${name} is missing from ${path}; run install --openclaw`};if(info.isSymbolicLink()||!info.isFile())return{error:`${path} is a symlink or not a regular file; move or remove it`};try{return{content:readFileSync14(path,"utf-8")}}catch(e){return{error:`Failed to read ${path}: ${e instanceof Error?e.message:String(e)}`}}}function parseJson(content){try{return JSON.parse(stripJsonComments(content))}catch{return}}function manifestError(dir){let file=readPluginFile(dir,OPENCLAW_PLUGIN_MANIFEST_FILE);if("error"in file)return file.error;if(readRecord(parseJson(file.content),"id")===OPENCLAW_PLUGIN_ID)return;return`${join16(dir,OPENCLAW_PLUGIN_MANIFEST_FILE)} is not a valid ${OPENCLAW_PLUGIN_ID} manifest; run install --openclaw`}function packageError(dir){let file=readPluginFile(dir,OPENCLAW_PLUGIN_PACKAGE_FILE);if("error"in file)return file.error;let extensions=readRecord(readRecord(parseJson(file.content),"openclaw"),"extensions");if(Array.isArray(extensions)&&extensions.includes(`./${OPENCLAW_PLUGIN_ENTRY_FILE}`))return;return`${join16(dir,OPENCLAW_PLUGIN_PACKAGE_FILE)} does not point OpenClaw at ${OPENCLAW_PLUGIN_ENTRY_FILE}; run install --openclaw`}function stringList(value){return Array.isArray(value)?value.filter((item)=>typeof item==="string"):[]}function enablementError(homeDir){let configPath=getOpenClawConfigPath(homeDir);if(!lstatOrUndefined2(configPath))return`${OPENCLAW_PLUGIN_ID} is not enabled; ${ENABLE_HINT}`;let config=(()=>{try{return JSON.parse(stripJsonComments(readFileSync14(configPath,"utf-8")))}catch{return}})();if(config===void 0)return`Failed to read ${configPath}; fix it, then ${ENABLE_HINT}`;let plugins=readRecord(config,"plugins");if(readRecord(plugins,"enabled")===!1)return`plugins.enabled is false in ${configPath}; no OpenClaw plugin loads`;let entryEnabled=readRecord(readRecord(readRecord(plugins,"entries"),OPENCLAW_PLUGIN_ID),"enabled");if(stringList(readRecord(plugins,"deny")).includes(OPENCLAW_PLUGIN_ID)||entryEnabled===!1)return`${OPENCLAW_PLUGIN_ID} is disabled in ${configPath}; ${ENABLE_HINT}`;let allow=stringList(readRecord(plugins,"allow"));if(allow.length>0&&!allow.includes(OPENCLAW_PLUGIN_ID))return`plugins.allow in ${configPath} does not list ${OPENCLAW_PLUGIN_ID}; add it, then ${ENABLE_HINT}`;if(allow.includes(OPENCLAW_PLUGIN_ID)||entryEnabled===!0)return;return`${OPENCLAW_PLUGIN_ID} is not enabled; ${ENABLE_HINT}`}function artifactVersion2(content){return/^\/\/ version:\s*(.+)$/m.exec(content)?.[1]?.trim()}function modifiedFileErrors(dir,installedVersion,packagedDir){if(packagedDir===void 0)return[];let packagedEntry=readPluginFile(packagedDir,OPENCLAW_PLUGIN_ENTRY_FILE);if("error"in packagedEntry||artifactVersion2(packagedEntry.content)!==installedVersion)return[];return[OPENCLAW_PLUGIN_ENTRY_FILE,OPENCLAW_PLUGIN_MANIFEST_FILE,OPENCLAW_PLUGIN_PACKAGE_FILE].flatMap((name)=>{let installed=readPluginFile(dir,name),packaged=readPluginFile(packagedDir,name);if("error"in installed||"error"in packaged||installed.content===packaged.content)return[];return[`Modified ${name} occupies ${join16(dir,name)}; run install --openclaw to restore it`]})}function detect10(context){let configPath=getOpenClawPluginDir(context.homeDir),unusable=inspectManagedPluginDir(PLATFORM2,configPath);if(unusable)return unusable;let entry=readPluginFile(configPath,OPENCLAW_PLUGIN_ENTRY_FILE),shapeErrors=["error"in entry?entry.error:entry.content.startsWith(OPENCLAW_MANAGED_HEADER)?void 0:`Unmanaged ${OPENCLAW_PLUGIN_ENTRY_FILE} occupies ${join16(configPath,OPENCLAW_PLUGIN_ENTRY_FILE)}; move or remove it`,manifestError(configPath),packageError(configPath)].filter((error)=>error!==void 0),installed="content"in entry?artifactVersion2(entry.content):void 0,errors=shapeErrors.length>0?shapeErrors:modifiedFileErrors(configPath,installed,findOpenClawArtifactDir());if(errors.length>0)return{platform:PLATFORM2,status:"n/a",configPath,errors};let outdatedError=installed===getPackageVersion()?[]:["Installed OpenClaw plugin is outdated; run install --openclaw to update"],disabled=enablementError(context.homeDir);if(disabled)return{platform:PLATFORM2,status:"disabled",method:"plugin directory",configPath,errors:[disabled,...outdatedError]};return{platform:PLATFORM2,status:"configured",method:"plugin directory",configPath,errors:outdatedError.length>0?outdatedError:void 0}}import{existsSync as existsSync12,readFileSync as readFileSync15}from"node:fs";import{join as join17}from"node:path";function detect11(context){let errors=[],configDir=join17(context.homeDir,".config","opencode"),candidates=["opencode.json","opencode.jsonc"];for(let filename of candidates){let configPath=join17(configDir,filename);if(existsSync12(configPath))try{let content=readFileSync15(configPath,"utf-8"),json=stripJsonComments(content);if((JSON.parse(json).plugin??[]).some((p)=>p.includes("cc-safety-net")))return{platform:"opencode",status:"configured",method:"plugin array",configPath,errors:errors.length>0?errors:void 0}}catch(e){errors.push(`Failed to parse ${filename}: ${e instanceof Error?e.message:String(e)}`)}}return{platform:"opencode",status:"n/a",errors:errors.length>0?errors:void 0}}import{join as join18}from"node:path";function getPiSettingsPath(homeDir){return join18(homeDir,".pi","agent","settings.json")}function isPiSafetyNetPackageSource(source){if(typeof source!=="string")return!1;return source==="npm:cc-safety-net"||source.startsWith("npm:cc-safety-net@")}function detect12(context){let settingsPath=getPiSettingsPath(context.homeDir),settings=readStateFile(settingsPath);if(settings.kind==="unreadable")return{platform:"pi",status:"not-inspected"};if(settings.kind==="missing")return{platform:"pi",status:"n/a"};let packages=readRecord(settings.value,"packages");if(!Array.isArray(packages))return{platform:"pi",status:"n/a"};let entry=packages.find((candidate)=>isPiSafetyNetPackageSource(typeof candidate==="string"?candidate:readRecord(candidate,"source")));if(entry===void 0)return{platform:"pi",status:"n/a"};let extensions=readRecord(entry,"extensions");if(Array.isArray(extensions)&&extensions.some((resource)=>typeof resource==="string"&&resource.startsWith("-")))return{platform:"pi",status:"disabled",method:"package config",configPath:settingsPath,errors:["npm:cc-safety-net is installed but its extension is disabled in Pi settings"]};return{platform:"pi",status:"configured",method:"package config",configPath:settingsPath}}var detectors={amp:detect,"antigravity-cli":detect2,"claude-code":detect3,codex:detect4,"copilot-cli":detect5,cursor:detect6,"gemini-cli":detect7,"hermes-agent":detect8,"kimi-code":detect9,openclaw:detect10,opencode:detect11,pi:detect12};function detectAllHooks(cwd,options){let context={...options,cwd,homeDir:options?.homeDir??homedir2()};return doctorIntegrationOrder.map((platform)=>toHookStatus(detectors[platform](context)))}function toHookStatus(detection){if(detection.status==="not-inspected")return{platform:detection.platform,detected:!1,configured:!1,inspectionStatus:"not-inspected"};return{platform:detection.platform,detected:detection.status!=="n/a",configured:detection.status==="configured",inspectionStatus:detection.status!=="n/a"?"verified":detection.errors&&detection.errors.length>0?"failed":"not-applicable",method:detection.method,configPath:detection.configPath,configPaths:detection.configPaths,errors:detection.errors}}import{tmpdir}from"node:os";import{join as join19}from"node:path";var CASES=Object.freeze([{command:"git reset --hard",description:"git reset --hard",expectBlocked:!0},{command:"rm -rf /",description:"rm -rf /",expectBlocked:!0},{command:"rm -rf ./node_modules",description:"rm in cwd (safe)",expectBlocked:!1}]),SNAPSHOT=Object.freeze({state:"ready",diagnostics:Object.freeze([]),ruleMetadata:Object.freeze({}),policy:Object.freeze({rules:Object.freeze([]),transparentWrappers:Object.freeze([]),safety:Object.freeze({}),worktreeMode:!1,destructiveCommandProtectionEnabled:!0,destructiveCommandRuleOverrides:Object.freeze({}),destructiveCommandAllowPaths:Object.freeze([]),secretProtection:Object.freeze({enabled:!0,disabledRules:Object.freeze([]),denyPaths:Object.freeze([])})})}),STANDARD_MODES={strict:!1,paranoidRm:!1,paranoidInterpreters:!1,worktreeMode:!1,effectiveLevel:"standard",capabilities:{fail_closed:{enabled:!1,source:"preset",sources:[]},paranoid_rm:{enabled:!1,source:"preset",sources:[]},paranoid_interpreters:{enabled:!1,source:"preset",sources:[]}},sources:{failClosed:[],paranoidRm:[],paranoidInterpreters:[],worktreeMode:[]}};function runIntegrationSelfTest(){let cwd=join19(tmpdir(),"cc-safety-net-self-test"),results=CASES.map((testCase)=>{let evaluation=evaluateRuntimeGuard(createToolInvocation("self-test",{command:testCase.command},{kind:"command",shell:"auto"},{configCwd:cwd,executionCwd:cwd},testCase.command),{guard:{dependencies:{loadPolicySnapshot:()=>SNAPSHOT,getModes:()=>STANDARD_MODES,findPolicyMutation:()=>null}},audit:{agent:"self-test",getSessionId:()=>{return}}}),expected=testCase.expectBlocked?"blocked":"allowed",actual=evaluation.decision.kind==="deny"?"blocked":"allowed";return{command:testCase.command,description:testCase.description,expected,actual,passed:expected===actual,reason:evaluation.decision.kind==="deny"?evaluation.decision.reason:void 0,ruleId:evaluation.decision.kind==="deny"?evaluation.decision.ruleId:void 0}});return{passed:results.filter((result)=>result.passed).length,failed:results.filter((result)=>!result.passed).length,total:results.length,results}}function parseDoctorFlags(args){let parsed=parseCommandArgs({label:"doctor",booleans:{json:["--json"],skipUpdateCheck:["--skip-update-check"]}},args);if(reportCommandArgErrors(parsed.errors))return null;return{json:parsed.flags.json,skipUpdateCheck:parsed.flags.skipUpdateCheck}}async function runDoctor(options={}){let report=await resolveAfterOptionalBanner(!options.json,()=>{let reportPromise=collectDoctorReport(options);return{ready:reportPromise,finish:()=>reportPromise}},()=>printInstallBanner(),{loadingMessage:"Checking system status…"});if(options.json)console.log(JSON.stringify(report,null,2));else printReport(report);return doctorHasFailure(report.hooks,report.engineSelfTest,{userConfig:report.userConfig,projectConfig:report.projectConfig})?1:0}async function collectDoctorReport(options){let cwd=options.cwd??process.cwd(),system=await getSystemInfo(),hooks=detectAllHooks(cwd,{codexPluginListOutput:system.codexPluginListOutput,copilotCliVersion:system.copilotCliVersion}),configInfo=getConfigInfo(cwd),environment=getEnvironmentInfo(),snapshot=loadPolicySnapshot({cwd}),policy=snapshot.policy,modes=getCCSafetyNetEnvModes(policy),ruleStates=resolveEffectiveDestructiveCommandRules(policy,modes.capabilities),activity=getActivitySummary(7),update=options.skipUpdateCheck?{currentVersion:getPackageVersion(),latestVersion:null,updateAvailable:!1}:await checkForUpdates(),report={hooks,engineSelfTest:runIntegrationSelfTest(),userConfig:configInfo.userConfig,projectConfig:configInfo.projectConfig,configState:describeConfigState(snapshot),effectiveRules:configInfo.effectiveRules,shadowedRules:configInfo.shadowedRules,environment,effectiveSafety:{selectedPreset:policy.safety.level??"standard",level:modes.effectiveLevel,capabilities:modes.capabilities,ruleOverrides:policy.destructiveCommandRuleOverrides,weakenedRuleOverrides:Object.entries(ruleStates).filter(([,state])=>state.source==="rule_override"&&state.override==="off"&&state.inheritedEnabled&&state.changesInherited).map(([id])=>id),ruleCounts:{stored:Object.keys(policy.destructiveCommandRuleOverrides).length,effective:Object.values(ruleStates).filter((state)=>state.changesInherited).length}},posture:getDoctorPosture(configInfo.userConfig.path),activity,update,system};return{...report,findings:deriveDoctorFindings(report)}}function doctorHasFailure(hooks,engineSelfTest,configInfo){return hooks.length>0&&hooks.every((hook)=>!hook.configured)||hooks.some((hook)=>hook.inspectionStatus==="failed")||engineSelfTest.failed>0||configInfo.userConfig.exists&&!configInfo.userConfig.valid||configInfo.projectConfig.exists&&!configInfo.projectConfig.valid}function printReport(report){console.log(),console.log(formatHooksSection(report.hooks)),console.log(),console.log(formatEngineSelfTestSection(report.engineSelfTest)),console.log(),console.log(formatConfigSection(report)),console.log(),console.log(formatEnvironmentSection(report.environment)),console.log(),console.log(formatEffectiveSafetySection(report)),console.log(),console.log(formatFindingsSection(report.findings)),console.log(),console.log(formatActivitySection(report.activity)),console.log(),console.log(formatSystemInfoSection(report.system)),console.log(),console.log(formatUpdateSection(report.update)),console.log(formatSummary(report))}import{existsSync as existsSync13}from"node:fs";var SHELL_SAFE_ARGUMENT=/^[A-Za-z0-9_@%+=:,./-]+$/,USAGE="Usage: cc-safety-net explain [--json] [--cwd <path>] <command>";function parseExplainFlags(args){let parsed=parseCommandArgs({label:"explain",booleans:{json:["--json"]},values:{cwd:["--cwd"]},positionals:"tail"},args);if(reportCommandArgErrors(parsed.errors))return console.error(USAGE),console.error("Pass -- before a command that starts with dashes."),null;if(parsed.values.cwd!==void 0&&!existsSync13(parsed.values.cwd))return console.error(`Error: --cwd path does not exist: ${parsed.values.cwd}`),null;let command=parsed.positionals.length===1?parsed.positionals[0]:parsed.positionals.map((argument)=>SHELL_SAFE_ARGUMENT.test(argument)?argument:`'${argument.replaceAll("'","'\\''")}'`).join(" ");if(!command)return console.error("Error: No command provided"),console.error(USAGE),null;return{json:parsed.flags.json,cwd:parsed.values.cwd,command}}function getBoxChars(asciiOnly){if(asciiOnly)return{dh:"=",dv:"|",dtl:"+",dtr:"+",dbl:"+",dbr:"+",h:"-",v:"|",tl:"+",tr:"+",bl:"+",br:"+",sh:"="};return{dh:"═",dv:"║",dtl:"╔",dtr:"╗",dbl:"╚",dbr:"╝",h:"─",v:"│",tl:"┌",tr:"┐",bl:"└",br:"┘",sh:"━"}}function formatHeader(box,width){let padding=width-18;return[`${box.dtl}${box.dh.repeat(width)}${box.dtr}`,`${box.dv}  Command Analysis${" ".repeat(padding)}${box.dv}`,`${box.dbl}${box.dh.repeat(width)}${box.dbr}`]}function formatTokenArray(tokens){return JSON.stringify(tokens)}function formatColoredTokenArray(tokens,seed=0){return`[${tokens.map((token,index)=>colorizeToken(token,index,seed)).join(",")}]`}function wrapReason(reason,indent,maxWidth=70){let words=reason.split(" "),lines=[],current="";for(let word of words)if(current&&current.length+word.length+1>maxWidth)lines.push(current),current=word;else current=current?`${current} ${word}`:word;if(current)lines.push(current);return lines.map((line,i)=>i===0?line:`${indent}${line}`)}function formatStepStyleD(step,stepNum,box){let lines=[];switch(step.type){case"parse":return null;case"env-strip":{lines.push(""),lines.push(`STEP ${stepNum} ${box.h} Strip environment variables`);let envKeys=Object.keys(step.envVars);return lines.push(`  Removed: ${envKeys.map((k)=>`${k}=<redacted>`).join(", ")}`),lines.push(`  Tokens:  ${formatTokenArray(step.output)}`),{lines,incrementStep:!0}}case"leading-tokens-stripped":return lines.push(""),lines.push(`STEP ${stepNum} ${box.h} Strip wrappers`),lines.push(`  Removed: ${step.removed.join(", ")}`),lines.push(`  Tokens:  ${formatTokenArray(step.output)}`),{lines,incrementStep:!0};case"shell-wrapper":return lines.push(""),lines.push(`STEP ${stepNum} ${box.h} Detect shell wrapper`),lines.push(`  Wrapper: ${step.wrapper} -c`),lines.push(`  Inner:   ${step.innerCommand}`),{lines,incrementStep:!0};case"interpreter":{if(lines.push(""),lines.push(`STEP ${stepNum} ${box.h} Detect interpreter`),lines.push(`  Interpreter: ${step.interpreter}`),lines.push(`  Code:        ${step.codeArg}`),step.paranoidBlocked)lines.push("  Result:      ✗ BLOCKED (paranoid mode)");return{lines,incrementStep:!0}}case"busybox":return lines.push(""),lines.push(`STEP ${stepNum} ${box.h} Busybox wrapper`),lines.push(`  Subcommand: ${step.subcommand}`),{lines,incrementStep:!0};case"transparent-wrapper":return lines.push(""),lines.push(`STEP ${stepNum} ${box.h} Transparent wrapper`),lines.push(`  Wrapper: ${step.wrapper}`),lines.push(`  Tokens:  ${formatTokenArray(step.output)}`),{lines,incrementStep:!0};case"recurse":return{lines:[],incrementStep:!1};case"rule-check":{lines.push(""),lines.push(`STEP ${stepNum} ${box.h} Match rules`);let ruleRef=`${step.ruleModule}:${step.ruleFunction}()`;if(lines.push(`  Rule:   ${ruleRef}`),step.matched)lines.push("  Result: MATCHED");else lines.push("  Result: No match");return{lines,incrementStep:!0}}case"worktree-relaxation":return lines.push(""),lines.push(`STEP ${stepNum} ${box.h} Worktree relaxation`),lines.push(`  Mode:   ${ENV_FLAGS.worktree.name}`),lines.push(`  Git cwd: ${step.gitCwd}`),lines.push("  Result: Allowed local discard in linked worktree"),{lines,incrementStep:!0};case"tmpdir-check":return null;case"fallback-scan":{if(step.embeddedCommandFound)return lines.push(""),lines.push(`STEP ${stepNum} ${box.h} Fallback scan`),lines.push(`  Found: ${step.embeddedCommandFound}`),{lines,incrementStep:!0};return null}case"custom-rules-check":{if(step.rulesChecked){if(lines.push(""),lines.push(`STEP ${stepNum} ${box.h} Custom rules`),step.matched)lines.push("  Result: MATCHED");else lines.push("  Result: No match");return{lines,incrementStep:!0}}return null}case"cwd-change":return null;case"dangerous-text":{if(step.matched)return lines.push(""),lines.push(`STEP ${stepNum} ${box.h} Dangerous text check`),lines.push(`  Token:  ${step.token}`),lines.push("  Result: MATCHED"),{lines,incrementStep:!0};return null}case"strict-unparseable":return lines.push(""),lines.push(`STEP ${stepNum} ${box.h} Strict mode check`),lines.push(`  Command: ${step.rawCommand}`),lines.push("  Result:  ✗ UNPARSEABLE"),{lines,incrementStep:!0};case"segment-skipped":return null;case"error":return lines.push(""),lines.push(`ERROR: ${step.message}`),{lines,incrementStep:!1};default:return null}}function formatTraceHuman(result,options){let box=getBoxChars(options?.asciiOnly??!1),width=58,lines=[],stepNum=1;lines.push(...formatHeader(box,58)),lines.push("");let errorStep=result.trace.steps.find((s)=>s.type==="error");if(errorStep&&errorStep.type==="error"){lines.push("ERROR"),lines.push(`  ${errorStep.message}`),lines.push(""),lines.push("RESULT"),lines.push(`  Status: ${result.result==="blocked"?colors.red("BLOCKED"):colors.green("ALLOWED")}`),lines.push(""),lines.push("CONFIG");let configPath2=result.configSource??"none";return lines.push(`  Path: ${configPath2}`),lines.join(`
...
event = "PreToolUse"
command = "${KIMI_HOOK_COMMAND}"`,KIMI_INLINE_HOOK=`{ event = "PreToolUse", command = "${KIMI_HOOK_COMMAND}" }`;function getKimiConfigPath(homeDir){return join21(process.env.KIMI_CODE_HOME??join21(homeDir,".kimi-code"),"config.toml")}function removeTopLevelEmptyHooksArray(content){return content.split(`
...
if ($dialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { [Console]::Out.Write($dialog.SelectedPath) }`,LINUX_DIALOGS=[{binary:"zenity",args:["--file-selection","--directory",`--title=${PROMPT}`]},{binary:"kdialog",args:["--getexistingdirectory",".","--title",PROMPT]}],onPath=(binary,env)=>(env.PATH??"").split(delimiter2).some((dir)=>dir.length>0&&existsSync20(join29(dir,binary)));function isDirectoryPickerAvailable(platform,env){if(platform==="darwin"||platform==="win32")return!0;if(platform!=="linux")return!1;if(!env.DISPLAY&&!env.WAYLAND_DISPLAY)return!1;return LINUX_DIALOGS.some((dialog)=>onPath(dialog.binary,env))}function getDialogCommand(platform,env){if(platform==="darwin")return{cmd:"osascript",args:["-e",MACOS_SCRIPT]};if(platform==="win32")return{cmd:"powershell.exe",args:["-NoProfile","-STA","-Command",WINDOWS_SCRIPT]};let dialog=LINUX_DIALOGS.find((candidate)=>onPath(candidate.binary,env));return dialog?{cmd:dialog.binary,args:dialog.args}:null}function chooseDirectory(platform=process.platform,env=process.env){let command=getDialogCommand(platform,env);if(!command)return Promise.resolve({error:"No folder dialog is available on this system"});return new Promise((resolve7)=>{let child=spawn3(command.cmd,command.args,{env,stdio:["ignore","pipe","pipe"]}),stdout="",settled=!1,finish=(result)=>{if(settled)return;settled=!0,clearTimeout(timeout),resolve7(result)},timeout=setTimeout(()=>{child.kill(),finish({error:"The folder dialog timed out"})},DIALOG_TIMEOUT_MS);child.stdout.on("data",(chunk)=>{stdout+=chunk.toString()}),child.on("error",()=>finish({error:`Could not open the folder dialog (${command.cmd})`})),child.on("close",()=>{let picked=stdout.trim().replace(/\/+$/,"");if(!picked)return finish({cancelled:!0});if(!existsSync20(picked)||!statSync2(picked).isDirectory())return finish({error:"That selection is not a folder on disk"});finish({path:picked})})})}var guiDocument=`<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
