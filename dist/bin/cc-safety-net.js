...
`),`Enabled ${t2} plugin in ${t}`}function Ni(n){let t=Wr(n);if(!Le(t))return;let r=Gl(t);if(!Array.isArray(r.packages))return;let e=r.packages.find((a)=>!!a&&typeof a==="object"&&!Array.isArray(a)&&Rr(a.source)&&("extensions"in a));if(!e)return;return delete e.extensions,B(t,`${JSON.stringify(r,null,2)}
`),`Enabled npm:cc-safety-net extensions in ${t}`}function Al(n,t){let r=F({label:t,booleans:Object.fromEntries(C.map((l)=>[l.target,[l.flag]]))},n),e=r.errors[0];if(e)throw Error(e);let a=C.filter((l)=>r.flags[l.target]).map((l)=>l.target);if(a.length!==1)throw Error(`Choose exactly one ${t} target: ${[...bl.keys()].join(", ")}`);return a[0]}async function Cl(n=ft(),t=k2){let[r,e,a]=await Promise.all([t(["amp","plugins","list"],30000),t(["codex","plugin","list"],30000),t(["copilot","--binary-version"])]);return{codexPluginListOutput:e,hooks:tn(process.cwd(),{homeDir:n,ampPluginListOutput:r,codexPluginListOutput:e,copilotCliVersion:a})}}async function Ii(n,t=k2){let r=await Cl(ft(),t);return r.hooks.filter((e)=>n==="install"?e.configured:e.detected||e.inspectionStatus==="not-inspected").filter((e)=>e.platform!=="codex"||!ce(r.codexPluginListOutput)||Ml(r.codexPluginListOutput)).map((e)=>e.platform)}function n5(n,t,r){if(t.length>0)return{finish:async()=>[Al(t,n)]};if(!r.selectTargets&&!Or(r.input,r.output))return{finish:async()=>[Al(t,n)]};let e=r.detectConfiguredTargets??(()=>Ii(n,r.fetchVersion)),a=Promise.all([hl(r.probeTargets),e()]);return{ready:a,finish:async()=>{let[l,d]=await a,s=wl(l,{action:n,configuredTargets:d}),L=r.selectTargets?await r.selectTargets(n,Rl(n,s)):await E0(n,Rl(n,s),{input:r.input,output:r.output});if(L==="update")return L;if(!L||L.length===0)return null;return Pr(L)}}}async function J2(n,t,r=!1){let e=Vl[n];e.beforeInstall?.(t);let a=typeof e.installCommands==="function"?await e.installCommands(t):{commands:e.installCommands};return await Yr(a.commands),await v0(a.cleanupCommands??[]),[`${a.update||r?"Updated":"Installed"} ${b2(n)} integration`,e.postInstallMessage].filter(Boolean).join(`
