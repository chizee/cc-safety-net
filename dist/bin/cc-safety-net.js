...
`;return atomicWriteFile(configPath,updated),{path:configPath,alreadyInstalled:!0}}import{spawnSync as spawnSync2}from"node:child_process";function formatNativeCommand(command){return command.join(" ")}function formatCommandFailure(command,status,output){return[`Failed to run ${formatNativeCommand(command)}${status===null?"":` (exit ${status})`}.`,output.trim()].filter(Boolean).join(`
`)}function runNativeCommand(command){let result=spawnSync2(command[0],command.slice(1),{encoding:"utf-8",stdio:"pipe"}),output=[result.stdout,result.stderr].filter(Boolean).join(`
