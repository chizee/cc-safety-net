...
event = "PreToolUse"
command = "${KIMI_HOOK_COMMAND}"`,KIMI_INLINE_HOOK=`{ event = "PreToolUse", command = "${KIMI_HOOK_COMMAND}" }`;function getKimiConfigPath(homeDir){return join10(process.env.KIMI_CODE_HOME??join10(homeDir,".kimi-code"),"config.toml")}function removeTopLevelEmptyHooksArray(content){return content.split(`
...
${output}`.trim()));if(result.status!==0)throw Error(formatCommandFailure(command,result.status,output));return output}function runNativeCommands(commands2){commands2.forEach((command)=>{runNativeCommand(command)})}import{existsSync as existsSync7,readFileSync as readFileSync8,rmSync as rmSync2}from"node:fs";import{join as join11}from"node:path";var OPENCODE_PACKAGE="cc-safety-net",OPENCODE_CACHE_PACKAGE=`${OPENCODE_PACKAGE}@latest`,OPENCODE_CONFIG_FILES=["opencode.json","opencode.jsonc"];function getDefaultOpenCodeConfigPath(homeDir){return join11(homeDir,".config","opencode",OPENCODE_CONFIG_FILES[0])}function getOpenCodeConfigPaths(homeDir){return OPENCODE_CONFIG_FILES.map((filename)=>join11(homeDir,".config","opencode",filename))}function getOpenCodeCachePath(homeDir){return join11(homeDir,".cache","opencode","packages",OPENCODE_CACHE_PACKAGE)}function clearOpenCodeCache(homeDir){rmSync2(getOpenCodeCachePath(homeDir),{recursive:!0,force:!0})}function skipJsonComment(content,index){if(content[index]==="/"&&content[index+1]==="/"){let newlineIndex=content.indexOf(`
