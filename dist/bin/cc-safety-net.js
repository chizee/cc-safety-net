...
`)}function formatHooksTable(hooks){let headers=["Platform","Discovery","Configuration","Inspection"],rowData=hooks.map((h)=>{let platformName=getIntegrationDisplayName(h.platform);if(h.inspectionStatus==="not-inspected"){let notInspected=colors.dim("Not inspected");return[platformName,notInspected,notInspected,notInspected]}let discovery=h.detected?colors.green("Detected"):h.inspectionStatus==="failed"?colors.red("Unknown"):colors.dim("Not detected"),configuration=h.configured?colors.green("Configured"):h.detected?colors.yellow("Not configured"):h.inspectionStatus==="failed"?colors.red("Unknown"):colors.dim("Not applicable"),inspection=h.inspectionStatus==="verified"?colors.green("Verified"):h.inspectionStatus==="failed"?colors.red("Failed"):colors.dim("Not applicable");return[platformName,discovery,configuration,inspection]});return formatAsciiTable({headers,rows:rowData})}function formatEngineSelfTestSection(selfTest){let lines=["Guard Engine Verification",`   Synthetic self-test: ${selfTest.failed>0?colors.red(`${selfTest.passed}/${selfTest.total} FAIL`):colors.green(`${selfTest.passed}/${selfTest.total} passed`)}`],failures=selfTest.results.filter((result)=>!result.passed);if(failures.length>0){lines.push(""),lines.push(colors.red("   Failures:"));for(let failure of failures)lines.push(colors.red(`   • ${failure.description}`)),lines.push(colors.red(`     expected ${failure.expected}, got ${failure.actual}`))}return lines.join(`
...
`)}function formatConfigTable(userConfig,projectConfig){let headers=["Scope","Status"],getStatusDisplay=(config)=>{if(!config.exists)return colors.dim("N/A");if(!config.valid)return colors.red(`Invalid (${config.errors?.[0]??"unknown error"})`);return colors.green("Configured")},rows=[["User",getStatusDisplay(userConfig)],["Project",getStatusDisplay(projectConfig)]];return formatAsciiTable({headers,rows})}function formatEnvironmentSection(envVars){let lines=[];return lines.push("Environment"),lines.push(formatEnvironmentTable(envVars)),lines.join(`
...
`)}function formatEnvironmentTable(envVars){let headers=["Variable","Status","Legacy"],rows=envVars.map((v)=>{let statusIcon=v.isSet?colors.green("✓"):colors.dim("✗"),legacyStatus=v.legacyName&&v.legacyIsSet?`${v.legacyName} ${colors.green("✓")}`:v.legacyName??"";return[v.name,statusIcon,legacyStatus]});return formatAsciiTable({headers,rows})}function formatActivitySection(activity){let lines=[];if(activity.totalBlocked===0)lines.push("Recent Activity"),lines.push("   No blocked commands in the last 7 days"),lines.push("   Tip: This is normal for new installations");else lines.push(`Recent Activity · last 7 days (${activity.totalBlocked} blocked / ${activity.sessionCount} sessions)`),lines.push(formatActivityTable(activity.recentEntries));if(activity.unreadable>0)lines.push(`   Warning: ${activity.unreadable} audit log ${activity.unreadable===1?"source":"sources"} could not be read; this summary is incomplete`);return lines.join(`
`)}function formatActivityTable(entries){let headers=["Time","Command"],rows=entries.map((e)=>{let command=renderTerminalText(e.command.replace(/\r\n|\r|\n/g," ↵ ").replace(/\t/g," ")),cmd=command.length>40?`${command.slice(0,37)}...`:command;return[e.relativeTime,cmd]});return formatAsciiTable({headers,rows})}function formatUpdateSection(update){let lines=[];if(lines.push("Update Check"),update.latestVersion===null&&!update.error)return lines.push(formatUpdateTable([["Status",colors.dim("Skipped")],["Installed",update.currentVersion]])),lines.join(`
`);if(update.error)return lines.push(formatUpdateTable([["Status",`${colors.yellow("⚠")} Error`],["Installed",update.currentVersion],["Error",colors.dim(update.error)]])),lines.join(`
`);if(update.updateAvailable)return lines.push(formatUpdateTable([["Status",`${colors.yellow("⚠")} Update Available`],["Current",update.currentVersion],["Latest",colors.green(update.latestVersion??"")]])),lines.push(""),lines.push("   Run: bunx cc-safety-net@latest doctor"),lines.push("   Or:  npx cc-safety-net@latest doctor"),lines.join(`
`);return lines.push(formatUpdateTable([["Status",`${colors.green("✓")} Up to date`],["Version",update.currentVersion]])),lines.join(`
`)}function formatUpdateTable(rows){return formatAsciiTable({rows})}function formatSystemInfoSection(system){let lines=[];return lines.push("System Info"),lines.push(formatSystemInfoTable(system)),lines.join(`
`)}function formatSystemInfoTable(system){let headers=["Component","Version"],formatValue=(value)=>{if(value===null)return colors.dim("not found");return value},rows=[{label:"cc-safety-net",value:system.version},{label:"Claude Code",value:system.claudeCodeVersion},{label:"Amp Code",value:system.ampVersion},{label:"Antigravity CLI",value:system.antigravityCliVersion},{label:"Codex",value:system.codexCliVersion},{label:"Cursor",value:system.cursorVersion},{label:"Gemini CLI",value:system.geminiCliVersion},{label:"GitHub Copilot CLI",value:system.copilotCliVersion},{label:"Kimi Code",value:system.kimiCodeVersion},{label:"OpenCode",value:system.openCodeVersion},{label:"Pi",value:system.piCliVersion},{label:"Node.js",value:system.nodeVersion},{label:"npm",value:system.npmVersion},{label:"Bun",value:system.bunVersion},{label:"Platform",value:system.platform}].map((r)=>[r.label,formatValue(r.value)]);return formatAsciiTable({headers,rows})}function formatSummary(report){if(report.findings.length===0)return colors.green(`
...
${report.findings.length} ${label}: ${parts.join(", ")}.`;if(counts.error>0)return colors.red(message);if(counts.warning>0)return colors.yellow(message);return colors.blue(message)}import{existsSync as existsSync5,lstatSync as lstatSync2,readdirSync as readdirSync3,readFileSync as readFileSync5}from"node:fs";import{homedir as homedir2}from"node:os";import{join as join9}from"node:path";var handleAmpToolCall=createAmpToolCallHandler();function createAmpToolCallHandler(options={}){return(event,amp)=>handleAmpToolCallWithDependencies(event,amp,options)}function handleAmpToolCallWithDependencies(event,amp,options){let toolCall=getAmpToolInvocation(event,amp),getSessionId=()=>ampThreadId(event);if("malformed"in toolCall)return writeIntegrationDenialAudit(toolCall.denial,getSessionId,{agent:"amp",toolName:toolCall.denial.toolName,cwd:toolCall.cwd}),rejectAmpToolCall(toolCall.denial);try{let evaluation=evaluateRuntimeGuard(toolCall,{guard:{auditAllowed:shouldRecordAllowedCommands(),policyOptions:options.policyOptions,dependencies:options.guardDependencies},audit:{agent:"amp",getSessionId}});return projectAmpEvaluation(evaluation,evaluation.stage!=="config-state")}catch(error){if(!(error instanceof GuardEvaluationError))throw error;if(envTruthy(ENV_FLAGS.debug))console.error(`CC Safety Net debug: amp tool.call analysis failed: ${formatIntegrationError(error.cause)}`);return projectAmpEvaluation(error.evaluation,toolCall.route.kind==="command")}}function getAmpToolInvocation(event,amp){if(!event||typeof event!=="object")return malformedAmpToolCall(null);let toolCall=event;if(typeof toolCall.tool!=="string"||toolCall.tool.trim()==="")return malformedAmpToolCall(null);if(!toolCall.input||typeof toolCall.input!=="object")return malformedAmpToolCall(null,toolCall.tool);let workspaceRoot=resolveAmpWorkspaceRoot(amp);if(!workspaceRoot)return malformedAmpToolCall(null,toolCall.tool);let shell=extractAmpShellCommand(amp,event);if(!shell.ok)return malformedAmpToolCall(workspaceRoot,toolCall.tool);if(!shell.command)return createToolInvocation(toolCall.tool,toolCall.input,{kind:getNonCommandToolInputKind(toolCall.tool)},{configCwd:workspaceRoot,executionCwd:workspaceRoot},null);if(typeof shell.command.command!=="string"||shell.command.command.trim()==="")return malformedAmpToolCall(workspaceRoot,toolCall.tool);let executionCwd=typeof shell.command.dir==="string"?resolveContainedCwd(shell.command.dir,[workspaceRoot]):workspaceRoot;if(!executionCwd)return malformedAmpToolCall(workspaceRoot,toolCall.tool,shell.command.command,shell.command.dir);return createToolInvocation(toolCall.tool,toolCall.input,{kind:"command",shell:"posix"},{configCwd:workspaceRoot,executionCwd},shell.command.command)}function resolveAmpWorkspaceRoot(amp){let workspaceRoot=amp.system.workspaceRoot;if(!workspaceRoot)return;try{let rootPath=amp.helpers.filePathFromURI(workspaceRoot);if(typeof rootPath!=="string"||rootPath.trim()==="")return;return resolveContainedCwd(".",[rootPath])}catch{return}}function extractAmpShellCommand(amp,event){try{return{ok:!0,command:amp.helpers.shellCommandFromToolCall(event)}}catch{return{ok:!1}}}function ampThreadId(event){if(!event||typeof event!=="object")return;let id=event.thread?.id;return typeof id==="string"&&id.trim()!==""?id:void 0}function malformedAmpToolCall(cwd,toolName,command,segment){return{malformed:!0,denial:createFailedClosedDenial({command,segment,toolName}),cwd}}function projectAmpEvaluation(evaluation,includeEvidence){let denial=projectGuardDenial(evaluation,{includeEvidence});return denial?rejectAmpToolCall(denial):{action:"allow"}}function rejectAmpToolCall(denial){return{action:"reject-and-continue",message:formatDenial(denial)}}var AMP_MANAGED_HEADER="// cc-safety-net managed Amp plugin. Do not edit. Reinstall with: npx -y cc-safety-net install --amp";function stripJsonComments(content){let result="",i=0,inString=!1,isEscaped=!1,lastCommaIndex=-1;while(i<content.length){let char=content[i],next=content[i+1];if(isEscaped){result+=char,isEscaped=!1,i++;continue}if(char==='"'&&!inString){inString=!0,lastCommaIndex=-1,result+=char,i++;continue}if(char==='"'&&inString){inString=!1,result+=char,i++;continue}if(char==="\\"&&inString){isEscaped=!0,result+=char,i++;continue}if(inString){result+=char,i++;continue}if(char==="/"&&next==="/"){while(i<content.length&&content[i]!==`
`)i++;continue}if(char==="/"&&next==="*"){i+=2;while(i<content.length-1){if(content[i]==="*"&&content[i+1]==="/"){i+=2;break}i++}continue}if(char===","){lastCommaIndex=result.length,result+=char,i++;continue}if(char==="}"||char==="]"){if(lastCommaIndex!==-1){let between=result.slice(lastCommaIndex+1);if(/^\s*$/.test(between))result=result.slice(0,lastCommaIndex)+between}lastCommaIndex=-1,result+=char,i++;continue}if(!/\s/.test(char))lastCommaIndex=-1;result+=char,i++}return result}import{spawn}from"node:child_process";import{existsSync as existsSync2}from"node:fs";import{delimiter,extname,join as join5}from"node:path";import{stripVTControlCharacters}from"node:util";var CURRENT_VERSION="1.0.6",VERSION_FETCH_TIMEOUT_MS=5000,TEST_SPAWN_PLATFORM_ENV="_CC_SAFETY_NET_TEST_SPAWN_PLATFORM";function getPackageVersion(){return CURRENT_VERSION}function getEnvValue(env,name){let direct=env[name];if(direct)return direct;let matchingName=Object.keys(env).find((key)=>key.toLowerCase()===name.toLowerCase()&&!!env[key]);return matchingName?env[matchingName]:direct}function getWindowsExecutableExtensions(env){return(getEnvValue(env,"PATHEXT")||".COM;.EXE;.BAT;.CMD").split(";").filter((extension)=>extension.length>0)}function resolveWindowsCommand(command,env){let candidates=extname(command)?[command]:[...getWindowsExecutableExtensions(env).map((extension)=>`${command}${extension}`),command];if(command.includes("/")||command.includes("\\"))return candidates.find((candidate)=>existsSync2(candidate))??command;return(getEnvValue(env,"PATH")??"").split(delimiter).flatMap((dir)=>candidates.map((candidate)=>join5(dir,candidate))).find((candidate)=>existsSync2(candidate))??command}function quoteWindowsCommandArg(value){if(!/[\s"&|<>^]/.test(value))return value;return`"${value.replace(/"/g,'""')}"`}function getSpawnCommand(args,env){let[command,...rest]=args,platform=env[TEST_SPAWN_PLATFORM_ENV]==="win32"?"win32":process.platform;if(!command||platform!=="win32")return{cmd:command??"",args:rest};let resolved=resolveWindowsCommand(command,env);if(!/\.(?:bat|cmd)$/i.test(resolved))return{cmd:resolved,args:rest};return{cmd:getEnvValue(env,"COMSPEC")??"cmd.exe",args:["/d","/c",["call",quoteWindowsCommandArg(resolved),...rest.map(quoteWindowsCommandArg)].join(" ")]}}var defaultVersionFetcher=async(args,timeoutMs=VERSION_FETCH_TIMEOUT_MS)=>{let result=await runCommand(args,{timeoutMs});if(result.code!==0)return null;return stripVTControlCharacters(result.stdout).trim()||stripVTControlCharacters(result.stderr).trim()||null};function runCommand(args,options){let[cmd,...rest]=args;if(!cmd)return Promise.resolve({code:null,stdout:"",stderr:""});return new Promise((resolve4)=>{try{let spawnCommand=getSpawnCommand([cmd,...rest],process.env),proc=spawn(spawnCommand.cmd,spawnCommand.args,{stdio:["ignore","pipe","pipe"]}),isSettled=!1,stdout="",stderr="";proc.stdout.on("data",(data)=>{stdout+=data.toString()}),proc.stderr.on("data",(data)=>{stderr+=data.toString()});let finish=(result)=>{if(isSettled)return;isSettled=!0,clearTimeout(timeoutId),resolve4(result)},timeoutId=setTimeout(()=>{proc.kill(),finish({code:null,stdout,stderr})},options.timeoutMs);proc.on("close",(code)=>{finish({code,stdout,stderr})}),proc.on("error",()=>{finish({code:null,stdout,stderr})})}catch{resolve4({code:null,stdout:"",stderr:""})}})}function parseVersion(output){if(!output)return null;let claudeMatch=/Claude Code\s+(\d+\.\d+\.\d+)/i.exec(output);if(claudeMatch)return claudeMatch[1]??null;let versionMatch=/v?(\d+\.\d+\.\d+(?:-[a-zA-Z0-9.]+)?)/i.exec(output);if(versionMatch)return versionMatch[1]??null;return output.split(`
`)[0]?.trim()||null}async function getSystemInfo(fetcher=defaultVersionFetcher){let[claudeRaw,antigravityRaw,openCodeRaw,codexRaw,codexPluginListOutput,geminiRaw,copilotRaw,kimiRaw,piRaw,cursorRaw,ampRaw,nodeRaw,npmRaw,bunRaw]=await Promise.all([fetcher(["claude","--version"]),fetcher(["agy","--version"]),fetcher(["opencode","--version"]),fetcher(["codex","--version"]),fetcher(["codex","plugin","list"]),fetcher(["gemini","--version"]),fetcher(["copilot","--binary-version"]),fetcher(["kimi","--version"]),fetcher(["pi","--version"]),fetcher(["cursor","--version"]),fetcher(["amp","--version"]),fetcher(["node","--version"]),fetcher(["npm","--version"]),fetcher(["bun","--version"])]);return{version:CURRENT_VERSION,claudeCodeVersion:parseVersion(claudeRaw),antigravityCliVersion:parseVersion(antigravityRaw),openCodeVersion:parseVersion(openCodeRaw),codexCliVersion:parseVersion(codexRaw),codexPluginListOutput,geminiCliVersion:parseVersion(geminiRaw),copilotCliVersion:parseVersion(copilotRaw),kimiCodeVersion:parseVersion(kimiRaw),piCliVersion:parseVersion(piRaw),cursorVersion:parseVersion(cursorRaw),ampVersion:parseVersion(ampRaw),nodeVersion:parseVersion(nodeRaw),npmVersion:parseVersion(npmRaw),bunVersion:parseVersion(bunRaw),platform:`${process.platform} ${process.arch}`}}import{join as join6}from"node:path";function getAntigravityHooksPath(homeDir){return join6(homeDir,".gemini","config","hooks.json")}import{existsSync as existsSync3,lstatSync,mkdirSync,readFileSync as readFileSync3,rmSync}from"node:fs";import{dirname as dirname3,join as join7}from"node:path";import{fileURLToPath}from"node:url";import{renameSync,writeFileSync}from"node:fs";function atomicWriteFile(dest,content){let tmp=`${dest}.${process.pid}.tmp`;writeFileSync(tmp,content),renameSync(tmp,dest)}var AMP_ARTIFACT_RELATIVE=join7("amp","cc-safety-net.ts");function getAmpPluginPath(homeDir){return join7(homeDir,".config","amp","plugins","cc-safety-net.ts")}function ampArtifactCandidates(){let moduleDir=dirname3(fileURLToPath(import.meta.url));return[join7(moduleDir,"..",AMP_ARTIFACT_RELATIVE),join7(moduleDir,"..","..","..","..","dist",AMP_ARTIFACT_RELATIVE)]}function resolveAmpArtifactPath(candidates=ampArtifactCandidates()){let found=candidates.find((path)=>existsSync3(path)&&lstatSync(path).isFile());if(!found)throw Error("Packaged Amp plugin artifact not found. Reinstall cc-safety-net and try again.");return found}function lstatOrUndefined(path){try{return lstatSync(path)}catch{return}}function isManagedAmpArtifact(content){return content.subarray(0,Buffer.byteLength(AMP_MANAGED_HEADER)).toString("utf-8")===AMP_MANAGED_HEADER}function installAmp(homeDir,artifactPath=resolveAmpArtifactPath()){let artifact=readFileSync3(artifactPath),dest=getAmpPluginPath(homeDir),info=lstatOrUndefined(dest);if(!info)return mkdirSync(dirname3(dest),{recursive:!0}),atomicWriteFile(dest,artifact),{path:dest,alreadyInstalled:!1};if(info.isSymbolicLink()||!info.isFile())throw Error(`Refusing to overwrite ${dest}: not a regular file. Move or remove it and rerun install --amp.`);let current=readFileSync3(dest);if(current.equals(artifact))return{path:dest,alreadyInstalled:!0};if(isManagedAmpArtifact(current))return atomicWriteFile(dest,artifact),{path:dest,alreadyInstalled:!1};throw Error(`Refusing to overwrite unmanaged file at ${dest}. Move or remove it and rerun install --amp.`)}function uninstallAmp(homeDir){let dest=getAmpPluginPath(homeDir),info=lstatOrUndefined(dest);if(!info)return{path:dest,alreadyInstalled:!1};if(info.isSymbolicLink()||!info.isFile())throw Error(`Refusing to remove ${dest}: not a regular file. Move or remove it manually.`);if(!isManagedAmpArtifact(readFileSync3(dest)))throw Error(`Refusing to remove unmanaged file at ${dest}. Move or remove it manually.`);return rmSync(dest),{path:dest,alreadyInstalled:!0}}import{existsSync as existsSync4,mkdirSync as mkdirSync2,readFileSync as readFileSync4}from"node:fs";import{dirname as dirname4,join as join8}from"node:path";var CURSOR_HOOK_COMMAND="npx -y cc-safety-net hook --cursor",CURSOR_HOOK_TIMEOUT=30;function getCursorHooksPath(homeDir){return join8(homeDir,".cursor","hooks.json")}function isRecord(value){return typeof value==="object"&&value!==null&&!Array.isArray(value)}function canonicalCursorEntry(){return{command:CURSOR_HOOK_COMMAND,timeout:CURSOR_HOOK_TIMEOUT,failClosed:!0}}function isManagedCursorEntry(entry){return isRecord(entry)&&entry.command===CURSOR_HOOK_COMMAND}function isCanonicalCursorEntry(entry){return Object.keys(entry).length===3&&entry.command===CURSOR_HOOK_COMMAND&&entry.timeout===CURSOR_HOOK_TIMEOUT&&entry.failClosed===!0}function readCursorJson(configPath){try{return JSON.parse(readFileSync4(configPath,"utf-8"))}catch(error){if(error instanceof SyntaxError)throw Error(`Failed to parse Cursor hooks config ${configPath}: ${error.message}`);throw error}}function parseCursorHooksConfig(configPath){let parsed=readCursorJson(configPath);if(!isRecord(parsed))throw Error(`Cursor hooks config ${configPath} must be a JSON object`);if(parsed.version!==1)throw Error(`Cursor hooks config ${configPath} must set "version": 1`);if(parsed.hooks!==void 0&&!isRecord(parsed.hooks))throw Error(`Cursor hooks config ${configPath} "hooks" must be an object`);let preToolUse=isRecord(parsed.hooks)?parsed.hooks.preToolUse:void 0;if(preToolUse!==void 0&&!Array.isArray(preToolUse))throw Error(`Cursor hooks config ${configPath} "hooks.preToolUse" must be an array`);return parsed}function getCursorPreToolUse(config){let preToolUse=isRecord(config.hooks)?config.hooks.preToolUse:void 0;return Array.isArray(preToolUse)?preToolUse:[]}function canonicalizeCursorEntries(entries){if(!entries.some(isManagedCursorEntry))return[...entries,canonicalCursorEntry()];return entries.reduce((state,entry)=>{if(!isManagedCursorEntry(entry))return state.result.push(entry),state;if(!state.inserted)state.result.push(canonicalCursorEntry()),state.inserted=!0;return state},{result:[],inserted:!1}).result}function writeCursorHooksConfig(configPath,config,preToolUse){let hooks=isRecord(config.hooks)?config.hooks:{},next={...config,hooks:{...hooks,preToolUse}};atomicWriteFile(configPath,`${JSON.stringify(next,null,2)}
`)}function installCursor(homeDir){let configPath=getCursorHooksPath(homeDir);if(!existsSync4(configPath))return mkdirSync2(dirname4(configPath),{recursive:!0}),atomicWriteFile(configPath,`${JSON.stringify({version:1,hooks:{preToolUse:[canonicalCursorEntry()]}},null,2)}
`),{path:configPath,alreadyInstalled:!1};let config=parseCursorHooksConfig(configPath),existing=getCursorPreToolUse(config),managed=existing.filter(isManagedCursorEntry);if(isRecord(config.hooks)&&Array.isArray(config.hooks.preToolUse)&&managed.length===1&&managed[0]!==void 0&&isCanonicalCursorEntry(managed[0]))return{path:configPath,alreadyInstalled:!0};return writeCursorHooksConfig(configPath,config,canonicalizeCursorEntries(existing)),{path:configPath,alreadyInstalled:!1}}function uninstallCursor(homeDir){let configPath=getCursorHooksPath(homeDir);if(!existsSync4(configPath))return{path:configPath,alreadyInstalled:!1};let config=parseCursorHooksConfig(configPath),existing=getCursorPreToolUse(config),filtered=existing.filter((entry)=>!isManagedCursorEntry(entry));if(filtered.length===existing.length)return{path:configPath,alreadyInstalled:!1};return writeCursorHooksConfig(configPath,config,filtered),{path:configPath,alreadyInstalled:!0}}var COPILOT_PLUGIN_ID="cc-safety-net@cc-marketplace",COPILOT_PLUGIN_DIR=["cc-marketplace","cc-safety-net"],COPILOT_LEGACY_PLUGIN_DIR=["_direct","copilot-safety-net"];function hasIdentifier(output,identifier){let escaped=identifier.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");return new RegExp(`(^|[^a-z0-9-])${escaped}([^a-z0-9-]|$)`,"m").test(output??"")}function hasCopilotSafetyNetPlugin(output){return hasIdentifier(output,"cc-safety-net@cc-marketplace")}function hasCopilotMarketplace(output){return hasIdentifier(output,"cc-marketplace")}function hasCopilotLegacyPlugin(output){return hasIdentifier(output,"copilot-safety-net")}var CLAUDE_SAFETY_NET_PLUGIN_ID="cc-safety-net@cc-marketplace",CODEX_PLUGIN_LIST_CONFIG_PATH="codex plugin list",CODEX_SAFETY_NET_SOURCE="https://github.com/kenryu42/cc-safety-net.git",GEMINI_SAFETY_NET_EXTENSION="gemini-safety-net",ANTIGRAVITY_HOOK_COMMAND_PATTERN=/cc-safety-net\s+hook\s+(?:[^\s]+\s+)*(?:--agy-cli|-ac)(\s|["']|$)/,KIMI_HOOK_COMMAND_PATTERN=/cc-safety-net\s+hook\s+(?:[^\s]+\s+)*--kimi-code(\s|["']|$)/;function readStateFile(path,preprocess=(raw)=>raw){if(!existsSync5(path))return{kind:"missing"};try{return{kind:"ok",value:JSON.parse(preprocess(readFileSync5(path,"utf-8")))}}catch{return{kind:"unreadable"}}}function readRecord(value,key){return typeof value==="object"&&value!==null?value[key]:void 0}function getClaudeInstalledPluginsPath(homeDir){return join9(homeDir,".claude","plugins","installed_plugins.json")}function isInstalledPluginRecord(value,pluginId){let record=readRecord(readRecord(value,"plugins"),pluginId);return Array.isArray(record)&&record.length>0}function hasClaudeInstalledPlugin(homeDir,pluginId){let installed=readStateFile(getClaudeInstalledPluginsPath(homeDir));return installed.kind==="ok"&&isInstalledPluginRecord(installed.value,pluginId)}function detectClaudeCode(homeDir){let installedPath=getClaudeInstalledPluginsPath(homeDir),installed=readStateFile(installedPath);if(installed.kind==="unreadable")return{platform:"claude-code",status:"not-inspected"};if(installed.kind==="missing")return{platform:"claude-code",status:"n/a"};if(!isInstalledPluginRecord(installed.value,CLAUDE_SAFETY_NET_PLUGIN_ID))return{platform:"claude-code",status:"n/a"};let settingsPath=join9(homeDir,".claude","settings.json"),settings=readStateFile(settingsPath);if(settings.kind==="unreadable")return{platform:"claude-code",status:"not-inspected"};if(!(settings.kind==="ok"&&readRecord(readRecord(settings.value,"enabledPlugins"),CLAUDE_SAFETY_NET_PLUGIN_ID)===!0))return{platform:"claude-code",status:"disabled",method:"plugin config",configPath:settingsPath,errors:[`${CLAUDE_SAFETY_NET_PLUGIN_ID} is installed but not enabled in Claude Code`]};return{platform:"claude-code",status:"configured",method:"plugin config",configPath:installedPath}}function detectOpenCode(homeDir){let errors=[],configDir=join9(homeDir,".config","opencode"),candidates=["opencode.json","opencode.jsonc"];for(let filename of candidates){let configPath=join9(configDir,filename);if(existsSync5(configPath))try{let content=readFileSync5(configPath,"utf-8"),json=stripJsonComments(content);if((JSON.parse(json).plugin??[]).some((p)=>p.includes("cc-safety-net")))return{platform:"opencode",status:"configured",method:"plugin array",configPath,errors:errors.length>0?errors:void 0}}catch(e){errors.push(`Failed to parse ${filename}: ${e instanceof Error?e.message:String(e)}`)}}return{platform:"opencode",status:"n/a",errors:errors.length>0?errors:void 0}}function detectGeminiCLI(homeDir){let extensionsDir=join9(homeDir,".gemini","extensions"),extensionDir=join9(extensionsDir,GEMINI_SAFETY_NET_EXTENSION);if(!existsSync5(extensionDir))return{platform:"gemini-cli",status:"n/a"};let enablementPath=join9(extensionsDir,"extension-enablement.json"),enablement=readStateFile(enablementPath);if(enablement.kind==="unreadable")return{platform:"gemini-cli",status:"not-inspected"};let overrides=enablement.kind==="ok"?readRecord(readRecord(enablement.value,GEMINI_SAFETY_NET_EXTENSION),"overrides"):void 0;if(Array.isArray(overrides)&&overrides.some((entry)=>typeof entry==="string"&&entry.startsWith("!")))return{platform:"gemini-cli",status:"disabled",method:"extension config",configPath:enablementPath,errors:[`${GEMINI_SAFETY_NET_EXTENSION} is disabled in Gemini CLI`]};return{platform:"gemini-cli",status:"configured",method:"extension config",configPath:extensionDir}}function _getKimiConfigPath(homeDir){return join9(process.env.KIMI_CODE_HOME||join9(homeDir,".kimi-code"),"config.toml")}function _findAntigravitySafetyNetHooks(config){if(!config||typeof config!=="object"||Array.isArray(config))return[];return Object.values(config).flatMap((definition)=>{if(!definition||typeof definition!=="object"||Array.isArray(definition))return[];let record=definition,preToolUse=record.PreToolUse;if(!Array.isArray(preToolUse))return[];return preToolUse.flatMap((entry)=>{if(!entry||typeof entry!=="object"||Array.isArray(entry))return[];let hooks=entry.hooks;if(!Array.isArray(hooks))return[];return hooks.flatMap((hook)=>{if(!hook||typeof hook!=="object"||Array.isArray(hook))return[];let command=hook.command;if(typeof command!=="string"||!ANTIGRAVITY_HOOK_COMMAND_PATTERN.test(command))return[];return[{command,enabled:record.enabled!==!1}]})})})}function detectAntigravityCli(homeDir){let configPath=getAntigravityHooksPath(homeDir);if(!existsSync5(configPath))return{platform:"antigravity-cli",status:"n/a",configPath};let matches;try{matches=_findAntigravitySafetyNetHooks(JSON.parse(readFileSync5(configPath,"utf-8")))}catch(e){return{platform:"antigravity-cli",status:"n/a",configPath,errors:[`Failed to parse Antigravity hooks config ${configPath}: ${e instanceof Error?e.message:String(e)}`]}}if(matches.some((match)=>match.enabled))return{platform:"antigravity-cli",status:"configured",method:"hook config",configPath};if(matches.length>0)return{platform:"antigravity-cli",status:"disabled",method:"hook config",configPath};return{platform:"antigravity-cli",status:"n/a",configPath}}function detectKimiCode(homeDir){let configPath=_getKimiConfigPath(homeDir);if(!existsSync5(configPath))return{platform:"kimi-code",status:"n/a",configPath};try{if(!KIMI_HOOK_COMMAND_PATTERN.test(readFileSync5(configPath,"utf-8")))return{platform:"kimi-code",status:"n/a",configPath}}catch(e){return{platform:"kimi-code",status:"n/a",configPath,errors:[`Failed to read ${configPath}: ${e instanceof Error?e.message:String(e)}`]}}return{platform:"kimi-code",status:"configured",method:"hook config",configPath}}function getPiSettingsPath(homeDir){return join9(homeDir,".pi","agent","settings.json")}function isPiSafetyNetPackageSource(source){if(typeof source!=="string")return!1;return source==="npm:cc-safety-net"||source.startsWith("npm:cc-safety-net@")}function detectPi(homeDir){let settingsPath=getPiSettingsPath(homeDir),settings=readStateFile(settingsPath);if(settings.kind==="unreadable")return{platform:"pi",status:"not-inspected"};if(settings.kind==="missing")return{platform:"pi",status:"n/a"};let packages=readRecord(settings.value,"packages");if(!Array.isArray(packages))return{platform:"pi",status:"n/a"};let entry=packages.find((candidate)=>isPiSafetyNetPackageSource(typeof candidate==="string"?candidate:readRecord(candidate,"source")));if(entry===void 0)return{platform:"pi",status:"n/a"};let extensions=readRecord(entry,"extensions");if(Array.isArray(extensions)&&extensions.some((resource)=>typeof resource==="string"&&resource.startsWith("-")))return{platform:"pi",status:"disabled",method:"package config",configPath:settingsPath,errors:["npm:cc-safety-net is installed but its extension is disabled in Pi settings"]};return{platform:"pi",status:"configured",method:"package config",configPath:settingsPath}}function detectCodex(pluginListOutput){if(!pluginListOutput)return{platform:"codex",status:"n/a"};let pluginLine=pluginListOutput.split(`
`).find((line)=>line.includes(CODEX_SAFETY_NET_SOURCE));if(!pluginLine)return{platform:"codex",status:"n/a"};if(!pluginLine.includes("installed, enabled"))return{platform:"codex",status:"disabled",method:CODEX_PLUGIN_LIST_CONFIG_PATH,configPath:CODEX_PLUGIN_LIST_CONFIG_PATH,errors:[`Codex plugin line for ${CODEX_SAFETY_NET_SOURCE} must contain installed, enabled.`]};return{platform:"codex",status:"configured",method:CODEX_PLUGIN_LIST_CONFIG_PATH,configPath:CODEX_PLUGIN_LIST_CONFIG_PATH}}function _isSafetyNetCopilotCommand(command){if(!command?.includes("cc-safety-net"))return!1;return/(^|\s)hook\s+(?:[^\s]+\s+)*(--copilot-cli|-cp)(\s|$)/.test(command)}function _isAtLeastVersion(version,threshold){if(!version)return null;let match=version.match(/(\d+)\.(\d+)\.(\d+)/);if(!match)return null;let parsed=[Number(match[1]),Number(match[2]),Number(match[3])];for(let index=0;index<threshold.length;index++){let left=parsed[index]??0,right=threshold[index]??0;if(left!==right)return left>right}return!0}function _supportsCopilotUserHookFiles(version){return _isAtLeastVersion(version,[0,0,422])}function _supportsCopilotInlineHooks(version){return _isAtLeastVersion(version,[1,0,8])}function _getCopilotConfigHome(homeDir){return process.env.COPILOT_HOME||join9(homeDir,".copilot")}function _hasSafetyNetCopilotHook(config){return(config.hooks?.preToolUse??[]).some((hook)=>{if(hook.type!=="command")return!1;return _isSafetyNetCopilotCommand(hook.command)||_isSafetyNetCopilotCommand(hook.bash)||_isSafetyNetCopilotCommand(hook.powershell)})}function _readCopilotConfigFile(configPath,errors){try{return JSON.parse(stripJsonComments(readFileSync5(configPath,"utf-8")))}catch(e){errors?.push(`Failed to parse ${configPath}: ${e instanceof Error?e.message:String(e)}`);return}}function _listJsonFiles(dirPath,errors){try{return readdirSync3(dirPath).filter((name)=>name.endsWith(".json")).sort((a,b)=>a.localeCompare(b))}catch(e){return errors?.push(`Failed to read ${dirPath}: ${e instanceof Error?e.message:String(e)}`),[]}}function _collectSafetyNetCopilotHookFiles(dirPath,errors){if(!existsSync5(dirPath))return[];let matches=[];for(let filename of _listJsonFiles(dirPath,errors)){let configPath=join9(dirPath,filename),config=_readCopilotConfigFile(configPath,errors);if(config&&_hasSafetyNetCopilotHook(config))matches.push(configPath)}return matches}function _collectCopilotInlineConfig(configPath,errors){if(!existsSync5(configPath))return;let config=_readCopilotConfigFile(configPath,errors);if(!config)return;return{path:configPath,config}}function _warnOnUnsupportedCopilotSource(errors,version,sourceDescription,requiredVersion){if(version){errors.push(`GitHub Copilot CLI ${version} does not support ${sourceDescription}; requires ${requiredVersion}+`);return}errors.push(`GitHub Copilot CLI version unavailable; skipping ${sourceDescription} because it requires ${requiredVersion}+`)}function _resolveCopilotInlineDisableSource(inlineSources){let precedence=[inlineSources.localSettings,inlineSources.repoSettings,inlineSources.userConfig];for(let source of precedence){if(source?.config.disableAllHooks===!0)return source.path;if(source?.config.disableAllHooks===!1)return}return}function _checkCopilotEnabled(homeDir,cwd,copilotCliVersion,errors){let configHome=_getCopilotConfigHome(homeDir),repoHookDir=join9(cwd,".github","hooks"),userHookDir=join9(configHome,"hooks"),repoConfigDir=join9(cwd,".github","copilot"),inlineSupport=_supportsCopilotInlineHooks(copilotCliVersion),inlineErrors=inlineSupport===!0?errors:void 0,inlineSources={userConfig:_collectCopilotInlineConfig(join9(configHome,"config.json"),inlineErrors),repoSettings:_collectCopilotInlineConfig(join9(repoConfigDir,"settings.json"),inlineErrors),localSettings:_collectCopilotInlineConfig(join9(repoConfigDir,"settings.local.json"),inlineErrors)};if(inlineSupport!==!1){let disableSource=_resolveCopilotInlineDisableSource(inlineSources);if(disableSource){if(inlineSupport===null)errors.push(`GitHub Copilot CLI version unavailable; treating disableAllHooks in ${disableSource} as active`);return{activeConfigPaths:[],disabledBy:disableSource}}}let repoHookPaths=_collectSafetyNetCopilotHookFiles(repoHookDir,errors),userHookSupport=_supportsCopilotUserHookFiles(copilotCliVersion),userHookErrors=userHookSupport===!0?errors:void 0,userHookFiles=existsSync5(userHookDir)?_listJsonFiles(userHookDir,userHookErrors):[],userHookPaths=[];for(let filename of userHookFiles){let configPath=join9(userHookDir,filename),config=_readCopilotConfigFile(configPath,userHookErrors);if(config&&_hasSafetyNetCopilotHook(config))userHookPaths.push(configPath)}if(userHookSupport!==!0&&userHookPaths.length>0)_warnOnUnsupportedCopilotSource(errors,copilotCliVersion,`user hook files in ${userHookDir}`,"0.0.422"),userHookPaths.length=0;let inlinePaths=[],inlineSourcesByPrecedence=[inlineSources.localSettings,inlineSources.repoSettings,inlineSources.userConfig];for(let source of inlineSourcesByPrecedence){if(!source)continue;if(!_hasSafetyNetCopilotHook(source.config))continue;if(inlineSupport===!0){inlinePaths.push(source.path);continue}_warnOnUnsupportedCopilotSource(errors,copilotCliVersion,"inline hook definitions in Copilot config files","1.0.8");break}return{activeConfigPaths:[...inlinePaths.filter((path)=>path.endsWith("settings.local.json")),...inlinePaths.filter((path)=>path.endsWith("settings.json")),...repoHookPaths,...inlinePaths.filter((path)=>path.endsWith("config.json")),...userHookPaths]}}function _findCursorManagedEntries(config){if(!config||typeof config!=="object"||Array.isArray(config))return[];let hooks=config.hooks;if(!hooks||typeof hooks!=="object"||Array.isArray(hooks))return[];let preToolUse=hooks.preToolUse;if(!Array.isArray(preToolUse))return[];return preToolUse.filter((entry)=>!!entry&&typeof entry==="object"&&!Array.isArray(entry)&&entry.command===CURSOR_HOOK_COMMAND)}function _cursorDriftErrors(entries){let errors=[];if(entries.length>1)errors.push("Multiple managed cc-safety-net hooks found; reinstall to collapse duplicates");let entry=entries[0];if(entry&&entry.failClosed!==!0)errors.push('Managed hook is missing "failClosed": true; reinstall to repair');if(entry&&entry.timeout!==30)errors.push('Managed hook "timeout" is not 30; reinstall to repair');return errors}function detectCursor(homeDir){let configPath=getCursorHooksPath(homeDir);if(!existsSync5(configPath))return{platform:"cursor",status:"n/a",configPath};let parsed;try{parsed=JSON.parse(readFileSync5(configPath,"utf-8"))}catch(e){return{platform:"cursor",status:"n/a",configPath,errors:[`Failed to parse Cursor hooks config ${configPath}: ${e instanceof Error?e.message:String(e)}`]}}let entries=_findCursorManagedEntries(parsed);if(entries.length===0)return{platform:"cursor",status:"n/a",configPath};let errors=_cursorDriftErrors(entries);return{platform:"cursor",status:"configured",method:"hook config",configPath,errors:errors.length>0?errors:void 0}}function _ampArtifactVersion(content){return/^\/\/ version:\s*(.+)$/m.exec(content)?.[1]?.trim()}function detectAmp(homeDir){let configPath=getAmpPluginPath(homeDir),info=(()=>{try{return lstatSync2(configPath)}catch{return}})();if(!info)return{platform:"amp",status:"n/a",configPath};if(info.isSymbolicLink()||!info.isFile())return{platform:"amp",status:"n/a",configPath,errors:[`${configPath} is a symlink or not a regular file; move or remove it before installing`]};let content;try{content=readFileSync5(configPath,"utf-8")}catch(e){return{platform:"amp",status:"n/a",configPath,errors:[`Failed to read ${configPath}: ${e instanceof Error?e.message:String(e)}`]}}if(!content.startsWith(AMP_MANAGED_HEADER))return{platform:"amp",status:"n/a",configPath,errors:[`Unmanaged file occupies ${configPath}; move or remove it before installing`]};let outdated=_ampArtifactVersion(content)!==getPackageVersion();return{platform:"amp",status:"configured",method:"plugin file",configPath,errors:outdated?["Installed Amp plugin is outdated; run install --amp to update"]:void 0}}function detectAllHooks(cwd,options){let homeDir=options?.homeDir??homedir2(),detectCopilotCLI=()=>{let errors=[],hooksCheck=_checkCopilotEnabled(homeDir,cwd,options?.copilotCliVersion,errors);if(hooksCheck.disabledBy)return{platform:"copilot-cli",status:"disabled",method:"hook config",configPath:hooksCheck.disabledBy,configPaths:[hooksCheck.disabledBy],errors:errors.length>0?errors:void 0};let configHome=_getCopilotConfigHome(homeDir),pluginDir=join9(configHome,"installed-plugins",...COPILOT_PLUGIN_DIR),pluginInstalled=existsSync5(pluginDir),settingsPath=join9(configHome,"settings.json"),settings=readStateFile(settingsPath,stripJsonComments);if(pluginInstalled&&settings.kind==="unreadable")return{platform:"copilot-cli",status:"not-inspected"};if(pluginInstalled&&settings.kind==="ok"&&readRecord(readRecord(settings.value,"enabledPlugins"),COPILOT_PLUGIN_ID)===!1)return{platform:"copilot-cli",status:"disabled",method:"plugin config",configPath:settingsPath,errors:[`${COPILOT_PLUGIN_ID} is installed but not enabled in Copilot CLI`]};if(pluginInstalled||hooksCheck.activeConfigPaths.length>0){let viaPlugin=pluginInstalled,primaryConfigPath=hooksCheck.activeConfigPaths[0];return{platform:"copilot-cli",status:"configured",method:viaPlugin?"plugin config":"hook config",configPath:primaryConfigPath??(viaPlugin?pluginDir:void 0),configPaths:hooksCheck.activeConfigPaths.length>0?hooksCheck.activeConfigPaths:void 0,errors:errors.length>0?errors:void 0}}return{platform:"copilot-cli",status:"n/a",errors:errors.length>0?errors:void 0}};return doctorIntegrationOrder.map((platform)=>{let detection=(()=>{switch(platform){case"claude-code":return detectClaudeCode(homeDir);case"antigravity-cli":return detectAntigravityCli(homeDir);case"opencode":return detectOpenCode(homeDir);case"gemini-cli":return detectGeminiCLI(homeDir);case"copilot-cli":return detectCopilotCLI();case"kimi-code":return detectKimiCode(homeDir);case"pi":return detectPi(homeDir);case"codex":return detectCodex(options?.codexPluginListOutput);case"cursor":return detectCursor(homeDir);case"amp":return detectAmp(homeDir)}return platform})();return _toHookStatus(detection)})}function _toHookStatus(detection){if(detection.status==="not-inspected")return{platform:detection.platform,detected:!1,configured:!1,inspectionStatus:"not-inspected"};return{platform:detection.platform,detected:detection.status!=="n/a",configured:detection.status==="configured",inspectionStatus:detection.status!=="n/a"?"verified":detection.errors&&detection.errors.length>0?"failed":"not-applicable",method:detection.method,configPath:detection.configPath,configPaths:detection.configPaths,errors:detection.errors}}import{lstatSync as lstatSync3}from"node:fs";import{dirname as dirname5}from"node:path";function inspectDirectory(kind,path){try{let stat=lstatSync3(path);if(stat.isSymbolicLink())return{kind,path,status:"unsafe",issues:["symlink"]};if(!stat.isDirectory())return{kind,path,status:"unsafe",issues:["not-directory"]};if(process.platform==="win32"||typeof process.getuid!=="function")return{kind,path,status:"unknown",issues:[]};let issues=[...stat.uid!==process.getuid()?["ownership"]:[],...(stat.mode&18)!==0?["permissions"]:[]];return{kind,path,status:issues.length>0?"unsafe":"safe",issues}}catch(error){if(typeof error==="object"&&error!==null&&"code"in error&&error.code==="ENOENT")return{kind,path,status:"not-applicable",issues:[]};return{kind,path,status:"unknown",issues:[]}}}function getDoctorPosture(userConfigPath){let auditPath=getAuditLogsDir();return{directories:[inspectDirectory("policy",dirname5(dirname5(userConfigPath))),inspectDirectory("config",dirname5(userConfigPath)),...auditPath?[inspectDirectory("audit",auditPath)]:[{kind:"audit",status:"unknown",issues:[]}]]}}function isNewerVersion(latest,current){if(current==="dev")return!1;let latestParts=latest.split(".").map(Number),currentParts=current.split(".").map(Number),[latestMajor=0,latestMinor=0,latestPatch=0]=latestParts,[currentMajor=0,currentMinor=0,currentPatch=0]=currentParts;if(latestMajor!==currentMajor)return latestMajor>currentMajor;if(latestMinor!==currentMinor)return latestMinor>currentMinor;return latestPatch>currentPatch}async function checkForUpdates(){let currentVersion=getPackageVersion(),controller=new AbortController,timeout=setTimeout(()=>controller.abort(),3000);try{let res=await fetch("https://registry.npmjs.org/cc-safety-net/latest",{signal:controller.signal});if(!res.ok)return{currentVersion,latestVersion:null,updateAvailable:!1,error:`npm registry returned ${res.status}`};let data=await res.json(),updateAvailable=isNewerVersion(data.version,currentVersion);return{currentVersion,latestVersion:data.version,updateAvailable}}catch(e){return{currentVersion,latestVersion:null,updateAvailable:!1,error:e instanceof Error?e.message:"Network error"}}finally{clearTimeout(timeout)}}import*as readline from"node:readline";var CURSOR_DOWN=(rows)=>`\x1B[${rows}B`,CURSOR_UP=(rows)=>`\x1B[${rows}A`;var SCRAMBLE_POOL=["░","▒","▓","╱","╲","┃","━","┏","┓","┗","┛","╋"];function wait(milliseconds){return new Promise((resolve4)=>setTimeout(resolve4,milliseconds))}function waitForAnimationFrame(milliseconds,sleep,signal){if(!signal)return sleep(milliseconds);if(signal.aborted)return Promise.resolve();return new Promise((resolve4,reject)=>{let cleanup=()=>signal.removeEventListener("abort",onAbort),onAbort=()=>{cleanup(),resolve4()};signal.addEventListener("abort",onAbort,{once:!0}),sleep(milliseconds).then(()=>{cleanup(),resolve4()},(error)=>{cleanup(),reject(error)})})}function positiveOrDefault(value,fallback){return value&&value>0?value:fallback}function clamp01(value){return Math.max(0,Math.min(1,value))}function byte(value){return Math.max(0,Math.min(255,Math.round(value)))}function linearToSrgb(value){return value<=0.0031308?12.92*value:1.055*value**0.4166666666666667-0.055}function oklchToSrgb(lightness,chroma,hueDegrees){let hueRadians=hueDegrees*Math.PI/180,a=chroma*Math.cos(hueRadians),b=chroma*Math.sin(hueRadians),l=(lightness+0.3963377774*a+0.2158037573*b)**3,m=(lightness-0.1055613458*a-0.0638541728*b)**3,s=(lightness-0.0894841775*a-1.291485548*b)**3;return{blue:byte(linearToSrgb(clamp01(-0.0041960863*l-0.7034186147*m+1.707614701*s))*255),green:byte(linearToSrgb(clamp01(-1.2684380046*l+2.6097574011*m-0.3413193965*s))*255),red:byte(linearToSrgb(clamp01(4.0767416621*l-3.3077115913*m+0.2309699292*s))*255)}}function rainbow(frequency,offset){let hueDegrees=(offset*frequency*180/Math.PI%360+360)%360;return oklchToSrgb(0.72,0.15,hueDegrees)}function rainbowColorEscape(offset,frequency=0.1){let color=rainbow(frequency,offset);return`\x1B[38;2;${color.red};${color.green};${color.blue}m`}function mixTowardWhite(color,amount){return{blue:byte(color.blue+(255-color.blue)*amount),green:byte(color.green+(255-color.green)*amount),red:byte(color.red+(255-color.red)*amount)}}function hash01(a,b,c){let mixed=Math.imul(a+2654435769,2246822507)^Math.imul(b+3266489909,668265263)^Math.imul(c+374761393,2654435761),x1=mixed^mixed>>>15,x2=Math.imul(x1,739982445),x3=x2^x2>>>12,x4=Math.imul(x3,695872825);return((x4^x4>>>15)>>>0)/4294967296}function scrambleGlyph(lineIndex,columnIndex,frame){let index=Math.floor(hash01(lineIndex,columnIndex,frame)*SCRAMBLE_POOL.length);return SCRAMBLE_POOL[index]??"░"}function smootherstep(progress){let t=clamp01(progress);return t*t*t*(t*(t*6-15)+10)}function buildLine(cells){if(cells.length===0)return"";let parts=[],activeBold=!1,activeColor="";for(let cell of cells){let color=`${cell.red};${cell.green};${cell.blue}`;if(cell.bold!==activeBold)parts.push(cell.bold?"\x1B[1m":"\x1B[22m"),activeBold=cell.bold;if(color!==activeColor)parts.push(`\x1B[38;2;${color}m`),activeColor=color;parts.push(cell.character)}return`${parts.join("")}\x1B[22m\x1B[39m`}function settledLineCells(line,lineIndex,frequency,seed,spread){return line.map((character,columnIndex)=>({...rainbow(frequency,seed+lineIndex+columnIndex/spread),bold:!1,character}))}function wavefrontLineCells(line,lineIndex,frame,frameCount,width,frequency,seed,spread){let revealFrames=Math.max(1,frameCount*0.75),revealProgress=Math.min(1,frame/revealFrames),front=width*smootherstep(revealProgress),settleProgress=Math.max(0,(frame-revealFrames)/Math.max(1,frameCount-revealFrames)),seedOffset=(1-smootherstep(frame/frameCount))*spread*2,settleFlash=0.35*Math.max(0,1-settleProgress*2),revealed=revealProgress>=1,cutoff=Math.min(line.length,Math.ceil(front+2+1));return line.slice(0,cutoff).map((character,columnIndex)=>{let base=rainbow(frequency,seed+lineIndex+columnIndex/spread+seedOffset),position=columnIndex+hash01(lineIndex,columnIndex,7919)*2-1;if(position>front+2)return{...base,bold:!1,character:" "};let distance=front-position,glow=0.8*Math.exp(-(distance*distance)/12.5),boost=Math.min(0.9,glow+settleFlash),scrambling=!revealed&&position>front-4;return{...mixTowardWhite(base,boost),bold:boost>0.3,character:scrambling?scrambleGlyph(lineIndex,columnIndex,frame):character}})}function buildFrame(cellsPerLine){return`\x1B[?2026h${cellsPerLine.map((cells,lineIndex)=>`\x1B8${lineIndex>0?CURSOR_DOWN(lineIndex):""}${buildLine(cells)}`).join("")}\x1B[?2026l`}async function writeAnimatedLolcat(text,options={}){if(!text)return;let output=options.output??process.stdout,sleep=options.sleep??wait,frequency=positiveOrDefault(options.frequency,0.1),seed=options.seed??0,speed=positiveOrDefault(options.speed,40),spread=positiveOrDefault(options.spread,3),frameRate=positiveOrDefault(options.frameRate,60),duration=Math.max(1,Math.floor(positiveOrDefault(options.duration,12))),lines=text.split(`
...
event = "PreToolUse"
command = "${KIMI_HOOK_COMMAND}"`,KIMI_INLINE_HOOK=`{ event = "PreToolUse", command = "${KIMI_HOOK_COMMAND}" }`;function getKimiConfigPath(homeDir){return join11(process.env.KIMI_CODE_HOME??join11(homeDir,".kimi-code"),"config.toml")}function removeTopLevelEmptyHooksArray(content){return content.split(`
...
${KIMI_HOOK_BLOCK}
`}function removeKimiTableHookBlocks(content){return content.split(/(?=^\s*\[)/m).filter((block)=>!/^\s*\[\[hooks]]\s*$/m.test(block)||!block.includes(KIMI_HOOK_COMMAND)).join("").trimEnd()}function removeKimiInlineHook(content,hooksRange){let itemStart=content.indexOf(KIMI_INLINE_HOOK,hooksRange.start);if(itemStart===-1||itemStart>hooksRange.end)return content;return removeArrayRangeItem(content,{start:itemStart,end:itemStart+KIMI_INLINE_HOOK.length})}function installKimiCode(homeDir){let configPath=getKimiConfigPath(homeDir);if(mkdirSync4(dirname7(configPath),{recursive:!0}),!existsSync8(configPath))return atomicWriteFile(configPath,`${KIMI_HOOK_BLOCK}
`),{path:configPath,alreadyInstalled:!1};let content=readFileSync7(configPath,"utf-8");if(content.includes(KIMI_HOOK_COMMAND))return{path:configPath,alreadyInstalled:!0};return atomicWriteFile(configPath,appendKimiHook(content)),{path:configPath,alreadyInstalled:!1}}function uninstallKimiCode(homeDir){let configPath=getKimiConfigPath(homeDir);if(!existsSync8(configPath))return{path:configPath,alreadyInstalled:!1};let content=readFileSync7(configPath,"utf-8");if(!content.includes(KIMI_HOOK_COMMAND))return{path:configPath,alreadyInstalled:!1};let inlineHooksRange=findTopLevelInlineHooksArray(content),updated=inlineHooksRange?removeKimiInlineHook(content,inlineHooksRange):`${removeKimiTableHookBlocks(content)}
`;return atomicWriteFile(configPath,updated),{path:configPath,alreadyInstalled:!0}}import{spawnSync}from"node:child_process";function formatNativeCommand(command){return command.join(" ")}function formatCommandFailure(command,status,output){return[`Failed to run ${formatNativeCommand(command)}${status===null?"":` (exit ${status})`}.`,output.trim()].filter(Boolean).join(`
...
`)}var import_sisteransi=__toESM(require_src(),1);function findCursor(s,o,l){if(!l.some((r)=>!r.disabled))return s;let t=s+o,n=Math.max(l.length-1,0),e=t<0?n:t>n?0:t;return l[e]?.disabled?findCursor(e,o<0?-1:1,l):e}function findTextCursor(s,o,l,i){let t=i.split(`
`),n=0,e=s;for(let r of t){if(e<=r.length)break;e-=r.length+1,n++}for(n=Math.max(0,Math.min(t.length-1,n+l)),e=Math.min(e,t[n].length)+o;e<0&&n>0;)n--,e+=t[n].length+1;for(;e>t[n].length&&n<t.length-1;)e-=t[n].length+1,n++;e=Math.max(0,Math.min(t[n].length,e));let h=0;for(let r=0;r<n;r++)h+=t[r].length+1;return h+e}var a$1=["up","down","left","right","space","enter","cancel"],t=["January","February","March","April","May","June","July","August","September","October","November","December"],settings={actions:new Set(a$1),aliases:new Map([["k","up"],["j","down"],["h","left"],["l","right"],["\x03","cancel"],["escape","cancel"]]),messages:{cancel:"Canceled",error:"Something went wrong"},withGuide:!0,date:{monthNames:[...t],messages:{required:"Please enter a valid date",invalidMonth:"There are only 12 months in a year",invalidDay:(n,e)=>`There are only ${n} days in ${e}`,afterMin:(n)=>`Date must be on or after ${n.toISOString().slice(0,10)}`,beforeMax:(n)=>`Date must be on or before ${n.toISOString().slice(0,10)}`}}};function isActionKey(n,e){if(typeof n=="string")return settings.aliases.get(n)===e;for(let s of n)if(s!==void 0&&isActionKey(s,e))return!0;return!1}function diffLines(i,s){if(i===s)return;let e=i.split(`
`),t2=s.split(`
`),r=Math.max(e.length,t2.length),f=[];for(let n=0;n<r;n++)e[n]!==t2[n]&&f.push(n);return{lines:f,numLinesBefore:e.length,numLinesAfter:t2.length,numLines:r}}var R=globalThis.process.platform.startsWith("win"),CANCEL_SYMBOL=Symbol("clack:cancel");function isCancel(e){return e===CANCEL_SYMBOL}function setRawMode(e,r){let o=e;o.isTTY&&o.setRawMode(r)}var getRows=(e)=>("rows"in e)&&typeof e.rows=="number"?e.rows:20;function runValidation(e,n){if("~standard"in e){let a=e["~standard"].validate(n);if(a instanceof Promise)throw TypeError("Schema validation must be synchronous. Update `validate()` and remove any asynchronous logic.");return a.issues?.at(0)?.message}return e(n)}class V{input;output;_abortSignal;rl;opts;_render;_track=!1;_prevFrame="";_subscribers=new Map;_cursor=0;state="initial";error="";value;userInput="";constructor(t2,e=!0){let{input:i=stdin,output:n=stdout,render:s,signal:r,...o}=t2;this.opts=o,this.onKeypress=this.onKeypress.bind(this),this.close=this.close.bind(this),this.render=this.render.bind(this),this._render=s.bind(this),this._track=e,this._abortSignal=r,this.input=i,this.output=n}unsubscribe(){this._subscribers.clear()}setSubscriber(t2,e){let i=this._subscribers.get(t2)??[];i.push(e),this._subscribers.set(t2,i)}on(t2,e){this.setSubscriber(t2,{cb:e})}once(t2,e){this.setSubscriber(t2,{cb:e,once:!0})}emit(t2,...e){let i=this._subscribers.get(t2)??[],n=[];for(let s of i)s.cb(...e),s.once&&n.push(()=>i.splice(i.indexOf(s),1));for(let s of n)s()}prompt(){return new Promise((t2)=>{if(this._abortSignal){if(this._abortSignal.aborted)return this.state="cancel",this.close(),t2(CANCEL_SYMBOL);this._abortSignal.addEventListener("abort",()=>{this.state="cancel",this.close()},{once:!0})}this.rl=l__default.createInterface({input:this.input,tabSize:2,prompt:"",escapeCodeTimeout:50,terminal:!0}),this.rl.prompt(),this.opts.initialUserInput!==void 0&&this._setUserInput(this.opts.initialUserInput,!0),this.input.on("keypress",this.onKeypress),setRawMode(this.input,!0),this.output.on("resize",this.render),this.render(),this.once("submit",()=>{this.output.write(import_sisteransi.cursor.show),this.output.off("resize",this.render),setRawMode(this.input,!1),t2(this.value)}),this.once("cancel",()=>{this.output.write(import_sisteransi.cursor.show),this.output.off("resize",this.render),setRawMode(this.input,!1),t2(CANCEL_SYMBOL)})})}_isActionKey(t2,e){return t2==="\t"}_shouldSubmit(t2,e){return!0}_setValue(t2){this.value=t2,this.emit("value",this.value)}_setUserInput(t2,e){this.userInput=t2??"",this.emit("userInput",this.userInput),e&&this._track&&this.rl&&(this.rl.write(this.userInput),this._cursor=this.rl.cursor)}_clearUserInput(){this.rl?.write(null,{ctrl:!0,name:"u"}),this._setUserInput("")}onKeypress(t2,e){if(this._track&&e.name!=="return"&&(e.name&&this._isActionKey(t2,e)&&this.rl?.write(null,{ctrl:!0,name:"h"}),this._cursor=this.rl?.cursor??0,this._setUserInput(this.rl?.line)),this.state==="error"&&(this.state="active"),e?.name&&(!this._track&&settings.aliases.has(e.name)&&this.emit("cursor",settings.aliases.get(e.name)),settings.actions.has(e.name)&&this.emit("cursor",e.name)),t2&&(t2.toLowerCase()==="y"||t2.toLowerCase()==="n")&&this.emit("confirm",t2.toLowerCase()==="y"),this.emit("key",t2,e),e?.name==="return"&&this._shouldSubmit(t2,e)){if(this.opts.validate){let i=runValidation(this.opts.validate,this.value);i&&(this.error=i instanceof Error?i.message:i,this.state="error",this.rl?.write(this.userInput))}this.state!=="error"&&(this.state="submit")}isActionKey([t2,e?.name,e?.sequence],"cancel")&&(this.state="cancel"),(this.state==="submit"||this.state==="cancel")&&this.emit("finalize"),this.render(),(this.state==="submit"||this.state==="cancel")&&this.close()}close(){this.input.unpipe(),this.input.removeListener("keypress",this.onKeypress),this.output.write(`
...
`).length-1;this.output.write(import_sisteransi.cursor.move(-999,t2*-1))}render(){let t2=wrapAnsi(this._render(this)??"",process.stdout.columns,{hard:!0,trim:!1});if(t2!==this._prevFrame){if(this.state==="initial")this.output.write(import_sisteransi.cursor.hide);else{let e=diffLines(this._prevFrame,t2),i=getRows(this.output);if(this.restoreCursor(),e){let n=Math.max(0,e.numLinesAfter-i),s=Math.max(0,e.numLinesBefore-i),r=e.lines.find((o)=>o>=n);if(r===void 0){this._prevFrame=t2;return}if(e.lines.length===1){this.output.write(import_sisteransi.cursor.move(0,r-s)),this.output.write(import_sisteransi.erase.lines(1));let o=t2.split(`
`);this.output.write(o[r]),this._prevFrame=t2,this.output.write(import_sisteransi.cursor.move(0,o.length-r-1));return}else if(e.lines.length>1){if(n<s)r=n;else{let h=r-s;h>0&&this.output.write(import_sisteransi.cursor.move(0,h))}this.output.write(import_sisteransi.erase.down());let f=t2.split(`
`).slice(r);this.output.write(f.join(`
`)),this._prevFrame=t2;return}}this.output.write(import_sisteransi.erase.down())}this.output.write(t2),this.state==="initial"&&(this.state="active"),this._prevFrame=t2}}}function p$1(l,e){if(l===void 0||e.length===0)return 0;let i=e.findIndex((s)=>s.value===l);return i!==-1?i:0}function g(l,e){return(e.label??String(e.value)).toLowerCase().includes(l.toLowerCase())}function m(l,e){if(e)return l?e:e[0]}var T$1=class extends V{filteredOptions;multiple;isNavigating=!1;selectedValues=[];focusedValue;#e=0;#s="";#t;#i;#n;get cursor(){return this.#e}get userInputWithCursor(){if(!this.userInput)return styleText(["inverse","hidden"],"_");if(this._cursor>=this.userInput.length)return`${this.userInput}█`;let e=this.userInput.slice(0,this.cursor),t2=this.userInput.slice(this.cursor,this.cursor+1),i=this.userInput.slice(this.cursor+1);return`${e}${styleText("inverse",t2)}${i}`}get options(){return typeof this.#i=="function"?this.#i():this.#i}constructor(e){super(e),this.#i=e.options,this.#n=e.placeholder;let t2=this.options;this.filteredOptions=[...t2],this.multiple=e.multiple===!0,this.#t=typeof e.options=="function"?e.filter:e.filter??g;let i;if(e.initialValue&&Array.isArray(e.initialValue)?this.multiple?i=e.initialValue:i=e.initialValue.slice(0,1):!this.multiple&&this.options.length>0&&(i=[this.options[0]?.value]),i)for(let s of i){let n=t2.findIndex((o)=>o.value===s);n!==-1&&(this.toggleSelected(s),this.#e=n)}this.focusedValue=this.options[this.#e]?.value,this.on("key",(s,n)=>this.#l(s,n)),this.on("userInput",(s)=>this.#u(s))}_isActionKey(e,t2){return e==="\t"||this.multiple&&this.isNavigating&&t2.name==="space"&&e!==void 0&&e!==""}#l(e,t2){let i=t2.name==="up",s=t2.name==="down",n=t2.name==="return",o=this.userInput===""||this.userInput==="\t",u=this.#n,a=this.options,f=u!==void 0&&u!==""&&a.some((r)=>!r.disabled&&(this.#t?this.#t(u,r):!0));if(t2.name==="tab"&&o&&f){this.userInput==="\t"&&this._clearUserInput(),this._setUserInput(u,!0),this.isNavigating=!1;return}i||s?(this.#e=findCursor(this.#e,i?-1:1,this.filteredOptions),this.focusedValue=this.filteredOptions[this.#e]?.value,this.multiple||(this.selectedValues=[this.focusedValue]),this.isNavigating=!0):n?this.value=m(this.multiple,this.selectedValues):this.multiple?this.focusedValue!==void 0&&(t2.name==="tab"||this.isNavigating&&t2.name==="space")?this.toggleSelected(this.focusedValue):this.isNavigating=!1:(this.focusedValue&&(this.selectedValues=[this.focusedValue]),this.isNavigating=!1)}deselectAll(){this.selectedValues=[]}toggleSelected(e){this.filteredOptions.length!==0&&(this.multiple?this.selectedValues.includes(e)?this.selectedValues=this.selectedValues.filter((t2)=>t2!==e):this.selectedValues=[...this.selectedValues,e]:this.selectedValues=[e])}#u(e){if(e!==this.#s){this.#s=e;let t2=this.options;e&&this.#t?this.filteredOptions=t2.filter((n)=>this.#t?.(e,n)):this.filteredOptions=[...t2];let i=p$1(this.focusedValue,this.filteredOptions);this.#e=findCursor(i,0,this.filteredOptions);let s=this.filteredOptions[this.#e];s&&!s.disabled?this.focusedValue=s.value:this.focusedValue=void 0,this.multiple||(this.focusedValue!==void 0?this.toggleSelected(this.focusedValue):this.deselectAll())}}};var _={Y:{type:"year",len:4},M:{type:"month",len:2},D:{type:"day",len:2}};function M(r){return[...r].map((t2)=>_[t2])}function P(r){let i=new Intl.DateTimeFormat(r,{year:"numeric",month:"2-digit",day:"2-digit"}).formatToParts(new Date(2000,0,15)),s=[],n="/";for(let e of i)e.type==="literal"?n=e.value.trim()||e.value:(e.type==="year"||e.type==="month"||e.type==="day")&&s.push({type:e.type,len:e.type==="year"?4:2});return{segments:s,separator:n}}function p(r){return Number.parseInt((r||"0").replace(/_/g,"0"),10)||0}function f(r){return{year:p(r.year),month:p(r.month),day:p(r.day)}}function c(r,t2){return new Date(r||2001,t2||1,0).getDate()}function b(r){let{year:t2,month:i,day:s}=f(r);if(!t2||t2<0||t2>9999||!i||i<1||i>12||!s||s<1)return;let n=new Date(Date.UTC(t2,i-1,s));if(!(n.getUTCFullYear()!==t2||n.getUTCMonth()!==i-1||n.getUTCDate()!==s))return{year:t2,month:i,day:s}}function C(r){let t2=b(r);return t2?new Date(Date.UTC(t2.year,t2.month-1,t2.day)):void 0}function T2(r,t2,i,s){let n=i?{year:i.getUTCFullYear(),month:i.getUTCMonth()+1,day:i.getUTCDate()}:null,e=s?{year:s.getUTCFullYear(),month:s.getUTCMonth()+1,day:s.getUTCDate()}:null;return r==="year"?{min:n?.year??1,max:e?.year??9999}:r==="month"?{min:n&&t2.year===n.year?n.month:1,max:e&&t2.year===e.year?e.month:12}:{min:n&&t2.year===n.year&&t2.month===n.month?n.day:1,max:e&&t2.year===e.year&&t2.month===e.month?e.day:c(t2.year,t2.month)}}class U extends V{#i;#o;#t;#h;#u;#e={segmentIndex:0,positionInSegment:0};#n=!0;#s=null;inlineError="";get segmentCursor(){return{...this.#e}}get segmentValues(){return{...this.#t}}get segments(){return this.#i}get separator(){return this.#o}get formattedValue(){return this.#l(this.#t)}#l(t2){return this.#i.map((i)=>t2[i.type]).join(this.#o)}#r(){this._setUserInput(this.#l(this.#t)),this._setValue(C(this.#t)??void 0)}constructor(t2){let i=t2.format?{segments:M(t2.format),separator:t2.separator??"/"}:P(t2.locale),s=t2.separator??i.separator,n=t2.format?M(t2.format):i.segments,e=t2.initialValue??t2.defaultValue,m2=e?{year:String(e.getUTCFullYear()).padStart(4,"0"),month:String(e.getUTCMonth()+1).padStart(2,"0"),day:String(e.getUTCDate()).padStart(2,"0")}:{year:"____",month:"__",day:"__"},o=n.map((a)=>m2[a.type]).join(s);super({...t2,initialUserInput:o},!1),this.#i=n,this.#o=s,this.#t=m2,this.#h=t2.minDate,this.#u=t2.maxDate,this.#r(),this.on("cursor",(a)=>this.#f(a)),this.on("key",(a,u)=>this.#y(a,u)),this.on("finalize",()=>this.#p(t2))}#a(){let t2=Math.max(0,Math.min(this.#e.segmentIndex,this.#i.length-1)),i=this.#i[t2];if(i)return this.#e.positionInSegment=Math.max(0,Math.min(this.#e.positionInSegment,i.len-1)),{segment:i,index:t2}}#m(t2){this.inlineError="",this.#s=null;let i=this.#a();i&&(this.#e.segmentIndex=Math.max(0,Math.min(this.#i.length-1,i.index+t2)),this.#e.positionInSegment=0,this.#n=!0)}#d(t2){let i=this.#a();if(!i)return;let{segment:s}=i,n=this.#t[s.type],e=!n||n.replace(/_/g,"")==="",m2=Number.parseInt((n||"0").replace(/_/g,"0"),10)||0,o=T2(s.type,f(this.#t),this.#h,this.#u),a;e?a=t2===1?o.min:o.max:a=Math.max(Math.min(o.max,m2+t2),o.min),this.#t={...this.#t,[s.type]:a.toString().padStart(s.len,"0")},this.#n=!0,this.#s=null,this.#r()}#f(t2){if(t2)switch(t2){case"right":return this.#m(1);case"left":return this.#m(-1);case"up":return this.#d(1);case"down":return this.#d(-1)}}#y(t2,i){if(i?.name==="backspace"||i?.sequence===""||i?.sequence==="\b"||t2===""||t2==="\b"){this.inlineError="";let n=this.#a();if(!n)return;if(!this.#t[n.segment.type].replace(/_/g,"")){this.#m(-1);return}this.#t[n.segment.type]="_".repeat(n.segment.len),this.#n=!0,this.#e.positionInSegment=0,this.#r();return}if(i?.name==="tab"){this.inlineError="";let n=this.#a();if(!n)return;let e=i.shift?-1:1,m2=n.index+e;m2>=0&&m2<this.#i.length&&(this.#e.segmentIndex=m2,this.#e.positionInSegment=0,this.#n=!0);return}if(t2&&/^[0-9]$/.test(t2)){let n=this.#a();if(!n)return;let{segment:e}=n,m2=!this.#t[e.type].replace(/_/g,"");if(this.#n&&this.#s!==null&&!m2){let h=this.#s+t2,d={...this.#t,[e.type]:h},g2=this.#g(d,e);if(g2){this.inlineError=g2,this.#s=null,this.#n=!1;return}this.inlineError="",this.#t[e.type]=h,this.#s=null,this.#n=!1,this.#r(),n.index<this.#i.length-1&&(this.#e.segmentIndex=n.index+1,this.#e.positionInSegment=0,this.#n=!0);return}this.#n&&!m2&&(this.#t[e.type]="_".repeat(e.len),this.#e.positionInSegment=0),this.#n=!1,this.#s=null;let o=this.#t[e.type],a=o.indexOf("_"),u=a>=0?a:Math.min(this.#e.positionInSegment,e.len-1);if(u<0||u>=e.len)return;let l=o.slice(0,u)+t2+o.slice(u+1),D=!1;if(u===0&&o==="__"&&(e.type==="month"||e.type==="day")){let h=Number.parseInt(t2,10);l=`0${t2}`,D=h<=(e.type==="month"?1:2)}if(e.type==="year"&&(l=(o.replace(/_/g,"")+t2).padStart(e.len,"_")),!l.includes("_")){let h={...this.#t,[e.type]:l},d=this.#g(h,e);if(d){this.inlineError=d;return}}this.inlineError="",this.#t[e.type]=l;let y=l.includes("_")?void 0:b(this.#t);if(y){let{year:h,month:d}=y,g2=c(h,d);this.#t={year:String(Math.max(0,Math.min(9999,h))).padStart(4,"0"),month:String(Math.max(1,Math.min(12,d))).padStart(2,"0"),day:String(Math.max(1,Math.min(g2,y.day))).padStart(2,"0")}}this.#r();let S=l.indexOf("_");D?(this.#n=!0,this.#s=t2):S>=0?this.#e.positionInSegment=S:a>=0&&n.index<this.#i.length-1?(this.#e.segmentIndex=n.index+1,this.#e.positionInSegment=0,this.#n=!0):this.#e.positionInSegment=Math.min(u+1,e.len-1)}}#g(t2,i){let{month:s,day:n}=f(t2);if(i.type==="month"&&(s<0||s>12))return settings.date.messages.invalidMonth;if(i.type==="day"&&(n<0||n>31))return settings.date.messages.invalidDay(31,"any month")}#p(t2){let{year:i,month:s,day:n}=f(this.#t);if(i&&s&&n){let e=c(i,s);this.#t={...this.#t,day:String(Math.min(n,e)).padStart(2,"0")}}this.value=C(this.#t)??t2.defaultValue??void 0}}var u$2=class extends V{options;cursor=0;#t;getGroupItems(t2){return this.options.filter((r)=>r.group===t2)}isGroupSelected(t2){let r=this.getGroupItems(t2),e=this.value;return e===void 0?!1:r.every((s)=>e.includes(s.value))}toggleValue(){let t2=this.options[this.cursor];if(t2!==void 0)if(this.value===void 0&&(this.value=[]),t2.group===!0){let r=t2.value,e=this.getGroupItems(r);this.isGroupSelected(r)?this.value=this.value.filter((s)=>e.findIndex((i)=>i.value===s)===-1):this.value=[...this.value,...e.map((s)=>s.value)],this.value=Array.from(new Set(this.value))}else{let r=this.value.includes(t2.value);this.value=r?this.value.filter((e)=>e!==t2.value):[...this.value,t2.value]}}constructor(t2){super(t2,!1);let{options:r}=t2;this.#t=t2.selectableGroups!==!1,this.options=Object.entries(r).flatMap(([e,s])=>[{value:e,group:!0,label:e},...s.map((i)=>({...i,group:e}))]),this.value=[...t2.initialValues??[]],this.cursor=Math.max(this.options.findIndex(({value:e})=>e===t2.cursorAt),this.#t?0:1),this.on("cursor",(e)=>{switch(e){case"left":case"up":{this.cursor=this.cursor===0?this.options.length-1:this.cursor-1;let s=this.options[this.cursor]?.group===!0;!this.#t&&s&&(this.cursor=this.cursor===0?this.options.length-1:this.cursor-1);break}case"down":case"right":{this.cursor=this.cursor===this.options.length-1?0:this.cursor+1;let s=this.options[this.cursor]?.group===!0;!this.#t&&s&&(this.cursor=this.cursor===this.options.length-1?0:this.cursor+1);break}case"space":this.toggleValue();break}})}},o=new Set(["up","down","left","right"]);class h extends V{#t=!1;#s;focused="editor";get userInputWithCursor(){if(this.state==="submit")return this.userInput;let t2=this.userInput;if(this.cursor>=t2.length)return`${t2}█`;let s=t2.slice(0,this.cursor),r=t2.slice(this.cursor,this.cursor+1),i=t2.slice(this.cursor+1);return r===`
`?`${s}█
${i}`:`${s}${styleText("inverse",r)}${i}`}get cursor(){return this._cursor}#r(t2){if(this.userInput.length===0){this._setUserInput(t2);return}this._setUserInput(this.userInput.slice(0,this.cursor)+t2+this.userInput.slice(this.cursor))}#i(t2){let s=this.value??"";switch(t2){case"up":this._cursor=findTextCursor(this._cursor,0,-1,s);return;case"down":this._cursor=findTextCursor(this._cursor,0,1,s);return;case"left":this._cursor=findTextCursor(this._cursor,-1,0,s);return;case"right":this._cursor=findTextCursor(this._cursor,1,0,s);return}}_shouldSubmit(t2,s){if(this.#s)return this.focused==="submit"?!0:(this.#r(`
`),this._cursor++,!1);let r=this.#t;return this.#t=!0,r&&this.cursor===this.userInput.length?(this.userInput[this.cursor-1]===`
`&&(this._setUserInput(this.userInput.slice(0,this.cursor-1)+this.userInput.slice(this.cursor)),this._cursor--),!0):(this.#r(`
`),this._cursor++,!1)}constructor(t2){let s=t2.initialUserInput??t2.initialValue;super({...t2,initialUserInput:s},!1),s!==void 0&&(this._cursor=s.length),this.#s=t2.showSubmit??!1,this.on("key",(r,i)=>{if(i?.name&&o.has(i.name)){this.#t=!1,this.#i(i.name);return}if(r==="\t"&&this.#s){this.focused=this.focused==="editor"?"submit":"editor";return}if(i?.name!=="return"){if(this.#t=!1,i?.name==="backspace"&&this.cursor>0){this._setUserInput(this.userInput.slice(0,this.cursor-1)+this.userInput.slice(this.cursor)),this._cursor--;return}if(i?.name==="delete"&&this.cursor<this.userInput.length){this._setUserInput(this.userInput.slice(0,this.cursor)+this.userInput.slice(this.cursor+1));return}r&&(this.#s&&this.focused==="submit"&&(this.focused="editor"),this.#r(r??""),this._cursor++)}}),this.on("userInput",(r)=>{this._setValue(r)}),this.on("finalize",()=>{this.value||(this.value=t2.defaultValue),this.value===void 0&&(this.value="")})}}class a extends V{options;cursor=0;get _value(){return this.options[this.cursor]?.value}get _enabledOptions(){return this.options.filter((e)=>e.disabled!==!0)}toggleAll(){let e=this._enabledOptions,i=this.value!==void 0&&this.value.length===e.length;this.value=i?[]:e.map((t2)=>t2.value)}toggleInvert(){let e=this.value;if(!e)return;let i=this._enabledOptions.filter((t2)=>!e.includes(t2.value));this.value=i.map((t2)=>t2.value)}toggleValue(){this.value===void 0&&(this.value=[]);let e=this.value.includes(this._value);this.value=e?this.value.filter((i)=>i!==this._value):[...this.value,this._value]}constructor(e){super(e,!1),this.options=e.options,this.value=[...e.initialValues??[]];let i=Math.max(this.options.findIndex(({value:t2})=>t2===e.cursorAt),0);this.cursor=this.options[i]?.disabled?findCursor(i,1,this.options):i,this.on("key",(t2,l)=>{l.name==="a"&&this.toggleAll(),l.name==="i"&&this.toggleInvert()}),this.on("cursor",(t2)=>{switch(t2){case"left":case"up":this.cursor=findCursor(this.cursor,-1,this.options);break;case"down":case"right":this.cursor=findCursor(this.cursor,1,this.options);break;case"space":this.toggleValue();break}})}}var INSTALL_TARGETS=installIntegrationMetadata.map((integration)=>({target:integration.id,flag:integration.flag,label:integration.installLabel,probeCommand:integration.probeCommand})),TARGET_FLAGS=new Map(INSTALL_TARGETS.map((target)=>[target.flag,target.target]));function orderInstallTargets(targets){let selectedTargets=new Set(targets);return INSTALL_TARGETS.map((target)=>target.target).filter((target)=>selectedTargets.has(target))}function runInstallTargetsInOrder(targets,runTarget){targets.forEach(runTarget)}var PROBE_TIMEOUT_MS=5000;function titleCaseAction(action){return action==="install"?"Install":"Uninstall"}function activeVerb(action){return action==="install"?"Installing":"Uninstalling"}function targetPreposition(action){return action==="install"?"into":"from"}function defaultInstallTargetProbe(command){let result=spawnSync2(command[0],command.slice(1),{env:process.env,stdio:"ignore",timeout:PROBE_TIMEOUT_MS});return!result.error&&result.status===0}function probeInstallTarget(command){return new Promise((resolve5)=>{let proc=spawn2(command[0],command.slice(1),{env:process.env,stdio:"ignore"}),settled=!1,finish=(available)=>{if(settled)return;settled=!0,clearTimeout(timeoutId),resolve5(available)},timeoutId=setTimeout(()=>{proc.kill(),finish(!1)},PROBE_TIMEOUT_MS);proc.on("error",()=>finish(!1)),proc.on("close",(code)=>finish(code===0))})}function buildInstallTargetChoices(probe=defaultInstallTargetProbe,options={}){let configuredTargets=new Set(options.configuredTargets??[]);if(options.async)return Promise.all(INSTALL_TARGETS.map(async(target)=>({target:target.target,flag:target.flag,label:target.label,...getChoiceAvailability(options.action,await probe(target.probeCommand),configuredTargets.has(target.target))})));let syncProbe=probe;return INSTALL_TARGETS.map((target)=>({target:target.target,flag:target.flag,label:target.label,...getChoiceAvailability(options.action,syncProbe(target.probeCommand),configuredTargets.has(target.target))}))}function buildInstallTargetChoicesAsync(probe=probeInstallTarget,options={}){return buildInstallTargetChoices(probe,{...options,async:!0})}function applyInstallTargetState(choices,options){let configuredTargets=new Set(options.configuredTargets??[]);return choices.map((choice)=>({...choice,...getChoiceAvailability(options.action,choice.available,configuredTargets.has(choice.target))}))}function getChoiceAvailability(action,cliAvailable,configured){if(action==="uninstall")return configured?{available:!0}:{available:!1,unavailableReason:"not installed"};if(action==="install"&&configured)return{available:!1,unavailableReason:"already installed"};if(!cliAvailable)return{available:!1,unavailableReason:"CLI not installed"};return{available:!0}}var CHECKBOX_ON="◉",CHECKBOX_OFF="◯",CURSOR_ON=">",CURSOR_OFF=" ";function renderInstallSelection(action,choices,state,options={}){let useColor=options.color!==!1,formatDim=useColor?colors.dim:(value)=>value,formatCheckboxOn=useColor?colors.green:(value)=>value,formatFocus=useColor?colors.bold:(value)=>value;return["",`${titleCaseAction(action)} CC Safety Net ${targetPreposition(action)}:`,"",...choices.map((choice,index)=>{let selected=state.selected.includes(choice.target),focused=index===state.cursor&&choice.available,marker=selected?CHECKBOX_ON:CHECKBOX_OFF,cursor2=focused?CURSOR_ON:CURSOR_OFF,suffix=choice.available?"":` (${choice.unavailableReason??"not installed"})`,rowBody=`${marker} ${choice.label}${suffix}`,formatted=!choice.available?formatDim(rowBody):selected?formatCheckboxOn(rowBody):focused?formatFocus(rowBody):rowBody;return`${cursor2} ${formatted}`}),"",action==="install"?"Space: select  Enter: confirm  u: update installed  Up/Down: move  q/Esc: cancel":choices.some((choice)=>choice.available)?"Space: select  Enter: confirm  Up/Down: move  q/Esc: cancel":`No selectable integrations found for ${action}. q/Esc: close`].join(`
`)}function canPromptInstallTargets(input=process.stdin,output=process.stdout){return Boolean(input.isTTY&&output.isTTY&&typeof input.setRawMode==="function")}async function promptInstallTargets(action,choices,options={}){let output=options.output??process.stdout,abort=new AbortController,lastKey="",prompt=new a({input:options.input??process.stdin,output,signal:abort.signal,options:choices.map((choice)=>({value:choice.target,disabled:!choice.available})),render(){return renderInstallSelection(action,choices,{cursor:this.cursor,selected:this.value??[]})},validate:(selected2)=>selected2?.length?void 0:"Select at least one integration"});prompt.on("key",(char,key)=>{if(lastKey=char??"",key.name==="return"&&prompt.value?.length===0)output.write("\x07");if(char==="q"||action==="install"&&(char==="u"||char==="U"))abort.abort()});let selected=await prompt.prompt();if(!isCancel(selected))return output.write(`${activeVerb(action)} selected integrations...
`),selected??[];if(action==="install"&&(lastKey==="u"||lastKey==="U"))return"update";if(lastKey==="\x03")(options.onInterrupt??(()=>process.kill(process.pid,"SIGINT")))();return null}var AMP_RESTART_NOTE='Restart Amp or run "plugins: reload" to apply the change.',CLAUDE_LEGACY_PLUGIN_ID="safety-net@cc-marketplace",NATIVE_UPDATE_TARGETS=new Set(["claude-code","codex","copilot-cli","gemini-cli","opencode","pi"]);function hasCodexLegacyPlugin(output){return/^\s*safety-net@cc-marketplace[^a-z0-9-][^\n]*installed,/m.test(output??"")}function hasCodexReplacementPlugin(output){return/^\s*cc-safety-net[^a-z0-9-][^\n]*installed,/m.test(output??"")}var NATIVE_INSTALLS={"claude-code":{installCommands:(homeDir)=>{let update=hasClaudeInstalledPlugin(homeDir,"cc-safety-net@cc-marketplace");return{commands:[...update?[["claude","plugin","marketplace","update","cc-marketplace"],["claude","plugin","update","cc-safety-net@cc-marketplace"]]:[["claude","plugin","marketplace","add","kenryu42/cc-marketplace"],["claude","plugin","install","cc-safety-net@cc-marketplace"]],...detectClaudeCode(homeDir).status==="disabled"?[["claude","plugin","enable","cc-safety-net@cc-marketplace"]]:[],...hasClaudeInstalledPlugin(homeDir,CLAUDE_LEGACY_PLUGIN_ID)?[["claude","plugin","uninstall",CLAUDE_LEGACY_PLUGIN_ID]]:[]],update}},uninstallCommands:[["claude","plugin","uninstall","cc-safety-net@cc-marketplace"],["claude","plugin","marketplace","remove","cc-marketplace"]]},codex:{installCommands:()=>{let pluginList=runNativeCommand(["codex","plugin","list"]),update=hasCodexReplacementPlugin(pluginList);return{commands:[...update?[["codex","plugin","marketplace","upgrade","cc-marketplace"],["codex","plugin","add","cc-safety-net@cc-marketplace"]]:[["codex","plugin","marketplace","add","kenryu42/cc-marketplace"],["codex","plugin","add","cc-safety-net@cc-marketplace"]],...hasCodexLegacyPlugin(pluginList)?[["codex","plugin","remove","safety-net@cc-marketplace"]]:[]],update}},uninstallCommands:[["codex","plugin","remove","cc-safety-net@cc-marketplace"],["codex","plugin","marketplace","remove","cc-marketplace"]],postInstallMessage:"Start Codex, open `/hooks`, select the cc-safety-net PreToolUse hook, and press `t` to trust it."},"copilot-cli":{installCommands:()=>{let pluginList=runNativeCommand(["copilot","plugin","list"]),legacyUninstall=hasCopilotLegacyPlugin(pluginList)?[["copilot","plugin","uninstall","copilot-safety-net"]]:[];if(hasCopilotSafetyNetPlugin(pluginList))return{commands:[["copilot","plugin","marketplace","update","cc-marketplace"],["copilot","plugin","update",COPILOT_PLUGIN_ID],...legacyUninstall],update:!0};return{commands:[...hasCopilotMarketplace(runNativeCommand(["copilot","plugin","marketplace","list"]))?[]:[["copilot","plugin","marketplace","add","kenryu42/cc-marketplace"]],["copilot","plugin","install",COPILOT_PLUGIN_ID],...legacyUninstall]}},uninstallCommands:[["copilot","plugin","uninstall","cc-safety-net@cc-marketplace"],["copilot","plugin","marketplace","remove","cc-marketplace"]]},"gemini-cli":{installCommands:(homeDir)=>{let detection=detectGeminiCLI(homeDir);if(detection.status==="configured")return{commands:[["gemini","extensions","update","gemini-safety-net"]],update:!0};if(detection.status==="disabled")return{commands:[["gemini","extensions","update","gemini-safety-net"],["gemini","extensions","enable","gemini-safety-net"]],update:!0};return{commands:[["gemini","extensions","install","https://github.com/kenryu42/gemini-safety-net","--consent"]]}},uninstallCommands:[["gemini","extensions","uninstall","gemini-safety-net"]]},opencode:{beforeInstall:clearOpenCodeCache,installCommands:[["opencode","plugin","-g","-f","cc-safety-net@latest"]]},pi:{installCommands:[["pi","install","npm:cc-safety-net"]],uninstallCommands:[["pi","uninstall","npm:cc-safety-net"]]}};function getHomeDir(){return process.env.HOME??homedir3()}function parseJsonSettings(configPath,preprocess=(raw)=>raw){try{let config=JSON.parse(preprocess(readFileSync9(configPath,"utf-8")));if(!config||typeof config!=="object"||Array.isArray(config))throw Error(`Settings file ${configPath} must be a JSON object`);return config}catch(error){if(error instanceof SyntaxError)throw Error(`Failed to parse ${configPath}: ${error.message}`);throw error}}function enableCopilotPlugin(homeDir){let settingsPath=join14(_getCopilotConfigHome(homeDir),"settings.json");if(!existsSync11(settingsPath))return;let settings2=parseJsonSettings(settingsPath,stripJsonComments),enabledPlugins=settings2.enabledPlugins;if(!enabledPlugins||typeof enabledPlugins!=="object"||Array.isArray(enabledPlugins))return;if(enabledPlugins[COPILOT_PLUGIN_ID]!==!1)return;let raw=readFileSync9(settingsPath,"utf-8"),flipped=raw.replace(new RegExp(`("${COPILOT_PLUGIN_ID}"\\s*:\\s*)false`),"$1true");enabledPlugins[COPILOT_PLUGIN_ID]=!0,atomicWriteFile(settingsPath,flipped!==raw?flipped:`${JSON.stringify(settings2,null,2)}
`),console.log(`Enabled ${COPILOT_PLUGIN_ID} plugin in ${settingsPath}`)}function removePiExtensionsFilter(homeDir){let settingsPath=getPiSettingsPath(homeDir);if(!existsSync11(settingsPath))return;let settings2=parseJsonSettings(settingsPath);if(!Array.isArray(settings2.packages))return;let entry=settings2.packages.find((candidate)=>!!candidate&&typeof candidate==="object"&&!Array.isArray(candidate)&&isPiSafetyNetPackageSource(candidate.source)&&("extensions"in candidate));if(!entry)return;delete entry.extensions,atomicWriteFile(settingsPath,`${JSON.stringify(settings2,null,2)}
`),console.log(`Enabled npm:cc-safety-net extensions in ${settingsPath}`)}function parseInstallTarget(args,action){let parsed=parseCommandArgs({label:action,booleans:Object.fromEntries(INSTALL_TARGETS.map((target)=>[target.target,[target.flag]]))},args),error=parsed.errors[0];if(error)throw Error(error);let targets=INSTALL_TARGETS.filter((target)=>parsed.flags[target.target]).map((target)=>target.target);if(targets.length!==1)throw Error(`Choose exactly one ${action} target: ${[...TARGET_FLAGS.keys()].join(", ")}`);return targets[0]}async function detectInstallHookState(homeDir=getHomeDir()){let[codexPluginListOutput,copilotCliVersion]=await Promise.all([defaultVersionFetcher(["codex","plugin","list"]),defaultVersionFetcher(["copilot","--binary-version"])]);return{codexPluginListOutput,hooks:detectAllHooks(process.cwd(),{homeDir,codexPluginListOutput,copilotCliVersion})}}async function detectConfiguredInstallTargets(action){let state=await detectInstallHookState();return state.hooks.filter((hook)=>action==="install"?hook.configured:hook.detected||hook.inspectionStatus==="not-inspected").filter((hook)=>hook.platform!=="codex"||!hasCodexLegacyPlugin(state.codexPluginListOutput)||hasCodexReplacementPlugin(state.codexPluginListOutput)).map((hook)=>hook.platform)}function startResolveInstallTargets(action,args,options){if(args.length>0)return{finish:async()=>[parseInstallTarget(args,action)]};if(!options.selectTargets&&!canPromptInstallTargets(options.input,options.output))return{finish:async()=>[parseInstallTarget(args,action)]};let detectConfiguredTargets=options.detectConfiguredTargets??(()=>detectConfiguredInstallTargets(action)),ready=Promise.all([buildInstallTargetChoicesAsync(options.probeTargets),detectConfiguredTargets()]);return{ready,finish:async()=>{let[choices,configuredTargets]=await ready,targetChoices=applyInstallTargetState(choices,{action,configuredTargets}),selected=options.selectTargets?await options.selectTargets(action,targetChoices):await promptInstallTargets(action,targetChoices,{input:options.input,output:options.output});if(selected==="update")return selected;if(!selected||selected.length===0)return null;return orderInstallTargets(selected)}}}function installNativeTarget(target,homeDir,updating=!1){let definition=NATIVE_INSTALLS[target];definition.beforeInstall?.(homeDir);let plan=typeof definition.installCommands==="function"?definition.installCommands(homeDir):{commands:definition.installCommands};runNativeCommands(plan.commands),console.log([`${plan.update||updating?"Updated":"Installed"} ${getIntegrationInstallLabel(target)} integration`,definition.postInstallMessage].filter(Boolean).join(`
`))}function uninstallNativeTarget(target){let definition=NATIVE_INSTALLS[target];if(!definition.uninstallCommands)throw Error(`${getIntegrationInstallLabel(target)} uninstall is not supported`);runNativeCommands(definition.uninstallCommands),console.log(`Uninstalled ${getIntegrationInstallLabel(target)} integration`)}function uninstallOpenCodeTarget(homeDir){let result=uninstallOpenCode(homeDir);console.log(result.alreadyInstalled?`Uninstalled OpenCode plugin from ${result.path}`:`OpenCode plugin not installed in ${result.path}`)}var CONFIG_INSTALLS={"antigravity-cli":{install:installAntigravityCli,uninstall:uninstallAntigravityCli},cursor:{install:installCursor,uninstall:uninstallCursor},"kimi-code":{install:installKimiCode,uninstall:uninstallKimiCode}};function runConfigInstallTarget(action,target,homeDir,updating=!1){if(action==="install")clearNpxSafetyNetCache(homeDir);let result=CONFIG_INSTALLS[target][action](homeDir),name=getIntegrationInstallLabel(target),pastTense=action!=="install"?"Uninstalled":updating?"Updated":"Installed";console.log(action==="install"&&result.alreadyInstalled?updating?`${name} hook up to date in ${result.path}`:`${name} hook already installed in ${result.path}`:action==="uninstall"&&!result.alreadyInstalled?`${name} hook not installed in ${result.path}`:`${pastTense} ${name} hook ${action==="install"?"in":"from"} ${result.path}`)}function runAmpInstallTarget(action,homeDir,updating=!1){let result=action==="install"?installAmp(homeDir):uninstallAmp(homeDir),name=getIntegrationInstallLabel("amp"),noChange=action==="install"&&result.alreadyInstalled||action==="uninstall"&&!result.alreadyInstalled,message=noChange?action==="install"?`${name} plugin ${updating?"up to date":"already installed"} at ${result.path}`:`${name} plugin not installed at ${result.path}`:`${action!=="install"?"Uninstalled":updating?"Updated":"Installed"} ${name} plugin ${action==="install"?"at":"from"} ${result.path}`;console.log([message,noChange?void 0:AMP_RESTART_NOTE].filter(Boolean).join(`
`))}var INSTALL_OPERATIONS={amp:{install:(homeDir,updating)=>runAmpInstallTarget("install",homeDir,updating),uninstall:(homeDir)=>runAmpInstallTarget("uninstall",homeDir)},"antigravity-cli":{install:(homeDir,updating)=>runConfigInstallTarget("install","antigravity-cli",homeDir,updating),uninstall:(homeDir)=>runConfigInstallTarget("uninstall","antigravity-cli",homeDir)},"claude-code":{install:(homeDir,updating)=>installNativeTarget("claude-code",homeDir,updating),uninstall:()=>uninstallNativeTarget("claude-code")},codex:{install:(homeDir,updating)=>installNativeTarget("codex",homeDir,updating),uninstall:()=>uninstallNativeTarget("codex")},"copilot-cli":{install:(homeDir,updating)=>{installNativeTarget("copilot-cli",homeDir,updating),enableCopilotPlugin(homeDir)},uninstall:()=>uninstallNativeTarget("copilot-cli")},cursor:{install:(homeDir,updating)=>runConfigInstallTarget("install","cursor",homeDir,updating),uninstall:(homeDir)=>runConfigInstallTarget("uninstall","cursor",homeDir)},"gemini-cli":{install:(homeDir,updating)=>installNativeTarget("gemini-cli",homeDir,updating),uninstall:()=>uninstallNativeTarget("gemini-cli")},"kimi-code":{install:(homeDir,updating)=>runConfigInstallTarget("install","kimi-code",homeDir,updating),uninstall:(homeDir)=>runConfigInstallTarget("uninstall","kimi-code",homeDir)},opencode:{install:(homeDir,updating)=>installNativeTarget("opencode",homeDir,updating),uninstall:(homeDir)=>uninstallOpenCodeTarget(homeDir)},pi:{install:(homeDir,updating)=>{installNativeTarget("pi",homeDir,updating),removePiExtensionsFilter(homeDir)},uninstall:()=>uninstallNativeTarget("pi")}};function runSingleInstallTarget(action,target,homeDir,updating=!1){INSTALL_OPERATIONS[target][action](homeDir,updating)}function parseUpdateArgs(args){let error=parseCommandArgs({label:"update"},args).errors[0];if(error)throw Error(error)}async function detectUpdateTargets(homeDir){let state=await detectInstallHookState(homeDir),copilotPluginsDir=join14(_getCopilotConfigHome(homeDir),"installed-plugins");return orderInstallTargets([...state.hooks.filter((hook)=>hook.platform!=="copilot-cli"&&hook.detected).map((hook)=>hook.platform),...[COPILOT_PLUGIN_DIR,COPILOT_LEGACY_PLUGIN_DIR].flatMap((dir)=>existsSync11(join14(copilotPluginsDir,...dir))?["copilot-cli"]:[]),...hasClaudeInstalledPlugin(homeDir,CLAUDE_LEGACY_PLUGIN_ID)?["claude-code"]:[],...hasCodexLegacyPlugin(state.codexPluginListOutput)?["codex"]:[]])}async function updateInstalledIntegrations(){let homeDir=getHomeDir(),targets=await detectUpdateTargets(homeDir);if(targets.length===0){console.log("No installed integrations found. Run `cc-safety-net install` to set one up.");return}let targetSet=new Set(targets),available=new Map(await Promise.all(INSTALL_TARGETS.filter((target)=>targetSet.has(target.target)&&NATIVE_UPDATE_TARGETS.has(target.target)).map(async(target)=>[target.target,await probeInstallTarget(target.probeCommand)])));runInstallTargetsInOrder(targets,(target)=>{if(NATIVE_UPDATE_TARGETS.has(target)&&!available.get(target)){console.log(`${getIntegrationInstallLabel(target)} not found; skipped`);return}runSingleInstallTarget("install",target,homeDir,!0)})}function runUpdateCommand(args){return Promise.resolve().then(()=>parseUpdateArgs(args)).then(updateInstalledIntegrations).then(()=>0).catch((error)=>{return console.error(formatInstallError(error)),1})}async function runInstallCommand(action,args,options={}){try{let targets=await resolveAfterOptionalBanner(!0,()=>startResolveInstallTargets(action,args,options),()=>printInstallBanner({input:options.input??process.stdin,output:options.output??process.stdout}),{loadingMessage:action==="install"?"Checking available integrations…":"Checking installed integrations…",output:options.output??process.stdout});if(!targets)return(options.output??process.stdout).write(`Cancelled: nothing was ${action}ed.
`),0;if(targets==="update")return(options.runUpdate??(()=>runUpdateCommand([])))();let homeDir=getHomeDir();return runInstallTargetsInOrder(targets,(target)=>runSingleInstallTarget(action,target,homeDir)),0}catch(e){return console.error(formatInstallError(e)),1}}function formatInstallError(error){let message=error instanceof Error?error.message:String(error),code=typeof error==="object"&&error!==null&&"code"in error?error.code:null;if(code==="EACCES"||code==="EPERM")return`${message}
...
/* The secret group head carries a bulk action, so the collapse control is a
   button inside the head rather than the head itself. */
.tier-collapse {
  display: flex;
  min-width: 0;
  flex: 1;
...
<svg xmlns="http://www.w3.org/2000/svg" width="1254" height="1254" viewBox="54 23 1140 1140" role="img" aria-label="Safety net logo mesh variant">
  <defs>
    <radialGradient id="spot-0" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stop-color="#f8fafc" stop-opacity="0.68"/>
      <stop offset="56%" stop-color="#f8fafc" stop-opacity="0.29"/>
      <stop offset="100%" stop-color="#f8fafc" stop-opacity="0"/>
    </radialGradient>
    <radialGradient id="spot-1" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stop-color="#7dd3fc" stop-opacity="0.58"/>
      <stop offset="56%" stop-color="#7dd3fc" stop-opacity="0.24"/>
      <stop offset="100%" stop-color="#7dd3fc" stop-opacity="0"/>
    </radialGradient>
    <radialGradient id="spot-2" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stop-color="#64748b" stop-opacity="0.7"/>
      <stop offset="56%" stop-color="#64748b" stop-opacity="0.29"/>
      <stop offset="100%" stop-color="#64748b" stop-opacity="0"/>
    </radialGradient>
    <radialGradient id="spot-3" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stop-color="#0f172a" stop-opacity="0.9"/>
      <stop offset="56%" stop-color="#0f172a" stop-opacity="0.38"/>
      <stop offset="100%" stop-color="#0f172a" stop-opacity="0"/>
    </radialGradient>
    <linearGradient id="edge" x1="14%" y1="8%" x2="88%" y2="94%">
      <stop offset="0%" stop-color="#ffffff" stop-opacity="0.7"/>
      <stop offset="50%" stop-color="#bae6fd" stop-opacity="0.24"/>
      <stop offset="100%" stop-color="#1e293b" stop-opacity="0.86"/>
    </linearGradient>
    <mask id="net-mask" maskUnits="userSpaceOnUse">
      <rect width="1254" height="1254" fill="black"/>
      <rect x="-46.32" y="-47.38" width="92.63" height="94.75" rx="12.00" ry="12.00" transform="translate(628.75 127.25) rotate(-45.00)" fill="white"/>
      <rect x="-66.82" y="-41.01" width="133.64" height="82.02" rx="12.00" ry="12.00" transform="translate(713.75 230.25) rotate(-45.00)" fill="white"/>
      <rect x="-39.95" y="-134.00" width="79.90" height="267.99" rx="12.00" ry="12.00" transform="translate(588.00 275.50) rotate(-45.00)" fill="white"/>
      <rect x="-39.60" y="-65.05" width="79.20" height="130.11" rx="12.00" ry="12.00" transform="translate(444.50 320.50) rotate(-45.00)" fill="white"/>
      <rect x="-133.29" y="-40.31" width="266.58" height="80.61" rx="12.00" ry="12.00" transform="translate(759.75 369.25) rotate(-45.00)" fill="white"/>
      <rect x="-77.07" y="-39.24" width="154.15" height="78.49" rx="12.00" ry="12.00" transform="translate(533.25 407.25) rotate(-45.00)" fill="white"/>
      <rect x="-67.10" y="-39.74" width="134.21" height="79.48" rx="12.00" ry="12.00" transform="translate(895.22 413.86) rotate(-45.00)" fill="white"/>
      <rect x="-39.84" y="-134.04" width="79.68" height="268.08" rx="12.00" ry="12.00" transform="translate(401.36 461.24) rotate(-45.00)" fill="white"/>
      <rect x="-39.60" y="-74.60" width="79.20" height="149.20" rx="12.00" ry="12.00" transform="translate(812.25 500.25) rotate(-45.00)" fill="white"/>
      <rect x="-39.60" y="-77.43" width="79.20" height="154.86" rx="12.00" ry="12.00" transform="translate(625.75 500.75) rotate(-45.00)" fill="white"/>
      <rect x="-39.24" y="-67.18" width="78.49" height="134.35" rx="12.00" ry="12.00" transform="translate(263.25 505.75) rotate(-45.00)" fill="white"/>
      <rect x="-133.28" y="-40.02" width="266.56" height="80.04" rx="12.00" ry="12.00" transform="translate(941.36 551.76) rotate(-45.00)" fill="white"/>
      <rect x="-54.80" y="-53.74" width="109.60" height="107.48" rx="12.00" ry="12.00" transform="translate(1096.75 593.25) rotate(-45.00)" fill="white"/>
      <rect x="-77.43" y="-40.31" width="154.86" height="80.61" rx="12.00" ry="12.00" transform="translate(719.75 594.25) rotate(-45.00)" fill="white"/>
      <rect x="-51.97" y="-54.45" width="103.94" height="108.89" rx="12.00" ry="12.00" transform="translate(155.25 594.75) rotate(-45.00)" fill="white"/>
      <rect x="-76.37" y="-40.31" width="152.74" height="80.61" rx="12.00" ry="12.00" transform="translate(534.50 595.50) rotate(-45.00)" fill="white"/>
      <rect x="-135.12" y="-40.16" width="270.23" height="80.32" rx="12.00" ry="12.00" transform="translate(307.96 634.94) rotate(-45.00)" fill="white"/>
      <rect x="-40.02" y="-70.64" width="80.05" height="141.27" rx="12.00" ry="12.00" transform="translate(989.66 680.72) rotate(-45.00)" fill="white"/>
      <rect x="-38.90" y="-77.27" width="77.80" height="154.54" rx="12.00" ry="12.00" transform="translate(442.49 687.00) rotate(-45.00)" fill="white"/>
      <rect x="-39.95" y="-77.43" width="79.90" height="154.86" rx="12.00" ry="12.00" transform="translate(628.50 689.00) rotate(-45.00)" fill="white"/>
      <rect x="-39.40" y="-134.46" width="78.80" height="268.92" rx="12.00" ry="12.00" transform="translate(853.69 727.31) rotate(-45.00)" fill="white"/>
      <rect x="-69.65" y="-38.18" width="139.30" height="76.37" rx="12.00" ry="12.00" transform="translate(353.25 771.75) rotate(-45.00)" fill="white"/>
      <rect x="-78.44" y="-39.44" width="156.88" height="78.88" rx="12.00" ry="12.00" transform="translate(720.61 782.02) rotate(-45.00)" fill="white"/>
      <rect x="-133.77" y="-39.86" width="267.53" height="79.71" rx="12.00" ry="12.00" transform="translate(493.85 820.81) rotate(-45.00)" fill="white"/>
      <rect x="-39.24" y="-66.82" width="78.49" height="133.64" rx="12.00" ry="12.00" transform="translate(806.50 868.00) rotate(-45.00)" fill="white"/>
      <rect x="-40.02" y="-133.39" width="80.05" height="266.79" rx="12.00" ry="12.00" transform="translate(666.35 914.10) rotate(-45.00)" fill="white"/>
      <rect x="-67.18" y="-39.60" width="134.35" height="79.20" rx="12.00" ry="12.00" transform="translate(540.00 960.00) rotate(-45.00)" fill="white"/>
      <rect x="-49.85" y="-49.50" width="99.70" height="98.99" rx="12.00" ry="12.00" transform="translate(627.25 1064.75) rotate(-45.00)" fill="white"/>
    </mask>
  </defs>
  <g>
    <g mask="url(#net-mask)">
      <rect width="1254" height="1254" fill="#07090d"/>
      <circle cx="360" cy="240" r="430" fill="url(#spot-0)"/>
      <circle cx="820" cy="300" r="430" fill="url(#spot-1)"/>
      <circle cx="760" cy="830" r="500" fill="url(#spot-2)"/>
      <circle cx="300" cy="780" r="390" fill="url(#spot-3)"/>
      <rect width="1254" height="1254" fill="url(#edge)" opacity="0.18"/>
    </g>
    <g fill="none" stroke="url(#edge)" stroke-width="14" stroke-linejoin="round">
      <rect x="-46.32" y="-47.38" width="92.63" height="94.75" rx="12.00" ry="12.00" transform="translate(628.75 127.25) rotate(-45.00)"/>
      <rect x="-66.82" y="-41.01" width="133.64" height="82.02" rx="12.00" ry="12.00" transform="translate(713.75 230.25) rotate(-45.00)"/>
      <rect x="-39.95" y="-134.00" width="79.90" height="267.99" rx="12.00" ry="12.00" transform="translate(588.00 275.50) rotate(-45.00)"/>
      <rect x="-39.60" y="-65.05" width="79.20" height="130.11" rx="12.00" ry="12.00" transform="translate(444.50 320.50) rotate(-45.00)"/>
      <rect x="-133.29" y="-40.31" width="266.58" height="80.61" rx="12.00" ry="12.00" transform="translate(759.75 369.25) rotate(-45.00)"/>
      <rect x="-77.07" y="-39.24" width="154.15" height="78.49" rx="12.00" ry="12.00" transform="translate(533.25 407.25) rotate(-45.00)"/>
      <rect x="-67.10" y="-39.74" width="134.21" height="79.48" rx="12.00" ry="12.00" transform="translate(895.22 413.86) rotate(-45.00)"/>
      <rect x="-39.84" y="-134.04" width="79.68" height="268.08" rx="12.00" ry="12.00" transform="translate(401.36 461.24) rotate(-45.00)"/>
      <rect x="-39.60" y="-74.60" width="79.20" height="149.20" rx="12.00" ry="12.00" transform="translate(812.25 500.25) rotate(-45.00)"/>
      <rect x="-39.60" y="-77.43" width="79.20" height="154.86" rx="12.00" ry="12.00" transform="translate(625.75 500.75) rotate(-45.00)"/>
      <rect x="-39.24" y="-67.18" width="78.49" height="134.35" rx="12.00" ry="12.00" transform="translate(263.25 505.75) rotate(-45.00)"/>
      <rect x="-133.28" y="-40.02" width="266.56" height="80.04" rx="12.00" ry="12.00" transform="translate(941.36 551.76) rotate(-45.00)"/>
      <rect x="-54.80" y="-53.74" width="109.60" height="107.48" rx="12.00" ry="12.00" transform="translate(1096.75 593.25) rotate(-45.00)"/>
      <rect x="-77.43" y="-40.31" width="154.86" height="80.61" rx="12.00" ry="12.00" transform="translate(719.75 594.25) rotate(-45.00)"/>
      <rect x="-51.97" y="-54.45" width="103.94" height="108.89" rx="12.00" ry="12.00" transform="translate(155.25 594.75) rotate(-45.00)"/>
      <rect x="-76.37" y="-40.31" width="152.74" height="80.61" rx="12.00" ry="12.00" transform="translate(534.50 595.50) rotate(-45.00)"/>
      <rect x="-135.12" y="-40.16" width="270.23" height="80.32" rx="12.00" ry="12.00" transform="translate(307.96 634.94) rotate(-45.00)"/>
      <rect x="-40.02" y="-70.64" width="80.05" height="141.27" rx="12.00" ry="12.00" transform="translate(989.66 680.72) rotate(-45.00)"/>
      <rect x="-38.90" y="-77.27" width="77.80" height="154.54" rx="12.00" ry="12.00" transform="translate(442.49 687.00) rotate(-45.00)"/>
      <rect x="-39.95" y="-77.43" width="79.90" height="154.86" rx="12.00" ry="12.00" transform="translate(628.50 689.00) rotate(-45.00)"/>
      <rect x="-39.40" y="-134.46" width="78.80" height="268.92" rx="12.00" ry="12.00" transform="translate(853.69 727.31) rotate(-45.00)"/>
      <rect x="-69.65" y="-38.18" width="139.30" height="76.37" rx="12.00" ry="12.00" transform="translate(353.25 771.75) rotate(-45.00)"/>
      <rect x="-78.44" y="-39.44" width="156.88" height="78.88" rx="12.00" ry="12.00" transform="translate(720.61 782.02) rotate(-45.00)"/>
      <rect x="-133.77" y="-39.86" width="267.53" height="79.71" rx="12.00" ry="12.00" transform="translate(493.85 820.81) rotate(-45.00)"/>
      <rect x="-39.24" y="-66.82" width="78.49" height="133.64" rx="12.00" ry="12.00" transform="translate(806.50 868.00) rotate(-45.00)"/>
      <rect x="-40.02" y="-133.39" width="80.05" height="266.79" rx="12.00" ry="12.00" transform="translate(666.35 914.10) rotate(-45.00)"/>
      <rect x="-67.18" y="-39.60" width="134.35" height="79.20" rx="12.00" ry="12.00" transform="translate(540.00 960.00) rotate(-45.00)"/>
      <rect x="-49.85" y="-49.50" width="99.70" height="98.99" rx="12.00" ry="12.00" transform="translate(627.25 1064.75) rotate(-45.00)"/>
    </g>
    <g fill="none" stroke="#ffffff" stroke-opacity="0.2" stroke-width="5" stroke-linejoin="round" transform="translate(-10 -14)">
      <rect x="-46.32" y="-47.38" width="92.63" height="94.75" rx="12.00" ry="12.00" transform="translate(628.75 127.25) rotate(-45.00)"/>
      <rect x="-66.82" y="-41.01" width="133.64" height="82.02" rx="12.00" ry="12.00" transform="translate(713.75 230.25) rotate(-45.00)"/>
      <rect x="-39.95" y="-134.00" width="79.90" height="267.99" rx="12.00" ry="12.00" transform="translate(588.00 275.50) rotate(-45.00)"/>
      <rect x="-39.60" y="-65.05" width="79.20" height="130.11" rx="12.00" ry="12.00" transform="translate(444.50 320.50) rotate(-45.00)"/>
...
<head>
  ...
  <link rel="icon" href="__CC_SAFETY_NET_FAVICON__">
  <script>
    (() => {
...
<body>
  <div class="app-shell">
    <aside class="sidebar">
      <div class="brand">
        <h1 class="brand-logo"><a class="brand-home" href="#overview" title="Overview"><!-- __CC_SAFETY_NET_LOGO__ --></a></h1>
      </div>
...
✗ ${label}: ${path}`),console.error("  Errors:");let errorNum=1;for(let error of errors)for(let part of error.split("; "))console.error(`    ${errorNum}. ${part}`),errorNum++}function addRulesSchemaIfMissing(target){try{let content=readPolicyFile(target);if(content===null)return!1;let parsed=JSON.parse(content);if(parsed.$schema)return!1;return writePolicyFileAtomic(target,JSON.stringify({$schema:RULES_SCHEMA_URL,...parsed},null,2)),!0}catch(error){if(error instanceof PolicyFilesystemError)throw error;return!1}}var RULE_SUBCOMMANDS=new Set(["init","add","remove","update","sync","list","wrapper","migrate","doc","verify"]),RULE_WRAPPER_ACTIONS=new Set(["add","remove","list"]);async function runRuleCommand(args){try{return await runRuleCommandInternal(args)}catch(error){if(error instanceof PolicyFilesystemError)return console.error(error.message),1;throw error}}async function runRuleCommandInternal(args){let flags=parseRuleFlags(args),helpCommand=flags.help?getRuleHelpCommand(flags.positionals):null;if(helpCommand)return printCommandHelp(helpCommand),0;if(flags.errors.length>0){for(let error of flags.errors)console.error(error);return 1}let subcommand=flags.positionals[0];if(!subcommand)return printCommandHelp(ruleCommand,console.error),1;let value=flags.positionals[1],options={global:flags.global,check:flags.check};if(subcommand==="init"){let scope=getScopePaths(options),dir=scope.configDir;ensureRulesConfig(scope.configTarget),ensurePolicyDirectory(getPolicyFilesystemTargetForPath(scope.filesystemScope,getRulebookCacheRoot({...options,cacheConfigDir:dir})));let rulebookPath=join19(dir,"example-rules","rulebook.json"),rulebookTarget=getPolicyFilesystemTargetForPath(scope.filesystemScope,rulebookPath);if(flags.example&&readPolicyFile(rulebookTarget)===null)writeStarterRulebook(rulebookTarget,"example-rules");let result=await syncRulesConfig(options);return printRuleChangeResult(result,"Rule config initialized."),result.ok?0:1}if(subcommand==="add"){if(!value)return console.error("rule add requires a source"),1;let result=await addRulebookSource(value,options);return printRuleChangeResult(result,`Added rulebook source: ${value}`),result.ok?0:1}if(subcommand==="remove"){if(!value)return console.error("rule remove requires a source"),1;let result=await removeRulebookSource(value,{...options,deleteSource:flags.deleteSource});return printRuleChangeResult(result,`Removed rulebook source: ${value}`),result.ok?0:1}if(subcommand==="update"||subcommand==="sync"){let result=await syncRulesConfig({...options,only:subcommand==="update"?value:void 0});return printRuleChangeResult(result,flags.check?"Rule config checked.":"Rule config synced."),result.ok?0:1}if(subcommand==="list"){let policy=loadRulesPolicy(),paths=getPolicyPaths({});return printRulesListReport(policy,{user:getRulesConfigSourceDisplayMap(policy.userConfigPath,paths.userScope),project:getRulesConfigSourceDisplayMap(policy.projectConfigPath,paths.projectScope)}),policy.errors.length>0?1:0}if(subcommand==="wrapper")return runRuleWrapperCommand(flags);if(subcommand==="migrate")return runRulesMigrate({cleanup:flags.cleanup,cwd:process.cwd()});if(subcommand==="doc"){console.log(RULE_DOC);let notice=await getUpdateNotice();if(notice)console.error(notice);return 0}if(subcommand==="verify")return runRulesVerify();return 1}function getRuleHelpCommand(positionals){if(positionals.length===0)return ruleCommand;let leaves=ruleCommand.subcommands.filter((leaf2)=>leaf2.usage.split(" ")[0]===positionals[0]);if(leaves.length===0)return null;if(positionals.length===1&&leaves.length>1)return{name:`rule ${positionals[0]}`,description:`Subcommands of rule ${positionals[0]}`,usage:`rule ${positionals[0]} <subcommand>`,subcommands:leaves,options:[]};let leaf=positionals.length===1?leaves[0]:leaves.find((entry)=>entry.usage.split(" ")[1]===positionals[1]);if(!leaf)return null;return{name:`rule ${positionals[0]}`,description:leaf.description,usage:`rule ${leaf.usage}`,options:[]}}function parseRuleFlags(args){let parsed=parseCommandArgs({label:"rule",booleans:{global:["-g","--global"],check:["--check"],cleanup:["--cleanup"],deleteSource:["--delete-source"],example:["--example"]},positionals:"list"},args),flags={...parsed.flags,help:parsed.help,positionals:parsed.positionals,errors:parsed.errors};return validateRuleFlags(flags),flags}function validateRuleFlags(flags){let[subcommand]=flags.positionals;if(subcommand&&!RULE_SUBCOMMANDS.has(subcommand))flags.errors.push(`Unknown rule subcommand: ${subcommand}`);if(flags.deleteSource&&subcommand!=="remove")if(subcommand&&RULE_SUBCOMMANDS.has(subcommand))flags.errors.push(`Unknown option for rule ${subcommand}: --delete-source`);else flags.errors.push("--delete-source is only valid with 'rule remove'");if(flags.cleanup&&subcommand!=="migrate")flags.errors.push(unknownRuleOption(subcommand,"--cleanup"));if(flags.example&&subcommand!=="init")flags.errors.push(unknownRuleOption(subcommand,"--example"));if(subcommand==="migrate"){if(flags.global)flags.errors.push(unknownRuleOption(subcommand,"--global"));if(flags.check)flags.errors.push(unknownRuleOption(subcommand,"--check"));if(flags.positionals.length>1)flags.errors.push(`Unexpected rule migrate argument: ${flags.positionals[1]}`)}else if(subcommand==="wrapper")validateRuleWrapperFlags(flags);else if(flags.positionals.length>2)flags.errors.push(`Unexpected rule argument: ${flags.positionals[2]}`);if(subcommand==="list"&&flags.global)flags.errors.push("Unknown option for rule list: --global")}function unknownRuleOption(subcommand,option){return subcommand?`Unknown option for rule ${subcommand}: ${option}`:`Unknown option for rule: ${option}`}function validateRuleWrapperFlags(flags){let action=flags.positionals[1],command=flags.positionals[2];if(!action){flags.errors.push("rule wrapper requires add, remove, or list");return}if(!RULE_WRAPPER_ACTIONS.has(action)){flags.errors.push(`Unknown rule wrapper action: ${action}`);return}if(action==="list"){if(command)flags.errors.push(`Unexpected rule wrapper argument: ${command}`);return}if(!command){flags.errors.push(`rule wrapper ${action} requires a command`);return}if(flags.positionals.length>3)flags.errors.push(`Unexpected rule wrapper argument: ${flags.positionals[3]}`)}function ensureRulesConfig(configPath){if(readPolicyFile(configPath)===null){writeDefaultRulesConfig(configPath);return}let loaded=readRulesConfig(configPath);if(!loaded.config)return;writeJsonAtomic(configPath,{version:1,rules:loaded.config.rules,overrides:loaded.config.overrides??{},transparent_wrappers:loaded.config.transparent_wrappers??[]})}async function runRuleWrapperCommand(flags){let action=flags.positionals[1],command=flags.positionals[2],configPath=getScopePaths({global:flags.global}).configTarget;if(action==="list"){let loaded2=readRulesConfig(configPath);if(loaded2.errors.length>0){for(let error of loaded2.errors)console.error(error);return 1}return printTransparentWrappers(loaded2.config?.transparent_wrappers??[]),0}if(!command||!COMMAND_PATTERN.test(command))return console.error("transparent wrapper must match command pattern"),1;if(isReservedTransparentWrapper(command))return console.error(`reserved command "${command}" cannot be a wrapper`),1;let loaded=readRulesConfig(configPath);if(loaded.errors.length>0){for(let error of loaded.errors)console.error(error);return 1}let config=loaded.config??{version:1,rules:[],overrides:{},transparent_wrappers:[]},wrappers=action==="add"?[...new Set([...config.transparent_wrappers??[],command])]:(config.transparent_wrappers??[]).filter((wrapper)=>wrapper!==command);return writeJsonAtomic(configPath,{version:1,rules:config.rules,overrides:config.overrides??{},transparent_wrappers:wrappers}),console.log(action==="add"?`Added transparent wrapper: ${command}`:`Removed transparent wrapper: ${command}`),0}function printTransparentWrappers(wrappers){if(wrappers.length===0){console.log("Transparent wrappers: (none)");return}console.log(`Transparent wrappers (${wrappers.length}):`);for(let wrapper of wrappers)console.log(`  - ${wrapper}`)}import{homedir as homedir6}from"node:os";import{existsSync as existsSync13,readFileSync as readFileSync10}from"node:fs";import{homedir as homedir5}from"node:os";import{join as join20}from"node:path";async function readStdinAsync(){if(process.stdin.isTTY)return null;return(await readBoundedHookInput(process.stdin).catch(()=>null))?.trim()||null}function getSettingsPath(){if(process.env.CLAUDE_SETTINGS_PATH)return process.env.CLAUDE_SETTINGS_PATH;return join20(homedir5(),".claude","settings.json")}function isPluginEnabled(){let settingsPath=getSettingsPath();if(!existsSync13(settingsPath))return!1;try{let content=readFileSync10(settingsPath,"utf-8"),settings2=JSON.parse(content);if(!settings2.enabledPlugins)return!1;let pluginKey="cc-safety-net@cc-marketplace";if(!(pluginKey in settings2.enabledPlugins))return!1;return settings2.enabledPlugins[pluginKey]===!0}catch(error){if(envTruthy(ENV_FLAGS.debug))console.error(`CC Safety Net debug: failed to read Claude settings: ${settingsPath}: ${error instanceof Error?error.message:String(error)}`);return!1}}async function printStatusline(){let enabled=isPluginEnabled(),status;if(!enabled)status="\uD83D\uDEE1️ CC Safety Net ❌";else{let snapshot=loadPolicySnapshot({cwd:process.cwd()}),policy=snapshot.policy,modes=getCCSafetyNetEnvModes(policy),hasEffectiveRuleCustomization=Object.values(resolveEffectiveDestructiveCommandRules(policy,modes.capabilities)).some((rule)=>rule.changesInherited);status=`\uD83D\uDEE1️ CC Safety Net ${{standard:"✅",strict:"\uD83D\uDD12",paranoid:"\uD83D\uDC41️",custom:"\uD83D\uDD27"}[hasEffectiveRuleCustomization?"custom":modes.effectiveLevel]}${modes.worktreeMode?"\uD83C\uDF33":""}${snapshot.state==="degraded"?"⚠️":""}`}let stdinInput=await readStdinAsync();if(stdinInput&&!stdinInput.startsWith("{"))console.log(`${stdinInput} | ${status}`);else console.log(status)}function printStatus(){let snapshot=loadPolicySnapshot({cwd:process.cwd()}),policy=snapshot.policy,modes=getCCSafetyNetEnvModes(policy),asciiOnly=!!process.env.NO_COLOR||!process.stdout.isTTY,width=Math.min(process.stdout.columns||80,100),on=asciiOnly?"ok":"✔",off=asciiOnly?"OFF":"✘",row=(label,value)=>{let line=`  ${label.padEnd(13)}${value}`;return(line.length>width?`${line.slice(0,width-1)}…`:line).replaceAll(off,colors.red(off))},hasEffectiveRuleCustomization=Object.values(resolveEffectiveDestructiveCommandRules(policy,modes.capabilities)).some((rule)=>rule.changesInherited),policyPath=getUserPolicyPath(),paintVerdict={ready:colors.green,degraded:colors.yellow}[snapshot.state],issues=[...isPluginEnabled()?[]:["plugin cc-safety-net@cc-marketplace is disabled in Claude Code; nothing is enforced in Claude Code until it is re-enabled. Other integrations are not affected."],...snapshot.diagnostics],bullet=asciiOnly?"-":"·";console.log([`${asciiOnly?"":"\uD83D\uDEE1️  "}CC Safety Net — ${paintVerdict(snapshot.state)}`,"",row("Protection",`destructive ${policy.destructiveCommandProtectionEnabled?on:off}   secrets ${policy.secretProtection.enabled?on:off}`),row("Level",hasEffectiveRuleCustomization?`${modes.effectiveLevel} (customised)`:modes.effectiveLevel),row("Rules",policy.rules.length===0?"none active":`${policy.rules.length} active`),row("Policy",policyPath.startsWith(homedir6())?`~${policyPath.slice(homedir6().length)}`:policyPath),...modes.worktreeMode?[row("Worktree","relaxations active")]:[],"",...issues.length===0?["  Everything configured is active."]:["  Not active",...issues.flatMap((issue)=>wrapReason(issue,"      ",width-6).map((line,index)=>index===0?`    ${bullet} ${line}`:line)),"","  Full report: cc-safety-net doctor"]].join(`
`))}function handleHelpCommand(args){if(args[0]!=="help")return!1;let commandName=args[1];if(!commandName)printHelp(),process.exit(0);if(showCommandHelp(commandName))process.exit(0);console.error(`Unknown command: ${commandName}`),console.error("Run 'cc-safety-net --help' for available commands."),process.exit(1)}var commandHandlers={hook:async(args)=>{let integration=findHookIntegrationByFlag(args);if(integration){await integration.run();return}console.error("hook requires exactly one integration flag. Try: cc-safety-net hook --kimi-code"),showCommandHelp("hook",console.error),process.exit(1)},install:async(args)=>{process.exit(await runInstallCommand("install",args))},update:async(args)=>{process.exit(await runUpdateCommand(args))},uninstall:async(args)=>{process.exit(await runInstallCommand("uninstall",args))},rule:async(args)=>{process.exit(await runRuleCommand(args))},status:async(args)=>{if(reportCommandArgErrors(parseCommandArgs({label:"status"},args).errors))process.exit(1);printStatus()},statusline:async(args)=>{let parsed=parseCommandArgs({label:"statusline",booleans:{claudeCode:["-cc","--claude-code"]}},args);if(parsed.errors.length===0&&parsed.flags.claudeCode){await printStatusline();return}if(reportCommandArgErrors(parsed.errors),!parsed.flags.claudeCode)console.error("statusline requires --claude-code (-cc)");showCommandHelp("statusline",console.error),process.exit(1)},doctor:async(args)=>{let flags=parseDoctorFlags(args);if(!flags)process.exit(1);let exitCode=await runDoctor({json:flags.json,skipUpdateCheck:flags.skipUpdateCheck});process.exit(exitCode)},logs:async(args)=>{process.exit(await runLogsCommand(args))},gui:async(args)=>{process.exit(await runGuiCommand(args))},explain:async(args)=>{let flags=parseExplainFlags(args);if(!flags)process.exit(1);let result=explainCommand2(flags.command,{cwd:flags.cwd}),asciiOnly=!!process.env.NO_COLOR||!process.stdout.isTTY;if(flags.json)console.log(formatTraceJson(result));else console.log(formatTraceHuman(result,{asciiOnly}));process.exit(0)}};async function main(){let args=process.argv.slice(2),globalScan=parseCommandArgs({label:"cc-safety-net",booleans:{version:["-V","--version"]},positionals:"list"},args);if(handleHelpCommand(args))return;let commandName=args[0],command=commandName?findCommand(commandName):void 0;if(globalScan.help&&command&&command.name!=="rule")showCommandHelp(command.name),process.exit(0);if(!commandName||globalScan.help&&!command)printHelp(),process.exit(0);if(globalScan.flags.version)printVersion(),process.exit(0);if(command){await commandHandlers[command.name](args.slice(1));return}let legacyIntegration=findLegacyTopLevelHookIntegration(commandName);if(legacyIntegration){await legacyIntegration.run();return}if(commandName==="--statusline"){await printStatusline();return}console.error(commandName.startsWith("-")?`Unknown option: ${commandName}`:`Unknown command: ${commandName}`),console.error("Run 'cc-safety-net --help' for usage."),process.exit(1)}main().catch((error)=>{console.error("CC Safety Net error:",error),process.exit(1)});
