 ...
 * exactly that file.
 */
export function readRuntimeUserBaseline(options?: RulesPolicyOptions): {
  baseline: GuiPolicy;
  diagnostics: string[];
} {
  ...
  return {
    baseline: normalizeGuiPolicy(file.value),
    diagnostics: file.errors.length > 0 ? file.errors : getUserPolicyDiagnostics(file.value),
  };
}
