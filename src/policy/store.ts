...
export function loadPolicyConfig(options: RulesPolicyOptions = {}): PolicyConfig {
  ...
  const projectPath = getProjectPolicyPath(options.cwd);
  const projectFile = readPolicyFile(projectPath);
  const project = projectPolicyProjection(projectFile.parsed);
  const merged =
 ...
 */
export function projectPolicyProjection(value: unknown): {
  ...
  const destructiveSection = {
    ...
    ...(destructive.allow_paths !== undefined
      ? { allow_paths: repairAllowPaths(destructive.allow_paths) }
      : {}),
  };
  const secretSection = {
    ...
    ...(secret.overrides !== undefined
      ...
      : {}),
    ...(secret.deny_paths !== undefined ? { deny_paths: repairDenyPaths(secret.deny_paths) } : {}),
 ...
 */
function readPolicyFile(path: string): {
  exists: boolean;
  parsed?: unknown;
  errors: string[];
  fallback?: PolicyFallback;
...
function readPolicyConfig(path: string): {
  ...
  const gui = normalizeGuiPolicy(file.parsed);
  return {
    policy: normalizePolicyConfig(gui),
    gui,
    errors: file.errors,
