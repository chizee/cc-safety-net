...
function readHermesConfig(homeDir: string): string | undefined {
  try {
    return readFileSync(join(getHermesHomeDir(homeDir), 'config.yaml'), 'utf-8');
  } catch {
    return undefined;
  }
}
 ...
 * plugin back on.
 */
export function isHermesAgentPluginEnabled(homeDir: string): boolean {
  const config = readHermesConfig(homeDir) ?? '';
  return (
...
export function detect(context: DetectContext): HookDetection {
  const configPath = getHermesAgentPluginDir(context.homeDir);
  ...
  if (!isHermesAgentPluginEnabled(context.homeDir))
    return {
      platform: PLATFORM,
      status: 'disabled',
