 ...
 */
export function getHermesHomeDir(homeDir: string): string {
  const hermesHome = process.env.HERMES_HOME?.trim();
  return hermesHome ? hermesHome : join(homeDir, '.hermes');
}

export function getHermesAgentPluginDir(homeDir: string): string {
 ...
 */
function resolvePluginDir(homeDir: string, action: 'install' | 'remove'): string {
...
export function installHermesAgent(homeDir: string): InstallResult {
  const dir = resolvePluginDir(homeDir, 'install');
 ...
 */
export function readOwnedHermesAgentFiles(homeDir: string): readonly { name: string }[] {
  const dir = resolvePluginDir(homeDir, 'remove');
...
export function uninstallHermesAgent(homeDir: string): InstallResult {
  const dir = resolvePluginDir(homeDir, 'remove');
  if (!lstatOrUndefined(dir)) return { path: dir, alreadyInstalled: false };

  const present = readOwnedHermesAgentFiles(homeDir);
